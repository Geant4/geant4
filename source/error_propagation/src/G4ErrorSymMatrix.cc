...
#include "../include/G4ErrorSymMatrix.hh"
#include "../include/G4ErrorMatrix.hh"

#ifdef HEP_DEBUG_INLINE
//#include "CLHEP/Matrix/G4ErrorSymMatrix.icc"
#endif
...
#define SIMPLE_TOP(OPER)          \
    ...
    mcIter e=m1.m.begin()+m1.num_size(); \
...
G4ErrorSymMatrix::G4ErrorSymMatrix(int p)
   : m(p*(p+1)/2), nrow(p)
{
   size = nrow * (nrow+1) / 2;
   m.assign(size,0);
...
G4ErrorSymMatrix::G4ErrorSymMatrix(int p, int init)
   : m(p*(p+1)/2), nrow(p)
{
   size = nrow * (nrow+1) / 2;
   ...
   default:
      G4ErrorMatrix::error("G4ErrorSymMatrix: initialization must be either 0 or 1.");
   }
}
...
G4ErrorSymMatrix G4ErrorSymMatrix::sub(int min_row, int max_row) const
...
#else
{
  ...
  mcIter b1 = m.begin() + (min_row+2)*(min_row-1)/2;
...
G4ErrorSymMatrix G4ErrorSymMatrix::sub(int min_row, int max_row) 
{
  G4ErrorSymMatrix mret(max_row-min_row+1);
  if(max_row > num_row())
    G4ErrorMatrix::error("G4ErrorSymMatrix::sub: Index out of range");
  mIter a = mret.m.begin();
  mIter b1 = m.begin() + (min_row+2)*(min_row-1)/2;
  for(int irow=1; irow<=mret.num_row(); irow++) {
    mIter b = b1;
    for(int icol=1; icol<=irow; icol++) {
      *(a++) = *(b++);
...
void G4ErrorSymMatrix::sub(int row,const G4ErrorSymMatrix &m1)
{
  if(row <1 || row+m1.num_row()-1 > num_row() )
    G4ErrorMatrix::error("G4ErrorSymMatrix::sub: Index out of range");
  mcIter a = m1.m.begin();
  mIter b1 = m.begin() + (row+2)*(row-1)/2;
...
#else
{
  G4ErrorSymMatrix mret(m1.num_row() + m2.num_row(),
				       0);
#endif
  mret.sub(1,m1);
...
const G4ErrorSymMatrix &m1,double t)
#ifdef HEP_GNU_OPTIMIZED_RETURN
     return mret(m1);
{
#else
...
G4ErrorSymMatrix operator*(double t,const G4ErrorSymMatrix &m1)
#ifdef HEP_GNU_OPTIMIZED_RETURN
     return mret(m1);
{
#else
     ...
     return mret(m1.num_row(),m2.num_col());
{
#else
  {
    ...
    mcIter mit1, mit2, sp,snp; //mit2=0
    double temp;
    mIter mir=mret.m.begin();
    int step,stept;
...
#else
{
  ...
  mcIter snp1,sp1,snp2,sp2;
  double temp;
  mIter mr = mret.m.begin();
...
G4ErrorSymMatrix & G4ErrorSymMatrix::operator/=(double t)
{
  SIMPLE_UOP(/=)
  return (*this);
}
...
// So there is no need to check dimensions again.
  int n = m1.num_col();
  mIter mr = mret.m.begin();
  mIter tempr1 = temp.m.begin();
  for(int r=1;r<=mret.num_row();r++) {
    mcIter m1c1 = m1.m.begin();
    for(int c=1;c<=r;c++) {
       double tmp = 0.0;
      mIter tempri = tempr1;
      mcIter m1ci = m1c1;
...
/*
double G4ErrorSymMatrix::similarity(const CLHEP::HepVector &m1)
const {
   double mret = 0.0;
  CLHEP::HepVector temp = (*this) *m1;
// If m1*(*this) has correct dimensions, then so will the m1.T multiplication.
// So there is no need to check dimensions again.
  mIter a=temp.m.begin();
  mcIter b=m1.m.begin();
  mIter e=a+m1.num_row();
  for(;a<e;) mret += (*(a++)) * (*(b++));
  return mret;
}
...
#else
{
  ...
  int n = m1.num_col();
  mIter mrc = mret.m.begin();
  mIter temp1r = temp.m.begin();
  for(int r=1;r<=mret.num_row();r++) {
    mcIter m11c = m1.m.begin();
    for(int c=1;c<=r;c++) {
       double tmp = 0.0;
       mIter tempir = temp1r;
       mcIter m1ic = m11c;
...
void G4ErrorSymMatrix::invert(int &ifail) {
...
double G4ErrorSymMatrix::determinant() const {
...
double G4ErrorSymMatrix::trace() const {
   double t = 0.0;
...
void G4ErrorSymMatrix::invertBunchKaufman(int &ifail) {
...
double G4ErrorSymMatrix::posDefFraction5x5 = 1.0;
double G4ErrorSymMatrix::posDefFraction6x6 = 1.0;
double G4ErrorSymMatrix::adjustment5x5 = 0.0;
double G4ErrorSymMatrix::adjustment6x6 = 0.0;
const double G4ErrorSymMatrix::CHOLESKY_THRESHOLD_5x5 = .5;
const double G4ErrorSymMatrix::CHOLESKY_THRESHOLD_6x6 = .2;
const double G4ErrorSymMatrix::CHOLESKY_CREEP_5x5 = .005;
const double G4ErrorSymMatrix::CHOLESKY_CREEP_6x6 = .002;
...

void G4ErrorSymMatrix::invert5(int & ifail) { 
...
void G4ErrorSymMatrix::invert6(int & ifail) { 
...

void G4ErrorSymMatrix::invertHaywood5  (int & ifail) {
...

void G4ErrorSymMatrix::invertHaywood6  (int & ifail) {
...

void G4ErrorSymMatrix::invertCholesky5 (int & ifail) {
...

void G4ErrorSymMatrix::invertCholesky6 (int & ifail) {
...

void G4ErrorSymMatrix::invert4  (int & ifail) {
...
void G4ErrorSymMatrix::invertHaywood4  (int & ifail) {
