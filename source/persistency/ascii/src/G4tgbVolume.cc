    ...
    for (auto cite = children.first; cite != children.second; ++cite)
    {
      ...
      if (G4tgrMessenger::GetVerboseLevel() >= 2)
      {
        G4cout << " G4tgbVolume::ConstructG4Volumes - construct daughter "
               << pl->GetVolume()->GetName() << " # " << pl->GetCopyNo() << G4endl;
      }
#endif
...
{
  G4double angularTolerance = G4GeometryTolerance::GetInstance()->GetAngularTolerance();
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 2)
  {
    G4cout << " G4tgbVolume::FindOrConstructG4Solid():" << G4endl << "   SOLID = " << sol << G4endl
           << "   " << sol->GetName() << " of type " << sol->GetType() << G4endl;
  }
#endif
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 2)
  {
    G4cout << " G4tgbVolume::FindOrConstructG4Solid() - " << sol->GetSolidParams().size() << G4endl;
  }
#endif
  ...
  else if (stype == "TRAP")
  {
    ...
    {
      solid =
        new G4Trap(sname, solParam[0], solParam[1], solParam[2], solParam[3], solParam[4],
                   solParam[5], solParam[6], solParam[7], solParam[8], solParam[9], solParam[10]);
    }
    else if (solParam.size() == 4)
    {
      solid = new G4Trap(sname, solParam[0], solParam[1] / deg, solParam[2] / deg, solParam[3]);
    }
    else
    {
      ...
      G4Exception("G4tgbVolume::FindOrConstructG4Solid()", "InvalidSetup", FatalException,
                  ErrMessage);
      return 0;
    }
  }
  ...
  {
    CheckNoSolidParams(stype, 5, (G4int)solParam.size());
    solid = new G4Trd(sname, solParam[0], solParam[1], solParam[2], solParam[3], solParam[4]);
  }
  else if (stype == "PARA")
  {
    CheckNoSolidParams(stype, 6, (G4int)solParam.size());
    solid = new G4Para(sname, solParam[0], solParam[1], solParam[2], solParam[3], solParam[4],
                       solParam[5]);
  }
  else if (stype == "CONE")
  {
    CheckNoSolidParams(stype, 5, (G4int)solParam.size());
    solid = new G4Cons(sname, solParam[0], solParam[1], solParam[2], solParam[3], solParam[4], 0.,
                       360. * deg);
  }
  else if (stype == "CONS")
  {
    ...
    solid = new G4Cons(sname, solParam[0], solParam[1], solParam[2], solParam[3], solParam[4],
                       solParam[5], phiDelta);
  }
  else if (stype == "SPHERE")
  {
    ...
    solid =
      new G4Sphere(sname, solParam[0], solParam[1], solParam[2], phiDelta, solParam[4], thetaDelta);
  }
  else if (stype == "ORB")
  {
  ...
  else if (stype == "TORUS")
  {
    ...
    solid = new G4Torus(sname, solParam[0], solParam[1], solParam[2], solParam[3], phiDelta);
  }
  else if (stype == "POLYCONE" || stype == "GENERICPOLYCONE")
  {
    ...
    else
    {
      ...
      solid = new G4GenericPolycone(sname, solParam[0], phiTotal,  // start,delta-phi
                                    (G4int)nplanes,  // sections
                                    &((*R_c)[0]), &((*Z_c)[0]));
    }
  }
  else if (stype == "POLYHEDRA")
  {
    ...
    if (!genericPoly)
    {
      ...
      solid = new G4Polyhedra(sname, solParam[0], phiTotal, G4int(solParam[2]), (G4int)nplanes,
                              &((*z_p)[0]), &((*rmin_p)[0]), &((*rmax_p)[0]));
    }
    else
    {
  ...
  {
    CheckNoSolidParams(stype, 5, (G4int)solParam.size());
    solid = new G4Ellipsoid(sname, solParam[0], solParam[1], solParam[2], solParam[3], solParam[4]);
  }
  else if (stype == "ELLIPTICALCONE")
  {
    CheckNoSolidParams(stype, 4, (G4int)solParam.size());
    solid = new G4EllipticalCone(sname, solParam[0], solParam[1], solParam[2], solParam[3]);
  }
  else if (stype == "HYPE")
  {
    CheckNoSolidParams(stype, 5, (G4int)solParam.size());
    solid = new G4Hype(sname, solParam[0], solParam[1], solParam[2], solParam[3], solParam[4]);
  }
  else if (stype == "TET")
  {
  ...
  {
    CheckNoSolidParams(stype, 4, (G4int)solParam.size());
    solid = new G4TwistedBox(sname, solParam[0], solParam[1], solParam[2], solParam[3]);
  }
  else if (stype == "TWISTEDTRAP")
  {
    CheckNoSolidParams(stype, 11, (G4int)solParam.size());
    solid = new G4TwistedTrap(sname, solParam[0], solParam[1], solParam[2], solParam[3],
                              solParam[4], solParam[5], solParam[6], solParam[7], solParam[8],
                              solParam[9], solParam[10]);
  }
  else if (stype == "TWISTEDTRD")
  {
    CheckNoSolidParams(stype, 6, (G4int)solParam.size());
    solid = new G4TwistedTrd(sname, solParam[0], solParam[1], solParam[2], solParam[3], solParam[4],
                             solParam[5]);
  }
  ...
  else if (stype == "TWISTEDTUBS")
  {
    ...
    solid = new G4TwistedTubs(sname, solParam[0], solParam[1], solParam[2], solParam[3], phiTotal);
  }
  else if (stype == "TESSELLATED")
  {
    ...
    for (G4int ii = 0; ii < nFacets; ++ii)
    {
      ...
      if (G4int(solParam.size()) < jj + nPoints * 3 + 2)
      {
        ...
        G4String Err3 =
          " number of parameters is " + G4UIcommand::ConvertToString(G4int(solParam.size()));
  ...
  else if (stype.substr(0, 7) == "Boolean")
  {
    ...
    G4RotationMatrix* relRotMat =
      G4tgbRotationMatrixMgr::GetInstance()->FindOrBuildG4RotMatrix(sol->GetRelativeRotMatName());
  ...
  {
    G4String ErrMessage = "Solids of type " + stype + " not implemented yet, sorry...";
    G4Exception("G4tgbVolume::FindOrConstructG4Solid()", "NotImplemented", FatalException,
                ErrMessage);
    return nullptr;
  }
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 2)
  {
    G4cout << " G4tgbVolume::FindOrConstructG4Solid()" << G4endl << "   Created solid " << sname
...
// --------------------------------------------------------------------
void G4tgbVolume::CheckNoSolidParams(const G4String& solidType, const unsigned int NoParamExpected,
                                     const unsigned int NoParam)
{
  if (NoParamExpected != NoParam)
  {
    ...
    G4String ErrMessage = Err1 + Err2 + Err3 + " !";
    G4Exception("G4tgbVolume::CheckNoSolidParams()", "InvalidSetup", FatalException, ErrMessage);
  }
}
...
G4LogicalVolume* G4tgbVolume::ConstructG4LogVol(const G4VSolid* solid)
{
  ...
  G4Material* mate =
    G4tgbMaterialMgr::GetInstance()->FindOrBuildG4Material(theTgrVolume->GetMaterialName());
  if (mate == nullptr)
  {
    G4String ErrMessage =
      "Material not found " + theTgrVolume->GetMaterialName() + " for volume " + GetName() + ".";
    G4Exception("G4tgbVolume::ConstructG4LogVol()", "InvalidSetup", FatalException, ErrMessage);
  }
#ifdef G4VERBOSE
  if (G4tgrMessenger::GetVerboseLevel() >= 2)
  {
  ...
  //---------- Construct the LV
  logvol =
    new G4LogicalVolume(const_cast<G4VSolid*>(solid), const_cast<G4Material*>(mate), GetName());

#ifdef G4VERBOSE
  if (G4tgrMessenger::GetVerboseLevel() >= 1)
  {
    G4cout << " Constructing new G4LogicalVolume: " << logvol->GetName() << " mate "
           << mate->GetName() << G4endl;
  }
#endif
                                                   ...
                                                   const G4LogicalVolume* parentLV)
{
  ...
  if (place == nullptr)
  {
    ...
    if (G4tgrMessenger::GetVerboseLevel() >= 2)
    {
      G4cout << " G4tgbVolume::ConstructG4PhysVol() - World: " << GetName() << G4endl;
    }
#endif
    physvol = new G4PVPlacement(nullptr, G4ThreeVector(), const_cast<G4LogicalVolume*>(currentLV),
                                GetName(), 0, false, 0, theTgrVolume->GetCheckOverlaps());
#ifdef G4VERBOSE
    if (G4tgrMessenger::GetVerboseLevel() >= 1)
    {
      G4cout << " Constructing new : G4PVPlacement " << physvol->GetName() << G4endl;
    }
#endif
  }
  else
  {
    ...
    if (G4tgrMessenger::GetVerboseLevel() >= 2)
    {
      G4cout << " G4tgbVolume::ConstructG4PhysVol() - " << GetName() << G4endl << "   inside "
             << parentLV->GetName() << " copy No: " << copyNo
    ...
    if (theTgrVolume->GetType() == "VOLSimple")
    {
      ...
      if (G4tgrMessenger::GetVerboseLevel() >= 2)
      {
        G4cout << " G4tgbVolume::ConstructG4PhysVol() - Placement type = " << place->GetType()
               << G4endl;
      }
#endif
      ...
      if (place->GetType() == "PlaceSimple")
      {
        ...
        {
          G4cerr << " Matrix : " << rmName << " " << rotmat->colX() << " " << rotmat->colY() << " "
                 << rotmat->colZ() << G4endl << " product x X y * z = " << check << " x X y "
                 << rotmat->colX().cross(rotmat->colY()) << G4endl;
          G4String ErrMessage = "Rotation is not ortogonal " + rmName + " !";
          G4Exception("G4tgbVolume::ConstructG4PhysVol()", "InvalidSetup", FatalException,
                      ErrMessage);
        ...
        else if (1 + check <= tol)
        {
          ...
          physvol = (G4ReflectionFactory::Instance()->Place(
        ...
        else
        {
          ...
          if (G4tgrMessenger::GetVerboseLevel() >= 1)
          {
            G4cout << "Construction new G4VPhysicalVolume"
                   << " through G4ReflectionFactory " << GetName() << " in volume "
                   << parentLV->GetName() << " copyNo " << copyNo << " position "
                   << place->GetPlacement() << " ROT " << rotmat->colX() << " " << rotmat->colY()
                   << " " << rotmat->colZ() << G4endl;
          }
#endif
          physvol = new G4PVPlacement(rotmat, place->GetPlacement(),
                                      const_cast<G4LogicalVolume*>(currentLV), GetName(),
                                      const_cast<G4LogicalVolume*>(parentLV), false, copyNo,
      ...
      else if (place->GetType() == "PlaceParam")
      {
        ...
        if (G4tgrMessenger::GetVerboseLevel() >= 2)
        {
          G4cout << " G4tgbVolume::ConstructG4PhysVol() -" << G4endl << "   param: " << GetName()
                 << " in " << parentLV->GetName() << " param type= " << dp->GetParamType()
                 << G4endl;
        }
#endif
        ...
        if ((dp->GetParamType() == "CIRCLE") || (dp->GetParamType() == "CIRCLE_XY")
            || (dp->GetParamType() == "CIRCLE_XZ") || (dp->GetParamType() == "CIRCLE_YZ"))
        {
          param = new G4tgbPlaceParamCircle(dp);
        }
        else if ((dp->GetParamType() == "LINEAR") || (dp->GetParamType() == "LINEAR_X")
                 || (dp->GetParamType() == "LINEAR_Y") || (dp->GetParamType() == "LINEAR_Z"))
        {
          param = new G4tgbPlaceParamLinear(dp);
        }
        else if ((dp->GetParamType() == "SQUARE") || (dp->GetParamType() == "SQUARE_XY")
                 || (dp->GetParamType() == "SQUARE_XZ") || (dp->GetParamType() == "SQUARE_YZ"))
        {
        ...
        {
          G4String ErrMessage =
            "Parameterisation has wrong type, TYPE: " + G4String(dp->GetParamType()) + " !";
          G4Exception("G4tgbVolume::ConstructG4PhysVol", "WrongArgument", FatalException,
                      ErrMessage);
          return nullptr;
        }
#ifdef G4VERBOSE
        if (G4tgrMessenger::GetVerboseLevel() >= 1)
        {
          G4cout << " G4tgbVolume::ConstructG4PhysVol() -" << G4endl
                 << "   New G4PVParameterised: " << GetName() << " vol " << currentLV->GetName()
                 << " in vol " << parentLV->GetName() << " axis " << param->GetAxis() << " nCopies "
                 << param->GetNCopies() << G4endl;
        }
#endif
        physvol = new G4PVParameterised(GetName(), const_cast<G4LogicalVolume*>(currentLV),
                                        const_cast<G4LogicalVolume*>(parentLV),
                                        EAxis(param->GetAxis()), param->GetNCopies(), param);
#ifdef G4VERBOSE
        if (G4tgrMessenger::GetVerboseLevel() >= 1)
        {
          G4cout << " Constructing new G4PVParameterised: " << physvol->GetName() << " in volume "
                 << parentLV->GetName() << " N copies " << param->GetNCopies() << " axis "
      ...
      else if (place->GetType() == "PlaceReplica")
      {
        ...
        if (G4tgrMessenger::GetVerboseLevel() >= 2)
        {
          G4cout << " G4tgbVolume::ConstructG4PhysVol() -" << G4endl << "   replica"
                 << " " << currentLV->GetName() << " in " << parentLV->GetName() << " NDiv "
                 << dpr->GetNDiv() << " Width " << dpr->GetWidth() << " offset " << dpr->GetOffset()
                 << G4endl;
        }
#endif
        physvol = new G4PVReplica(GetName(), const_cast<G4LogicalVolume*>(currentLV),
                                  const_cast<G4LogicalVolume*>(parentLV), EAxis(dpr->GetAxis()),
                                  dpr->GetNDiv(), dpr->GetWidth(), dpr->GetOffset());
#ifdef G4VERBOSE
        if (G4tgrMessenger::GetVerboseLevel() >= 1)
        {
          G4cout << " Constructing new G4PVReplica: " << currentLV->GetName() << " in "
                 << parentLV->GetName() << " NDiv " << dpr->GetNDiv() << " Width "
                 << dpr->GetWidth() << " offset " << dpr->GetOffset() << G4endl;
        }
#endif
      }
    }
    else if (theTgrVolume->GetType() == "VOLDivision")
    {
      ...
      G4Material* mate =
        G4tgbMaterialMgr::GetInstance()->FindOrBuildG4Material(theTgrVolume->GetMaterialName());
      G4LogicalVolume* divLV = new G4LogicalVolume(solid, const_cast<G4Material*>(mate), GetName());
#ifdef G4VERBOSE
      if (G4tgrMessenger::GetVerboseLevel() >= 1)
      {
        G4cout << " Constructed new G4LogicalVolume for division: " << divLV->GetName() << " mate "
               << mate->GetName() << G4endl;
      }
#endif
      ...
      switch (divType)
      {
        case DivByNdiv:
          physvol = new G4PVDivision(GetName(), (G4LogicalVolume*)divLV,
                                     ...
                                     placeDiv->GetNDiv(), placeDiv->GetOffset());
#ifdef G4VERBOSE
          if (G4tgrMessenger::GetVerboseLevel() >= 1)
          {
            G4cout << " Constructing new G4PVDivision by number of divisions: " << GetName()
                   << " in " << parentLV->GetName() << " axis " << placeDiv->GetAxis() << " Ndiv "
                   << placeDiv->GetNDiv() << " offset " << placeDiv->GetOffset() << G4endl;
          }
#endif
          break;
        case DivByWidth:
          physvol = new G4PVDivision(GetName(), (G4LogicalVolume*)divLV,
                                     const_cast<G4LogicalVolume*>(parentLV), placeDiv->GetAxis(),
                                     placeDiv->GetWidth(), placeDiv->GetOffset());
#ifdef G4VERBOSE
          if (G4tgrMessenger::GetVerboseLevel() >= 1)
          {
            G4cout << " Constructing new G4PVDivision by width: " << GetName() << " in "
                   << parentLV->GetName() << " axis " << placeDiv->GetAxis() << " width "
                   << placeDiv->GetWidth() << " offset " << placeDiv->GetOffset() << G4endl;
          }
#endif
          break;
        case DivByNdivAndWidth:
          physvol = new G4PVDivision(
            GetName(), (G4LogicalVolume*)divLV, const_cast<G4LogicalVolume*>(parentLV),
            placeDiv->GetAxis(), placeDiv->GetNDiv(), placeDiv->GetWidth(), placeDiv->GetOffset());
#ifdef G4VERBOSE
          if (G4tgrMessenger::GetVerboseLevel() >= 1)
          {
            G4cout << " Constructing new G4PVDivision by width"
                   << " and number of divisions: " << GetName() << " in " << parentLV->GetName()
                   << " axis " << placeDiv->GetAxis() << " Ndiv " << placeDiv->GetNDiv()
                   << " width " << placeDiv->GetWidth() << " offset " << placeDiv->GetOffset()
                   << G4endl;
          }
#endif
          break;
    ...
    else if (theTgrVolume->GetType() == "VOLAssembly")
    {
      ...
      if (!theG4AssemblyVolume)
      {
        ...
        for (G4int ii = 0; ii < tgrAssembly->GetNoComponents(); ++ii)
        {
          ...
          if (G4tgrMessenger::GetVerboseLevel() >= 1)
          {
            G4cout << " G4AssemblyVolume->AddPlacedVolume " << ii << " " << logvol->GetName()
                   << " translation " << transl << " rotmat " << rotmat->colX() << " "
      ...
      G4LogicalVolume* parentLV_nonconst = const_cast<G4LogicalVolume*>(parentLV);
      theG4AssemblyVolume->MakeImprint(parentLV_nonconst, transl, rotmat);
    }
    else  // If it is G4tgrVolumeAssembly
    {
      G4String ErrMessage = "Volume type not supported: " + theTgrVolume->GetType() + ", sorry...";
      G4Exception("G4tgbVolume::ConstructG4PhysVol()", "NotImplemented", FatalException,
                  ErrMessage);
    }
  }
...
G4VSolid* G4tgbVolume::BuildSolidForDivision(G4VSolid* parentSolid, EAxis axis)
{
  ...
  if (parentSolid->GetEntityType() == "G4Box")
  {
    ...
    solid = new G4Box(GetName(), psolid->GetXHalfLength() * redf, psolid->GetZHalfLength() * redf,
                      psolid->GetZHalfLength() * redf);
  }
  else if (parentSolid->GetEntityType() == "G4Tubs")
  {
    G4Tubs* psolid = (G4Tubs*)(parentSolid);
    solid = new G4Tubs(GetName(), psolid->GetInnerRadius() * redf, psolid->GetOuterRadius() * redf,
                       psolid->GetZHalfLength() * redf, psolid->GetStartPhiAngle(),
  ...
  else if (parentSolid->GetEntityType() == "G4Cons")
  {
    ...
    solid = new G4Cons(GetName(), psolid->GetInnerRadiusMinusZ() * redf,
                       psolid->GetOuterRadiusMinusZ() * redf, psolid->GetInnerRadiusPlusZ() * redf,
                       psolid->GetOuterRadiusPlusZ() * redf, psolid->GetZHalfLength() * redf,
  ...
  else if (parentSolid->GetEntityType() == "G4Trd")
  {
    ...
    if (axis == kXAxis
        && std::fabs(mpDx1 - mpDx2) > G4GeometryTolerance::GetInstance()->GetSurfaceTolerance())
    {
      solid =
        new G4Trap(GetName(), psolid->GetZHalfLength() * redf, psolid->GetYHalfLength1() * redf,
                   psolid->GetXHalfLength2() * redf, psolid->GetXHalfLength1() * redf);
    }
    else
    {
  ...
  else if (parentSolid->GetEntityType() == "G4Para")
  {
    ...
    solid = new G4Para(GetName(), psolid->GetXHalfLength() * redf, psolid->GetYHalfLength() * redf,
                       psolid->GetZHalfLength() * redf, std::atan(psolid->GetTanAlpha()),
                       psolid->GetSymAxis().theta(), psolid->GetSymAxis().phi());
  }
  else if (parentSolid->GetEntityType() == "G4Polycone")
  {
    ...
    solid =
      new G4Polycone(GetName(), psolid->GetStartPhi(), psolid->GetEndPhi(), origParam.Num_z_planes,
  ...
  else if (parentSolid->GetEntityType() == "G4GenericPolycone")
  {
    ...
    solid = new G4GenericPolycone(GetName(), psolid->GetStartPhi(),
                                  psolid->GetEndPhi() - psolid->GetStartPhi(), numRZ, r, z);
    delete[] r;
    delete[] z;
  }
  else if (parentSolid->GetEntityType() == "G4Polyhedra")
  {
    ...
    solid =
      new G4Polyhedra(GetName(), psolid->GetStartPhi(), psolid->GetEndPhi(), psolid->GetNumSide(),
                      origParam.Num_z_planes, origParam.Z_values, origParam.Rmin, origParam.Rmax);
  }
  else
  {
    ...
    G4Exception("G4tgbVolume::BuildSolidForDivision()", "NotImplemented", FatalException,
                ErrMessage);
    return nullptr;
  }
