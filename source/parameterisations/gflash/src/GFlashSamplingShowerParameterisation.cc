...
GFlashSamplingShowerTuning * aPar): GFlashShowerParameterisation()
{  
...
GFlashSamplingShowerParameterisation::ComputeLongitudinalParameters(G4double y)
{
	...
	// if sampling 
	AveLogTmax  =std:: max(0.1,log(exp(AveLogTmaxh) + ParsAveT1/Fs + ParsAveT2*(1-ehat)));  //ok
	AveLogAlpha =std::max(0.1,log(exp(AveLogAlphah) + (ParsAveA1/Fs)));  //ok
	//
	...
	//std::cout<<"ComputeLongitudinalParameters logTmax= "<<  AveLogTmax  <<std::endl;
	//std::cout<<"ComputeLongitudinalParameters LogAlpha = "<<  AveLogAlpha<<std::endl;
	//std:: cout<<"ComputeLongitudinalParameters LogAlphah = "<<  AveLogAlphah<<  std::endl;
...
void GFlashSamplingShowerParameterisation::GenerateNSpotProfile(const G4double y)
{
	...
	//	std::cout  <<    Tmaxh<< " "<< TNSpot    << std::endl;
	AlphaNSpot =    Alphah * (ParsSpotA1+ParsSpotA2*Zeff);   	
	//	std::cout  <<	AlphaNSpot << std::endl;
	BetaNSpot  = (AlphaNSpot-1.00)/TNSpot;           // ok 
      	NSpot      =   ParsSpotN1 /SamplingResolution * std::pow(y*Eceff/GeV,ParsSpotN2 ); //ok
	std::cout  <<	NSpot   << std::endl;
}
...
G4double GFlashSamplingShowerParameterisation::ApplySampling(const G4double DEne,const G4double )
{
	...
	if(Resolution >0.0 && DEne > 0.00 )
	{
		...
		G4float x2 = RandGamma::shoot(x1, 1.0)*Resolution; 		
		DEneFluctuated=x2;
	}
...
ComputeRadialParameters(G4double Energy, G4double Tau)
{
	...
	//std::cout<<" ComputeRadialParameters"<<RadiusCore  <<" "<<WeightCore << " " <<  RadiusTail   <<std::endl;
	RadiusCore   =   RadiusCore + ParsRC1*(1-ehat) + ParsRC2/Fs*exp(-Tau); //ok
	WeightCore   =   WeightCore + (1-ehat) *(ParsWC1+ParsWC2/Fs     
	*exp(-pow((Tau-1.),2))); //ok
