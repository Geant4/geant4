...
unsigned long crc32ul(const std::string & s) {
  static __thread std::vector<unsigned long> *crc_table_G4MT_TLS_ = 0 ; if (!crc_table_G4MT_TLS_) {crc_table_G4MT_TLS_ = new  std::vector<unsigned long>  ; *crc_table_G4MT_TLS_=  gen_crc_table() ; }  std::vector<unsigned long> &crc_table = *crc_table_G4MT_TLS_;
  unsigned long crc = 0;
  int end = s.length();
  for (int j = 0; j != end; ++j) {
