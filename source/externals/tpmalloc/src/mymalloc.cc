...
* MSPACES
  ...
  mspaces as thread-locals. For example:
    static G4ThreadLocal mspace tlms = 0;
...
static G4ThreadLocal int dev_zero_fd = -1; /* Cached file descriptor for /dev/zero. */
...
static G4ThreadLocal MLOCK_T malloc_global_mutex = { 0, 0, 0};
...
static G4ThreadLocal MLOCK_T malloc_global_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static G4ThreadLocal MLOCK_T malloc_global_mutex;
static G4ThreadLocal volatile long malloc_global_mutex_status;
...
static void* internal_memalign(mstate m, size_t alignment, size_t bytes) {
  ...
  if (bytes >= MAX_REQUEST - alignment) {
    ...
    void* mem = internal_malloc(m, req);
    if (mem != 0) {
      void* leader = 0;
      void* trailer = 0;
      mchunkptr p = mem2chunk(mem);
      ...
      if ((((size_t)(mem)) % alignment) != 0) { /* misaligned */
        /*
          ...
          possible.
        */
        char* br = (char*)mem2chunk((size_t)(((size_t)((size_t)mem +
                                                       alignment -
                                                       SIZE_T_ONE)) &
                                             -alignment));
...
static void** ialloc(mstate m,
  ...
  */
  ...
  if (chunks != 0) {
    ...
    if (n_elements == 0)
      return (void**)NULL;//internal_malloc(m, 0); Changed by A. Dotti (14Jan2013)
    marray = 0;
