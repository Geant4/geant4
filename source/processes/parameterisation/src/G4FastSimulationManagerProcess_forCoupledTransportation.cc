  ...
  fFastSimulationTrigger(false)
{
  ...
  if (verboseLevel>0) G4cout << "G4FastSimulationManagerProcess_forCoupledTransportation `" << GetProcessName() 
...
G4FastSimulationManagerProcess_forCoupledTransportation::~G4FastSimulationManagerProcess_forCoupledTransportation()
{
...
{
  if (fIsTrackingTime)
    G4cout << "!!! G4FastSimulationManagerProcess_forCoupledTransportation `" << GetProcessName() 
  ...
  else
    {
      ...
      if (newWorld == 0) G4Exception("G4FastSimulationManagerProcess_forCoupledTransportation::SetWorldVolume(const G4String&, G4bool verbose)",
				     "InvalidWorld",
				     FatalException,
				     tellWhatIsWrong);
      if (verboseLevel>0)
	if (fWorldVolume) G4cout << "G4FastSimulationManagerProcess_forCoupledTransportation `" << GetProcessName()
				 << "': changing world volume from '"  << fWorldVolume->GetName() 
				 << "' to `" << newWorld << "'." << G4endl;
	else              G4cout << "G4FastSimulationManagerProcess_forCoupledTransportation `" << GetProcessName()
...
SetWorldVolume(G4VPhysicalVolume* newWorld)
{
  ...
  else G4cout << "!!! G4FastSimulationManagerProcess_forCoupledTransportation::SetWorldVolume(const G4VPhysicalVolume* newWorld) : null pointer passed. !!! Continuing at your own risks..." << G4endl;
}
...
//                  AtRestDoIt:
//-----------------------------------------------
G4VParticleChange* G4FastSimulationManagerProcess_forCoupledTransportation::AtRestDoIt(const G4Track&, const G4Step&)
{
  return fFastSimulationManager->InvokeAtRestDoIt();
}


void G4FastSimulationManagerProcess_forCoupledTransportation::Verbose() const
{
