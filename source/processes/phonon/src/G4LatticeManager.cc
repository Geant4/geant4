...
G4LatticeManager::~G4LatticeManager() {
  for (LatticeMatMap::iterator lm=fLLatticeList.begin();
       lm != fLLatticeList.end(); ++lm) {
    delete lm->second; lm->second = 0;
  }

  for (LatticeVolMap::iterator pm=fPLatticeList.begin();
       pm != fPLatticeList.end(); ++pm) {
    delete pm->second; pm->second = 0;
  }
}
...
#ifdef G4VERBOSE
  if (verboseLevel) {
    G4cout << "G4LatticeManager::registerLattice: Registering Lattice."
	   << " Total number of logical lattices:"
	   << fLLatticeList.size() << G4endl;
  }
#endif
...
G4LatticeLogical* G4LatticeManager::GetLattice(G4Material* Mat) const {
  ...
  if (latFind != fLLatticeList.end()) return latFind->second;
...
G4LatticePhysical* G4LatticeManager::GetLattice(G4VPhysicalVolume* Vol) const {
  ...
  if (latFind != fPLatticeList.end()) return latFind->second;
