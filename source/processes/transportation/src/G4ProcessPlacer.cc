...
//
//
// $Id: G4ProcessPlacer.cc,v 1.10 2002-10-16 16:27:00 dressel Exp $
...
void G4ProcessPlacer::RemoveProcess(G4VProcess *process){
  G4std::G4cout << "=== G4ProcessPlacer::RemoveProcess: for: " <<  fParticleName 
	 << G4endl;
  G4std::G4cout << "  ProcessName: " << process->GetProcessName() 
	 << ", will be removed!" << G4endl;

  G4std::G4cout << "  The initial Vectors: " << G4endl;
  PrintPostStepGPILVec();
  PrintPostStepDoItVec();

  GetProcessManager()->RemoveProcess(process);

  G4std::G4cout << "  The final Vectors: " << G4endl;
  PrintPostStepGPILVec();
  PrintPostStepDoItVec();
...
{
  G4std::G4cout << "  ProcessName: " << process->GetProcessName() << G4endl;
  ...
  if (sol == eLast) {  
    ...
    if (!transportation) {
      G4std::G4Exception(" G4ProcessPlacer:: could not get process id=0");
    }
    if (transportation->GetProcessName() != "Transportation") {
      G4std::G4cout << transportation->GetProcessName() << G4endl;
      G4std::G4Exception(" G4ProcessPlacer:: process id=0 is not Transportation");
    }
...
{
  G4std::G4cout << "=== G4ProcessPlacer::AddProcessAsSecondDoIt: for: " 
...
{
  G4std::G4cout << "=== G4ProcessPlacer::AddProcessAsLastDoIt: for: " 
	 << fParticleName << G4endl;
...
{ 
  ...
  // ---------------------------------------------------------
  if (!processmanager) {
    G4std::G4Exception(" G4ProcessPlacer::GetProcessManager: no ProcessSampler");
  }
...
void G4ProcessPlacer::PrintPostStepGPILVec(){
  G4std::G4cout << "GPIL Vector: " << G4endl;
...
void G4ProcessPlacer::PrintPostStepDoItVec(){
  G4std::G4cout << "DoIt Vector: " << G4endl;
...
{
  if (!processVec) {
    G4std::G4Exception("G4ProcessPlacer::G4ProcessPlacer: no processVec");
  }
  G4int len = processVec->length();
  if (len==0) {
    G4std::G4Exception("G4ProcessPlacer::G4ProcessPlacer:processVec len = 0");
  }
  for (int pi=0; pi<len; pi++) {
    G4VProcess *p = (*processVec)[pi];
    if (p) {
      G4std::G4cout << "   " << p->GetProcessName() << G4endl;
    } 
    else {
      G4std::G4cout << "   " << "no process found for position: " << pi 
