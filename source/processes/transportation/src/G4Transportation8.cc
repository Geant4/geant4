...
G4Transportation8::G4Transportation8( G4int verboseLevel )
...
G4Transportation8::~G4Transportation8()
{
  ...
  if( (fVerboseLevel > 0) && (fSumEnergyKilled > 0.0 ) ){ 
    G4cout << " G4Transportation8: Statistics for looping particles " << G4endl;
     ...
     else
     {
        ...
        if( fVerboseLevel > 1 )
        {
          if( std::fabs(startEnergy- endEnergy) > perThousand * endEnergy )
          {
            ...
            {
               no_warnings++;
               G4cout << "WARNING - G4Transportation8::AlongStepGetPIL() " 
		 ...
		 G4cout << " These include EpsilonStepMax(/Min) in G4FieldManager "
			...
			<< G4endl;
               ...
               G4cerr << "ERROR - G4Transportation8::AlongStepGetPIL()" << G4endl
...
G4VParticleChange* G4Transportation8::AlongStepDoIt( const G4Track& track,
                                                    const G4Step&  stepData )
{
  ...
  if ( fParticleIsLooping )
  {
	...
	if( (fVerboseLevel > 1) || 
	  ...
	  G4cout << " G4Transportation8 is killing track that is looping or stuck "
		 << G4endl
...
G4VParticleChange* G4Transportation8::PostStepDoIt( const G4Track& track,
                                                   const G4Step& )
{
...
// New method takes over the responsibility to reset the state of G4Transportation8
//   object at the start of a new track or the resumption of a suspended track. 

void 
G4Transportation8::StartTracking(G4Track* aTrack)
{
