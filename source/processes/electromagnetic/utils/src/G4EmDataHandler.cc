...
{
  if (!fUseBaseParticleTable && 0 < tLength) {
...
{
  if (idx >= tLength) {
    for (std::size_t j=tLength; j<=idx; ++j) {
...
G4PhysicsTable* G4EmDataHandler::MakeTable(G4PhysicsTable* ptr, std::size_t idx)
{
  if (idx >= tLength) {
    for (std::size_t j=tLength; j<=idx; ++j) {
      data.push_back(nullptr);
