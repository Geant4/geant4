...
// ********************************************************************
//
// $Id: G4BetheBlochModel70.cc,v 1.1 2005-02-26 22:01:07 vnivanch Exp $
...
G4BetheBlochModel70::G4BetheBlochModel70(const G4ParticleDefinition* p, const G4String& nam)
  : G4VEmModel(nam),
  particle(0),
...
G4BetheBlochModel70::~G4BetheBlochModel70()
{}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

G4double G4BetheBlochModel70::HighEnergyLimit(const G4ParticleDefinition* p)
{
...
G4double G4BetheBlochModel70::LowEnergyLimit(const G4ParticleDefinition* p)
{
  if(!particle) SetParticle(p);
  return lowKinEnergy;
}
...
G4double G4BetheBlochModel70::MinEnergyCut(const G4ParticleDefinition*,
...
G4bool G4BetheBlochModel70::IsInCharge(const G4ParticleDefinition* p)
{
  if(!particle) SetParticle(p);
...
void G4BetheBlochModel70::Initialise(const G4ParticleDefinition* p,
                                     const G4DataVector&)
{
  if(!particle) SetParticle(p);
...
G4double G4BetheBlochModel70::ComputeDEDX(const G4Material* material,
                                                ...
                                                G4double cut)
{
  ...
  // shell correction
  // dedx -= 2.0*corr->ShellCorrectionSTD(p,material,kineticEnergy);
...
G4double G4BetheBlochModel70::CrossSection(const G4MaterialCutsCouple* couple,
                                         const G4ParticleDefinition* p,
                                               G4double kineticEnergy,
                                               G4double cutEnergy,
...
G4DynamicParticle* G4BetheBlochModel70::SampleSecondary(
                                   ...
                                   G4double maxEnergy)
{
  ...
  do {
    ...
    if(f > 1.0) {
        G4cout << "G4BetheBlochModel70::SampleSecondary Warning! "
               << "Majorant 1.0 < "
...
vector<G4DynamicParticle*>* G4BetheBlochModel70::SampleSecondaries(
                             const G4MaterialCutsCouple* couple,
                             const G4DynamicParticle* dp,
                                   G4double tmin,
                                   G4double maxEnergy)
