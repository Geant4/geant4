...
void G4PAIySection::Initialize( const G4Material* material,
				...
				G4double betaGammaSq, G4double** photoAbsCof, G4int intNumber)
{
  fSandia = 0;
  // fMatSandiaMatrix = 0;
  G4int i, j;
  ...
  for( i = 0; i < G4int(theMaterialTable->size()); i++ )
  {
    if( material->GetName() == (*theMaterialTable)[i]->GetName() ) break;
    else continue;
  }
  // fMaterialIndex = i;
  ...
  // G4cout<<"i last = "<<i<<"; "<<"fIntervalNumber = "<<fIntervalNumber<<G4endl;   
        ...
        else
	{
	 ...
	 //    <<"; dNdxPAI = "<<fDifPAIySection[i]<<G4endl;
      }
      IntegralCerenkov();
      // IntegralMM();
      IntegralPlasmon();
