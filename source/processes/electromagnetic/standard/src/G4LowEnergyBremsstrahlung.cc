...
// and all its terms.
//
// $Id: G4LowEnergyBremsstrahlung.cc,v 1.1 1999-01-08 14:16:29 gunter Exp $
...
//      ------------ G4LowEnergyeBremsstrahlung physics process --------
...
G4LowEnergyBremsstrahlung::G4LowEnergyBremsstrahlung(const G4String& processName)
  : G4eEnergyLoss("eBremsstrahlung"),      // initialization
    ...
    theGamma (G4Gamma::Gamma() )
{
    ...
    theEmittedPhotonSpectrumTable = 0;
    theEmittedPhotonEnergiesTable = 0;
...
G4LowEnergyBremsstrahlung::~G4LowEnergyBremsstrahlung(){
  ...
  if(theEmittedPhotonSpectrumTable){
    theEmittedPhotonSpectrumTable->clearAndDestroy();
    delete theEmittedPhotonSpectrumTable;
  }

  if(theEmittedPhotonEnergiesTable){
    theEmittedPhotonEnergiesTable->clearAndDestroy();
    delete theEmittedPhotonEnergiesTable;
  }
...
void G4LowEnergyBremsstrahlung::BuildCrossSectionTable(const G4ParticleDefinition& aParticleType){

  // Subshells Cross Section Tables
  HepString file;
  G4double param[4] = {81, 0, 0, 0}; 
...
void G4LowEnergyBremsstrahlung::BuildEmittedPhotonSpectrumTables(const G4ParticleDefinition& aParticleType){

  HepString file;
  G4double param[4] = {82, 21, 0, 0};
  ...
  G4EpdlData c(file, param);
  c.FillDataTable(1., 1.); // 
  theEmittedPhotonEnergiesTable = new G4PhysicsTable((*c.GetFstDataTable()));
  theEmittedPhotonSpectrumTable = new G4PhysicsTable((*c.GetSndDataTable()));
}
...
// 'Migdal' version , this is the default in GEANT3 
{
  ...
  G4double CM[36],CP[36],CCM[25],CCP[25];

  //  CMbarn,CPbarn,CCMbarn,CCPbarn in units barn .............
  for (G4int i=0; i<36; i++)
  {
    CM[i] = CMbarn[i]*barn ;
    CP[i] = CPbarn[i]*barn ;
  }
  for (G4int ii=0; ii<25; ii++)
  {
    CCM[ii] = CCMbarn[ii]*barn ;
    CCP[ii] = CCPbarn[ii]*barn ;
  }
  //  -----------------------------------------------------------

  G4double loss=0.;
  G4double TotalEnergy,factor,corr,rate ;

  if(Z<1.) return loss ;

  TotalEnergy = T + electron_mass_c2 ;

  G4double y=log(Cut/(ve*TotalEnergy)) ;

  if (y<=0.)
  {
    loss = ComputeXYPolynomial(x, y, 6, 6, CM)
           + Z * ComputeXYPolynomial(x, y, 5, 5, CCM);
  }
  else
  {  
    loss = ComputeXYPolynomial(x, y, 6, 6, CP)
           + Z * ComputeXYPolynomial(x, y, 5, 5, CCP);
  }

  rate = TotalEnergy/Cut ;
  corr = 1./(1.+corrfac*natom*rate*rate) ;

  factor = beta*log(Cut*corr/T);
  factor = exp(factor);
...
void G4LowEnergyBremsstrahlung::BuildLambdaTable(const G4ParticleDefinition& ParticleType)
...
// tables are Build for MATERIALS. 
{
 ...
 // cout << " brems material=" << material->GetName() << endl;
        for ( G4int i = 0 ; i < TotBin ; i++ )      
           {
             LowEdgeEnergy = ptrVector->GetLowEdgeEnergy( i ) ;
             Value = ComputeMeanFreePath( &ParticleType, LowEdgeEnergy,
                                         material );  
  //G4double sigma = 1./Value ;
  //cout << setw(15) << setprecision(6) <<LowEdgeEnergy << "    "
  //     << setw(15) << setprecision(6) << sigma << endl;
...
void G4LowEnergyBremsstrahlung::ComputePartialSumSigma(const G4ParticleDefinition* ParticleType,
...
G4double G4LowEnergyBremsstrahlung::ComputeMicroscopicCrossSection(
 ...
 
{
 ...
 // ++++++++++++++++++++++++++++++++++
 static const G4double Tlim = 1.*MeV ;
 //static const G4double Tlim = 0.*MeV ;
 if( KineticEnergy < Tlim )
     LocalKineticEnergy = Tlim ;
 // ++++++++++++++++++++++++++++++++++
 ...
 if( KineticEnergy < Tlim)
 {
   CrossSection *= log(KineticEnergy/GammaEnergyCut)/
...
G4double G4LowEnergyBremsstrahlung::ComputePositronCorrFactorSigma( G4double AtomicNumber,
...
// 
{
  ...
  static const G4double MigdalConstant = classic_electr_radius*electron_Compton_length
