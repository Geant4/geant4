...
// ********************************************************************
//
// $Id: G4WaterStoppingRange.cc,v 1.1 2008-08-08 18:52:40 antoni Exp $
...
G4WaterStoppingRange::G4WaterStoppingRange(G4EmCorrections* corr, G4bool splineFlag) 
{
  spline = splineFlag;
  Initialise(corr);
}
...
G4double 
G4WaterStoppingRange::GetDEDX(G4int ionZ, G4double kinEnergy)
{
  G4double res = 0.0;
...
void G4WaterStoppingRange::Initialise(G4EmCorrections* corr)
{
  G4int i;
dedx.resize(16*1);
...
if(corr) {
  G4int n = 0;
    for(i=0; i<16; i++) {
      corr->AddStoppingData(Z[i], Z[i], "G4_WATER", dedx[n]);
      n++;
  }
}
}
