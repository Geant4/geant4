...
// and all its terms.
//
// $Id: G4PhotoAbsorption.cc,v 1.1 1999-01-07 16:11:23 gunter Exp $
...
G4PhotoAbsorption::G4PhotoAbsorption(const G4String& processName)
...
G4PhotoAbsorption::~G4PhotoAbsorption()
{
...
void G4PhotoAbsorption::SetPhysicsTableBining(G4double lowE, G4double highE, G4int nBins)
{
  LowestEnergyLimit = lowE; HighestEnergyLimit = highE; NumbBinTable = nBins;
}  
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo.... 
 
void G4PhotoAbsorption::BuildPhysicsTable(const G4ParticleDefinition& PhotonType)
...
G4double G4PhotoAbsorption::ComputeCrossSectionPerAtom (G4double PhotonEnergy,
                                                        G4double AtomicNumber)
...
G4VParticleChange* G4PhotoAbsorption::PostStepDoIt(const G4Track& aTrack,
                                                   const G4Step&  aStep)
//
 ...
 
{
   ...
   // Photo electron
   //
   G4double ElecKineEnergy;
   
   if (PhotonEnergy < LowestEnergyLimit)
     {
       G4int i = aMaterial->GetSandiaTable()->GetMatNbOfIntervals() - 1;  
       while ((i>0)&&(PhotonEnergy<aMaterial->GetSandiaTable()
                                            ->GetSandiaCofForMaterial(i,0))) i--;  
       ElecKineEnergy = PhotonEnergy - aMaterial->GetSandiaTable()
                                                ->GetSandiaCofForMaterial(i,0);  
     }
   else
    {   
      // select randomly one element constituing the material.
      G4double Z = SelectRandomAtom(aDynamicPhoton, aMaterial)->GetZ();
      ElecKineEnergy = PhotonEnergy - ComputeKBindingEnergy(Z);
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4Element*
G4PhotoAbsorption::SelectRandomAtom(const G4DynamicParticle* aDynamicPhoton,
...
void G4PhotoAbsorption::PrintInfoDefinition()
{
