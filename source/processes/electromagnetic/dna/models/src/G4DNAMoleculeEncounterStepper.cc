...
G4DNAMoleculeEncounterStepper:: Utils::Utils(const G4Track& tA, const G4Molecule* mB) :
...
void G4DNAMoleculeEncounterStepper::Prepare()
{
	...
	G4cout << "\t || MEM || G4DNAMoleculeEncounterStepper::Prepare || After computing G4ITManager<G4Molecule>::Instance()->UpdatePositionMap, diff is : " << mem_diff << G4endl;
#endif
}
...
//	fReactants = new vector<G4Track*>();
	...
	for (G4int i=0 ; i<nbReactives ; i++)
	{
		...
		G4KDTreeResultHandle resultsNearest (G4ITManager<G4Molecule>::Instance()
...
//			G4cout << "fUserMinTimeStep =" << fUserMinTimeStep << " isInf = "<< IsInf(fUserMinTimeStep) << G4endl;

			if(tempMinET <= fSampledMinTimeStep)
			{
				if( fUserMinTimeStep < DBL_MAX/*IsInf(fUserMinTimeStep) == false*/ && tempMinET <= fUserMinTimeStep) // ==> Record in range
				{
				...
				{
					if(tempMinET < fSampledMinTimeStep) // to avoid cases where fSampledMinTimeStep == tempMinET
					{
...
//	       G4cout << GetMolecule(trackA)->GetName() << " ("<< trackA.GetTrackID()<< ") " << " + ..." << G4endl; //<< " | " << trackB->GetTrackID() << G4endl;
//
//	       for(int j = 0 ; j < fReactants->size() ; j++)
//	       {
//	    	   G4cout << GetMolecule(fReactants->at(j) )->GetName() <<" ("<< fReactants->at(j)->GetTrackID() << ")" << G4endl;
//	       }
//		}
	}
	...
	if(fVerbose)
	{
		...
		if(fVerbose > 1)
		{
			G4cout << "Selected reactants for trackA: " << moleculeA->GetName() << " (" << trackA.GetTrackID() << ") are: ";
			...
			results->Next())
	{
		...
		if (trackB->GetTrackStatus() != fAlive)
		{
			...
			exceptionDescription << "The incomming trackID "
					...
					<< utils.trackA.GetTrackID()  <<"("<< GetMolecule(utils.trackA)->GetName()<<")" << G4endl;
		...
		if(fabs(trackB->GetGlobalTime() - utils.trackA.GetGlobalTime()) > utils.trackA.GetGlobalTime()*(1-1/100) )
		{
			// DEBUG
			G4ExceptionDescription exceptionDescription;
