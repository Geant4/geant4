...
G4TrackKiller::G4TrackKiller(const G4String& processName, G4ProcessType aType)
 : G4VDiscreteProcess(processName, aType)
{
  // set Process Sub Type
  SetProcessSubType(static_cast<int>(NEUTRON_KILLER));

  fMinKinEnergyThreshold = 0.0;
  timeThreshold = DBL_MAX;
  pMess = new G4TrackKillerMessenger(this);
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

G4TrackKiller::~G4TrackKiller() 
{ 
  delete pMess; 
}
...
G4bool G4TrackKiller::IsApplicable(const G4ParticleDefinition& particle)
{
...
void G4TrackKiller::SetTimeLimit(G4double val)
{
  timeThreshold = val;
  if(verboseLevel > 0) 
...
void G4TrackKiller::SetMinKinEnergyLimit(G4double val)
{
  fMinKinEnergyThreshold = val;
  if(verboseLevel > 0) 
    G4cout << "### G4NeutronKiller: Tracking cut E(MeV) = " 
	   << fMinKinEnergyThreshold/MeV << G4endl;
}
