...
const vector<const G4MolecularDissociationChannel*>* G4MoleculeDefinition::GetDecayChannels(const G4String& ExState) const
{
	if (fDecayTable)
	{
		const vector<const G4MolecularDissociationChannel*>* output = fDecayTable->GetDecayChannels(ExState);
		return output;
	}
	else
	{
...
//___________________________________________________________________________
const vector<const G4MolecularDissociationChannel*>* G4MoleculeDefinition::GetDecayChannels(const G4ElectronOccupancy* occ) const
{
	if (fDecayTable)
	{
		const vector<const G4MolecularDissociationChannel*>* output = fDecayTable->GetDecayChannels(occ);
		return output;
	}
	else
	{
