		...
		double diffusion_coefficient)
{
	...
	{
		definition = new G4MoleculeDefinition(name, -1 /* mass*/, diffusion_coefficient);
	...
	else
	{
		...
		description << "The molecule definition " << name << " was already recorded in the table" << G4endl;
		G4Exception("G4MoleculeTable::CreateMoleculeDefinition","DEFINITION_ALREADY_CREATED",FatalException, description);
	}
	return definition;
}

G4Molecule* G4MoleculeTable::CreateMoleculeModel(const G4String& name,
		G4MoleculeDefinition* molDef, int charge, double diffusion_coefficient)
{
	...
	if(it == fMoleculeTable.end())
	{
		...
		if(diffusion_coefficient != -1)
			molecule->SetDiffusionCoefficient(diffusion_coefficient);
	...
	else
	{
		// exception
		G4ExceptionDescription description;
		description << "The molecule model " << name << " was already recorded in the table" << G4endl;
		G4Exception("G4MoleculeTable::CreateMoleculeModel","MODEL_ALREADY_CREATED",FatalException, description);
	}
	return molecule;
}
...
void G4MoleculeTable::RecordMoleculeModel(const G4String& name, G4Molecule* molecule)
{
	MoleculeTable::iterator it = fMoleculeTable.find(name);
	if(it == fMoleculeTable.end())
	{
