...
int G4MoleculeCounter::SearchUpperBoundTime(double time, bool sameTypeOfMolecule)
{
	...
	if(sameTypeOfMolecule == true)
	{
		...
		if(fpLastSearch->fLowerBoundSet && fpLastSearch->fLowerBoundTime != end_time)
		{
...
//			G4cout << "fpLastSearch->fLowerBoundTime != timeMap.end() " << time << G4endl;
			...
			{
				NbMoleculeAgainstTime::iterator upperToLast = fpLastSearch->fLowerBoundTime;
				upperToLast++;
...
int G4MoleculeCounter::GetNMoleculesAtTime(const G4Molecule &molecule, double time)
{
...
RecordedTimes G4MoleculeCounter::GetRecordedTimes()
{
    ...
    for(it = fCounterMap.begin(), ite = fCounterMap.end(); it != ite; ++it)
    {
        ...
        for(it2 = (it->second).begin(), ite2 = (it->second).end(); it2 != ite2; ++it2)
        {
            //time = it2->first;
