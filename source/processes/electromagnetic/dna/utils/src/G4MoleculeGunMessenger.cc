...
G4MoleculeGunMessenger::G4MoleculeGunMessenger()
{
	...
	fpGunNewGunType = new G4UIcmdWithAString("/process/em/dna/chem/gun/newType", this);
}
		...
		G4MoleculeGunMessenger*)
{
	...
	G4String tmp = dir; tmp += "/moleculeModel";
	fpGunMoleculeModel = new G4UIcmdWithAString(tmp, this);
	tmp = dir; tmp += "/position";
...
G4String G4MoleculeGunMessenger::MultipleGun::GetCurrentValue(
		G4UIcommand* command) {
