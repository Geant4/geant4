...
{
	if(requestedState == G4State_Idle && G4StateManager::GetStateManager()->GetPreviousState() == G4State_PreInit)
	{
		Initialize();
	}
	else if (requestedState == G4State_Quit)
	{
		G4cout << "G4DNAMolecularMaterial::Notify ---> received G4State_Quit" << G4endl;
		Clear();
		//DeleteInstance();
	}
	return true;
