...
public:
	...
	int GetNbNodes() const  { return fNbNodes; }
	G4KDNode_Base* GetRoot() { return fRoot; }

	template <typename PointT> G4KDNode_Base* InsertMap(PointT* pos);
	 ...
	 */
	template<typename Position> G4KDTreeResultHandle NearestInRange(const Position& pos,
			const double& range);
...
protected:
	...
	public:
		...
		template<typename Position> void SetMinMax(const Position& min, const Position& max)
		{
...
protected :
	...
	template<typename Position> int __NearestInRange(G4KDNode_Base *node,
			const Position& pos,
	...
	template<typename Position> void __NearestToPosition(G4KDNode_Base *node,
			const Position& pos,
			G4KDNode_Base *&result,
			double *result_dist_sq,
			HyperRect* fRect);

	template<typename Position> void __NearestToNode(G4KDNode_Base *source_node,
			G4KDNode_Base *node,
			const Position& pos,
