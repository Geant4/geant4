...
{
	if(fgStepManager == 0)
		fgStepManager = new G4ITStepManager();
...
{
	G4cout << "G4ITStepManager::DeleteInstance is called" << G4endl;
...
G4ITStepManager::G4ITStepManager() : G4ITTrackHolder(), G4ITTimeStepper(),
									G4VStateDependent()
{
	Create();
}
...
{
	if(fVerbose)
		G4cout << "*** G4ITStepManager is being cleared ***" << G4endl;
...
void G4ITStepManager::Process()
{
	...
	if(fVerbose)
	{
		...
		{
			G4cout << "*** G4ITStepManager did not start because no track was found to be processed"<< G4endl;
			G4cout  << "___________________________________" << G4endl;
		}
	}
#endif
...
void G4ITStepManager::Stepping()
{
	...
	if(fTimeStep == 0)// < fTimeTolerance)
	{
		...
		if(fZeroTimeCount >= fMaxNZeroTimeStepsAllowed)
		{
			...
			exceptionDescription << "The maximum number of zero time steps is currently : " << fMaxNZeroTimeStepsAllowed;
	...
	mem_second = MemoryUsage();
	mem_diff = mem_second-mem_first;
	G4cout << "|| MEM || After computing reactions + UserPostTimeStepAction, diff is : " << mem_diff << G4endl;
#endif
	...
	if(fVerbose)
	{
		if(fVerbose == 2)
		{
			...
			G4cout  << "\33[1;31m";
#endif
			G4cout << "Step N°"<< fNbSteps << "\t T= " << G4BestUnit(fGlobalTime,"Time") << "\t dt= " << G4BestUnit(fTimeStep,"Time")<<G4endl;
#ifdef USE_COLOR
			G4cout  << "\33[0m" ;
#endif
		}
...
double G4ITStepManager::GetLimitingTimeStep() const
{
	...
	map<double, double>::const_iterator it_fpUserTimeSteps_i   = fpUserTimeSteps->upper_bound(fGlobalTime) ;
	map<double, double>::const_iterator it_fpUserTimeSteps_low = fpUserTimeSteps->lower_bound(fGlobalTime) ;
...
void G4ITStepManager::ExtractTimeStepperData(G4ITModelProcessor* MP)
{
	...
	const std::vector<std::vector<G4VITStepModel*> >* model = MP->GetCurrentModel();
...
void G4ITStepManager::ExtractDoItData(G4ITStepProcessor* SP)
{
	...
	assert(fabs(track->GetGlobalTime() - (fGlobalTime + fTimeStep)) < fTimeTolerance);
}
//_________________________________________________________________________
		...
		//        if(fInteractionStep == false)
	{
		...
		for( ; reactionInfo_i != reactionInfo_v->end() ; reactionInfo_i++)
		{
			...
			if(fpUserTimeStepAction)
			{
				...
				fpUserTimeStepAction->UserReactionAction(*trackA, *trackB, *productsVector);
			}
...
void G4ITStepManager::KillTracks()
{
	...
	if(fVerbose > 5)
	{
		G4cout << "*** G4ITStepManager::KillTracks , step #" << fNbSteps << " ***" << G4endl;
...
G4ITStepManager::G4ITStepManager(const G4ITStepManager&) : G4ITTrackHolder(), 
								G4ITTimeStepper(), G4VStateDependent()
{
	Create();
}
