...
{
    if(fSteppingMsg) delete fSteppingMsg;
...
void G4ITStepManager::Stepping()
{
    ...
    else fDefinedMinTimeStep = DBL_MAX ;
    ...
    if(fpUserTimeStepAction) fpUserTimeStepAction->TimeStepAction();
    // TODO: pre/post
...
//    }
    ...
    if(fVerbose)
    {
        ...
        G4cout  << "\33[1;31m";
#endif
            G4cout << "Step N°"<< fNbSteps << "\t dt = " << G4BestUnit(fTimeStep,"Time")<<G4endl;
#ifdef USE_COLOR
            G4cout  << "\33[0m" ;
#endif
        }
...
//        if(fInteractionStep == false)
    {
        ...
        for( ; reactionInfo_i != reactionInfo_v->end() ; reactionInfo_i++)
        {
            ...
            if(fpUserTimeStepAction)
            {
                ...
                fpUserTimeStepAction->UserReactionAction(*trackA, *trackB, *productsVector, nbSecondaries);
            }
