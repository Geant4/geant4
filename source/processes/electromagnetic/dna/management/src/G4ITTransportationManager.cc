...
void G4ITTransportationManager::Initialize()
{
    ...
    std::vector<G4VPhysicalVolume*>::iterator it = G4TransportationManager::GetTransportationManager()->GetWorldsIterator();
...
G4ITNavigator* G4ITTransportationManager::GetNavigator( const G4String& worldName )
{
   ...
   else
   {
      ...
      G4Exception("G4ITTransportationManager::GetNavigator(name)",
...
G4int G4ITTransportationManager::ActivateNavigator( G4ITNavigator* aNavigator )
{
   ...
   if (pNav == fNavigators.end())
   {
      G4String message
         = "Navigator for volume -" + aNavigator->GetWorldVolume()->GetName()
         + "- not found in memory!";
      G4Exception("G4ITTransportationManager::ActivateNavigator()",
                  "GeomNav1002", JustWarning, message);
      return -1;
   }
...
void G4ITTransportationManager::DeRegisterWorld( G4VPhysicalVolume* aWorld )
{
   ...
   else
   {
     ...
     G4Exception("G4ITTransportationManager::DeRegisterWorld()",
                 "GeomNav1002", JustWarning, message);
   }
}
