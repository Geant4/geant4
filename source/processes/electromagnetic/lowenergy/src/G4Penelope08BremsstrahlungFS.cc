...
// ********************************************************************
//
// $Id: G4Penelope08BremsstrahlungFS.cc,v 1.1 2010-12-20 14:11:37 pandola Exp $
...
G4Penelope08BremsstrahlungFS::G4Penelope08BremsstrahlungFS() : 
  ...
  thePBcut(0)
{
  ...
  theElementData = new std::map<const G4int,CLHEP::HepMatrix* >;
...
G4Penelope08BremsstrahlungFS::~G4Penelope08BremsstrahlungFS()
{
  ...
  std::map<const G4int,CLHEP::HepMatrix*>::iterator i;
  if (theElementData)
    {
...
void G4Penelope08BremsstrahlungFS::ClearTables()
{  
...
G4double G4Penelope08BremsstrahlungFS::GetEffectiveZSquared(const G4Material* material)
{
  if (!theEffectiveZSq)
    {
...
void G4Penelope08BremsstrahlungFS::BuildScaledXSTable(const G4Material* material,
							 G4double cut)
{
 ...
 //*********************************************************************
  ...
  CLHEP::HepMatrix* tempMatrix = new CLHEP::HepMatrix(nBinsE,nBinsX,0);
  ...
  for (G4int iel=0;iel<nElements;iel++)
      ...
      if (!theElementData->count(iZ))
	{
	  ...
	  if (!theElementData->count(iZ))
	    {
	      G4cout << "Error in G4Penelope08BremsstrahlungFS::BuildScaledXSTable" << G4endl;
	      G4cout << "Unable to retrieve data for element " << iZ << G4endl;
	      G4Exception();
	    }
	}

      CLHEP::HepMatrix* atomData = theElementData->find(iZ)->second;
      
      for (size_t ie=0;ie<nBinsE;ie++)
	{
	  (*tempData)[ie] += wgt*(*atomData)[ie][32]; //last column contains total XS
	  for (size_t ix=0;ix<nBinsX;ix++)
	    (*tempMatrix)[ie][ix] += wgt*(*atomData)[ie][ix];	 
	}
    }
  ...
  for (size_t ie=0;ie<nBinsE;ie++)
	...
	(classic_electr_radius*classic_electr_radius*(theEGrid[ie]+2.0*electron_mass_c2));
      ...
      if (TST > 1.0)
	{
	  G4cout << "G4Penelope08BremsstrahlungFS. Corrupted data files?" << G4endl;
  ...
  for (size_t ix=0;ix<nBinsX;ix++)
      ...
      for (size_t ie=0;ie<nBinsE;ie++)
	{
	  ...
	  G4double aValue = (*tempMatrix)[ie][ix];
...
void G4Penelope08BremsstrahlungFS::ReadDataFile(G4int Z)
{
 
  char* path = getenv("G4LEDATA");
  if (!path)
    {
      G4String excep = "G4Penelope08BremsstrahlungFS - G4LEDATA environment variable not set!";
      G4Exception(excep);
      return;
    }
  /*
  ...
  */
  ...
  if (!file.is_open())
    {
      G4String excep = "G4Penelope08BremsstrahlungFS - data file " + 
  ...
  if (readZ != Z)
    {
      G4cout << "G4Penelope08BremsstrahlungFS::ReadDataFile()" << G4endl;
      G4cout << "Corrupted data file for Z=" << Z << G4endl;
      G4Exception();
    }

  CLHEP::HepMatrix* theMatrix = new CLHEP::HepMatrix(nBinsE,nBinsX+1,0); //inialized with zeros
  ...
  for (size_t ie=0;ie<nBinsE;ie++)
	...
	{
	  file >> myDouble;
	  (*theMatrix)[ie][ix] = myDouble*millibarn;
	}
      file >> myDouble; //total cross section
      (*theMatrix)[ie][nBinsX] = myDouble*millibarn;
    }
...
G4double G4Penelope08BremsstrahlungFS::GetMomentumIntegral(G4double* y,
...
//x is always the gridX
{
  ...
  if (momOrder<-1 || size<2 || theXGrid[0]<0)
    {
      G4cout << "Problem in G4Penelope08BremsstrahlungFS::GetMomentumIntegral" << G4endl;
...
G4PhysicsTable* G4Penelope08BremsstrahlungFS::GetScaledXSTable(const G4Material* mat,
							       G4double cut)
{
...
void G4Penelope08BremsstrahlungFS::InitializeEnergySampling(const G4Material* material,
							    G4double cut)
{  
  std::pair<const G4Material*,G4double> theKey = std::make_pair(material,cut);
...
G4double G4Penelope08BremsstrahlungFS::SampleGammaEnergy(G4double energy,const G4Material* mat, 
							 G4double cut)
{
  if (!theSamplingTable)    
    theSamplingTable = 
  ...
  if (!(theSamplingTable->count(theKey)))
      ...
      if (!(theSamplingTable->count(theKey)) || !(thePBcut->count(theKey)))
	{
	  ...
	  G4Exception("Unable to create the SamplingTable for G4Penelope08BremsstrahlungFS");
  ...
  do
	...
	{
	  G4cout << "Error in G4Penelope08BremsstrahlungFS::SampleGammaEnergy()" << G4endl;
	  G4Exception();
	}
      else
	{
	...
	w1 = wbcut;
      if (w2 < w1)
	{
	  G4cout << "Warning in G4Penelope08BremsstrahlungFS::SampleX()" << G4endl;
