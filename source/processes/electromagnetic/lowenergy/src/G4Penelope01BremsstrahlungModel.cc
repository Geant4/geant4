...
// ********************************************************************
//
// $Id: G4Penelope01BremsstrahlungModel.cc,v 1.8 2010-11-25 09:44:05 pandola Exp $
...
//
#include "G4Penelope01BremsstrahlungModel.hh"
#include "G4Penelope01BremsstrahlungContinuous.hh"
#include "G4Penelope01BremsstrahlungAngular.hh"
...
G4Penelope01BremsstrahlungModel::G4Penelope01BremsstrahlungModel(const G4ParticleDefinition*,
								 const G4String& nam)
  :G4VEmModel(nam),fParticleChange(0),isInitialised(false),energySpectrum(0),
   angularData(0),stoppingPowerData(0),crossSectionHandler(0)
{
  ...
  angularData = new std::map<G4int,G4Penelope01BremsstrahlungAngular*>;
...
G4Penelope01BremsstrahlungModel::~G4Penelope01BremsstrahlungModel()
{
  ...
  if (angularData)
    {
      std::map <G4int,G4Penelope01BremsstrahlungAngular*>::iterator i;
  ...
  if (stoppingPowerData)
    {
      std::map <std::pair<G4int,G4double>,G4Penelope01BremsstrahlungContinuous*>::iterator j;
...
void G4Penelope01BremsstrahlungModel::Initialise(const G4ParticleDefinition* particle,
					       const G4DataVector& cuts)
{
  if (verboseLevel > 3)
    G4cout << "Calling G4Penelope01BremsstrahlungModel::Initialise()" << G4endl;
  ...
  if (verboseLevel > 2)
    G4cout << "Loaded cross section files for Penelope01BremsstrahlungModel" << G4endl;
  
  if (verboseLevel > 0) {
    G4cout << "Penelope01 Bremsstrahlung model is initialized " << G4endl
	   << "Energy range: "
...
G4double G4Penelope01BremsstrahlungModel::MinEnergyCut(const G4ParticleDefinition*,
...
G4double G4Penelope01BremsstrahlungModel::ComputeCrossSectionPerAtom(const G4ParticleDefinition*,
								   G4double kinEnergy,
								   G4double Z,
								   G4double,
								   G4double cutEnergy,
								   G4double)
{
  // Penelope v2001 model to calculate cross section for hard bremsstrahlung emission 
  ...
  if (verboseLevel > 3)
    G4cout << "Calling ComputeCrossSectionPerAtom() of G4Penelope01BremsstrahlungModel" << G4endl;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4double 
G4Penelope01BremsstrahlungModel::ComputeDEDXPerVolume(const G4Material* theMaterial,
						      const G4ParticleDefinition* theParticle,
						      G4double kineticEnergy,
						      G4double cutEnergy)
{
  // Penelope model v2001 to calculate the stopping power (in [Energy]/[Length]) for soft 
  // bremsstrahlung emission (gamma energy < cutEnergy).
  //
  // The actual calculation is performed by the helper class 
  // G4Penelope01BremsstrahlungContinuous and its method CalculateStopping(). Notice:
  ...
  if (!stoppingPowerData)
    stoppingPowerData = new std::map<std::pair<G4int,G4double>,
      G4Penelope01BremsstrahlungContinuous*>;
  ...
  for (size_t iel=0;iel<theMaterial->GetNumberOfElements();iel++)
    {
      ...
      G4Penelope01BremsstrahlungContinuous* theContinuousCalculator = 
...
void G4Penelope01BremsstrahlungModel::SampleSecondaries(std::vector<G4DynamicParticle*>* fvect,
							const G4MaterialCutsCouple* couple,
							const G4DynamicParticle* aDynamicParticle,
							G4double cutG,G4double)
{
  // Penelope model v2001 to sample the final state for hard bremsstrahlung emission 
  // (gamma energy < cutEnergy).
  ...
  //  G4Penelope01BremsstrahlungAngular, method ExtractCosTheta()
  ...
  //
  if (verboseLevel > 3)
    G4cout << "Calling SampleSecondaries() of G4Penelope01BremsstrahlungModel" << G4endl;
  ...
  if (verboseLevel > 1)
    {
      G4cout << "-----------------------------------------------------------" << G4endl;
      G4cout << "Energy balance from G4Penelope01Bremsstrahlung" << G4endl;
  ...
  if (verboseLevel > 0)
    {
      ...
      if (energyDiff > 0.05*keV)
        G4cout << "Warning from G4Penelope01Bremsstrahlung: problem with energy conservation: " <<
...
G4Penelope01BremsstrahlungAngular* G4Penelope01BremsstrahlungModel::GetAngularDataForZ(G4int iZ)
{  
  if (!angularData)
    angularData = new std::map<G4int,G4Penelope01BremsstrahlungAngular*>;
  ...
  G4Penelope01BremsstrahlungAngular* theAngular = new G4Penelope01BremsstrahlungAngular(iZ);
...
G4Penelope01BremsstrahlungContinuous* 
G4Penelope01BremsstrahlungModel::GetStoppingPowerData(G4int iZ,G4double energyCut,
						      const G4ParticleDefinition* 
						      theParticle)
{  
  if (!stoppingPowerData)
    stoppingPowerData = new std::map<std::pair<G4int,G4double>,G4Penelope01BremsstrahlungContinuous*>;
