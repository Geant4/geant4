...
G4Penelope08RayleighModel::G4Penelope08RayleighModel(const G4ParticleDefinition*,
                                             const G4String& nam)
...
G4Penelope08RayleighModel::~G4Penelope08RayleighModel()
{
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
void G4Penelope08RayleighModel::ClearTables()
{
...
void G4Penelope08RayleighModel::Initialise(const G4ParticleDefinition* ,
					 const G4DataVector& )
{
  if (verboseLevel > 3)
    G4cout << "Calling G4Penelope08RayleighModel::Initialise()" << G4endl;
  ...
  if (verboseLevel > 0) {
    G4cout << "Penelope08 Rayleigh model is initialized " << G4endl
	   << "Energy range: "
...
G4double G4Penelope08RayleighModel::ComputeCrossSectionPerAtom(const G4ParticleDefinition*,
							       G4double energy,
							       G4double Z,
							       G4double,
							       G4double,
							       G4double)
{
  // Cross section of Rayleigh scattering in Penelope2008 is calculated by the EPDL97 
  ...
  // et al. J. Phys. Chem. Ref. Data 4 (1975) 471; Erratum ibid. 6 (1977) 615.
   ...
   if (verboseLevel > 3)
    G4cout << "Calling CrossSectionPerAtom() of G4Penelope08RayleighModel" << G4endl;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
void G4Penelope08RayleighModel::BuildFormFactorTable(const G4Material* material)
{
  ...
  */
  ...
  if (MaxStechiometricFactor<1e-16)
    {
      G4cout << "G4Penelope08RayleighModel::BuildFormFactorTable" << G4endl;
...
void G4Penelope08RayleighModel::SampleSecondaries(std::vector<G4DynamicParticle*>* ,
						...
						G4double)
{
  ...
  if (verboseLevel > 3)
    G4cout << "Calling SamplingSecondaries() of G4Penelope08RayleighModel" << G4endl;
...
void G4Penelope08RayleighModel::ReadDataFile(const G4int Z)
{
  if (verboseLevel > 2)
    {
  ...
  */
  ...
  if (!file.is_open())
    {
      G4String excep = "G4Penelope08RayleighModel - data file " + G4String(ost.str()) + " not found!";
      G4Exception(excep);
    }
  G4int readZ =0;
  size_t nPoints= 0;
  ...
  */
  ...
  if (!file.is_open())
    {
      G4String excep = "G4Penelope08RayleighModel - data file " + G4String(ost2.str()) + " not found!";
      G4Exception(excep);
    }
  file >> readZ >> nPoints;
  //check the right file is opened.
...
G4double G4Penelope08RayleighModel::GetFSquared(const G4Material* mat, const G4double QSquared)
{
  ...
  if (verboseLevel > 3)
    {
      G4cout << "G4Penelope08RayleighModel::GetFSquared() in " << mat->GetName() << G4endl;  
...
void G4Penelope08RayleighModel::InitializeSamplingAlgorithm(const G4Material* mat)
{
  G4double q2min = 0;
  G4double q2max = 0;
  ...
  if (np < 16)
    {
      G4cout << "G4Penelope08RayleighModel::InitializeSamplingAlgorithm" << G4endl;
  ...
  //Create an object of type G4Penelope08RayleighSamplingData --> store in a map::Material*
...
void G4Penelope08RayleighModel::GetPMaxTable(const G4Material* mat)
{
  if (!pMaxTable)
    {
...
void G4Penelope08RayleighModel::DumpFormFactorTable(const G4Material* mat)
{
  G4cout << "*****************************************************************" << G4endl;
  G4cout << "G4Penelope08RayleighModel: Form Factor Table for " << mat->GetName() << G4endl;
