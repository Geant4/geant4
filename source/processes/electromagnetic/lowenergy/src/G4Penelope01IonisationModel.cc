...
// ********************************************************************
//
// $Id: G4Penelope01IonisationModel.cc,v 1.18 2010-12-01 15:20:35 pandola Exp $
...
#include "G4Penelope01IonisationModel.hh"
...
G4Penelope01IonisationModel::~G4Penelope01IonisationModel()
{
...
void G4Penelope01IonisationModel::Initialise(const G4ParticleDefinition* particle,
                                       const G4DataVector& )
{
  if (verboseLevel > 3)
    G4cout << "Calling G4Penelope01IonisationModel::Initialise()" << G4endl;
  ...
  if (verboseLevel > 2) 
    G4cout << "Loaded cross section files for Penelope01IonisationModel" << G4endl;

  if (verboseLevel > 2) {
    G4cout << "Penelope Ionisation model v2001 is initialized " << G4endl
	   << "Energy range: "
...
G4double G4Penelope01IonisationModel::CrossSectionPerVolume(const G4Material* material,
                                           const G4ParticleDefinition* theParticle,
                                           G4double energy,
                                           G4double cutEnergy,
                                           G4double)		
{  
  // Penelope model v2001 to calculate the cross section for inelastic collisions above the 
  ...
  if (verboseLevel > 2)
  {
    G4cout << "G4Penelope01IonisationModel " << G4endl;
...
//G4EmCalculator.
G4double G4Penelope01IonisationModel::ComputeCrossSectionPerAtom(const G4ParticleDefinition*,
								 G4double,
								 G4double,
								 G4double,
								 G4double,
								 G4double)
{
  G4cout << "*** G4Penelope01IonisationModel -- WARNING ***" << G4endl;
  G4cout << "Penelope Ionisation model v2001 does not calculate cross section _per atom_ " << G4endl;
...
G4double G4Penelope01IonisationModel::ComputeDEDXPerVolume(const G4Material* material,
                             		   const G4ParticleDefinition* theParticle,
                               		   G4double kineticEnergy,
                               		   G4double cutEnergy)
{
  // Penelope model v2001 to calculate the stopping power for soft inelastic collisions 
  ...
  if (verboseLevel > 3)
    G4cout << "Calling ComputeDEDX() of G4Penelope01IonisationModel" << G4endl;
...
G4double G4Penelope01IonisationModel::MinEnergyCut(const G4ParticleDefinition*,
...
void G4Penelope01IonisationModel::SampleSecondaries(std::vector<G4DynamicParticle*>* fvect,
						    const G4MaterialCutsCouple* couple,
						    const G4DynamicParticle* aDynamicParticle,
						    G4double cutE, G4double)
{
  // Penelope model v2001 to sample the final state following an hard inelastic interaction.
  ...
  if (verboseLevel > 3)
    G4cout << "Calling SamplingSecondaries() of G4Penelope01IonisationModel" << G4endl;
  ...
  if (localEnergyDeposit < 0)
    {
      G4cout << "WARNING-" 
	     << "G4Penelope01IonisationModel::SampleSecondaries - Negative energy deposit"
	     << G4endl;
  ...
  if (verboseLevel > 1)
    {
      G4cout << "-----------------------------------------------------------" << G4endl;
      G4cout << "Energy balance from G4Penelope01Ionisation" << G4endl;
  ...
  if (verboseLevel > 0)
	...
	G4cout << "Warning from G4Penelope01Ionisation: problem with energy conservation: " << 
...
void G4Penelope01IonisationModel::ReadData()
{
  if (verboseLevel > 2)    
    G4cout << "Data from G4Penelope01IonisationModel going to be read " << G4endl;
...
G4double G4Penelope01IonisationModel::CalculateDeltaFermi(G4double kinEnergy ,G4int Z,
							  G4double electronVolumeDensity)
{
...
void 
G4Penelope01IonisationModel::CalculateDiscreteForElectrons(G4double kinEnergy,
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
 ...
 void 
G4Penelope01IonisationModel::CalculateDiscreteForPositrons(G4double kinEnergy,
							   G4double cutoffEnergy,
							   G4int Z,
							   G4double electronVolumeDensity)
{
...
G4double 
G4Penelope01IonisationModel::CalculateCrossSectionsRatio(G4double kinEnergy,
							 G4double cutoffEnergy,
...
G4Penelope01IonisationModel::CrossSectionsRatioForElectrons(G4double kineticEnergy,
							    G4double resEnergy,
...
std::pair<G4double,G4double> 
G4Penelope01IonisationModel::CrossSectionsRatioForPositrons(G4double kineticEnergy,
							    G4double resEnergy,
							    G4double densityCorrection,
							    G4double cutoffEnergy)
{
...
G4double G4Penelope01IonisationModel::ComputeStoppingPowerForElectrons(G4double kinEnergy,
								       G4double cutEnergy,
								       G4double deltaFermi,
								       G4double resEnergy)
{
...
G4double G4Penelope01IonisationModel::ComputeStoppingPowerForPositrons(G4double kinEnergy,
                                            		             G4double cutEnergy,
					                             G4double deltaFermi,
					                             G4double resEnergy)
{
...
G4Penelope01IonisationModel::BuildCrossSectionTable(const G4ParticleDefinition* theParticle)
{
...
G4int G4Penelope01IonisationModel::SampleRandomAtom(const G4MaterialCutsCouple* couple,
						    G4double e) const
{
...
void G4Penelope01IonisationModel::ActivateAuger(G4bool augerbool)
{
  if (!DeexcitationFlag() && augerbool)
    {
      G4cout << "WARNING - G4Penelope01IonisationModel" << G4endl;
