...
// ********************************************************************
//
// $Id: G4Penelope01ComptonModel.cc,v 1.11 2010-12-01 15:20:26 pandola Exp $
...
#include "G4Penelope01ComptonModel.hh"
...
G4Penelope01ComptonModel::~G4Penelope01ComptonModel()
{  
...
void G4Penelope01ComptonModel::Initialise(const G4ParticleDefinition* particle,
					  const G4DataVector& cuts)
{
  if (verboseLevel > 3)
    G4cout << "Calling G4Penelope01ComptonModel::Initialise()" << G4endl;

  InitialiseElementSelectors(particle,cuts);

  if (verboseLevel > 0) {
    G4cout << "Penelope Compton model v2001 is initialized " << G4endl
	   << "Energy range: "
...
G4double G4Penelope01ComptonModel::ComputeCrossSectionPerAtom(
                                       const G4ParticleDefinition*,
                                             G4double energy,
                                             G4double Z, G4double,
                                             G4double, G4double)
{
  // Penelope v2001 model to calculate the Compton scattering cross section:
  ...
  // which is integrated numerically in cos(theta), G4Penelope01ComptonModel::DifferentialCrossSection().
  ...
  //
  if (verboseLevel > 3)
    G4cout << "Calling ComputeCrossSectionPerAtom() of G4Penelope01ComptonModel" << G4endl;
  ...
  if (energy< 5*MeV)
    {
      ...
      G4PenelopeIntegrator<G4Penelope01ComptonModel,G4double (G4Penelope01ComptonModel::*)(G4double)> 
	theIntegrator;
      cs = theIntegrator.Calculate(this,&G4Penelope01ComptonModel::DifferentialCrossSection,
				   -1.0,1.0,1e-05);
    }
  else 
    {
...
void G4Penelope01ComptonModel::SampleSecondaries(std::vector<G4DynamicParticle*>* fvect,
						 ...
						 G4double)
{
  ...
  // Penelope model v2001 to sample the Compton scattering final state.
  // D. Brusa et al., Nucl. Instrum. Meth. A 379 (1996) 167
  ...
  // G4Penelope01ComptonModel::DifferentialCrossSection(). The rejection method is 
  ...
  if (verboseLevel > 3)
    G4cout << "Calling SampleSecondaries() of G4Penelope01ComptonModel" << G4endl;
  ...
  if (localEnergyDeposit < 0)
    {
      G4cout << "WARNING-" 
	     << "G4Penelope01ComptonModel::SampleSecondaries - Negative energy deposit"
	     << G4endl;
  ...
  if (verboseLevel > 1)
    {
      G4cout << "-----------------------------------------------------------" << G4endl;
      G4cout << "Energy balance from G4Penelope01Compton" << G4endl;
  ...
  if (verboseLevel > 0)
	...
	G4cout << "Warning from G4Penelope01Compton: problem with energy conservation: " << 
...
void G4Penelope01ComptonModel::ReadData()
{
  ...
  if (verboseLevel > 2)
    {
      G4cout << "Data from G4Penelope01ComptonModel read " << G4endl;
    }
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

G4double G4Penelope01ComptonModel::DifferentialCrossSection(G4double cosTheta)
{
  //
  // Penelope model v2001 for the Compton scattering differential cross section 
  // dSigma/dOmega.
  // D. Brusa et al., Nucl. Instrum. Meth. A 379 (1996) 167
...
void G4Penelope01ComptonModel::ActivateAuger(G4bool augerbool)
{
  if (!DeexcitationFlag() && augerbool)
    {
      G4cout << "WARNING - G4Penelope01ComptonModel" << G4endl;
