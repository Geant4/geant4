...
//                   by V.Ivanchenko on based of old A.Forti code
//
// **************************************************************
// 
                            ...
                            const G4ParticleDefinition& aParticleType)
{
  ...
  if( !theGenerator ) theGenerator = new G4LowEnergyDeltaGen();
  theGenerator->SetVerbose(verboseLevel);
  theGenerator->Clear();
  theGenerator->Initialize();

  if(verboseLevel > 0) {
    G4cout << "G4LowEnergyIonisationGen is initialized"
           << G4endl;
      }
  ...
  if( !shellCrossSectionHandler ) {
    ...
    shellCrossSectionHandler->LoadShellData("ioni/ion-ss-cs-");
  }
  shellCrossSectionHandler->SetVerbose(verboseLevel);
  ...
  theMeanFreePathData = shellCrossSectionHandler->
                        BuildMeanFreePathForMaterials(tcut, tmax);
                          ...
                          const G4ParticleDefinition& aParticleType)
{
  ...
  tcut.resize(numOfMaterials);
  tmax.resize(numOfMaterials);
  ...
  for (size_t J=0; J<numOfMaterials; J++) {
    ...
    G4double cut = G4std::max(theGenerator->MinSecondaryEnergy(material),
                             (G4Electron::Electron())->GetCutsInEnergy()[J]);
    tcut[J] = cut;

    if(cut == theGenerator->MinSecondaryEnergy(material) 
           || verboseLevel > 0) {
      G4cout<<"G4LowEnergyIonisation: delta-electron Tcut = "
            << cut/keV
    ...
    for (size_t i = 0; i<totBin; i++) {
      ...
      for (G4int iel=0; iel<NumberOfElements; iel++ ) {
        ...
        const G4VEMDataSet* data = shellCrossSectionHandler->ShellDataSet(Z);

	// loop over shells
        G4int nShells = data->NumberOfComponents();
        if(verboseLevel > 1) {
          G4cout << "G4LowEnergyIonisation: find ionisation for Z= " << Z
                 << "; nShells= " << nShells
                 << "; data= " << data
                 << G4endl;              
	}
        for (G4int sh=0; sh<nShells; sh++) {
          G4double e = theGenerator->AverageEnergy(Z, sh, lowEdgeEnergy, cut);
          if(verboseLevel > 2) {
            G4cout << "Z= " << Z << "; shell= " << sh
                   << "; T(MeV)= " << lowEdgeEnergy/MeV
                   << "; Tcut(MeV)= " << cut/MeV
                   << "; Eav(MeV)= " << e/MeV
                   << G4endl;
	  }
          const G4VEMDataSet* comp = data->GetComponent(sh);
          const G4DataVector ener = comp->GetEnergies(0);
          G4double ee = G4std::max(ener.front(),
                        G4std::min(ener.back(),lowEdgeEnergy));
          G4double cs = comp->FindValue(ee);
          if(verboseLevel > 2) {
            G4cout << "comp= " << comp 
                   << "; crossec(barn)= " << cs/barn
                   << "; n= " << theAtomicNumDensityVector[iel]
                   << G4endl;
	  }
