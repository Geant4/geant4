                                                     ...
                                                     const G4double Tcut)
{
  ...
  for (G4int ii = 1; ii < 7; ii++){
    norm += Parms[ii-1]*(saa-sbb)/float(ii) ;
    saa *= aa ;
    sbb *= bb ;
  }
  ...
  if (maxEn >= sndCut){
    ...
    for(G4int kk = 1; kk < 5; kk++){
      ...
      norm += Parms[kk+6]*(saa-sbb)/float(kk);
      saa *= aa;
      sbb *= bb;
    }
  }
  ...
  if(Tcut < fstCut)
  {
    ...
    high= G4std::min(fstCut+BindingEn,maxEn+BindingEn) ; 
      ...
      for(G4int i=1; i<7; i++)
      {
        xsec += Parms[i-1]*(saa-sbb)/G4float(i) ;
        saa *= aa ;
        sbb *= bb ;
      }
  }  
  ...
  if(maxEn >= sndCut)
  {
    ...
    for(G4int i=1; i<5; i++)
    {
      xsec += Parms[i+6]*(saa-sbb)/G4float(i) ;
      saa *= aa ;
      sbb *= bb ;
    }
  }
                                                     ...
                                                     const G4double Tcut)
{
  ...
  //  end of normalization *****************************************
    ...
    high= G4std::min(high,Tlim) ; 
      ...
      for(G4int i=2; i<7; i++)
      {
        w = (saa-sbb)/G4float(i-1) ;
        saa *= aa ;
        sbb *= bb ;
        w += BindingEn*(sbb-saa)/G4float(i) ;
        loss += Parms[i-1]*w ;
      }
