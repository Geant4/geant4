...
// ********************************************************************
//
// $Id: G4Penelope01PhotoElectricModel.cc,v 1.13 2010-11-26 11:51:11 pandola Exp $
...
#include "G4Penelope01PhotoElectricModel.hh"
...
G4Penelope01PhotoElectricModel::~G4Penelope01PhotoElectricModel()
{  
...
void G4Penelope01PhotoElectricModel::Initialise(const G4ParticleDefinition*,
						const G4DataVector& )
{
  if (verboseLevel > 3)
    G4cout << "Calling  G4Penelope01PhotoElectricModel::Initialise()" << G4endl;
  ...
  if (verboseLevel > 2) 
    G4cout << "Loaded cross section files for Penelope01PhotoElectric" << G4endl;

  if (verboseLevel > 0) { 
    G4cout << "Penelope Photo-Electric model v2001 is initialized " << G4endl
	   << "Energy range: "
...
G4double G4Penelope01PhotoElectricModel::ComputeCrossSectionPerAtom(
...
{
  //
  // Penelope model v2001. Use data-driven approach for cross section estimate (and 
...
void G4Penelope01PhotoElectricModel::SampleSecondaries(std::vector<G4DynamicParticle*>* fvect,
...
{
  //
  // Photoelectric effect, Penelope model v2001
  //
  ...
  if (verboseLevel > 3)
    G4cout << "Calling SamplingSecondaries() of G4Penelope01PhotoElectricModel" << G4endl;
  ...
  if (localEnergyDeposit < 0)
    {
      G4cout << "WARNING - "
	     << "G4Penelope01PhotoElectric::PostStepDoIt - Negative energy deposit"
	     << G4endl;
  ...
  if (verboseLevel > 1)
    {
      G4cout << "-----------------------------------------------------------" << G4endl;
      G4cout << "Energy balance from G4Penelope01PhotoElectric" << G4endl;
  ...
  if (verboseLevel > 0)
	...
	G4cout << "Warning from G4Penelope01PhotoElectric: problem with energy conservation: " << 
...
void G4Penelope01PhotoElectricModel::ActivateAuger(G4bool augerbool)
{
  if (!DeexcitationFlag() && augerbool)
    {
      G4cout << "WARNING - G4Penelope01PhotoElectricModel" << G4endl;
...
G4double G4Penelope01PhotoElectricModel::SampleElectronDirection(G4double energy)
{
