...
// ********************************************************************
//
// $Id: G4Penelope01RayleighModel.cc,v 1.8 2010-11-26 11:51:11 pandola Exp $
...
//
#include "G4Penelope01RayleighModel.hh"
...
G4Penelope01RayleighModel::~G4Penelope01RayleighModel()
{
...
void G4Penelope01RayleighModel::Initialise(const G4ParticleDefinition* ,
					   const G4DataVector& )
{
  if (verboseLevel > 3)
    G4cout << "Calling G4Penelope01RayleighModel::Initialise()" << G4endl;


  if (verboseLevel > 0) {
    G4cout << "Penelope Rayleigh model v2001 is initialized " << G4endl
	   << "Energy range: "
...
G4double 
G4Penelope01RayleighModel::CrossSectionPerVolume(const G4Material* material,
						 const G4ParticleDefinition* p,
						 G4double ekin,
						 G4double,
						 G4double)
{
  // Penelope model v2001 to calculate the Rayleigh scattering inverse mean 
  // free path. 
  //
  ...
  if (verboseLevel > 3)
    G4cout << "Calling CrossSectionPerVolume() of G4Penelope01RayleighModel" << G4endl;
  ...
  //
  G4PenelopeIntegrator<G4Penelope01RayleighModel,G4double(G4Penelope01RayleighModel::*)(G4double)> 
    theIntegrator;
  cs =
    theIntegrator.Calculate(this,&G4Penelope01RayleighModel::DifferentialCrossSection,
			    -1.0,0.90,1e-06);
  cs += theIntegrator.Calculate(this,&G4Penelope01RayleighModel::DifferentialCrossSection,
...
//G4EmCalculator.
G4double G4Penelope01RayleighModel::ComputeCrossSectionPerAtom(const G4ParticleDefinition*,
							       G4double,
							       G4double,
							       G4double,
							       G4double,
							       G4double)
{
  G4cout << "*** G4Penelope01RayleighModel -- WARNING ***" << G4endl;
  G4cout << "Penelope Rayleigh model v2001 does not calculate cross section _per atom_ " << G4endl;
...
void G4Penelope01RayleighModel::SampleSecondaries(std::vector<G4DynamicParticle*>* ,
						  const G4MaterialCutsCouple* couple,
						  const G4DynamicParticle* aDynamicGamma,
						  G4double,
						  G4double)
{
  // Penelope model v2001 to sample the Rayleigh scattering final state.
  //
  ...
  if (verboseLevel > 3)
    G4cout << "Calling SamplingSecondaries() of G4Penelope01RayleighModel" << G4endl;
...
G4double G4Penelope01RayleighModel::DifferentialCrossSection(G4double cosTheta)
{
...
G4double G4Penelope01RayleighModel::MolecularFormFactor(G4double x2)
{
...
void G4Penelope01RayleighModel::InitialiseSampling()
{
  ...
  if (!SamplingTable.count(theMaterial)) //material not defined yet
    { 
      ...
      G4PenelopeIntegrator<G4Penelope01RayleighModel,G4double(G4Penelope01RayleighModel::*)(G4double)> 
	theIntegrator;
      G4double sum = theIntegrator.Calculate(this,&G4Penelope01RayleighModel::MolecularFormFactor,
      ...
      for (G4int i=1;i<nPoints;i++)
	{
	  ...
	  sum += theIntegrator.Calculate(this,
					&G4Penelope01RayleighModel::MolecularFormFactor,
					 XlowInte,XhighInte,1e-10);
	  samplingFunction_y->push_back(sum);
	}
...
void G4Penelope01RayleighModel::PrepareConstants()
{
  if (verboseLevel > 3)
    G4cout << "Calling G4Penelope01RayleighModel::PrepareConstants()" << G4endl;
  nPoints=241;
  Xlow=1e-04;
  Xhigh=1e06;
