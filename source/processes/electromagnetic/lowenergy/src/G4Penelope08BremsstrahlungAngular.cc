...
// ********************************************************************
//
// $Id: G4Penelope08BremsstrahlungAngular.cc,v 1.1 2010-12-20 14:11:37 pandola Exp $
// GEANT4 tag $Name: not supported by cvs2svn $
// 
// --------------------------------------------------------------
//
// File name:     G4Penelope08BremsstrahlungAngular
//
...
void G4Penelope08BremsstrahlungAngular::ClearTables()
{
...
void G4Penelope08BremsstrahlungAngular::ReadDataFile()
{
  ...
  if (!path)
    {
      G4String excep = 
	"G4Penelope08BremsstrahlungAngular - G4LEDATA environment variable not set!";
      G4Exception(excep);
      return;
    }
  ...
  if (!file.is_open())
    {
      G4String excep = "G4Penelope08BremsstrahlungAngular - data file " + pathFile + " not found!";
      G4Exception(excep);
    }
  ...
  for (k=0;k<NumberofKPoints;k++)    
      ...
      for (j=0;j<NumberofEPoints;j++)
	{
	  ...
	  if ((iz1-1 == i) && (ik1-1 == k) && (ie1-1 == j))
	    ...
	    {	     
	      G4cout << "G4Penelope08BremsstrahlungAngular: corrupted data file " << 
		pathFile << "?" << G4endl;
	      G4Exception();
	    }
	}
...
void G4Penelope08BremsstrahlungAngular::PrepareInterpolationTables(G4double Zmat)
{
  ...
  if (!dataRead)
    {
      ReadDataFile();
      if (!dataRead)	
	G4Exception("G4Penelope08BremsstrahlungAngular::PrepareInterpolationTables");
    }
...
G4double G4Penelope08BremsstrahlungAngular::SampleCosTheta(G4double Zmat,
							   G4double ePrimary,
							   G4double eGamma)
{
  ...
  if (!(theLorentzTables1->count(Zmat)) || !(theLorentzTables2->count(Zmat)))
    {
      ...
      G4cout << "G4Penelope08BremsstrahlungAngular" << G4endl;
      G4Exception();
    }
