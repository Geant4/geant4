...
G4double G4QAOLowEnergyLoss::EnergyLoss(G4int zpart, 
                                      G4Material* thematerial,
			              G4double kinen) 
				      
{
SetShellMaterial(thematerial->GetName());
G4double pot_ionizz=0;
pot_ionizz = thematerial->GetIonisation()->GetMeanExcitationEnergy();
G4double dedx=0;
G4double v=0;
v= c_light * sqrt( 2 * kinen / proton_mass_c2 );
G4double coeff=0;
coeff= (twopi * proton_mass_c2 * thematerial-> GetTotNbOfElectPerVolume()) / ( electron_mass_c2);
...
G4double G4QAOLowEnergyLoss::GetL0(G4double _enorm0) 
{
...
G4double G4QAOLowEnergyLoss::GetL1(G4double _enorm1)
{
...
G4double G4QAOLowEnergyLoss::GetL2(G4double _enorm2) 
{
