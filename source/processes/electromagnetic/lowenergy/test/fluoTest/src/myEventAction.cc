...
#ifdef G4ANALYSIS_USE
myEventAction::myEventAction(myAnalysisManager* aMgr)
  :drawFlag("all"),sampleCollID(-1),siCollID(-1),hpgeCollID(-1),
   analysisManager(aMgr)
{
  //eventMessenger = new myEventActionMessenger(this);
}
#else
...
void myEventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  mySensorHitsCollection* SiHC = NULL;  
  G4int n_hit = 0;
  G4double totESi=0, totLSi=0; 
  
  if (HCE){ SiHC = (mySensorHitsCollection*)(HCE->GetHC(siCollID));}
  ...
  mySensorHitsCollection* SaHC = NULL;
  G4int n_hit2 = 0;
  G4double totESam=0, totLSam=0; 
    
  if (HCE) SaHC = (mySensorHitsCollection*)(HCE->GetHC(sampleCollID));
  ...
  if (evtNb%printModulo == 0) {
    ...
    G4cout << "\n     " << n_hit2
	   << " hits are stored in MySensorHitsCollection." 
	   << G4endl;  	  
    G4cout << "---> End of event: " << evtNb << G4endl;	    
  }
  HCE = evt->GetHCofThisEvent();  
  //a collection of hits for the HPGeDetector     
  mySensorHitsCollection* HPGeHC = NULL;
  G4int n_hit3 = 0;
  G4double totEHPGe=0, totLHPGe=0; 
    
  if (HCE) HPGeHC = (mySensorHitsCollection*)(HCE->GetHC(hpgeCollID));
  //SaHC now points to SamCollection
