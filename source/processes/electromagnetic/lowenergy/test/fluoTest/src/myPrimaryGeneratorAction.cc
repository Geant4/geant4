  ...
  :Detector(DC),rndmFlag("off"),randomizePrimary("off"),sigmaAngle(0),sigmaMomentum(0)
{
  ...
  gunMessenger = new myPrimaryGeneratorMessenger(this);
...
void myPrimaryGeneratorAction::SetSigmaMomentum(G4double val)
{
  sigmaMomentum = val;
}
...
void myPrimaryGeneratorAction::SetSigmaAngle(G4double val)
{
  sigmaAngle = val;
}
