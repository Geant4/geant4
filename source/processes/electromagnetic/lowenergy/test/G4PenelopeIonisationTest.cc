...
//
//
// $Id: G4PenelopeIonisationTest.cc,v 1.4 2006-06-29 19:44:22 gunter Exp $
...
//
//
//      File name:     G4PenelopeIonisationTest.cc
//
...
//                     Adapted in order to test G4PenelopeIonisation
...
int main()
{
  ...
  AIDA::ITuple* ntuple1 = tpf->create("1","Primary Ntuple",
				      "double eprimary,energyf,de,dedx,pxch,pych,pzch,pch,thetach,costhetach");
  
  // ---- secondary ntuple ------
  AIDA::ITuple* ntuple2 = tpf->create("2","Secondary Ntuple",
				      "double eprimary,px_el,py_el,pz_el,p_el,e_el,theta_el,ekin_el,costheta_el");
  ...
  G4int index = theCouple->GetIndex();
  G4double tCut = (*(cutsTable->GetEnergyCutsVector(1)))[index];

  G4cout << "Cut for delta in " << theCouple->GetMaterial()->GetName() << ": " << tCut/keV << " keV" << G4endl;

  // Processes 
  G4int processType;
  G4cout << "Standard [1] or LowEnergy[2] or Penelope [3] Ionisation?" << G4endl;
  G4cin >> processType;
  ...
  if (processType == 1)
    {
      bremProcess = new G4eIonisation();
      G4cout << "The selected model is Standard" << G4endl;
    }
  else if (processType == 2)
    {
      bremProcess = new G4LowEnergyIonisation();
      G4cout << "The selected model is Low Energy" << G4endl;
    }
  else if (processType == 3)
    {
     
      bremProcess = new G4PenelopeIonisation();
...
//   eProcessManager->
//     SetProcessOrdering(theeminusIonisation,     idxPostStep,3);
...
//   pProcessManager->
//     SetProcessOrdering(theeplusIonisation,     idxPostStep,3);
//   pProcessManager->
//     SetProcessOrdering(theeplusAnnihilation,       idxPostStep,4);
  ...
  // G4LowEnergyIonisation BremstrahlungProcess;
  ...
  G4PenelopeIonisation* LowEProcess =
    (G4PenelopeIonisation*) bremProcess;
  G4LowEnergyIonisation* LowEProcess2 =
    (G4LowEnergyIonisation*) bremProcess;
  G4eIonisation* StdProcess =
    (G4eIonisation*) bremProcess;
  ...
  for (iter=0; iter<nIterations; iter++)
	...
	std::sqrt(pxChange*pxChange + pyChange*pyChange + pzChange*pzChange);
      ...
      //      G4cout << "Stopping power (dE/dx)=" << dedxNow << G4endl;
       ...
       ntuple1->fill(ntuple1->findColumn("costhetach"),std::cos(particleChange->GetMomentumChange()->theta()));
       ntuple1->addRow();
      ...
      for (G4int i = 0; i < (particleChange->GetNumberOfSecondaries()); i++) 
	{
	  ...
	  // for gammas initEnergy=initP
	    ...
	    ntuple2->fill(ntuple2->findColumn("e_el"),e_el/MeV);
	    ntuple2->fill(ntuple2->findColumn("theta_el"),theta_el);
	    ntuple2->fill(ntuple2->findColumn("ekin_el"),eKin_el/MeV);
	    ntuple2->fill(ntuple2->findColumn("costheta_el"),std::cos(particleChange->GetMomentumChange()->theta()));
	    ntuple2->addRow();       
	}
