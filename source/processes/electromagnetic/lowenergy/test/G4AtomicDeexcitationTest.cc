...
int main() { 
  ...
  G4cin>> numberOfRun;
  G4AtomicDeexcitation* deexcitation = new G4AtomicDeexcitation::G4AtomicDeexcitation();
  ...
  for (G4int k=0; k< vectorOfParticles->size();k++)
    {
      G4DynamicParticle* newPhoton = (*vectorOfParticles)[k];
      if ( newPhoton == 0)
	{
	...
	G4ThreeVector photonDiraction =newPhoton ->GetMomentum();
      
      G4double  photonEnergy =newPhoton ->GetKineticEnergy();
