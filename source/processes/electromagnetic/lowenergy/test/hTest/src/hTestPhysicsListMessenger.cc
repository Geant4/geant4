...
void hTestPhysicsListMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  if(command == cutGCmd)
  ...
  if(command == eCmd)
    { hTestList->SetCutsByEnergy(eCmd->GetNewDoubleValue(newValue));}
  if(command == rCmd)
    { hTestList->GetRange(rCmd->GetNewDoubleValue(newValue));}
