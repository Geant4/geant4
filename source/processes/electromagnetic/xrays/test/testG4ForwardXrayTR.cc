...
// and all its terms.
//
// $Id: testG4ForwardXrayTR.cc,v 1.1 1999-01-08 16:33:08 gunter Exp $
...
int main()
{
  ...
  //---write results to the file  hloss -----
   ofstream outFile("XrayTR.cc", ios::out ) ;
...
//--------- Particle definition ---------
  ...
  for ( icut=0; icut<theMaterialTable->length(); icut++)
  {
    ...
    G4cout<<"Just before TR object creation"<<endl ;

  G4ForwardXrayTR theXrayTR  ;

    G4cout<<"Just after TR object creation"<<endl ;
    ...
    ****************************************************************  */
    ...
    G4PhysicsLogVector(G4ForwardXrayTR::GetMinProtonTkin(),
         ...
         if(jMat < iMat)
	 {
		  ...
		  <<(*theMaterialTable)[jMat]->GetName()<<endl<<endl ;
           outFile<<"Lorentz Factor"<<"\t\t"
               <<"energy TR number"<<"\t\t"  
		  <<"theta TR number"<<"\t\t"<<endl<<endl ;  
    ...
    G4cout<<"Start TR energy distribution"<<endl ;
    ...
    outFile<<"TR energy, keV"<<"\t\t"<<"Distribution"<<endl<<endl ;
    for(iLossTR=0;iLossTR<200;iLossTR++)
    {
      outFile<<energyTR[iLossTR]*1000.0<<"\t\t"<<spectrumTR[iLossTR]<<endl ;
      meanLossTR +=energyTR[iLossTR]*spectrumTR[iLossTR] ;
    }
    G4cout<<"Mean TR energy over spectrum = "
        <<meanLossTR*1000./iStatMaxTR<<" keV"<<endl ;
