...
// evenly along the track segment and uniformly into 4pi.
{
  ...
  if (!MPT) return G4VRestDiscreteProcess::PostStepDoIt(aTrack, aStep);
  ...
  if (fEnhancedTimeConstants) {
    if      (MPT->GetProperty(kSCINTILLATIONCOMPONENT3)) N_timeconstants = 3;
    else if (MPT->GetProperty(kSCINTILLATIONCOMPONENT2)) N_timeconstants = 2;
    ...
    if (!Fast_Intensity && !Slow_Intensity)
         return G4VRestDiscreteProcess::PostStepDoIt(aTrack, aStep);
    if (Fast_Intensity && Slow_Intensity) N_timeconstants = 2;
  }
  ...
  if (!fEnhancedTimeConstants) {
    ...
    if (fScintillationByParticleType) {
      ...
      MeanNumberOfPhotons = MPT->GetConstProperty(kSCINTILLATIONYIELD)*fYieldFactor;
      // Birk's correction via fEmSaturation and specifying scintillation by
      // by particle type are physically mutually exclusive
      if (fEmSaturation)
        MeanNumberOfPhotons *= (fEmSaturation->VisibleEnergyDepositionAtAStep(&aStep));
      else
    ...
    if (fScintillationByParticleType) {
      ...
      yield1 = MPT->GetConstProperty(kSCINTILLATIONYIELD1);  // this needs to exist
      yield2 = MPT->ConstPropertyExists(kSCINTILLATIONYIELD2) ?
                  MPT->GetConstProperty(kSCINTILLATIONYIELD2) : 0.;
      yield3 = MPT->ConstPropertyExists(kSCINTILLATIONYIELD3) ?
                  MPT->GetConstProperty(kSCINTILLATIONYIELD3) : 0.;
      // The default linear scintillation process
      // Units: [# scintillation photons / MeV]
      MeanNumberOfPhotons = MPT->GetConstProperty(kSCINTILLATIONYIELD)*fYieldFactor;
      // Birk's correction via fEmSaturation and specifying scintillation by
  ...
  for (G4int scnt=0; scnt<N_timeconstants; ++scnt) {
    ...
    if (!fEnhancedTimeConstants) {
      if (scnt == 0) {
        ...
        else {  /// N_timeconstants != 1  and still scnt == 0
    ...
    else {  // fEnhancedTimeConstants == true
    ...
    for (size_t i=0; i<numPhot; ++i) {
      ...
      if (fScintillationTrackInfo) secTrack->
        SetUserInformation(new G4ScintillationTrackInformation(scintType));
...
{
  if (fIntegralTable1 && fIntegralTable2 && fIntegralTable3) return;

  const G4MaterialTable* materialTable = G4Material::GetMaterialTable();
  size_t numOfMaterials = G4Material::GetNumberOfMaterials();

  // create new physics table
  if (!fIntegralTable1) fIntegralTable1 = new G4PhysicsTable(numOfMaterials);
  if (!fIntegralTable2) fIntegralTable2 = new G4PhysicsTable(numOfMaterials);
  if (!fIntegralTable3) fIntegralTable3 = new G4PhysicsTable(numOfMaterials);
  ...
  for (size_t i=0; i<numOfMaterials; ++i) {
    ...
    if (MPT) {
      ...
      if (!MPV) MPV = MPT->GetProperty(kSCINTILLATIONCOMPONENT1);
      if (MPV) {
      ...
      if (!MPV) MPV = MPT->GetProperty(kSCINTILLATIONCOMPONENT3);
      if (MPV) {
        // Retrieve the first intensity point in vector
        // of (photon energy, intensity) pairs
        G4double currentIN = (*MPV)[0];
...
GetScintillationYieldByParticleType(const G4Track &aTrack, const G4Step &aStep)
{
  ...
  else scintVector = MPT->GetProperty(kELECTRONSCINTILLATIONYIELD);
  ...
  if (PreStepKineticEnergy <= scintVector->GetMaxEnergy()) {
    ...
    G4double Yield2 = scintVector->Value(PreStepKineticEnergy - StepEnergyDeposit);
    ...
    ScintillationYield = 
      scintVector->GetMaxValue()/scintVector->GetMaxEnergy() * StepEnergyDeposit;
  }
...
G4double G4Scintillation::
GetScintillationYieldByParticleType(const G4Track &aTrack, const G4Step &aStep,
                                    G4double &yield1, G4double &yield2, G4double &yield3)
{
  ...
  // below the production cut from neutrons after hElastic
    ...
    yield1 = MPT->GetConstProperty(kIONSCINTILLATIONYIELD1);
    yield2 = MPT->ConstPropertyExists(kIONSCINTILLATIONYIELD2) ?
                MPT->GetConstProperty(kIONSCINTILLATIONYIELD2) : 0.;
    yield3 = MPT->ConstPropertyExists(kIONSCINTILLATIONYIELD3) ?
                MPT->GetConstProperty(kIONSCINTILLATIONYIELD3) : 0.;
  }
  ...
  // and, default for particles not enumerated/listed above
    ...
    yield1 = MPT->GetConstProperty(kELECTRONSCINTILLATIONYIELD1);
    yield2 = MPT->ConstPropertyExists(kELECTRONSCINTILLATIONYIELD2) ?
                MPT->GetConstProperty(kELECTRONSCINTILLATIONYIELD2) : 0.;
    yield3 = MPT->ConstPropertyExists(kELECTRONSCINTILLATIONYIELD3) ?
                MPT->GetConstProperty(kELECTRONSCINTILLATIONYIELD3) : 0.;
  }
  ...
  if (PreStepKineticEnergy <= yieldVector->GetMaxEnergy()) {
    ...
    //G4double Yield2 = yieldVector->Value(PreStepKineticEnergy - StepEnergyDeposit);
    //ScintillationYield = Yield1 - Yield2;
    ...
    // Units: [# scintillation photons]
    ScintillationYield = 
      yieldVector->GetMaxValue()/yieldVector->GetMaxEnergy() * StepEnergyDeposit;
  }
