                                 ...
                                 const G4DataVector&)
{
  ...
  /*
  if(p->GetPDGMass() > MeV) {
    G4cout << "### WARNING: G4UrbanAdjointMscModel model is used for "
  ...
  */
  ...
  //G4cout << "### G4UrbanAdjointMscModel::Initialise done!" << G4endl;
}
...
G4ThreeVector& 
G4UrbanAdjointMscModel::SampleScattering(const G4ThreeVector& oldDirection,
                                  G4double /*safety*/)
{
  ...
  if(cth < 1.0 - 1000*tPathLength/lambda0 && cth < 0.5 &&
    ...
    G4cout << "### G4UrbanAdjointMscModel::SampleScattering for "
  ...
  */
  ...
  /*
  G4cout << "G4UrbanAdjointMscModel::SampleSecondaries: e(MeV)= " << kineticEnergy
  ...
  if (tau >= taubig) { cth = -1.+2.*rndmEngineMod->flat(); }
    ...
    if(xsi < 1.9) { 
      ...
      if(KineticEnergy > 20*MeV && xsi < 1.6) {
        G4cout << "G4UrbanAdjointMscModel::SampleCosineTheta: E(GeV)= "
               << KineticEnergy/GeV 
               << " !!** c= " << xsi
