...
//
//
// $Id: G4GGNNCrossSectionDataTest.cc,v 1.1 2008-11-24 10:53:47 grichine Exp $
...
// Process definition
  ...
  G4double barashXsc, hpwXsc, geishaXsc, neutronhpXsc, xscIn, xscTot, xscEl;
  ...
  for(i = 0; i < iMax; i++)
  {
   ...
   theDynamicParticle = new G4DynamicParticle(theParticleDefinition,
    ...
    if(kinEnergy < 90*GeV)
    {
      xscIn  = barNucIn.GetCrossSection(theDynamicParticle,theElement, 273*kelvin);
      xscTot = barNucIn.GetTotalXsc();
    }
    else
    {
      xscTot = ggXsc.GetCrossSection(theDynamicParticle,theElement, 273*kelvin);
      xscIn  = ggXsc.GetInelasticGlauberGribovXsc();
      
    }
    G4cout << kinEnergy/GeV <<" GeV, \t"<< xscIn/millibarn << " mb \t"
           << xscTot/millibarn << " mb" << G4endl;
    writef << kinEnergy/GeV <<" \t"<< xscIn/millibarn << " \t"
           << xscTot/millibarn << " " << G4endl;
