...
void pipCineSum()
{
  ...
  Double_t massPip = 0.13957; // in GeV
  ...
  simRead.open("pip-C-inelastic.dat");
  ...
  for( i = 0; i < numberOfExpPoints; i++ )
  {
    ...
    Tkin = std::sqrt(Plab*Plab + massPip*massPip) - massPip*massPip;
    Tmin = std::sqrt(Pmin*Pmin + massPip*massPip) - massPip*massPip;
    Tmax = std::sqrt(Pmax*Pmax + massPip*massPip) - massPip*massPip;
  ...
  */
  ...
  gg->SetTitle("#pi^{+}-C inelastic cross-section");
  ...
  */
  ...
  TCanvas* myc = new TCanvas("myc", "#pi^{+}-C inelastic cross-section");
  ...
  gg->GetYaxis()->SetRangeUser(140,480);
