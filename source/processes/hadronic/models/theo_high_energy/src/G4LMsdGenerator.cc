...
G4HadFinalState * G4LMsdGenerator::ApplyYourself(const G4HadProjectile & thePrimary, G4Nucleus &theNucleus)
{
  ...
  if ( fLMsdChannel ) {
     ...
     if ( fLMsdChannel->GetFraction(theNucleus, aPart) > G4UniformRand() )
     {
       //G4cout<<"___G4LMsdGenerator: before Scatter (1) QE=" << theQuasielastic<<G4endl;
       G4KineticTrackVector *result= fLMsdChannel->Scatter(theNucleus, aPart);
       //G4cout << "^^G4LMsdGenerator: after Scatter (1) " << G4endl;
       if (result)
       {
...
//#define DEBUG_initial_result
  	  ...
  	  {
  		  //G4cout << "LMsd secondary, mom " << (*ir_iter)->GetDefinition()->GetParticleName() << " " << (*ir_iter)->Get4Momentum() << G4endl;
...
{                                                                   // Uzhi Nov. 2012
  ...
  if(hitCount != theHighEnergyGenerator->GetWoundedNucleus()->GetMassNumber() )
  {
    ...
    if ( !theTransportResult ) {
       G4cout << "G4LMsdGenerator: null ptr from transport propagate " << G4endl;
  ...
  else
  {
    ...
    if ( !theTransportResult ) {
       G4cout << "G4LMsdGenerator: null ptr from decay propagate " << G4endl;
...
std::pair<G4double, G4double> G4LMsdGenerator::GetEnergyMomentumCheckLevels() const
{
