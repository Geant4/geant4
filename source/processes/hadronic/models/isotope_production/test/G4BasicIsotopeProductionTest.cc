...
// and all its terms.
//
// $Id: G4BasicIsotopeProductionTest.cc,v 1.2 1999-12-15 14:53:01 gunter Exp $
...
#include "../../low_energy/src/G4LENeutronInelastic.cc"
 ...
 int main()
  {
   ...
   G4DummyModel theDummyModel;
//   G4LENeutronInelastic theLEModel;
   theDummyModel.SetMaxEnergy(100*GeV);
   theDummyModel.SetMinEnergy(0*GeV);
   G4cout << "Inelastic instanciated!!!"<<G4endl;
   theInelasticProcess.RegisterMe(&theDummyModel);
//   theInelasticProcess.RegisterMe(&theLEModel);
...
//   G4ParticleChange* aFinalState;
   G4IsoParticleChange* aFinalState;
...
//           incomingEnergy=theEnergies[j];
           ...
           G4cerr << "FINAL EVENTCOUNTER=" << hpw
                ...
                << " in material " << theMaterials[k]->GetZ() <<", "
                << theMaterials[k]->GetA()*mole/g << G4endl;
           if (hpw==debugThisOne)
           {
             debugThisOne+=0;
           }
...
//           aFinalState = (G4ParticleChange*) (theProcesses[i]->PostStepDoIt( *aTrack, aStep ));
           aFinalState = dynamic_cast<G4IsoParticleChange*> (theProcesses[i]->PostStepDoIt( *aTrack, aStep ));
           G4cout << "NUMBER OF SECONDARIES="<<aFinalState->GetNumberOfSecondaries();
           if(aFinalState!=NULL) 
           {
             cout << "ISOTOPE "<<aFinalState->GetIsotope()<<G4endl;
           }
           else
           {
             G4Exception("dynamic_cast did not work --> G4BasicIsotopeProductionTest");
           }
           G4Track * second;
