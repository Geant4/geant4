...
//      cout << energies[i]<<" "<<theCounter[j][i]/1000000.<<" ";
  ...
  ifstream aDataSet("/data/NeutronProductionData/G4NDL0.2//IsotopeProduction/CrossSection/29_65_Copper", ios::in);
  int niso;
  aDataSet >> niso;
  ...
  for(i=0; i<counter; i++)
  {
    if(6*(pictureCount/6) == pictureCount) 
    {
       ...
       cout << "fo/file 66 copper65_"<<pictureCount/6<<".ps"<<endl<<"meta 66 -113"<<endl;
    }
    if(theList[i]=="UNCHANGED") i++;
    for(j=0; j<niso; j++)
    {
    ...
    cout << "title_gl 'isotopes produced by neutrons on 65Cu'"<<endl;
