...
//
//
// $Id: G4StringModelWithChipsTest.cc,v 1.1 2003-10-08 12:36:20 hpw Exp $
...
#include "../src/G4QGSMSplitableHadron.cc"
...
//    G4Fancy3DNucleus * the3DNucleus = new G4Fancy3DNucleus;
    ...
    theTheoModel->SetMaxEnergy(100*TeV);
      ...
      G4cout << "Please enter the sigma in pt [GeV]"<<G4endl;
      G4double spt;
      G4cin >> spt;
      theFragmentation->SetSigmaTransverseMomentum(spt*GeV);
...
//    G4StringInfoDump * theCascade = new G4StringInfoDump(incomingEnergy);
	...
	for( l=0; l<nEvents; ++l ) {
	  ...
	  if( aFinalState->GetStatusChange() == fAlive ) {// current particle is still alive
	    ...
	    ipar = currentDefinition->GetPDGEncoding();     
            qvalue += kineticEnergy;
            if(currentDefinition!=G4Proton::Proton() && currentDefinition!=G4Neutron::Neutron())
              qvalue+=currentDefinition->GetPDGMass();
	  } else if (nSec !=0) {
	    istart = 1;
