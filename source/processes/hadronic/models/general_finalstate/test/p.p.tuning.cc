...
main()
{
  ...
  const double xtot[] = {39.3*millibarn, 38.2*millibarn, 40.0*millibarn};
  const double xel[] = {8.8*millibarn, 7.5*millibarn, 7.0*millibarn};
  ...
  double dPomeron_Gamma = 0.1/GeV/GeV;
  double epsPomeron_Gamma = 0/GeV/GeV;
  ...
  while(!done)
  {
    ...
    while(counter<3)
    {
      theProb->Pomeron_S(3.0*GeV*GeV + epsPomeron_S);
      theProb->Pomeron_Gamma(3.96/GeV/GeV + epsPomeron_Gamma);
      theProb->Pomeron_C(1.4 + epsPomeron_C);
      theProb->Pomeron_Rsquare(3.56/GeV/GeV + epsPomeron_Rsquare);
