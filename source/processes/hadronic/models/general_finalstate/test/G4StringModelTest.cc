...
//
//
// $Id: G4StringModelTest.cc,v 1.1 2003-10-08 12:04:17 hpw Exp $
 ...
 int main()
  {
    ...
    G4TouchableHandle theTouchable(theTouchableV);
   ...
   G4int numberOfParticles = 26;
    G4ParticleDefinition *theParticles[26];
    ...
    G4ProcessManager *theCarbonProcessManager = new G4ProcessManager(G4GenericIon::GenericIonDefinition());
    G4GenericIon::GenericIonDefinition()->SetProcessManager(theCarbonProcessManager);
    G4IonInelasticProcess *theIonInelasticProcess =
      new G4IonInelasticProcess();
    theIonInelasticProcess->RegisterMe( theTheoModel );
    theCarbonProcessManager->AddDiscreteProcess( theIonInelasticProcess );
    theProcesses[25] = theAntiOmegaMinusInelasticProcess;

    G4ParticleDefinition *theCarbon = G4ParticleTable::GetParticleTable()->FindIon(6, 12, 0, 6);
    theParticles[25] = theCarbon;
    ...
    *condition = NotForced;
    ...
    do {
      ...
      G4cout << "24) anti_omega-  25) Carbon "<< G4endl;
      G4cout << "Please enter the particle type code"<< G4endl;
      G4cout << "\tFrom: " << std::flush;
      G4cin >> il;
      G4cout << "\tTo: " << std::flush;
      G4cin >> ir;
    } while( il < 0 || ir > 25 || il > ir );
