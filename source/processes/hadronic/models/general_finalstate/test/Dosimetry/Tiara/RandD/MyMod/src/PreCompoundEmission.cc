...
//
//
// $Id: PreCompoundEmission.cc,v 1.1 2003-10-08 12:32:13 hpw Exp $
...
const PreCompoundEmission & PreCompoundEmission::operator=(const PreCompoundEmission &right)
{
...
G4bool PreCompoundEmission::operator==(const PreCompoundEmission &right) const
{
  return false;
}
...
PreCompoundEmission::PreCompoundEmission()
{
  // Assume that projectile is a proton
}


G4ReactionProduct * PreCompoundEmission::PerformEmission(G4Fragment & aFragment,G4double dLvlDensity)
{
#ifdef debug
    G4Fragment InitialState(aFragment);
#endif
    // Choose a Fragment for emission
    VPreCompoundFragment * theFragment = theFragmentsVector.ChooseFragment();
    if (theFragment == 0)
    {
    ...
    if(aFragment.GetNumberOfParticles() - theFragment->GetA() <
       aFragment.GetNumberOfCharged()-theFragment->GetZ()){
      //Number of charged cannot be greater than number of particles
      return NULL;
    }
    // Kinetic Energy of emitted fragment
    G4double KineticEnergyOfEmittedFragment = theFragment->GetKineticEnergy(aFragment,dLvlDensity);
    
    // Calculate the fragment momentum (three vector)
    G4ThreeVector momentum = AngularDistribution(theFragment,aFragment,KineticEnergyOfEmittedFragment,dLvlDensity);
...
//    aFragment.GetGroundStateMass() + aFragment.GetExcitationEnergy() // initial energy in cm
    ...
    momentum = -momentum + aFragment.GetMomentum().vect();
...
G4ThreeVector PreCompoundEmission::AngularDistribution(VPreCompoundFragment * theFragment,
							 const G4Fragment& aFragment,
							 const G4double KineticEnergyOfEmittedFragment,G4double dLvlDensity) const
{
  ...
  // Fermi energy
  G4double Ef = PreCompoundParameters::GetAddress()->GetFermiEnergy(aFragment);
...
//      theLDP.LevelDensityParameter(aFragment.GetA(),aFragment.GetZ(),U);
  G4double g = 0.595*aFragment.GetA()*dLvlDensity;
...
G4double PreCompoundEmission::rho(const G4double p, const G4double h, const G4double g, 
...
//    }
    ...
    G4double tot = 0.0,t1=1,t2,t3=E-aph;
    for (G4int j = 0; j <= h; j++) 
    {
      t2 = fact[j]/ (fact[G4int(h)-j]*fact[G4int(h)]);
      if (t3 < 0.0) break;
      tot += t1*t2*pow(t3,p+h-1);
      t1 *= -1;
      t3 -= Ef;
    }
...
#ifdef debug
void PreCompoundEmission::CheckConservation(const G4Fragment & theInitialState,
