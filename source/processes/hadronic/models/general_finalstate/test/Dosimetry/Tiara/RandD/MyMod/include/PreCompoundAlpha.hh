...
//
//
// $Id: PreCompoundAlpha.hh,v 1.1 2003-10-08 12:32:12 hpw Exp $
// GEANT4 tag $Name: not supported by cvs2svn $
//
// by V. Lara

#ifndef PreCompoundAlpha_h
#define PreCompoundAlpha_h 1
...
class PreCompoundAlpha : public VPreCompoundIon
{
public:
  // default constructor
  PreCompoundAlpha():VPreCompoundIon(4,2,&theAlphaCoulombBarrier,"Alpha") {}

  // copy constructor
  PreCompoundAlpha(const PreCompoundAlpha &right): VPreCompoundIon(right) {}

  // destructor
  ~PreCompoundAlpha() {}

  // operators  
  const PreCompoundAlpha & operator=(const PreCompoundAlpha &right) {
    if (&right != this) this->VPreCompoundIon::operator=(right);
    return *this;
  };

  G4bool operator==(const PreCompoundAlpha &right) const
...
private:
    ...
    virtual G4double GetBeta(const G4Fragment& fragment)
	{
	    ...
	    Common = -2*z*eplus*eplus/Common;
	    if(z<=10)
	      return Common*0.68*z*0.1;
	    else if(z<=20)
