...
#ifdef DEBUG
 G4cout << " ga " << ga << " v2 " << v2 << " pb " << pb << " pb * pb / pa " << pb * pb / pa
<< " pv " << pv << G4endl;
#endif
pscm = sqrt(pa);
gb = pb / pscm;
...
vector<G4double> G4LorentzConvertor::rotate(const vector<G4double> mom1, const vector<G4double> mom) const {
...
G4double xx = 0.0;
if(v2 > small) xx = 
               pv * (gamma - 1.0) / v2 - bullet_mom[0] * gamma;
for(G4int i = 1; i < 4; i++) {
...
if(v2 < small) {
  ...
  for(G4int i = 1; i < 4; i++) mom1[i] = mom[i] + velocity[i] * xx;
};
#ifdef DEBUG
