...
G4double Fissioner::getC2(G4double A1, G4double A2, G4double X3, G4double X4, G4double R12) const {
  G4double C2 = 124.57 * (1.0 / A1 + 1.0 / A2) + 0.78 * (X3 + X4) - 176.9 *
     (pow(X3, 4) + pow(X4, 4)) + 219.36 * (1.0 / (A1 * A1) + 1.0 / (A2 * A2)) - 1.108 / R12;
  return C2;   
}

G4double Fissioner::getZopt(G4double A1, G4double A2, G4double ZT, 
...
void G4Fissioner::potentialMinimization(G4double& VP, vector<G4double> & ED,
   G4double& VC, G4double AF, G4double AS, G4double ZF, G4double ZS,
