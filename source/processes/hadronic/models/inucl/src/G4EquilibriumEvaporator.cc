...
const G4double prob_cut_off = 1.e-15;
...
const G4double small_ekin = 1.e-6;
const G4int itry_gam_max = 100;

vector<G4double> W(8);
vector<G4double> A1(6);
vector<G4double> Z1(6);
vector<G4double> u(6);
vector<G4double> V(6);
vector<G4double> TM(6);
...
if(G4InuclNuclei* nuclei_target = dynamic_cast<G4InuclNuclei*>(target)) {
  ...
  if(verboseLevel > 1){
	...
	if(EEXS > cut_off_energy) { 
          ...
          pair<vector<G4double>, vector<G4double> > parms = paraMaker(Z);
	  vector<G4double> AK = parms.first;
	  vector<G4double> CPA = parms.second;
...
//                convert back to the lab
                for(G4int i = 0; i < 2; i++) {
		  vector<G4double> mom = nuclea[i].getMomentum();
...
G4bool G4EquilibriumEvaporator::timeToBigBang(G4double a, G4double z, G4double e) const {
...
G4bool G4EquilibriumEvaporator::goodRemnant(G4double a, G4double z) const {
  return a > 1.0 && z > 0.0 && a > z;
}

G4double G4EquilibriumEvaporator::getQF(G4double x, G4double x2, G4double a,
        G4double z, G4double e) const {
...
G4double G4EquilibriumEvaporator::getAF(G4double x, G4double a, G4double z, G4double e) const {
...
G4double G4EquilibriumEvaporator::getPARLEVDEN(G4double A, G4double Z) const {
