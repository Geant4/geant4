...
G4int nrain = 100000; // number of interactions to be generated

// auxiliarly stuff for ugly analysis
G4Analyser* analyser = new Analyser;
G4WatcherGun* gun = new WatcherGun;
gun->setWatchers();
...
G4IntraNucleiCascader* incas = new G4IntraNucleiCascader;
...
G4InuclCollider* inucl = new G4InuclCollider(colep, incas, noneq, eqil, fiss, bigb);
...
G4InuclParticle* bull = new G4InuclElementaryParticle(0.8, 1); 
...
for(G4int i = 0; i < nrain; i++) {
  if((i + 1) % to_report == 0)  
    ...
    G4CollisionOutput output = inucl->collide(bull, targ); // standard method
//  auxiliarly method to get more information about the different stages 
//  of interaction  
//  G4CollisionOutput output = inucl->collideAndTest(bull, targ, analyser);
//
//  G4cout << " ++++++++++++++++++++++++++++++++++++++++++++++++++ " << G4endl;
