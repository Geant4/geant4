...
void G4EXFORHadData::FillDoubleDiffXSC( G4String& fileName)
{
  G4String tmpString, tmpSs1, tmpSs2, tmpSs3, tmpSs4, tmpSs5, tmpSs6;
  size_t posValue, pos1, pos2, pos3, pos4, pos5, pos6, pos7, pos8, pos9;
  size_t position = 0;
  ...
  while( !fin.eof() )  // reading loop from a MSU file to vectors
  {
    ...
    */
    ...
    /*   // don't use the number of data lines
    ...
    */
    ...
    { 
      ...
      pos8 = tmpString.find_first_of(fAnyEmptySpace,pos7);
      pos9 = tmpString.find_first_of(fAnyNumber,pos8);

      //  G4cout<<position<<"\t"<<pos1<<"\t"<<pos2<<"\t"<<pos3<<G4endl; 
  
      tmpSs1 = tmpString.substr(pos3,pos4-pos3);
      tmpSs2 = tmpString.substr(pos5,pos6-pos5);
      tmpSs3 = tmpString.substr(pos7,pos8-pos7);
      tmpSs4 = tmpString.substr(pos9);          

      energy   = G4double(atof(tmpSs1));  
      deltaE   = G4double(atof(tmpSs2));
      xsc      = G4double(atof(tmpSs3));
      deltaXsc = G4double(atof(tmpSs4));        
