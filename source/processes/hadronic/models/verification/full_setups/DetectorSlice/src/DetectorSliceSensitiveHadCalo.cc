...
DetectorSliceSensitiveHadCalo( const G4String name )
  ...
  G4String HCname;
  collectionName.insert( HCname="hadCalCollection" );
}


DetectorSliceSensitiveHadCalo::~DetectorSliceSensitiveHadCalo() {}


void DetectorSliceSensitiveHadCalo::Initialize( G4HCofThisEvent* ) {
  calCollection = 
    new DetectorSliceCalorimeterHitsCollection( SensitiveDetectorName,collectionName[0] );
}


G4bool DetectorSliceSensitiveHadCalo::
ProcessHits( G4Step* aStep, G4TouchableHistory* ) {
  ...
  bool isScintillator = true;     //              true = Quenching in Scintillator
   ...
   if ( isBirksOn ) {
    ...
    if ( isFirstCall ) {
      G4cout << "\t *** DetectorSliceSensitiveHadCalo::ProcessHits *** " << G4endl
  ...
  if ( edep > 0.001*eV ) {
    ...
    if ( analysis ) {
      ...
      analysis->particleDepositingEnergy( "HAD", edep, particlePDG );
    }
  }
...
void DetectorSliceSensitiveHadCalo::EndOfEvent( G4HCofThisEvent* HCE ) {
  static G4int HCID = -1;
  if ( HCID < 0 ) { 
  ...
  if ( ! calCollection->entries() ) {
    //G4cout << " DetectorSliceSensitiveHadCalo::EndOfEvent : DEBUG Info " << G4endl
    //       << "\t NO calorimeter hit! " << G4endl; //***DEBUG***
  }   
}


void DetectorSliceSensitiveHadCalo::clear() {} 


void DetectorSliceSensitiveHadCalo::DrawAll() {} 


void DetectorSliceSensitiveHadCalo::PrintAll() {} 
