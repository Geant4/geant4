...
#endif 
#ifdef G4VIS_USE 
  #include "DetectorSliceVisManager.hh" 
#endif 
#ifdef G4UI_USE_XM 
#include "G4UIXm.hh" 
#endif 
...
int main(int argc,char** argv) { 
  ...
  runManager->SetUserInitialization( new DetectorSliceDetectorConstruction ); 

  QGSP *thePL = new QGSP;    //***LOOKHERE*** 

  //thePL->SetDefaultCutValue( 0.020 *mm ); // 20 microns 
  runManager->SetUserInitialization( thePL ); 
  runManager->SetUserAction( new DetectorSlicePrimaryGeneratorAction ); 
  runManager->SetUserAction( new DetectorSliceRunAction ); 
  runManager->SetUserAction( new DetectorSliceEventAction ); 
  runManager->SetUserAction( new DetectorSliceTrackingAction ); 
  //DetectorSliceAnalysis::getInstance()->setIsHistogramOn( false );
#ifdef G4VIS_USE 
  DetectorSliceVisManager *visManager = new DetectorSliceVisManager; 
