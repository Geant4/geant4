#ifndef RandomCaloStackingAction_H
#define RandomCaloStackingAction_H 1
...
class RandomCaloStackingAction : public G4UserStackingAction {

  public:

  RandomCaloStackingAction();
  virtual ~RandomCaloStackingAction();
  ...
  static inline void switchOn( const G4bool inputSwitchOn = true );
  // Switch on/off the biasing. 
  // This is meant to be used by the PrimaryGeneratorAction, 
  // according to the beam energy of each event. 

  private:

  static G4bool isSwitchedOn;
  // If this variable is "false" then no biasing is applied
  // (i.e. it is like if this class were not used at all.)
  // If instead it is "true", then biasing is applied.
...
inline void RandomCaloStackingAction::switchOn( const G4bool inputSwitchOn ) {
  isSwitchedOn = inputSwitchOn;
}
