...
//
//
// $Id: TargetConstruction.cc,v 1.1 2003-05-27 13:44:49 hpw Exp $
...
G4VPhysicalVolume* TargetConstruction::ConstructTarget()
{
  ...
  //                               
  // Make the target a sensitive detector
  //
  G4SDManager* SDman = G4SDManager::GetSDMpointer();

  if(!targetSD) {
    targetSD = new TargetSD("TargSD",this);
    SDman->AddNewDetector(targetSD);
  }
  if(logicTarg) logicTarg->SetSensitiveDetector(targetSD);
