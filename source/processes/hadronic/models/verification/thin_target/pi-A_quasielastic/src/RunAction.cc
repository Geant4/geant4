...
//
//
// $Id: RunAction.cc,v 1.1 2003-07-31 01:21:46 dwright Exp $
...
RunAction::RunAction(TargetConstruction* targ)
  : loBinEdge(0.), hiBinEdge(500.), binSize(5.), 
...
void RunAction::BeginOfRunAction(const G4Run* aRun)
{
  ...
  if (matName == "Deuterium") {
    theData = new EnergyAngleCrossSection("data/pimquasi821_D.dat");
 
  } else if (matName == "Lithium6") {
    theData = new EnergyAngleCrossSection("data/pimquasi821_Li6.dat");
 
  } else if (matName == "Carbon") {
    theData = new EnergyAngleCrossSection("data/pimquasi821_C.dat");
 
  } else if (matName == "Calcium") {
    theData = new EnergyAngleCrossSection("data/pimquasi821_Ca.dat");
 
  } else if (matName == "Zirconium") {
    theData = new EnergyAngleCrossSection("data/pimquasi821_Zr.dat");
 
  } else if (matName == "Lead") {
    theData = new EnergyAngleCrossSection("data/pimquasi821_Pb.dat");
  ...
  // Set up array of 5 degree wide angle bins
  ...
  for (G4int i = 0; i < (G4int)angles.size(); i++) {
    bin_edge_pair.first = cos(deg_to_rad*(angles[i] - 2.5));
    bin_edge_pair.second = cos(deg_to_rad*(angles[i] + 2.5));
...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  G4cout << " Natoms = " << Natoms 
         << ", Nevents = " << Nevents 
         << ", scale factor = " << scale_factor << G4endl;
