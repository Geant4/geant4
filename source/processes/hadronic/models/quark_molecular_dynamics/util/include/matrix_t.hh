...
template <class t> class mathArrayBase : public arrayBase<t>;
...
template <class t> class Matrix : public mathArrayBase<t>;
template <class t> class Vector : public mathArrayBase<t>;
...
template <class t> class diagonalMatrix : public Matrix<t>;
...
template <class t> class tridiagonalMatrix : public Matrix<t>;
...
template<class t>
tridiagonalMatrix<t>::tridiagonalMatrix(int n,const t& def): Matrix<t>(),N(n)
{ 
  arrayBase<t>::set(3*n,def); 
}
...
template<class t>
t&  tridiagonalMatrix<t>::operator()(int j,int k) 
{ 
  return abs((int)j-(int)k)<=1 ? (*this)[j-1+N*(j-k+1)] : mathType<t>::null; 
}

template<class t>
t  tridiagonalMatrix<t>::operator()(int j,int k) const 
{ 
  return abs((int)j-(int)k)<=1 ? (*this)[j-1+N*(j-k+1)] : mathType<t>::null; 
}
