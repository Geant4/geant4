...
G4int tCascadeInterface(int runId, int nCascades, int bulletType, double eKin, int A, int Z);
...
int main(int argc, char **argv ) {
  ...
  test("Cascade interface", tCascadeInterface(runId, nCollisions, bulletType, eKin, A, Z));
...
int tCascadeInterface(int runId, int nCollisions, int bulletType, double eKin, int A, int Z) {
  G4int verboseLevel = 1;                          
  if (verboseLevel > 3) {
  ...
  G4ParticleDefinition *aParticleDef = 0;
  if(bulletType == proton) {
    aParticleDef = G4Proton::Proton();
  }
  else if(bulletType == neutron) {
    aParticleDef = G4Neutron::Neutron();
  }
  
  G4DynamicParticle aParticle;
  aParticle.SetDefinition(aParticleDef);
  //  aParticle.SetDefinition(aProton);
  aParticle.SetKineticEnergy(eKin);
  aParticle.SetMomentumDirection(0.0, 0.0, 1.0);   
  aParticle.SetMomentum(sqrt(std::pow(eKin, 2) + 2*eKin*938.27));
  ...
  G4CascadeInterface *theCascadeBert        = new G4CascadeInterface();
  ...
  //  G4cout <<"Using projectile type: " << hadProj-> << G4endl;
  G4cout <<"Projectile energy: " << hadProj.GetKineticEnergy() / MeV << " MeV" << G4endl;
  //  G4cout <<"Projectile energy: " << hadProj.GetKineticEnergy() * MeV << " MeV" << G4endl; 

  // Open output datafile
  std::ofstream dataOut("data/data.out");
  dataOut << runId << " " << nCollisions << " " << bulletType << " " << eKin << " "
	  << A << " " << Z << G4endl;
  ...
  for (G4int cascadeID =1 ; cascadeID <= nCollisions; cascadeID++) { 
    ...
    if (runId==1) {
      ...
      o = theCascadeBert->ApplyYourself(hadProj, targetNucleus); // Interface to Bertini
    }
    ...
    G4int type;
    G4int nucA, nucZ;
    G4LorentzVector fourMomentum = 0, projFourMom = 0, targetFourMom = 0;
    projFourMom = hadProj.Get4Momentum();
    G4double projFourMom2 = projFourMom.mag2();
    //    targetFourMom = targetNucleus.Get4Momentum();
//     G4double projFourMom2 = std::pow(projFourMom.t() / MeV,2) - (std::pow(projFourMom.px() / MeV,2)
// 							    + std::pow(projFourMom.py() / MeV,2)
// 							    + std::pow(projFourMom.pz() / MeV,2));
//     G4double targetFourMom2 = std::pow(targetFourMom.e(),2) - (std::pow(targetFourMom.px(),2)
// 								+ std::pow(targetFourMom.py(),2)
// 								+ std::pow(targetFourMom.pz(),2));
    G4double targetFourMom2 = std::pow(targetNucleus.AtomicMass(double(A), double(Z)) / MeV, 2);
    G4double fourMom2 = projFourMom2 + targetFourMom2;
    ...
    for (G4int i =0 ; i < nPart; i++) {
      ...
      fourMomentum = op->Get4Momentum();
//       fourMom2 = fourMom2 - (std::pow(fourMomentum.t() / MeV,2) - (std::pow(fourMomentum.px() / MeV,2)
// 							      + std::pow(fourMomentum.py() / MeV,2)
// 							      + std::pow(fourMomentum.pz() / MeV,2)));
      fourMom2 = fourMom2 - fourMomentum.mag2();
      type=0; // default
      nucA = 0;
      nucZ = 0;
      ...
      if(type == 0) {
	nucA = op->GetDefinition()->GetAtomicMass();
	nucZ = op->GetDefinition()->GetAtomicNumber();
	type = 0;
      }
	...
	baryonNumber = baryonNumber + nucA;
      }
	...
	std::setw(13) << nucA         << // dummy A 
	std::setw(13) << nucZ         << //  dummy Z
...
//     if(std::fabs(fourMom2) > 1.0) {
//       G4cout <<"WARNING: Four momentum conservation problem!" << G4endl;
//       G4cout <<"Total remaining four momentum (squared) = " << fourMom2 << G4endl; 
//     }
    if(verboseLevel > -1) {
    if(baryonNumber != (A + bulletB)) {
      G4cout <<"Baryon number = " << baryonNumber << G4endl;
      G4cout <<"A = " << A << G4endl;
      G4cout <<"Bullet baryon number = " << bulletB << G4endl;
    }
    if (baryonNumber != (A + bulletB)) {
      ...
      //      if(verboseLevel > 2) {
      if(baryonNumber != A + bulletB) {
      for (G4int iSecondary =0 ; iSecondary < nPart; iSecondary++) { 
