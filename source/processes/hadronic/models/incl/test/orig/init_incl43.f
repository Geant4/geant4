...
C Input for INCL, to be known at the CALL PNUC 
...
cjcd
      ...
      write(io,*)'*****************************************************'
      write(io,*)'*    Version prepared for  4.3 on CVS    4/2006     *'
      write(io,*)'* Clusters out as in the paper or coulomb barrier   *'
      write(io,*)'*           at R0 + rms(clust) + parameter          *'
      write(io,*)'*****************************************************'
...
C                                                                       P-N02800
     ...
     s                                                 ,NPAULSTR
      write(io,*) ' '
      write(io,*) ' R space nuclear density NOSURF=  ',NOSURF,'  means'
...
C      write(io,*) '            K6=0, NO ANGULAR MOMENTUM CONSERVATION'
      IF(INT(F(10)+0.1).EQ.1) THEN
      ENDIF
      IF(INT(F(11)+0.1).NE.0) 
     s  write(io,*) '  Calculation of LIGHT CLUSTERS selected:',F(11)
      write(io,*)' '
...
C parametres moyens de densite de la cible (fermi 2 parametres)
	...
	RMAXWS=R0+2.5
      END IF
      DRWS = RMAXWS/29.
1806  FORMAT(' Nuclear pot:',F8.2,' Mev',/,
     s ' Fermi momentum and energy',F8.2,' MeV/c',F8.2,' MeV')
...
1792  FORMAT('FOR composite beams, BMAX=RMAX+rmsCOMPOSITE',/,
     s'   example d/t/3He/4He: ',4F10.3)
      write(io,*) ' '
c      write(io,1790)
