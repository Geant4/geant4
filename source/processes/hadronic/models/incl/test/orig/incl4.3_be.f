c      ###################################################################
c      04/2006 version INCL4.3 prete a l'usage
c      ###################################################################
c
c--------------------------------------------------------------------------------
c Version pour developpement contenant:				INCL4.3
C   Test supprime sur premiere collision (CDPP OK dans tous les cas N in , pi in)
C   pf1t(8) -> pf1n(8) (idem (9)) ligne 640
c   Pas de reflexions avant au moins un avatar du nucleon incident (comme 4.2)
C   Commentaire de k=0 Go To 255 pour calculer le dernier avatar...
C   Sub BARR modifiee pour les clusters
C   Version avec cluster optionnel
C   Delta final en energie d'excitation si bloque par CDPP
C   Nouveau BARR relativiste (propose par JC et PH). 
C   Temps d'arret des pions de Thibault Guillemin (Juin 2005)
C--------------------------------------------------------------------------------
C
...
C                temps d'arret pi de Thibault Guillemin si NOSURF=-2        5/2005
C   Sortie des 3 composantes du moment angulaire	AB 2/2004
C
C   Version for heavy ions; The spectator nucleons of the projectile are
C      combined as a nucleus in the COMMON/PROJ_SPECT to be treated outside
C      by a Fermi breakup.
C      The cascade is stopped when interacting nucleons (IA1+IA2-A_PROJSPEC)
C      minus extracted ones (nbquit) is smaller than a given value (IA_BREAKUP=f(12))
C      for a treatment by the Fermi-breakup.                         AB  7/2008
C      New developments (definition and energy of the beam spactator) AB 5/2009
        subroutine pnu(IBERT,F,NOPART,KIND,EP,ALPHA,BETA,GAM,IZREM,
     -           IAREM,ESREM,ERECREM,ALREM,BEREM,GAREM,BIMPACT,
     s           L,XL1,XL2,XL3)
C                                                                       P-N00010
...
C						and studied also for pions.                                 
C                               In all options, it is always possible to renormalize the 
C				stopping time by the factor F(6): t=t0*F(6)
...
C    F(10)= Cluster option
C    F(11)= 0 only p,n,pi output(standard)/
C           1 light clusters in addition computed as in Nuc. Phys A740 (2004) 195.
C           2 light clusters as above except coulomb barrier = R0 + rms(clust) + parameter
c
...
C           In this version extended to large composite projectiles, KIND is
C           negative for projectile spectators (not entering the potential)
C
...
C name of the version from INCL (subroutine pnu) for print
      CHARACTER*80 versionname
      COMMON/vname/versionname
...
C Spectator of the projectile:
      INTEGER A_PROJSPEC,Z_PROJSPEC,N_PROJSPEC(MAX_A_PROJ)
      REAL*4 M_PROJSPEC
      COMMON/PROJ_SPECT/A_PROJSPEC,Z_PROJSPEC,EX_PROJSPEC,T_PROJSPEC,
     s P1_PROJSPEC,P2_PROJSPEC,P3_PROJSPEC,M_PROJSPEC
      COMMON/QUATVECTPROJO/eps_c(MAX_A_PROJ),p3_c(MAX_A_PROJ),
     s  p1_s(MAX_A_PROJ),p2_s(MAX_A_PROJ),p3_s(MAX_A_PROJ),
     s  t_c(MAX_A_PROJ)
c (eps_c,p1_s,p2_s,p3_s,eps_c used to store the kinematics of nucleons
C for composit projectiles before entering the potential)
      DOUBLE PRECISION fja,fjz,ex2,rnmass,um
...
c     DIMENSION LTT(15)                                                 P-N00520
     ...
     s            CP0,CP1,CP2,CP3,CP4,CP5,CP6
      ...
      DATA LINE/132*1H*/
      DATA HC,FMP,FMD,FMPI/197.328,938.2796,1232.,138.00/ 
...
C******************************************************************** 
      DATA versionname/' This is INCL4.3 (4/2006) HI beams (2007-2008)'/       
C********************************************************************               
...
C     Kgel=F(10) (NOT USED!)
C     Kclst=F(11) Light clusters computed if not =0
C                                                                       P-N02090
...
c	endif
        ...
        ibw=-1
c        ibw=16872   !No d'evenement ibert pour writes de verif
...
c deutons
      ...
      ELSE
      ia1=ABS(kindf7)
C Check here that kindf7 is -A and ia1 is A of the projectile.
c      			WRITE(6,*) 'kindf7,ia1:',kindf7,ia1
      IA_BE=ia1
      iz1=IZ_BE
      fmpinc=ia1*FMP - BIND_BE
      rms1=RMS_BE
      pf1=PMS_BE
      IA_BREAKUP=int(f(12)+0.1)
      ENDIF
...
c deutons CV 22/01/2001
      if (kindf7.lt.6.AND.kindf7.gt.0)  then
       bmax=bmax     ! nucleon and pion IN
      else
         beproj=fmpinc-ia1*fmp
         bmax=rmaxws+rms1      ! maximum extension for composite proj.
c	 bmax=bmax*1.3		! augmentation artificielle pour IL
      endif   
c deutons     
       IRUN=0                                                            P-N03890
      call ribm(al,ial)
...
c        temfin=25.5*ia2**0.16  ! Pb208->60fm/c
c modified in June 2005 (function of A and TLAB)
         temfin = 30.18*ia2**0.17*(1-5.7*10**(-5)*tlab)
      	   ELSE
...
C Deuteron density from Paris potential in q space:      
      ...
      ELSE
C Composite heavier than a deuteron:
      s1t1=0.
      s2t1=0.
      s3t1=0.
      sp1t1=0.
...
C Make the choice here (by hand...)      
      IGAUSSFERMI=2
      IF(IGAUSSFERMI.EQ.1) THEN
C Density of composite as a gaussien in q space:
      call rgauss(xga)
      ...
      ELSE
C Density of composite as a Fermi sphere in q space:
      DO iloc=1,3
        CALL RIBM(T(iloc),IY14)
      ENDDO
      T(2)=-1+2.*T(2)
      T(3)=6.283185*T(3)
      T2=T(2)
...
C for rho(r),rho(q) checking (in the projectile rest frame)
C (here BEFORE the change to keep sum(vect_r)=sum(vect_p)=0)
      V_PROJ=0.
      IF(ia1.GT.4) V_PROJ=(sei-fmpinc)/ia1
      DO i=1,ia1
      r_dist = SQRT(X1(I)*X1(I)+X2(I)*X2(I)+X3(I)*X3(I))
      p_mod = SQRT(P1(I)*P1(I)+P2(I)*P2(I)+P3(I)*P3(I))
      unbound=V_PROJ-SQRT(p_mod**2+fmp**2)-fmp
      CALL HFILL(9,r_dist,0.,1.)
      CALL HFILL(10,p_mod,0.,1.)      
      unbound=V_PROJ-(SQRT(p_mod**2+fmp**2)-fmp)
      IF(unbound.LT.0.) CALL HFILL(26,p_mod,0.,1.)
      ENDDO
      ...
      sei=sei+eps(i)
      ENDDO
C Potential of the projectile:
      V_PROJ=0.
      IF(ia1.GT.4) V_PROJ=(sei-fmpinc)/ia1
c      WRITE(6,*) 'V_PROJ= ',V_PROJ
      CALL HFILL(28,V_PROJ,0.,1.)
        
C Verif des energies, impulsions pour composite,M,ES,PINC OK:
c      SEloc=0
c      SP1loc=0.
c      SP2loc=0.
c      SP3loc=0.
c      DO iloc=1,ia1
c      IF(iloc.NE.ilm) THEN
c         SEloc=SEloc+eps(iloc)
c	 SP1loc=SP1loc+p1(iloc)
c	 SP2loc=SP2loc+p2(iloc)
c	 SP3loc=SP3loc+p3(iloc)
c	 WRITE(6,*) 'i,e,p:',iloc,eps(iloc),p1(iloc),p2(iloc),p3(iloc)
c      ELSE
c            var_ab=p1(ilm)**2+p2(ilm)**2+p3(ilm)**2
c             GPSG=0.
c      IF(var_ab.GT.0.)
c     s       GPSG=SQRT(((EPS(ilm)-V0)**2-FMP*FMP)/var_ab)                 
c      SP1loc=SP1loc+GPSG*P1(ilm)                                              
c      SP2loc=SP2loc+GPSG*P2(ilm)                                        
c      SP3loc=SP3loc+GPSG*P3(ilm)                                         
c      SEloc=SEloc+EPS(ilm)-V0 
c	 WRITE(6,*) 'i,e,p:',eps(ilm)-V0,
c     s                 GPSG*p1(ilm),GPSG*p2(ilm),GPSG*p3(ilm)
                                            
c      ENDIF
c      ENDDO
c      PLOC=SQRT(SP1loc**2+SP2loc**2+SP3loc**2)
c      FMLOC=SQRT(SEloc**2-PLOC**2)
c      TLOC=SQRT(PLOC**2+FMLOC**2)-FMLOC
c      WRITE(6,*) 'IN the composit REST FRAME (before rearrange):'
c      WRITE(6,*) 'ES,PS,PS1,PS2,PS3:',SEloc,PLOC,SP1loc,SP2loc,SP3loc 
c      WRITE(6,*) 'M,PINC,MLOC,TLOC:',fmpinc,pinc,FMLOC,TLOC
c      WRITE(6,*) ''
C END Verif des energies, impulsions pour composite
	
      ENDIF
    9 continue
    
C for rho(r),rho(q) checking (in the projectile rest frame)
C (here AFTER the change to keep sum(vect_r)=sum(vect_p)=0)
      DO i=1,ia1
      r_dist = SQRT(X1(I)*X1(I)+X2(I)*X2(I)+X3(I)*X3(I))
      p_mod = SQRT(P1(I)*P1(I)+P2(I)*P2(I)+P3(I)*P3(I))
      unbound=V_PROJ-SQRT(p_mod**2+fmp**2)-fmp
      CALL HFILL(24,r_dist,0.,1.)
      CALL HFILL(25,p_mod,0.,1.)
      unbound=V_PROJ-(SQRT(p_mod**2+fmp**2)-fmp)
      IF(unbound.LT.0.) CALL HFILL(27,p_mod,0.,1.)
      ENDDO
...
C Random azimuthal direction of the impact parameter (sept 99)
      ...
      TBID=TBID*6.283185
        ...
        X3(1)=X3(1)-Z
C Counter of p(n) entering the potential 4/2008 AB
        IF(ind2(1).EQ.1) THEN
        npenter=1
        nnenter=0
        ELSE
...
C Keep memory for later calculation of the spect-proj excitation energy
	t_c(i)=eps(i)-fmp
        eps(i)=eps(i)-V_PROJ
      eps_c(i)=eps(i)
	p3_c(i)=p3(i)
...
C Verif des energies, impulsions pour composite,M,ES,PINC OK:
      IF(IBERT.EQ.IBW) THEN
      WRITE(6,*) 'IN the composit REST FRAME:',ibert
      SEloc=0
      SP1loc=0.
      SP2loc=0.
      SP3loc=0.
      DO iloc=1,ia1
c      IF(iloc.NE.ilm) THEN
      SEloc=SEloc+eps(iloc)
	 SP1loc=SP1loc+p1(iloc)
	 SP2loc=SP2loc+p2(iloc)
	 SP3loc=SP3loc+p3(iloc)
	 WRITE(6,*) 'i,e,p:',iloc,eps(iloc),p1(iloc),p2(iloc),p3(iloc)
c      ELSE
c            var_ab=p1(ilm)**2+p2(ilm)**2+p3(ilm)**2
c             GPSG=0.
c      IF(var_ab.GT.0.)
...
c      ENDIF
      ENDDO
      PLOC=SQRT(SP1loc**2+SP2loc**2+SP3loc**2)
      FMLOC=SQRT(SEloc**2-PLOC**2)
      TLOC=SQRT(PLOC**2+FMLOC**2)-FMLOC
      WRITE(6,*) 'ES,PS,PS1,PS2,PS3:',SEloc,PLOC,SP1loc,SP2loc,SP3loc 
      WRITE(6,*) 'M,PINC,MLOC,TLOC,ilm:',fmpinc,pinc,FMLOC,TLOC,ilm
      WRITE(6,*) ''
      ENDIF
C END Verif des energies, impulsions pour composite
...
C Verif
c	WRITE(6,*) 'Transfo lorentz:B1,G1',B1,G1
c	E_LAB=G1*FMLOC
c	P3_LAB=G1*B1*FMLOC
c	WRITE(6,*) 'E_LAB,P3_LAB',E_LAB,P3_LAB
c	WRITE(6,*) ''
C Verif des energies, impulsions pour composite,M,ES,PINC OK:
      IF(IBERT.EQ.IBW) THEN
      WRITE(6,*) 'IN the LAB FRAME:',ibert
      SEloc=0
      SP1loc=0.
      SP2loc=0.
      SP3loc=0.
	 ...
	 SP2loc=SP2loc+p2(iloc)
	 SP3loc=SP3loc+p3(iloc)
       WRITE(6,*) iloc,eps(iloc),p1(iloc),p2(iloc),p3(iloc)
      ENDDO
      PLOC=SQRT(SP1loc**2+SP2loc**2+SP3loc**2)
      FMLOC=SQRT(SEloc**2-PLOC**2)
      TLOC=SQRT(PLOC**2+FMLOC**2)-FMLOC
      WRITE(6,*) 'ES,PS,PS1,PS2,PS3:',SEloc,PLOC,SP1loc,SP2loc,SP3loc 
      WRITE(6,*) 'M,PINC,MLOC,TLOC:',fmpinc,pinc,FMLOC,TLOC
      WRITE(6,*) ''
      ENDIF
C END Verif des energies, impulsions pour composite
C Fin VERIF
      ...
      GO TO 987
C With a potential for the projectile it is nonsens to search for 
C nucleons ON shell  31/05/2010.      
...
C ici sauvegarde des energies et impulsions des nucleons du projectile I.L.:
C (hors potentiel)
      DO I=1,IA1
        eps_c(i)=eps(i)
	p1_s(i)=p1(i)
...
c First particle entering in the target potential:	
        eps(ilm)=eps(ilm)+v0
	  
C CORRECTION EN TEST 16/04/2009 (Pourquoi pas la correction aussi de l'impulsion
C comme pour un nucleon IN ???? ....On essaye de traiter les nucleons hors couche?)
C 29/4/2009: Je pense que c'est une erreur et je transforme aussi maintenant l'impulsion:
            var_ab=p1(ilm)**2+p2(ilm)**2+p3(ilm)**2
      IF(var_ab.GT.0.)
     s       GPSG=SQRT((EPS(ilm)**2-FMP*FMP)/var_ab)
             P1(ilm)=P1(ilm)*GPSG                 
             P2(ilm)=P2(ilm)*GPSG                 
             P3(ilm)=P3(ilm)*GPSG                 
...
1871  CONTINUE
C for rho(q) checking (in the projectile rest frame)
      DO I=1,IA1
      P3_c(I)=g1*(P3(I)-b1*eps(I))
      p_mod = SQRT(P1(I)**2+P2(I)**2+P3_c(I)**2)
c      CALL HFILL(10,p_mod,0.,1.)
      ENDDO
c deutons
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'First nuc entering the pot: ilm, eps (inside)',
     s   ilm,eps(ilm)
      ENDIF
...
c      SP3loc=0.
c      DO iloc=1,ia1
c         SEloc=SEloc+eps_c(iloc)
c	 SP1loc=SP1loc+p1_s(iloc)
c	 SP2loc=SP2loc+p2_s(iloc)
c	 SP3loc=SP3loc+p3_s(iloc)
c	 WRITE(6,*) 'i,e,p:',eps_c(iloc),p1_s(iloc),p2_s(iloc),p3_s(iloc)
c      ENDDO
c      PLOC=SQRT(SP1loc**2+SP2loc**2+SP3loc**2)
c      FMLOC=SQRT(SEloc**2-PLOC**2)
c      TLOC=SQRT(PLOC**2+FMLOC**2)-FMLOC
c      WRITE(6,*) 'In the LAB (apres rearangement):'
c      WRITE(6,*) 'ES,PS,PS1,PS2,PS3:',SEloc,PLOC,SP1loc,SP2loc,SP3loc 
c      WRITE(6,*) 'M,PINC,MLOC,TLOC,ilm:',fmpinc,pinc,FMLOC,TLOC,ilm
c      WRITE(6,*) ''
C END Verif des energies, impulsions pour composite
...
C Counting the spectators and transparents of composit projectiles:      
      ips=0
      NB_TRANSPROJO=0
...
C Pour garder trace des spectateurs du projectile:
      npproj(i)=1
...
c incoming nucleons enter potential at maximum radius (modif. 13/06/01)
      ...
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) ' Testing projo-particle L,EPS(L),T5,T3,TMAX5:',
     s I,EPS(I),T5,T3,TMAX5
      ENDIF
      
      IF (T5.LT.0.) THEN
c This is a projectile spectator:
      ips=ips+1
      N_PROJSPEC(ips) = i      
      GO TO 38
      ENDIF
      ...
      IF (TREF.GT.TMAX5) THEN
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) ' Particle entering AFTER Tstop L,EPS(L):',I,EPS(I)
      ENDIF
c This is also a projectile spectator:
      ips=ips+1
      N_PROJSPEC(ips) = i            
      GO TO 38
      ENDIF
C Test to avoid a nucleon entering in the past (3/06/2010 as in INCL4.6 A.B.)
      IF(TREF.LE.0.) THEN
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) ' Particle entering in the past L,EPS(L):',I,EPS(I)
      ENDIF
c This is also a projectile spectator:
      ips=ips+1
      N_PROJSPEC(ips) = i            
      ...
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) '     Clock when it enters L,TREF:',I,TREF
      ENDIF      
   ...
   37 K=K+1                                                            
      ...
      JND(K)=-1
   38 continue
c For composit beams, cannot decide that there is no interaction,
c   (another nucleon can enter later at RMAX).
      kcol=1
      
      N_ACTIVNUC = IA-ips	!number of active nucleons (not missing the target)
      N_enter_pot = IA1-ips   ! Number of projectile nucl which can enter
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'N_ACTIVNUC,N_enter_pot,ips:',
     s N_ACTIVNUC,N_enter_pot,ips
      ENDIF      
...
C Here transparent event (no interaction avatar found)
      nopart=-1                                                         P-N04970
...
C Here, stoping condition for heavy (light) ions:
      IF(kindf7.LT.0) THEN
         IF(N_ACTIVNUC-nbquit.LE.IA_BREAKUP) THEN
         IF((N_enter_pot-npenter-nnenter).LE.0) THEN
       IF(IBERT.EQ.ibw) THEN
	 WRITE(6,*) 'Stopped cascade for Fermi breakup, TIM:',IBERT,TIM
       WRITE(6,*) 'N_enter_pot,npenter,nnenter',
     s   N_enter_pot,npenter,nnenter
       ENDIF
	   GO TO 255
         ENDIF
	   ENDIF
...
C*****************************************************
C    POUR TEST LE 20/3/2003... Tue le dernier avatar?
       if (k.eq.0) go to 255
...
C Info delta ou nucleon:
		...
		DEL1avat(Iavat)=IND1(L1)
	      ...
	      ENDIF 
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'Collision tried L1,EPS(L1),L2,EPS(L2):',
     s   L1,EPS(L1),L2,EPS(L2)
      ENDIF                                              
...
C       IF(NCOL_2C.EQ.1) THEN
c       do icomp=1,ia1
C Test on the first collision modified 4/07/2001 for direct AND exchange.
C Test suppressed 5/2005( wrong for for incoming pions)(AB)
c         IF(icomp.EQ.L1.OR.icomp.EQ.L2) THEN
c           Xavant=MIN(T(24),T(28))
c           Xapres=MIN(EPS(L1),EPS(L2))             
c           IF(Xapres.LE.Xavant) THEN
c        	IF(Kveux.EQ.1) Bloc_CDPP(Iavat)=1
c	   GO TO 248
c	   ENDIF
c         ENDIF
c       enddo
C pour le ntuple des avatars:
C        	IF(Kveux.EQ.1) THEN
C		R1_first_avat(1)=X1(1)
C		R1_first_avat(2)=X2(1)
C		R1_first_avat(3)=X3(1)
C		ENDIF
C       ELSE
...
c trouble with forward scattering 22/3/95
      ...
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'Collision accepted L1,EPS(L1),L2,EPS(L2),',
     s'IND1(L1),IND1(L2):',L1,EPS(L1),L2,EPS(L2),IND1(L1),IND1(L2)
      ENDIF                                              
...
c      write(6,*)'ligne 1392', io,rrrr,rcorr
      ...
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'Valid decay of Delta L1,EPS(L1),NPION,Q4(NPION):',
     s   L1,EPS(L1),NPION,Q4(NPION)
      ENDIF                                              
...
C Ce nucleon (ici delta) devient un participant:
      ...
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'Pi-N interaction LP,Q4(LP),L2,EPS(L2),IND1(L2):',
     s   LP,Q4(LP),L2,EPS(L2),IND1(L2)
      ENDIF                                              
...
c deutons pas bien compris ici CV ?
     ...
     s      GPSG=SQRT(((EPS(L1)+v0)**2-PM2)/var_ab)    
      ...
      nesc(l1)=0
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'Nucleon entering the pot L1,EPS(L1) (inside):',
     s   L1,EPS(L1)
      ENDIF                                              
c      write (6,*) 'rentree'
c      go to 607    
...
608   CONTINUE   
  ...
  605 FM=FMP                                                            P-N11640
      POT=V0                                                            P-N11650
  606 TP=BARR(EPS(L1)-FM,IND2(L1),1,ITCH,R2,V0)                        
...
C Commande clusters ou non (1/0)! 
      IF(Kclst.EQ.0) GO TO 1610
C-------------------------------------------------------------------
C Ici la particule L1 s'echappe du noyau:
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'Particle can escape L1,EPS(L1),IND1(L1):',
     s   L1,EPS(L1),IND1(L1)
      ENDIF                                              

      IF (IND1(L1).EQ.1) GO TO 1610
      
C Appel de la recherche des clusters (Deltas exclus):
      CALL CLUSTER(L1,ITCH,V0,IOK,KINDCLST,ID_NUC_CLST,
     s                   EOUT,TOUT,POUT,LOUT,Kclst)
      
      IF (IOK.EQ.1) THEN	!cluster OK
      	       NB_CLST = NB_CLST + 1
	       K_CLST(NB_CLST)=KINDCLST
	       E_CLST(NB_CLST)=EOUT
	       T_CLST(NB_CLST)=TOUT
	       
	   DO iloc=1,3
	       POUT_CLST(iloc,NB_CLST)=POUT(iloc)
	       LOUT_CLST(iloc,NB_CLST)=LOUT(iloc)
	   END DO
	   
	       nb_nuc_clst = 0
	   DO iloc=1,4
	       NO_NUC = ID_NUC_CLST(iloc)
	       ID_CLST(iloc,NB_CLST) = NO_NUC
	       IF (NO_NUC.NE.0) THEN
	       		NESC(NO_NUC) = 2
			nb_nuc_clst = nb_nuc_clst + 1
			IF (NO_NUC.NE.L1) THEN
C On retire les nucleons autre que L1 des tableaux des avatars.
C    (Le decalage en temps (TAU) se fera en traitant L1)
                 IF (K.EQ.0) GO TO 1607                                     
      		    KD=0                                                          
	         DO kloc=1,K                                               
      		   I20=kloc-KD                                                      
      		   IF(JND(kloc).EQ.NO_NUC) GO TO 1604                              
      		   IF(IND(kloc).EQ.NO_NUC.AND.JND(kloc).NE.0) GO TO 1604                
      		   CROIS(I20)=CROIS(kloc)                                  
      		   IND(I20)=IND(kloc)                                               
      		   JND(I20)=JND(kloc)                                              
      		   GO TO 1603                                                   
 1604 		   KD=KD+1                                                     
 ...
 1607            CONTINUE			                                                     
	 ...
	 nbquit = nbquit + nb_nuc_clst
	 ITCH = ITCH - ((KINDCLST-6)/2 + 1)
	  
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'a composite escapes L list,EOUT,KINDCLST:',
     s  (ID_NUC_CLST(iloc),iloc=1,4),EOUT,KINDCLST
      ENDIF                                              
	 
	 GO TO  610 
		      
      ENDIF			!sortie de la particule (nucleon ou delta)
1610  NESC(L1)=1                                                        P-N11730
      nbquit=nbquit+1
     ...
     s       GPSG=SQRT(((EPS(L1)-POT)**2-FM*FM)/var_ab)                 P-N11750
      ...
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'Escaping particle L1,EPS(L1)outside,IND1(L1):',
     s   L1,EPS(L1),IND1(L1)
      ENDIF                                              
C Test for a transparent nucleon for composit beams and stored for FermiBreakup
      IF(ia1.GT.1.AND.NC(L1).EQ.0) THEN
      ips=ips+1
      N_PROJSPEC(ips)=L1
      NB_TRANSPROJO = NB_TRANSPROJO + 1
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'escaping is a transparent of HI beam L1,EPS(L1):',
     s   L1,EPS(L1)
      ENDIF                                              
      ENDIF
...
C        (X2COUR remplace R22 le rayon**2 fixe du noyau)
      ...
      IF(IBERT.EQ.ibw) THEN
      IF(JPARTICIP(L1).EQ.1) WRITE(6,*) 'Reflection of a participant',
     s ' L1,EPS(L1):',
     s   L1,EPS(L1)
      ENDIF                                              
C Fin modif surface A.B.             
  ...
  603 CONTINUE                                                          P-N11970
      K=K-KD                                                            P-N11980
C  607 IF (NESC(L1).EQ.1) GO TO 613                                      P-N11990
C modif 10/02 pour logique des clusters (nesc()=2!)
  607 IF (NESC(L1).NE.0) GO TO 613                                      
      CALL NEW1(L1)                                            
...
c      WRITE(6,*) 'N_ACTIVNUC,nbquit,IA_BREAKUP,A_PROJSPEC',
c     s     N_ACTIVNUC,nbquit,IA_BREAKUP,A_PROJSPEC
c      WRITE(6,*) (nesc(iloc),iloc=1,IA)
...
C Tableau des energies a la fin (avatar.hbk)
  ...
  256 ELEAD=0.                                                          P-N12510
      ...
      NPX=0                                                             P-N12530
      EREM=0.                                                           P-N12540
C Energie d'excitation des deltas finals de decroissance bloquee: (AB CV 6/2005)
          EREM = EREM + DESTAR
      IZREM=0                                                           P-N12550
C Restitution de la charge du remnant si delta final bloque (A.B., C.V. 6/2005)
          IZREM = IZREM + IDQ
      INREM=0                                                           P-N12560
      iarem=0
      RCM1=0.                                                           P-N12570
...
c       if (ncol.eq.0) then
c        write (6,*)'TRANSP:ncol=',ncol,nsum_col,timi,b,IBERT
            go to 9100
            endif
        endif
      endif
...
c fin ajout CV
 ...
 9101 nopart=0
      ...
      IF (Kclst.NE.0) THEN
C On traite les clusters si il y en a:
      IF (NB_CLST.NE.0) THEN
          DO iloc=1,NB_CLST
	     POUT1=POUT1+POUT_CLST(1,iloc)      ! Bilans
	     POUT2=POUT2+POUT_CLST(2,iloc)
	     POUT3=POUT3+POUT_CLST(3,iloc)
	     EOUT=EOUT+T_CLST(iloc)
	     XL1=XL1-LOUT_CLST(1,iloc)
	     XL2=XL2-LOUT_CLST(2,iloc)
	     XL3=XL3-LOUT_CLST(3,iloc)
	     
             nopart=nopart+1			! copie pour sortie
	     kind(nopart)=K_CLST(iloc)
	     ep(nopart)=T_CLST(iloc)
	     bmass(nopart)=E_CLST(iloc) - T_CLST(iloc)
             ekout=ekout+ep(nopart)
	     ptotl=SQRT(POUT_CLST(1,iloc)**2 
     s	              + POUT_CLST(2,iloc)**2
     s		      + POUT_CLST(3,iloc)**2)
             alpha(nopart)=POUT_CLST(1,iloc)/ptotl
             beta(nopart)=POUT_CLST(2,iloc)/ptotl
             gam(nopart)=POUT_CLST(3,iloc)/ptotl
C             teta=180.*acos(POUT_CLST(3,iloc)/ptotl)/3.1415926
	  END DO	  
      END IF
      END IF
 
C Treat here the projectile spectators (including transparents):
      CALL PROJO_SPEC(IA1,ips,MAX_A_PROJ,N_PROJSPEC,
     s    FMPINC,PINC,TLAB,IBERT)
...
c      ELEAD=EPS(I)                                                     P-N12690
  259 IF (NESC(I).EQ.0) GO TO 254                                       P-N12700
      IF (NESC(I).GT.1) GO TO 258   ! on evite les nucleons des clusters

C Here nesc=1 or negative value (emitted nucleons, spectators included)      
	XL1_I=X2(I)*P3(I)-X3(I)*P2(I)	! moment angulaire emporte
	XL2_I=X3(I)*P1(I)-X1(I)*P3(I)
	XL3_I=X1(I)*P2(I)-X2(I)*P1(I)
      ...
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'i,eps(i) (escaping nuc.)',i,eps(i)
      ENDIF		                                       
...
C Here we don't copy the spectators of the projectile bound in a
C   spectator nucleus (treated by the deexcitation module):
      IF(kindf7.LT.0.AND.i.LE.ia1.AND.A_PROJSPEC.NE.0) THEN
         DO iloc=1,A_PROJSPEC
	    IF(i.EQ.N_PROJSPEC(iloc)) THEN
	       P1SPEC=P1SPEC+P1(i)
	       P2SPEC=P2SPEC+P2(i)
	       P3SPEC=P3SPEC+P3(i)
C For balance, a transparent from projectile is out:
            IF(i.GT.ips-NB_TRANSPROJO) ekout=ekout+eps(i)-fmp
	    GO TO 258
	    ENDIF
         ENDDO
      ENDIF
      ...
      ekout=ekout+eps(i)-fmp
           
      kind(nopart)=3-IC33
c Spectators of composite projectiles (7/2006, AB)
      IF(npproj(I).EQ.1) kind(nopart)= -kind(nopart)
      ...
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'particles output:nopart,ep,kind:',
     s nopart,ep(nopart),kind(nopart)
      ENDIF
...
C   and the distance with respect to the barycenter of the actual target 10/2003
...
C   des energies pour les impulsions.)
           ...
           pfrem3=pinc-P3SPEC
         do ipart=1,nopart
C     FIXME ep(ipart), i.e. the energy can be negative!
            if(ep(ipart).LT.0) then
               write(6,*)'ep(ipart)=ep(',ipart,')=',ep(ipart)
            endif
...
C     Modified 4/2004 for relativistic expressions and pions.
      ...
      IF (E.GT.V0IA) GO TO 1
      BARR=0.                                                           P-N24520
      RETURN                                                            P-N24530

    1 X=SQRT((2.*MRE*E+E*E)*(2.*MRE*(E-V0IA)+(E-V0IA)**2))                                          
      BARR=4.*X/(2.*MRE*(2.*E-V0IA)+E**2+(E-V0IA)**2+2.*X)                                    
      IQ=(IA+IZ)/2.
      IF (IQ.GT.0) GO TO 2                                           
      RETURN                                                            P-N24570
    ...
    3 G=IQ*IZN/137.03*SQRT(2.*MRE/(E-V0IA)/(1.+(E-V0IA)/2./MRE))
     s                             *(ACOS(PX)-PX*SQRT(1.-PX*PX))
...
c Pauli strict
      ...
      XR2=XR*XR                                                         P-N24980
      PR2=PR*PR                                                         P-N24990
      VOL=(4.*3.1415926/3.)**2*(XR*PR/(2.*3.1415926*197.33))**3         P-N25000
...
C	    	ESREM=f(3)+SEP
      ...
      END      	
C***************************************************************************
C RECHERCHE DE CLUSTERS (d,t,3He,4He) autour du nucleon K, ITCH=charge courante
C   du noyau pour la barriere coulombienne.
C   IOK=1, le cluster de type KINDCLST sort (0=pas de sortie cluster).
C   ID_NUC_CLST(4)	Numeros des nucleons constituant ce cluster.
C   EOUT,TOUT	Energie totale et cinetique de ce cluster hors du noyau.
C   POUT, LOUT	Vecteur impulsion et moment angulaire hors du noyau.
C  
C  Correction de l'appel du coulombien le 21/8/2003 (appel avec isospin et non Z)
C  Coulombien evalue a R0 si f(11) =1	  4/2006
C  Coulombien evalue a R0+RMS_CLUST+fneck si f(11) =2
C***************************************************************************
      SUBROUTINE CLUSTER(K,ITCH,V0,IOK,KINDCLST,ID_NUC_CLST,
     s                   EOUT,TOUT,POUT,LOUT,Kclst)
      
      REAL*4 POUT(3),LOUT(3)
      DIMENSION ID_NUC_CLST(4)
      
C In this version, a delta is not considered as constituant of a cluster.
C Common de INCL:
      COMMON/BL1/P1(300),P2(300),P3(300),EPS(300),IND1(300),IND2(300),TA
      COMMON/BL3/R1,R2,X1(300),X2(300),X3(300),IA1,IA2,RAB2             
      COMMON/BL5/TLG(300),NESC(300)
      COMMON/hazard/ial,IY1,IY2,IY3,IY4,IY5,IY6,IY7,IY8,IY9,IY10,
     s               IY11,IY12,IY13,IY14,IY15,IY16,IY17,IY18,IY19
      COMMON/WS/R0,ADIF,RMAXWS,DRWS,NOSURF,XFOISA,NPAULSTR,BMAX,fneck 
C RMS espace R, espace P, Fermi momentum and energy for light gauss nuc.      
      COMMON/light_gaus_nuc/rms1t(9),pf1t(9),pfln(9),tfln(9),vnuc(9)
      COMMON/REFR/LREF,VREF,RREF,EOUTREF,POUTREF,PXREF,PYREF,PZREF
      
      INTEGER OK_CLUST(4)
      DIMENSION ID_NUC(4,4),R_CLUST(3,4),P_CLUST(3,4),
     s            ISO_C(4),Tin_clst(4)
      COMMON/CLST/OK_CLUST,ID_NUC,R_CLUST,P_CLUST,ISO_C,Tin_clst
     
      DIMENSION R_CLST(3),P_CLST(3),taille_c(4),bind_clst(4),TCOUL(4)
      INTEGER A_CLST(4)

C Taille mini pour les differents clusters d,t,3He,4He:      
C      DATA taille_c/86436.,99225.,99225.,57600./
      DATA taille_c/150000,150000.,150000.,150000./
C Binding energies, nucleon mass, mass number
      DATA bind_clst,FMP/2.22,8.48,7.72,28.30,938.2796/
      DATA A_CLST/2,3,3,4/
      
      IA=IA1+IA2	!nb de nucleons du probleme
      DO i=1,4
        ID_NUC(i,1)=k
      ENDDO

C on deplace le nucleon k vers une zone de plus haute densite
C Pour Bi,Pb,Au translat=2.6, Cu:2.4 (ou RO+1.75)
			translat=RMAXWS-(R0+1.75)
	PK=SQRT(P1(k)**2+P2(k)**2+P3(k)**2)
	X1K=X1(k)-P1(k)*translat/PK
	X2K=X2(k)-P2(k)*translat/PK
	X3K=X3(k)-P3(k)*translat/PK

C PRINT DE TEST:
C	R_NEW=SQRT(X1K**2+X2K**2+X3K**2)
C	WRITE(6,*) 'R_NEW,PK', R_NEW,PK                                                  
	
C      WRITE(6,*) ' Recherche autour de:',k,X1(k),X2(k),X3(k)
C      WRITE(6,*) '                     ',P1(k),P2(k),P3(k)
C      WRITE(6,*) IA
      
C Recherche d'un deuton:
        IF(Kclst.EQ.1) THEN 
           RB=R2
        ELSE
           RB=R2+rms1t(6)+fneck
	...
	N=1
        OK_CLUST(N)=0
      DO i=1,IA
        IF((i.EQ.k).OR.(ind1(i).EQ.1)			!Not a DELTA
     s             .OR.(NESC(i).NE.0)) GO TO 622	!Not escaping nuc.
C          taille deuton
      	taille=(X1(i)-X1k)**2 
     s                 + (X2(i)-X2k)**2 + (X3(i)-X3k)**2
     	taille = taille*((P1(i)-P1(k))**2
     s                 + (P2(i)-P2(k))**2 + (P3(i)-P3(k))**2)/4.
        IF(taille.GT.taille_c(N)) GO TO 622	
C           isospin deuton:
        ISO_C(N)=ind2(k)+ind2(i)
        IF(ISO_C(N).NE.0) GO TO 622
      	OK_CLUST(N)=1
	lq=i
	ID_NUC(N,2)=lq
c           barycentre deuton      
      	R_CLUST(1,N)=(X1k+X1(lq))/2.
      	R_CLUST(2,N)=(X2k+X2(lq))/2.
      	R_CLUST(3,N)=(X3k+X3(lq))/2.
      	P_CLUST(1,N)=(P1(k)+P1(lq))
      	P_CLUST(2,N)=(P2(k)+P2(lq))
      	P_CLUST(3,N)=(P3(k)+P3(lq))
	PXREF=P_CLUST(1,N)
	PYREF=P_CLUST(2,N)
	PZREF=P_CLUST(3,N)
	FM = 2.*FMP-bind_clst(N)
	Tin_clst(N)=EPS(k)+EPS(lq)-FM
	TCOUL(N)=BARR(Tin_clst(N),0,2,ITCH,RB,V0)
C      WRITE(6,*) ' nucleon OK deuton:  ',lq,X1(lq),X2(lq),X3(lq)
C      WRITE(6,*) '                     ',P1(lq),P2(lq),P3(lq)
	GO TO 1452
622   CONTINUE
      ENDDO
1452  CONTINUE
C      WRITE(6,*) 'Recherche deuton finie'     
C Recherche d'un t et 3He:
        IF(Kclst.EQ.1) THEN 
           RBT=R2
           RBH=R2
        ELSE
           RBT=R2+rms1t(7)+fneck
           RBH=R2+rms1t(8)+fneck
        ENDIF
      	N=2
	M=3	
	OK_CLUST(N)=0
	OK_CLUST(M)=0
      DO i=1,IA
        IF((i.EQ.k).OR.(ind1(i).EQ.1)			!Not a DELTA
     s             .OR.(NESC(i).NE.0)) GO TO 623	!Not escaping nuc.
C          taille premier couple         
      	taille=(X1(i)-X1k)**2 
     s                 + (X2(i)-X2k)**2 + (X3(i)-X3k)**2
     	taille = taille*((P1(i)-P1(k))**2
     s                 + (P2(i)-P2(k))**2 + (P3(i)-P3(k))**2)/4.
        IF(taille.GT.taille_c(N)) GO TO 623
		lq=i 	! particule potentielle
      		R_CLST(1)=(X1k+X1(lq))/2.
      		R_CLST(2)=(X2k+X2(lq))/2.
      		R_CLST(3)=(X3k+X3(lq))/2.
      		P_CLST(1)=(P1(k)+P1(lq))
      		P_CLST(2)=(P2(k)+P2(lq))
      		P_CLST(3)=(P3(k)+P3(lq))
C      WRITE(6,*) ' nucleon OK t ou 3He:',lq,X1(lq),X2(lq),X3(lq)
C      WRITE(6,*) '                     ',P1(lq),P2(lq),P3(lq)
		
C          taille avec second nucleon et isospin      
      		IF(lq.EQ.IA) GO TO 1453
      	DO i1=lq+1,IA
        IF((i1.EQ.k).OR.(ind1(i1).EQ.1)			!Not a DELTA
     s             .OR.(NESC(i1).NE.0)) GO TO 624	!Not escaping nuc.
      		taille=(X1(i1)-R_CLST(1))**2 
     s  + (X2(i1)-R_CLST(2))**2 + (X3(i1)-R_CLST(3))**2
     		taille = taille*((2.*P1(i1)-P_CLST(1))**2
     s  + (2.*P2(i1)-P_CLST(2))**2 + (2.*P3(i1)-P_CLST(3))**2)/9.
      	IF(taille.GT.taille_c(N)) GO TO 624
C            isospin triton
        ISO_C(N)=ind2(k)+ind2(lq)+ind2(i1)
	IF(ISO_C(N).EQ.-1) THEN
		OK_CLUST(N)=1
      		R_CLUST(1,N)=(X1(i1)+2.*R_CLST(1))/3.
    	  	R_CLUST(2,N)=(X2(i1)+2.*R_CLST(2))/3.
     	 	R_CLUST(3,N)=(X3(i1)+2.*R_CLST(3))/3.
      		P_CLUST(1,N)=(P1(i1)+P_CLST(1))
      		P_CLUST(2,N)=(P2(i1)+P_CLST(2))
      		P_CLUST(3,N)=(P3(i1)+P_CLST(3))
                ls=i1	! particule 3
		ID_NUC(N,2)=lq
		ID_NUC(N,3)=ls
	PXREF=P_CLUST(1,N)
	PYREF=P_CLUST(2,N)
	PZREF=P_CLUST(3,N)
	FM = 3.*FMP-bind_clst(N)
	Tin_clst(N)=EPS(k)+EPS(lq)+EPS(ls)-FM
	TCOUL(N)=BARR(Tin_clst(N),-1,3,ITCH,RBT,V0)
C      WRITE(6,*) ' nucleon OK t ou 3He:',ls,X1(ls),X2(ls),X3(ls)
C      WRITE(6,*) '                     ',P1(ls),P2(ls),P3(ls)
	ENDIF
C            isospin 3He
        ISO_C(M)=ind2(k)+ind2(lq)+ind2(i1)
	IF(ISO_C(M).EQ.1) THEN
		OK_CLUST(M)=1
      		R_CLUST(1,M)=(X1(i1)+2.*R_CLST(1))/3.
      		R_CLUST(2,M)=(X2(i1)+2.*R_CLST(2))/3.
      		R_CLUST(3,M)=(X3(i1)+2.*R_CLST(3))/3.
      		P_CLUST(1,M)=(P1(i1)+P_CLST(1))
      		P_CLUST(2,M)=(P2(i1)+P_CLST(2))
      		P_CLUST(3,M)=(P3(i1)+P_CLST(3))
                ls=i1	! particule 3
		ID_NUC(M,2)=lq
		ID_NUC(M,3)=ls
	PXREF=P_CLUST(1,M)
	PYREF=P_CLUST(2,M)
	PZREF=P_CLUST(3,M)
	FM = 3.*FMP-bind_clst(M)
	Tin_clst(M)=EPS(k)+EPS(lq)+EPS(ls)-FM
	TCOUL(M)=BARR(Tin_clst(M),1,3,ITCH,RBH,V0)
C      WRITE(6,*) ' nucleon OK t ou 3He:',ls,X1(ls),X2(ls),X3(ls)
C      WRITE(6,*) '                     ',P1(ls),P2(ls),P3(ls)
	ENDIF
	
	IF((OK_CLUST(N)+OK_CLUST(M)).NE.2) THEN
C         ....On continue
		GO TO 624
	ELSE
C         ....On a trouve un t et un 3He: Arret
		GO TO 1453
	ENDIF
...
C          taille premier couple         
      DO i=1,IA
        IF((i.EQ.k).OR.(ind1(i).EQ.1)			!Not a DELTA
     s             .OR.(NESC(i).NE.0)) GO TO 625	!Not escaping nuc.
      	taille=(X1(i)-X1k)**2 
     s                 + (X2(i)-X2k)**2 + (X3(i)-X3k)**2
     	taille = taille*((P1(i)-P1(k))**2
     s                 + (P2(i)-P2(k))**2 + (P3(i)-P3(k))**2)/4.
        IF(taille.GT.taille_c(N)) GO TO 625
		lq=i 	! particule potentielle
      		R_CLST(1)=(X1k+X1(i))/2.
      		R_CLST(2)=(X2k+X2(i))/2.
      		R_CLST(3)=(X3k+X3(i))/2.
      		...
      		P_CLST(2)=(P2(k)+P2(i))
      		P_CLST(3)=(P3(k)+P3(i))
C      WRITE(6,*) ' nucleon OK 4He:     ',lq,X1(lq),X2(lq),X3(lq)
C      WRITE(6,*) '                     ',P1(lq),P2(lq),P3(lq)
		
C          taille avec second nucleon       
      		IF(lq.EQ.IA) GO TO 1454
      	DO i1=lq+1,IA		
        IF((i1.EQ.k).OR.(ind1(i1).EQ.1)			!Not a DELTA
     s             .OR.(NESC(i1).NE.0)) GO TO 626	!Not escaping nuc.
      	taille=(X1(i1)-R_CLST(1))**2 
     s  + (X2(i1)-R_CLST(2))**2 + (X3(i1)-R_CLST(3))**2
     	taille = taille*((2.*P1(i1)-P_CLST(1))**2
     s  + (2.*P2(i1)-P_CLST(2))**2 + (2.*P3(i1)-P_CLST(3))**2)/9.
      	IF(taille.GT.taille_c(N)) GO TO 626
		ls=i1 	! particule potentielle
      		R_CLST(1)=(X1(i1)+2.*R_CLST(1))/3.
      		R_CLST(2)=(X2(i1)+2.*R_CLST(2))/3.
      		R_CLST(3)=(X3(i1)+2.*R_CLST(3))/3.
      		P_CLST(1)=(P1(i1)+P_CLST(1))
      		P_CLST(2)=(P2(i1)+P_CLST(2))
      		P_CLST(3)=(P3(i1)+P_CLST(3))
C      WRITE(6,*) ' nucleon OK 4He:     ',ls,X1(ls),X2(ls),X3(ls)
C      WRITE(6,*) '                     ',P1(ls),P2(ls),P3(ls)

 		ISO_C(N)=ind2(k)+ind2(lq)+ind2(ls)	!test introduit 7/2003
		IF((ISO_C(N).EQ.3).OR.(ISO_C(N).EQ.-3)) GO TO 626

		
C          taille avec dernier nucleon et test d'isospin       
      		IF(ls.EQ.IA) GO TO 1454
      		DO i2=ls+1,IA		
        IF((i2.EQ.k).OR.(ind1(i2).EQ.1)			!Not a DELTA
     s             .OR.(NESC(i2).NE.0)) GO TO 627	!Not escaping nuc.
      		taille=(X1(i2)-R_CLST(1))**2 
     s  + (X2(i2)-R_CLST(2))**2 + (X3(i2)-R_CLST(3))**2
     		taille = taille*((3.*P1(i2)-P_CLST(1))**2
     s  + (3.*P2(i2)-P_CLST(2))**2 + (3.*P3(i2)-P_CLST(3))**2)/16.
      		IF(taille.GT.taille_c(N)) GO TO 627
C            isospin 4He
        ISO_C(N)=ISO_C(N)+ind2(i2)
	IF(ISO_C(N).EQ.0) THEN
		OK_CLUST(N)=1
		ld=i2 	! particule potentielle
      		R_CLUST(1,N)=(X1(i2)+3.*R_CLST(1))/4.
      		R_CLUST(2,N)=(X2(i2)+3.*R_CLST(2))/4.
      		R_CLUST(3,N)=(X3(i2)+3.*R_CLST(3))/4.
      		P_CLUST(1,N)=(P1(i2)+P_CLST(1))
      		P_CLUST(2,N)=(P2(i2)+P_CLST(2))
      		P_CLUST(3,N)=(P3(i2)+P_CLST(3))
		ID_NUC(N,2)=lq
		ID_NUC(N,3)=ls
		ID_NUC(N,4)=ld
	PXREF=P_CLUST(1,N)
	PYREF=P_CLUST(2,N)
	PZREF=P_CLUST(3,N)
	FM = 4.*FMP-bind_clst(N)
	Tin_clst(N)=EPS(k)+EPS(lq)+EPS(ls)+EPS(ld)-FM
	TCOUL(N)=BARR(Tin_clst(N),0,4,ITCH,RB,V0)
C      WRITE(6,*) ' nucleon OK 4He:     ',ld,X1(ld),X2(ld),X3(ld)
C      WRITE(6,*) '                     ',P1(ld),P2(ld),P3(ld)
		GO TO 1454
	ENDIF	
...
C      WRITE(6,*) 'clusters trouves:',(OK_CLUST(i),i=1,4) 
C	DO i=1,4
C	 IF(OK_CLUST(i).EQ.1) THEN
C	   WRITE(6,*) (R_CLUST(j,i),j=1,3),(P_CLUST(j,i),j=1,3)
C	 ENDIF
C	ENDDO
C*************************************************************
C  Choix du cluster retenu:
C*************************************************************
      TRANS=0.
      call ribm(rndm,iy12)
C      DO i=1,4		! cluster de transmission maxi
      DO i=4,1,-1		! cluster maxi prioritaire
C            IF((OK_CLUST(i).EQ.1).AND.(TRANS.LT.TCOUL(i))) THEN
            IF((OK_CLUST(i).EQ.1).AND.(TCOUL(i).GT.0.)) THEN
			TRANS=TCOUL(i)	
			num_clst=i
                        call ribm(rndm,iy12)	! tir hasard transmission
			IF(rndm.LE.TRANS) GO TO 732
	    ENDIF
      END DO
732   CONTINUE
C      call ribm(rndm,iy12)	! tir hasard transmission
      IF (rndm.LE.TRANS) THEN 
C      IF(TRANS.GE.0.05) THEN
      		IOK = 1		! Ce cluster sort
		KINDCLST = num_clst + 5
	    DO i=1,A_CLST(num_clst)
	    	ID_NUC_CLST(i) = ID_NUC(num_clst,i)
	    END DO
	    IF (A_CLST(num_clst).LT.4) THEN
	    	DO i=A_CLST(num_clst)+1,4
			ID_NUC_CLST(i) = 0
		END DO
	    ENDIF
		TOUT = Tin_clst(num_clst)
     s		     - A_CLST(num_clst)*V0
     		FMCLST = A_CLST(num_clst)*FMP - bind_clst(num_clst)
		EOUT = TOUT + FMCLST
		PX = P_CLUST(1,num_clst)
		PY = P_CLUST(2,num_clst)
		PZ = P_CLUST(3,num_clst)
                COEF = SQRT(TOUT*(TOUT+2.*FMCLST)/(PX*PX+PY*PY+PZ*PZ))
		POUT(1)=PX*COEF
		POUT(2)=PY*COEF
		POUT(3)=PZ*COEF
		RX = R_CLUST(1,num_clst)
		RY = R_CLUST(2,num_clst)
		RZ = R_CLUST(3,num_clst)
		LOUT(1)=RY*PZ-RZ*PY
		LOUT(2)=RZ*PX-RX*PZ
		LOUT(3)=RX*PY-RY*PX		
C Pour verifs (histos des clusters en r et p):
      		CALL CLUST_DENS(num_clst,translat)
      ELSE
      		IOK = 0	    	! aucun cluster ne sort
      ENDIF
      ...
      END
C****************************************************************************
      SUBROUTINE CLUST_DENS(num_clst,translat)
      
C****************************************************************************
C   STRUCTURE des clusters (densites)
C****************************************************************************
      INTEGER OK_CLUST(4)
      DIMENSION ID_NUC(4,4),R_CLUST(3,4),P_CLUST(3,4),
     s            ISO_C(4),Tin_clst(4)
...
C      DO i=1,4
      i=num_clst
         IF (OK_CLUST(i).NE.0) THEN
	     r= SQRT((X1K-R_CLUST(1,i))**2
     s              +(X2K-R_CLUST(2,i))**2 
     s              +(X3K-R_CLUST(3,i))**2) 
	     CALL HFILL(i,r,0.,1.)
	   DO j=2,NB_NUC(i)
	     r= SQRT((X1(ID_NUC(i,j))-R_CLUST(1,i))**2
     s              +(X2(ID_NUC(i,j))-R_CLUST(2,i))**2 
     s              +(X3(ID_NUC(i,j))-R_CLUST(3,i))**2) 
	     CALL HFILL(i,r,0.,1.)
	   ENDDO
	   DO j=1,NB_NUC(i)
	     p= SQRT((P1(ID_NUC(i,j))-P_CLUST(1,i)/NB_NUC(i))**2
     s              +(P2(ID_NUC(i,j))-P_CLUST(2,i)/NB_NUC(i))**2 
     s              +(P3(ID_NUC(i,j))-P_CLUST(3,i)/NB_NUC(i))**2) 
	     CALL HFILL(i+4,p,0.,1.)
C	     WRITE(6,*) 'type clust,nuc,r,p',i,j,r,p
	   ENDDO
	 ENDIF
C      ENDDO
      RETURN
...
C****************************************************************************
     ...
     sN_PROJSPEC,FMPINC,PINC,TLAB,IBERT)
      COMMON/BL1/P1(300),P2(300),P3(300),EPS(300),IND1(300),IND2(300),TA
C Spectators of the projectile treated here (nucleus for Fermi-Breakup):
C ips is the number of spect, their label is in N_PROJSPEC(ips).
C E,P are outside the target potential for these nucleons.
C La transmission de MAX_A_PROJ=60 ne marche pas....Explicite ici!
...
C for composit projectiles before entering the potential)
      COMMON/Fermi/TF,PF,PF2     
      DIMENSION Tpart(20),Ttrou(20)
      DOUBLE PRECISION fja,fjz,ex2,rnmass,um
      DATA um/931.20793d0/

      Tpart(1:20) = 0.0

C IBERT value for print:
      ibw=-1
c      ibw=16872

      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*)'Number of projo-spectators',ips,IBERT 
      ENDIF
...
C End Verifs
	...
	GS_mass=0.
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*)'i_c,EPS,Mass' 
      ENDIF
	DO i_spec=1,ips
	        i_c = N_PROJSPEC(i_spec)
		E_SPEC = E_SPEC+EPS(i_c)
		P1_SPEC = P1_SPEC+P1(i_c)
		P2_SPEC = P2_SPEC+P2(i_c)
		P3_SPEC = P3_SPEC+P3(i_c)
		Pi_SPEC2= P1(i_c)**2+P2(i_c)**2+P3(i_c)**2
C Current Ground State mass:
		GS_mass=GS_mass+SQRT(EPS(i_c)**2-Pi_SPEC2)
	   IF(IND2(i_c).EQ.1) Z_PROJSPEC=Z_PROJSPEC+1
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*)i_c,EPS(i_c),SQRT(EPS(i_c)**2-Pi_SPEC2) 
      ENDIF
	ENDDO
	P_SPEC2=P1_SPEC**2+P2_SPEC**2+P3_SPEC**2
	S_SPEC = SQRT(E_SPEC**2-P_SPEC2)

C Verif: print des nucleons du projectile (avant reaction)	
      IF(IBERT.EQ.ibw) THEN
      WRITE(6,*) 'projectile avant reaction:'
      WRITE(6,*) 'i, E,  P1,  P2,  P3,  P3_c:'
      esloc=0.
      p1sloc=0.
      p2sloc=0.
      p3sloc=0.      
            DO i=1,ia1
            WRITE(6,*) i,eps_c(i),p1_s(i),p2_s(i),p3_s(i),p3_c(i)
            esloc=esloc+eps_c(i)
            p1sloc=p1sloc+p1_s(i)
            p2sloc=p2sloc+p2_s(i)
            p3sloc=p3sloc+p3_s(i)
            ENDDO
            ploc2=p1sloc**2+p2sloc**2+p3sloc**2
            fmloc=SQRT(esloc**2-ploc2)
            ploc=SQRT(ploc2)
            tloc=esloc-fmloc
            WRITE(6,*) 'projo: E,P1,P2,P3,T,M',
     s  esloc,p1sloc,p2sloc,p3sloc,tloc,fmloc
      ENDIF
	
C No projectile spectator if A>=4 and A=Z or A=N (No DRESNER breakup)
        IF(ips.GE.4.AND.(Z_PROJSPEC.EQ.ips.OR.Z_PROJSPEC.EQ.0)) THEN
	DO i_spec=1,ips
      i_c = N_PROJSPEC(i_spec)
c Single nucleon forced on shell! Momentum kept.
c      epsprint=eps(i_c)
c      eps(i_c)=SQRT(P1(i_c)**2+P2(i_c)**2+P3(i_c)**2+938.2796**2)     
c      WRITE(6,*) 'Off shell problems in PROJO_SPEC, IBERT:',IBERT
c      WRITE(6,*) '  Nucleon',i_c,' forced on shell, momentum kept',
c     s ' eps avant:',epsprint,' eps apres:',eps(i_c)
c Single nucleon forced on shell! Energy kept.
      arg=(eps(i_c)**2-938.2796**2)/
     s           (P1(i_c)**2+P2(i_c)**2+P3(i_c)**2)
      if(arg.LT.0.) then
      WRITE(6,*) 'Off shell problems in PROJO_SPEC'
      WRITE(6,*) 'IBERT, i_c, eps, p:',IBERT,i_c,
     s            eps(i_c),P1(i_c),P2(i_c),P3(i_c)
c Single nucleon forced on shell! Momentum kept in that case.
      eps(i_c)=SQRT(P1(i_c)**2+P2(i_c)**2+P3(i_c)**2+938.2796**2)
      WRITE(6,*) '   here momentum kept eps:',eps(i_c)     
      else
      coef=SQRT(arg) ! FIXME FPE if off shell problems!!!      
        P1(i_c)=P1(i_c)*coef
        P2(i_c)=P2(i_c)*coef
        P3(i_c)=P3(i_c)*coef           
      endif
...
C Excitation energy from particle-hole excitation:
C   Seconde methode de calcul EX4:
      DO i=1,IA1
      	nb_p=i
      ...
      DO i=1,ips
      	j=N_PROJSPEC(i)
      EX4=EX4+t_c(j)-Tpart(i)
      ENDDO
      
      EX_PROJSPEC =EX4
      
C Coherent rest mass:
      M_PROJSPEC = S_SPEC - EX_PROJSPEC
      
      A_PROJSPEC=ips
      
      IF((EX_PROJSPEC.LT.0.).OR.(M_PROJSPEC.LT.0.)) THEN
      WRITE(6,*) 'EX_PROJSPEC is negative,A,Z:',EX_PROJSPEC,fja,fjz
      WRITE(6,*) '...or M is negative,A,Z:',M_PROJSPEC,fja,fjz
      ENDIF
...
C ...end check module 
      ...
      A_PROJSPEC = 0	!number of nucleons in the spectator nucleus
      i_c = N_PROJSPEC(1)
c Single nucleon forced on shell! Momentum kept.
c      eps(i_c)=SQRT(P1(i_c)**2+P2(i_c)**2+P3(i_c)**2+938.2796**2)     
c Single nucleon forced on shell! Energy kept.
      arg=(eps(i_c)**2-938.2796**2)/
     s           (P1(i_c)**2+P2(i_c)**2+P3(i_c)**2)
      if(arg.LT.0.) WRITE(6,*) 'Off shell problems in PROJO_SPEC'
C     FIXME FPE (arg can be negative!)
      if(arg.LT.0) then
         write(6,*)'arg = ',arg
         write(6,*)'ERROR: Trying to calculate sqrt(',arg,')...'
      endif
      coef=SQRT(arg)
