...
void INCL()
{
  TFile *cf = new TFile("tmp/run16.root");
  TFile *ff = new TFile("tmp/run16ref.root");
  ...
  inclResult->SaveAs("run16Remnants.eps");
  bimpactPlot->SaveAs("run16ImpactParameter.eps");
}
...
void nCrossSection(){
  TFile* fref = new TFile("tmp/run16.root");
  TFile* ffref = new TFile("tmp/run16ref.root");

  Char_t* titre="p(3 GeV) + Al (INCL4+ABLA)";
	
  Char_t* psFileName="n_cross_section_run16.ps";
  ...
  first=0;
  plotTheorTheta(reffort,titre,c1,emin,emax,logE,neutron,thet0,fnor0,fnora,"0 Deg",first, kBlack);
  //  first=1;
  fnora=fnora/10.;
