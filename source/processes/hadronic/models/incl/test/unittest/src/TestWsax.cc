...
// ********************************************************************
//
// $Id: TestWsax.cc,v 1.1 2007-05-23 09:56:26 miheikki Exp $ 
...
ClassImp(TestWsax)
  ...
  /* BEGIN_HTML
     ...
     <p>
     This class tests method G4Incl::wsax.
     </p>
     <p>
     <h3>Comparison between C++ and FORTRAN implementations</h3>

     <img src="plots/TestWsax.png"/>
     </p>
     END_HTML */
  
TestWsax::TestWsax()
{
  setUnitName("G4Incl::wsax");
  setOriginalUnitName("wsax");
  setPlotFileName("htmldoc/plots/TestWsax.png");
  setLogFileName("htmldoc/logs/TestWsax.log");
  setLinesOfCode(5);
  setTestStatus(false);
}
...
void TestWsax::runMe()
{
  ...
  for(Int_t i = 1; i < points; i++) {
    r[i] = r[i-1] + r_step;	
    //    cpp_f_r[i] = wsax(r[i], ws, dton);
    cpp_f_r[i] = incl->wsax(r[i]);
    fort_f_r[i] = wsax_(&r[i]);
    //
  ...
  fort_graph->SetTitle("Function wsax");
