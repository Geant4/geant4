...
// ********************************************************************
//
// $Id: TestFlin2.cc,v 1.2 2007-09-11 13:28:42 miheikki Exp $ 
...
ClassImp(TestFlin2)
  ...
  /* BEGIN_HTML
     ...
     <p>
     This class tests method G4Incl::firstDerivative.
     </p>
     <p>
     <h3>Comparison between C++ and FORTRAN implementations</h3>

     <img src="plots/TestFlin2.png"/>
     </p>
     <p>
     Extra logs:
     <a href="logs/TestFlin2ExtraLogCpp.txt">C++</a> <a href="logs/TestFlin2ExtraLogFortran.txt">FORTRAN</a>
     </p>
     END_HTML */

TestFlin2::TestFlin2()
{
  setUnitName("G4Incl::firstDerivative");
  setOriginalUnitName("flin2");
  setPlotFileName("htmldoc/plots/TestFlin2.png");
  setLogFileName("htmldoc/logs/TestFlin2.log");
  setLinesOfCode(8);
  setTestStatus(false);
}
...
void TestFlin2::runMe()
{
  ...
  ofstream cppnumbers("htmldoc/logs/TestFlin2ExtraLogCpp.txt");
  ofstream fortnumbers("htmldoc/logs/TestFlin2ExtraLogFortran.txt");
  ...
  for(Int_t i = 0; i < points; i++) {
    r[i] = r[i-1] + r_step;	
    cpp_f_r[i] = saxw->s[i][0];
    fort_f_r[i] = gSaxw->s[0][i];
  ...
  fort_graph->GetYaxis()->SetTitle("flin2(x)");
  fort_graph->SetTitle("Function flin2");
