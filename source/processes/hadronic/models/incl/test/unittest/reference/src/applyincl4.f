C***********************************************************************
C     This is subroutine applyincl4. It is a modified version of the
C     program Thin Target Monte-Carlo Spallation Code developed at
C     Saclay by A. Boudard and C. Volant. The modifications to adapt it
C     for use with Geant4 were done by Pekka Kaitaniemi at Helsinki
C     Institute of Physics.
C
C     This subroutine is used to connect INCL4 model to Geant4. Every
C     time this subroutine is called we initialize INCL4 and process one
C     event. The subroutine gets input via common block called CALINCL
C     and writes its output into VARNTP common block. Note: This
C     subroutine calls only INCL4 and NOT ABLA!
C
C     The C++ part of this interface contains class
C     G4InclInterface. Whenever the method ApplyYourself of this class
C     is called it uses this subroutine to do the actual calculation.
C**********************************************************************
...
c      COMMON/hazard/ial,IY
	...
	INTEGER AVV,ZVV,JREMN,KFIS,IZFIS,IAFIS      
        COMMON/VARNTP/MASSINI,MZINI,EXINI,MULNCASC,MULNEVAP,
     ...
     +TETLAB(max),PHILAB(max)
      ...
      PARAMETER (avm=1000)
      COMMON/VARAVAT/Kveux,Bavat,NOPART,NCOL,
...
C For Geant4      
      character*300 datapath
      character*300 datapathname
      character*300 PATHONE,PATHTWO
...
C	OPEN(5,file='cu42.in',status='old')
C      READ(5,*) stringout	! output file
      stringout = 'inclablatest.out'
C      	OPEN(6,file=stringout)
...
C      READ(5,*)icoup,ievtest,inithaz
C      		INIT_GRAINE = 1		!init of seeds in INIT_INCL
C      IF (inithaz.EQ.1) THEN
C      	INIT_GRAINE = 0      
C        READ(5,*) ial,(IY(ihaz),ihaz=1,19)
C      ENDIF
C	write(6,*)' on est parti pour ',icoup,' runs',inithaz,ievtest
C	READ(5,*) f(7),f(3)
C	write(6,*)' type projectile, energie projectile ',f(7),f(3)
c      READ(5,*)icoup
C      write(6,*)' on est parti pour ',icoup,' runs'
C      READ(5,*) finput(7),finput(3)
C      write(6,*)' type projectile, energie projectile ',
C     s                                         finput(7),finput(3)
C      READ(5,*) finput(1),finput(2)
C      write(6,*)'Masse, Charge cible ',finput(1),finput(2)
      ZMAT(1)=finput(2)+0.5
      AMAT(1)=finput(1)+0.5
      NBMAT=1      
...
C ************************************************************************
C      RACINE = 'dapnia/dapx4'

      datapathname = 'HOME'
      call getenv(datapathname, datapath)

      PATHONE = '/inclabladata'
      ...
      DO WHILE(long.EQ.0)
         li=li+1
         IF(DATAPATH(li:li).EQ.' ') long=li-1
      END DO

      RACINE = DATAPATH(1:long)//PATHONE

      icoup = 1

C Use KHS evaporation
      choice_evap = 1
...
C      READ(5,*) finput(5),finput(6),NOSURF,XFOISA,NPAULSTR
...
C	  WRITE(6,*) '*********************************************'
C          WRITE(6,*) ' PARAMETERS of GEM (fission-evapo)'
C          WRITE(6,*) ' '
...
C	  WRITE(6,*) '*********************************************'
C          WRITE(6,*) ' PARAMETERS of KHSv3p (fission-evapo)'
C          WRITE(6,*) ' '
C	  		CALL INIT_EVAPORA(RACINE) 
      ENDIF                                     
...
C	  WRITE(6,*) '*********************************************'
C          WRITE(6,*) ' Output file for NTUPLE:'
         
C          CALL HBINIT(Je_veux)
...
C          WRITE(6,*) 'icoup, proba_trans',icoup,proba_trans
C Old main loop
...
C                            EVAPORATION step                                       
C **************************************************************************                            
C      IF (choice_evap.EQ.2) THEN
C **************************  EVAPORATION GEM *********
C           EE=EE/APRF 
C       ERECI=DBLE(ERECREM) 
C       BET0I(1)=DBLE(ALREM)
C       BET0I(2)=DBLE(BEREM)
C       BET0I(3)=DBLE(GAREM)
C       WT=1.
C       NOCASI=INUM
      
      
      	                                 
C C      CALL GEM(IAREM,IZREM,EE,ERECI,BET0I,WT,NOCASI,
C C     s  IZF_AB,IAF_AB,EKINF,BET1F,NBPART)
      
C C 
C C GEM donne NBPART particules (<300) dans les tableaux:
C C    IZF_AB    Z
C C    IAF_AB    A
C C    EKINF     Kinetic energy
C C    BET1F(300,3)  Cosinus directeurs                                                                      
C C 

C C Copie des resultats dans le NTUPLE
C       NTRACK=NBPART+NOPART
C       DO i33=1,NBPART
C          AVV(NOPART+i33)=IAF_AB(i33)
C 	 ZVV(NOPART+i33)=IZF_AB(i33)
C 	 ITYPCASC(NOPART+i33)=0
C 	 ENERJ(NOPART+i33)=EKINF(i33)
C 	 TETLAB(NOPART+i33)=180.*ACOS(BET1F(i33,3))/3.141592654                                                                      
C 	 PHILAB(NOPART+i33)=180.*ATAN2(BET1F(i33,2),BET1F(i33,1))
C      s	 /3.141592654
C 	 ALOC=IAF_AB(i33)
C 	 ZLOC=IZF_AB(i33)
C          AVVMASS=pace2(DBLE(ALOC),DBLE(ZLOC)) 
C      s                             + ALOC*UMA - ZLOC*MELEC
C          PLAB(NOPART+i33)=SQRT(EKINF(i33)*(EKINF(i33)+2.*AVVMASS))	 
C       ENDDO
      
C       iv=NBPART
C C *************************  FIN EVPORATION GEM ***************      
C       ELSE	  
C C *************************  EVAPORATION KHS *********	  
...
C          LMI_PF1 = NOPART + NBPEVAP+1	!indices min et max dans /VARNTP/
...
C          LMI_PF2 = NOPART + NBPEVAP+1	!indices min et max dans /VARNTP/
...
C       END IF 
C C *********************** FIN de l'EVAPO KHS ******************** 
                                                          
C       ENDIF 	!choix de l'evaporation (KHS-GEM)  
...
C		CALL HFNTB(101,'VARNTP')
C		IF(Je_veux.EQ.1) CALL HFNTB(102,'VARAVAT')
	ENDIF
...
C         WRITE(6,*) ' '
C 	WRITE(6,*) 'END OF SHOTS'
C         WRITE(6,*) '        ',nfis,' fissions'
C 	WRITE(6,*) '        ',ntrans,' transp nuclear   ',
C      s                        ntrans_coul,' transp coulomb'
C 	WRITE(6,*) '        ',nabs,' absorptions'
C 	WRITE(6,*) '        ',nretir,' retirages cascade (nopart=-100)'
C 	WRITE(6,*) 'Geometrical cross section (mb) ',31.4159*BMAX**2
C 	WRITE(6,*) 'React. cross section (mb): ',
C      s                      f_cross_sect*31.4159*BMAX**2
C         WRITE(6,*) ' '
C 	WRITE(6,*) ' Bad momentum conservation without fission:',
C      s             impulse1,impulse2,impulse3
C 	WRITE(6,*) ' Bad momentum conservation with fission:',
C      s             imp_f1,imp_f2,imp_f3
C         WRITE(6,*)' (number of evts above respect. 0.01, 2 and 5 MeV/c)'
C         WRITE(6,*) ' '
	
C        WRITE(6,*)' last random',ial
        fmuln=muln_2/(icoup-ntrans)
	ener_2=ener_2/(icoup-ntrans)
        err=SQRT(muln_2)/(icoup-ntrans)
C        WRITE(6,*) ' '
C        WRITE(6,1516) fmuln,err,ener_2
...
C        WRITE(6,1517) fmuln,err,ener_20
...
C        WRITE(6,1518) fmuln,err,ener_max
1518   FORMAT( F8.3,'+/-',F8.3,' neutrons (above 20 MeV)',
     s  ' per interaction',
     s  /,10x,F8.3,' MeV = Mean energy carried by ALL of them')
C        WRITE(6,*) ' '
      
C        call hbclose(Je_veux)
...
C*** PK end
      END
C*****************************************************************************          
C       subroutine hbinit(Je_veux)                                                         
C       implicit real*8 (a-h,o-z)
                                                                                     
C       character*80 string
C       parameter(lrecl=4096)
C       parameter (max=250)
C       COMMON/PAWC/W(6000000)
C 	real*4 EXINI,ENERJ,BIMPACT,PLAB,TETLAB,PHILAB,ESTFIS
C 	integer AVV,ZVV,JREMN,KFIS,IZFIS,IAFIS      
C       COMMON/VARNTP/MASSINI,MZINI,EXINI,MULNCASC,MULNEVAP,
C      +MULNTOT,BIMPACT,JREMN,KFIS,ESTFIS,IZFIS,IAFIS,NTRACK,
C      +ITYPCASC(max),AVV(max),ZVV(max),ENERJ(max),PLAB(max),
C      +TETLAB(max),PHILAB(max)
      
C       REAL*4 Bavat,TIME,ENERGY,EPSd,EPS2,EPS4,EPS6,EPSf
C       REAL*4 R1_in,R1_first_avat
C       INTEGER Bloc_Paul,Bloc_CDPP,GO_OUT,avm,DEL1,DEL2
C       PARAMETER (avm=1000)
C       COMMON/VARAVAT/Kveux,Bavat,NOPART,NCOL,
C      s R1_in(3),R1_first_avat(3),
C      s EPSd(250),EPS2(250),EPS4(250),EPS6(250),EPSf(250),      
C      s NB_AVAT,
C      s TIME(avm),L1(avm),L2(avm),JPARTL1(avm),JPARTL2(avm),
C      s DEL1(avm),DEL2(avm),ENERGY(avm),Bloc_Paul(avm),
C      s Bloc_CDPP(avm),GO_OUT(avm)
       
C       call hlimit(6000000)
C       READ(5,*)string
C c      string='/home/crash2/il2_crash/volant/kheinz/r2bis45.hbook'
C       write(6,*)string
C       id=11
C       call hropen(id,'NTP',string,'N',lrecl,ierr)
C       call hbnt(101,'Bidon test',' ')

C       call hbname(101,'VARNTP',
C      +MASSINI,'MASSINI,MZINI,EXINI,MULNCASC,MULNEVAP,
C      +MULNTOT,BIMPACT,JREMN,KFIS,ESTFIS,IZFIS,IAFIS,NTRACK[0,250],
C      +'//'ITYP(NTRACK),AVV(NTRACK):I*4,ZVV(NTRACK):I*4,
C      +ENERJ(NTRACK),PLAB(NTRACK),TETLAB(NTRACK),PHILAB(NTRACK)')
     
C C      CALL HBOOK1(1,'J_rem_INCL',100,0.,100.,0.)
C C      CALL HBOOK1(2,'J_rem_KHS',100,0.,100.,0.)

C C Pour verif de la densite du noyau cible:
C C	CALL HBOOK1(3,'r_distrib',900,0.,15.,0.)
C C	CALL HBOOK1(4,'p_distrib',900,0.,300.,0.)

C C Second NTUPLE optionnel pour etude des avatars en fonction du temps 
C C de la cascade.

C       READ(5,*) Je_veux,string
C            Kveux=Je_veux	!Pour transmission a INCL
C       IF (Je_veux.NE.1) RETURN
C       WRITE(6,*) ' Ntuple for avatars study:' 
C       write(6,*) string
      
C       id=12
C       call hropen(id,'AVATAR',string,'N',lrecl,ierr)
C       call hbnt(102,'Avatars test',' ')

C       call hbname(102,'VARAVAT',
...
C       end                                                                       
C C***************************************************************************
C       subroutine hbclose(Je_veux)                                              
C       implicit real*8 (a-h,o-z)                                                 
                                                          
C       call hrout(0,icycle,' ')
C       call hrend('NTP')
C       IF(Je_veux.EQ.1) CALL HREND('AVATAR')                                               
C       return                                                                    
C       end                                
C C ------------------------------------------------------------------

C       SUBROUTINE PRINTCOM
C c Impression des common
C 	parameter (max=250)                                                                       
C 	real*4 EXINI,ENERJ,BIMPACT,PLAB,TETLAB,PHILAB,ESTFIS
C 	integer AVV,ZVV,JREMN,KFIS,IZFIS,IAFIS
C         common/VARNTP/MASSINI,MZINI,EXINI,MULNCASC,MULNEVAP,
...
C 	WRITE(6,*) 'MASSINI,MZINI,EXINI,MULNCASC,MULNEVAP,
C      +MULNTOT,BIMPACT,NTRACK',MASSINI,MZINI,EXINI,MULNCASC,MULNEVAP,
C      +MULNTOT,BIMPACT,NTRACK
C         WRITE(6,*) 'ITYPCASC,AVV,ZVV,ENERJ,PLAB,TETLAB,PHILAB'
C      	DO i=1,NTRACK
C      	WRITE(6,*) ITYPCASC(i),AVV(i),ZVV(i),ENERJ(i),PLAB(i),
C      +TETLAB(i),PHILAB(i)
C         ENDDO
C       RETURN
C       END
C C ------------------------------------------------------------------
C       SUBROUTINE TRANSLAB(GAMREM,ETREM,CSREM,NOPART,NDEC)
C c Ce subroutine transforme dans un repere 1 les impulsions pcv des 
C c particules acv, zcv et de cosinus directeurs xcv, ycv, zcv calculees 
C c dans un repere 2.    
C c La transformation de lorentz est definie par GAMREM (gamma) et
C c ETREM (eta). La direction  du repere 2 dans 1 est donnees par les 
C c cosinus directeurs ALREM,BEREM,GAREM (axe oz du repere 2).
C c L'axe oy(2) est fixe par le produit vectoriel oz(1)*oz(2).
C c Le calcul est fait pour les particules de NDEC a iv du common volant.
C C Resultats dans le NTUPLE (common VARNTP) decale de NOPART (cascade).
    
C       REAL*8  GAMREM,ETREM,ER,PLABI(3),PLABF(3),R(3,3)
C       real*8  MASSE,PTRAV2,CSREM(3),UMA,MELEC,EL
C       real*4 acv,zpcv,pcv,xcv,ycv,zcv
C       common/volant/acv(200),zpcv(200),pcv(200),xcv(200),
C      s              ycv(200),zcv(200),iv
...
C       DATA UMA,MELEC/931.4942,0.511/

C C Matrice de rotation dans le labo:
C         SITET = SQRT(CSREM(1)**2+CSREM(2)**2)
C         IF(SITET.GT.1.E-6)THEN
C         CSTET = CSREM(3)
C         SIPHI = CSREM(2)/SITET
C         CSPHI = CSREM(1)/SITET	
...
C       DO i=NDEC,iv
C         intp = i + NOPART
C         AVV(intp) = NINT(acv(i))
C         ZVV(intp) = NINT(zpcv(i))
C         ITYPCASC(intp) = 0    
C C Transformation de Lorentz Remnan --> Labo:
C 	IF (AVV(intp).EQ.-1) THEN
C 		MASSE=138.00	!Cugnon
C C		IF (AVV(intp).EQ.1)  MASSE=938.2796	!Cugnon
C C		IF (AVV(intp).EQ.4)  MASSE=3727.42	!OK
C 	ELSE
C            CALL MGLMS(DBLE(acv(i)),DBLE(zpcv(i)),0,EL)
C 	   MASSE = zpcv(i)*938.27 + (acv(i)- zpcv(i))*939.56 + EL
C 	END IF
	
C 	ER = DSQRT(pcv(i)**2 + MASSE**2)
	
C 	PLABI(1) = pcv(i)*xcv(i)
C 	PLABI(2) = pcv(i)*ycv(i)
C 	PLABI(3) = ER*ETREM + GAMREM*pcv(i)*zcv(i)
	 
	
C 	PTRAV2 = PLABI(1)**2 +PLABI(2)**2 +PLABI(3)**2
C 	PLAB(intp) = DSQRT(PTRAV2) 
C 	ENERJ(intp) = DSQRT(PTRAV2 + MASSE**2) - MASSE
	
C C Rotation dans le labo:
C 	DO j=1,3
C 	    PLABF(j) = 0.
C 		DO k=1,3
C 		    PLABF(j) = PLABF(j) + R(j,k)*PLABI(k)
C 		END DO
C 	END DO
C C impulsions dans le nouveau systeme copiees dans /volant/
C 	pcv(i) = PLAB(intp)
C         ptrav2=sqrt(plabf(1)**2+plabf(2)**2+plabf(3)**2)
C       	IF(ptrav2.GE.1.e-6) THEN
C 	xcv(i) = PLABF(1)/ptrav2
C 	ycv(i) = PLABF(2)/ptrav2
C 	zcv(i) = PLABF(3)/ptrav2
C 	ELSE
C 	xcv(i)=1.
C 	ycv(i)=0.
C 	zcv(i)=0.
C 	ENDIF
C c impulsions dans le nouveau systeme copiees dans /VARNTP/	
C 	IF(PLAB(intp).GE.1.e-6) THEN

C 	bidon=PLABF(3)/PLAB(intp)
C 	if(bidon.gt.1)bidon=1.
C 	if(bidon.lt.-1.)bidon=-1.
C 	TETLAB(intp) = ACOS(bidon)
C           SITET = SIN(TETLAB(intp))
C           PHILAB(intp) = ATAN2(PLABF(2),PLABF(1))        
C           TETLAB(intp) = TETLAB(intp)*57.2957795
C           PHILAB(intp) = PHILAB(intp)*57.2957795
C 	ELSE
C 	  TETLAB(intp) = 90.
C 	  PHILAB(intp) = 0.
C 	ENDIF
...
C       END
C C-------------------------------------------------------------------------
C       SUBROUTINE TRANSLABPF(MASSE1,T1,P1,CTET1,PHI1,GAMREM,ETREM,R,
C      s   PLAB1,GAM1,ETA1,CSDIR)
C C Calcul de l'impulsion du PF (PLAB1, cos directeurs CSDIR(3)) dans le
C C systeme remnant et des coefs de Lorentz GAM1,ETA1 de passage  
C c du systeme PF --> systeme remnant.
C c 
C C Input: MASSE1, T1 (energie cinetique), CTET1,PHI1 (cosTHETA et PHI)
C C                    (le PF dans le systeme du Noyau de Fission (NF)).
C C	 GAMREM,ETREM les coefs de Lorentz systeme NF --> syst remnant, 
C C        R(3,3) la matrice de rotation systeme NF--> systeme remnant.
C C
C C      
C      	REAL*8 MASSE1,T1,P1,CTET1,PHI1,GAMREM,ETREM,R(3,3),
C      s   PLAB1,GAM1,ETA1,CSDIR(3),ER,SITET,PLABI(3),PLABF(3)
     
C 	ER = T1 + MASSE1
	
C 	SITET = DSQRT(1.-CTET1**2)
	
C C ----Transformation de Lorentz Noyau fissionnant --> Remnant:	
C 	PLABI(1) = P1*SITET*COS(PHI1)
C 	PLABI(2) = P1*SITET*SIN(PHI1)
C 	PLABI(3) = ER*ETREM + GAMREM*P1*CTET1
	
C C ----Rotation du syst Noyaut Fissionant vers syst remnant:
C 	DO j=1,3
C 	    PLABF(j) = 0.
C 		DO k=1,3
C 		    PLABF(j) = PLABF(j) + R(j,k)*PLABI(k)
C 		END DO
C 	END DO
C C ----Cosinus directeurs et coefs de la transf de Lorentz dans le
C c     nouveau systeme:	
C         PLAB1 = PLABF(1)**2+PLABF(2)**2+PLABF(3)**2
C         GAM1 = DSQRT(MASSE1**2 + PLAB1)/MASSE1
C         PLAB1 = DSQRT(PLAB1)
C         ETA1 = PLAB1/MASSE1
        
C 	IF(PLAB1.LE.1.E-6) THEN
C 	   CSDIR(1)=0.
C 	   CSDIR(2)=0.
C 	   CSDIR(3)=1.
C 	   ELSE   
C         DO i=1,3
C            CSDIR(i) = PLABF(i)/PLAB1
C         END DO
C 	ENDIF
...
C         END
C C-------------------------------------------------------------------------
C       subroutine getime(ic)
C       dimension jd(3),jt(3),ti(2)                                       
C       common/cput/cpa
...
C        call idate(jd)                                                    
C        call itime(jt)                                                    
C        write(*,10)jd(2),jd(1),jd(3),jt
C    10 format(20hCalculation starts :,2x,i2.2,1h/,i2.2,1h/,i4.4,1x,i2.2
C      &      ,1h:,i2.2,1h:,i2.2)       
C       else
C        cpe=etime(ti)
C        call idate(jd)                                                    
C        call itime(jt)                                                    
C        write(*,20)jd(2),jd(1),jd(3),jt
C    20 format(20hCalculation ends   :,2x,i2.2,1h/,i2.2,1h/,i4.4,1x,i2.2
C      &      ,1h:,i2.2,1h:,i2.2)       
C        write(*,30)cpe-cpa
C    30 format(20hExecution time is  :,f10.2,4h sec)       
C       endif
C       return
C       end
C C------------------------------------------------------------------------
C       SUBROUTINE LOR_AB(GAM,ETA,Ein,Pin,Eout,Pout)
C C  Transformation de lorentz brute pour vérifs.
C C	P(3) = P_longitudinal (transformé)
C C	P(1) et P(2) = P_transvers (non transformés)
C       DIMENSION Pin(3),Pout(3)
C       REAL*8 GAM,ETA,Ein

C       Pout(1) = Pin(1)
C       Pout(2) = Pin(2) 
C       Eout = GAM*Ein + ETA*Pin(3)
C       Pout(3) = + ETA*Ein + GAM*Pin(3)
C       RETURN
C       END
C C------------------------------------------------------------------------
C       SUBROUTINE ROT_AB(R,Pin,Pout)
C C  Rotation d'un vecteur
C       DIMENSION Pin(3),Pout(3)
C       REAL*8 R(3,3)
