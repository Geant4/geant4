...
// and all its terms.
//
// $Id: G4HEAntiLambdaInelastic.cc,v 1.2.8.1 1999/12/07 20:52:06 gunter Exp $
...
ApplyYourself( const G4Track &aTrack, G4Nucleus &targetNucleus )
  {
    ...
    if(incidentKineticEnergy < 1.)
      { 
        G4cout << "GHEAntiLambdaInelastic: incident energy < 1 GeV" << endl;
      }
    if(verboseLevel > 1)
      {
        G4cout << "G4HEAntiLambdaInelastic::ApplyYourself" << endl;
