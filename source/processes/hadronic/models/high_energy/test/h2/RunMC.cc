...
//*******************************************************
  ...
  //  Z = 1.;  
     ...
     G4Material *theMat = new G4Material(name="Uranium", density=18.95*g/cm3, nEl=1);
      G4Element *elMat  = new G4Element(name="Uranium", symbol="U", iz=79., a=196.96*g/mole);
    ...
    G4Step aStep;
     LogicalFrame->SetMaterial(theMat); 
   G4ThreeVector aPosition(0.,0.,0.);
   G4double aTime = 0.0;
     G4ParticleMomentum theDirection(0.,0.,1.);
     
     G4Nucleus aNucleus(196.96, 79);

    // ----------- the above is needed for building a track...... ------------
 
  G4String aFile = "KaonAu.plot";
  G4String aHFile = "KaonH.plot";
    ...
    do 
       ...
       for(i=0; i<maxpart; i++)
	 {
      ...
      G4DynamicParticle* aParticle =
             new G4DynamicParticle(G4KaonPlus::KaonPlus(), theDirection, 250.*GeV );
           G4Track* aTrack = new G4Track( aParticle, aTime, aPosition );
           aTrack->SetTouchable(theTouchable);
           aStep.SetTrack( aTrack );
       aFinalState = (G4ParticleChange*) theInteraction.ApplyYourself( *aTrack, aNucleus);
           G4Track * second;
	   ...
	   G4HEVector  * aVector;
	     ...
	     aVector = new G4HEVector(aSec);
	     pv[i] = *aVector;
	     delete aVector;
           }
       G4int vecLength = aFinalState->GetNumberOfSecondaries();
       G4int ngk = vecLength;
       G4int nchhmf = 0;
       G4double ymax = -100.;
       G4double ymax2 = -100.;
