...
//	ConstructParticle();
	...
	G4Nucleus Li(7.,3.); 		// Lithium
	G4Material* m_Li = new G4Material("Li",    3.,  6.941*g/mole, 0.535*g/cm3);

	G4Nucleus Be(9.,4.); 		// Beryllium
	G4Material* m_Be = new G4Material("Be",    4.,  9.01*g/mole, 1.848*g/cm3);

	G4Nucleus C(12.,6.); 		// Carbon
	G4Material* m_C = new G4Material("C",    6.,  12.0107*g/mole, 2.267*g/cm3);

	G4Nucleus Mg(24., 12.0);	// Magnesium
	G4Material* m_Mg = new G4Material("Mg",    12., 24.305*g/mole, 1.738*g/cm3);

	G4Nucleus Al(27.,13.); 		// Aluminum
	G4Material* m_Al = new G4Material("Al",   13.,  26.981538*g/mole, 2.700*g/cm3);

	G4Nucleus Au(200., 79.);	// gold
	G4Material* m_Au = new G4Material("Au",    79., 199.96655*g/mole, 19.300*g/cm3);

	G4Nucleus Ta(181., 73.);	// gold
	G4Material* m_Ta = new G4Material("Ta",    73., 180.95*g/mole, 16.65*g/cm3);

	G4Nucleus Pb(207., 82.);	// lead
	G4Material* m_Pb = new G4Material("Pb",    82., 207.19*g/mole, 11.35*g/cm3);

	G4Nucleus nuclei[] = {  H, Li, Be, C, Mg, Al, Au, Ta, Pb };  // 0 is dummy
	
	G4Material * materials[] = { m_H, m_Li, m_Be, m_C, m_Mg, m_Al, m_Au, m_Ta, m_Pb };
		
	G4StableIsotopes theIso;

	G4String material_name;
	G4cout << G4endl;
	for ( unsigned int achoice=0; achoice< sizeof(nuclei)/sizeof(H); achoice++)
	{   G4cout  << theIso.GetName(int(nuclei[achoice].GetZ())) 
//	         << "(" << nuclei[achoice].GetN() << "," << nuclei[achoice].GetZ() << ")"
	...
	for ( unsigned int achoice=0; achoice< sizeof(nuclei)/sizeof(H); achoice++)
	{
	   if ( materials[achoice]->GetName() == material_name ) inucleus=achoice;
	}
	if ( inucleus < 0 ) 
	{
	    G4cout << " material " << material_name << " not available" << G4endl;
	...
	G4cout << G4endl << " Projectile PDGcode " << std::flush;
	G4int proj_PDG;
	G4cin >> proj_PDG;
	G4ParticleDefinition* primary_def = G4ParticleTable::GetParticleTable()->FindParticle(proj_PDG);
	if (primary_def == NULL)
	{
             G4cout << "Particle with encoding "<<proj_PDG<<" does not exist!!!"<<G4endl;
	     G4Exception(" No such primary ");
	}
	...
	G4cout << G4endl << " Model to use FTF, QGSM ?"<< std::flush;
	G4String model_name="";
	G4cin >> model_name;
...
//   	aStringDecayMethod->Setpspin(pspin);
	   ...
	   model->SetFragmentationModel(stringDecay);
        } else if ( model_name == "FTF" )
	{
	   model= new G4FTFModel ;
//   		G4FTFModel model(ptparameter*GeV, minExtraMass*MeV,x0Mass*MeV);
	   G4VLongitudinalStringDecay * aStringDecayMethod=new G4LundStringFragmentation();
   //				new G4LundStringFragmentation(ptparameter*GeV);
   //	aStringDecayMethod->SetVectorMesonProbability(pspin);
	   G4VStringFragmentation * stringDecay=new G4ExcitedStringDecay(aStringDecayMethod);
	   model->SetFragmentationModel(stringDecay);
        } else {
	   G4cout << " model " << model_name << "not defined " << G4endl;
	   G4Exception(" No such model ");
	}
	
	hbfilename << model_name << "_";
	hbfilename << primary_def->GetParticleName() << "_";
	hbfilename << theIso.GetName(int(theNucleus.GetZ())) << "_"
	           << proj_momentum 
	           << ".hbook";
		   
	G4cout << "Creating Hbook file " << hbfilename << G4endl;
...
//------- 
    ...
    AIDA::ITree * tree( tf->create( hbfilename.str(),  "hbook", false, true ));
...
//-	HBookTuple Tuple_rap = hf->createHistogram1D(" rapidity",2,"//");
 	std::string description = "int PDG; float pt; float fx; float rapidity; float E; float Ekin;float p;float theta";
...
//G4cout << "indices " << Tinfo_PDG <<" " <<Tinfo_pt<<" " <<Tinfo_fx <<" " << Tinfo_rapidity<<" " <<Tinfo_E <<G4endl;
	...
	G4double Eprojectile = sqrt(sqr(proj_momentum) + sqr(primary_def->GetPDGMass()));
	G4double Ekinetic= Eprojectile - primary_def->GetPDGMass();
	G4double beta = proj_momentum / ( primary_def->GetPDGMass() + Eprojectile );
	G4double delta_eta = atanh ( beta );        
	G4cout << " offset for rapidity = " << delta_eta << G4endl;
	Info->fill(3.,delta_eta);
        
	G4VCrossSectionDataSet* cs = 0;
	if(primary_def == G4Proton::Proton() && material->GetElement(0)->GetZ() > 1.5) {
	  cs = new G4ProtonInelasticCrossSection();
	} else if(primary_def == G4Neutron::Neutron() && material->GetElement(0)->GetZ() > 1.5) {
	  cs = new G4NeutronInelasticCrossSection();
	} else {
	  cs = new G4HadronInelasticDataSet();
	}

	G4DynamicParticle dParticle(primary_def,G4ThreeVector(1., 0.,0.),Ekinetic);
	G4double cross_sec = 0.0;

	if(cs) {
	  cs->BuildPhysicsTable(*primary_def);
...
//           << "### factora = " << factor
      G4cout << " primary particle: " << primary_def->GetParticleName()  << 
...
//	G4ParticleMomentum direction(G4UniformRand(),G4UniformRand(),G4UniformRand());
	direction.setMag(1.);
	   ...
	   G4DynamicParticle primary(primary_def, 
				     direction,
				     Ekinetic);
...
//		  || pdef->GetPDGWidth() <= 0 || pdef->GetPDGLifeTime() > 1*ns 
		  )
		{
		  ...
		  if ( pname == "anti_neutron" ) NneutronBar++;
		   ...
		   for ( unsigned int aSec=0;secondaries!=NULL && aSec<secondaries->size(); aSec++)
		   {
		...
		if ( secondaries == NULL )
		{
		   Result.push_back(Result1[aResult]);
		   		   
		} else{
		  for (unsigned int aSecondary=0; aSecondary<secondaries->size(); aSecondary++)
		  {
		      Result1.push_back((*secondaries)[aSecondary]);
		  }
...
//  Now decay remaining Sigmas Lamda....
	   ...
	   for (unsigned int bResult=0; bResult < result1->size(); bResult++)
	   {
		G4ParticleDefinition * pdef;
...
//		  || pdef->GetPDGWidth() <= 0 || pdef->GetPDGLifeTime() > 1*ns 
		...
		if ( secondaries == NULL )
		{
		   (*result).push_back((*result1)[bResult]);
		   		   
		} else{
		  for (unsigned int aSecondary=0; aSecondary<secondaries->size(); aSecondary++)
		  {
		      (*result).push_back((*secondaries)[aSecondary]);
		  }
	   ...
	   numberOfStrings->fill(float(result->size()));
	   	
	   G4double Et=0., cutEt=0.,cutEta=0.;
	   G4double Etcurrent;
	   G4double rapidity;
	   G4double Epartsum=0;
	   debout << "Final Kinetic tracks  " << result->size() << G4endl;
	   G4LorentzVector Sum=0;
	   for (astring=0; astring < int(result->size()); astring++)
	   {
	   	if( (*result)[astring] == NULL ) G4cout << "got NULL" << G4endl;
		Etcurrent= (*result)[astring]->Get4Momentum().e() * fabs(sin((*result)[astring]->Get4Momentum().theta()));
		Et+= Etcurrent;
...
//		G4double fx2=2*(toCMS*px2).z()/Ecms;
		...
		G4double theta=(*result)[astring]->Get4Momentum().theta();
		
		int PDGCode = (*result)[astring]->GetDefinition()->GetPDGEncoding();
		Tinfo->fill( Tinfo_PDG,  PDGCode );
   	        Tinfo->fill( Tinfo_pt,  float(pt) );
	        Tinfo->fill( Tinfo_fx,  float(feynmanX) );
	        Tinfo->fill( Tinfo_E,  float(((*result)[astring]->Get4Momentum()).e()) );
		Tinfo->fill( Tinfo_Ekin,float(((*result)[astring]->Get4Momentum()).e() -
		                     (*result)[astring]->GetDefinition()->GetPDGMass()));
	        Tinfo->fill( Tinfo_rapidity, float(rapidity) );
		Tinfo->fill( Tinfo_p, float(((*result)[astring]->Get4Momentum()).vect().mag()) );
...
// 	   std::for_each(result->begin(),result->end(),DeleteKineticTrack());
	   for (unsigned int i1=0;i1<result->size();i1++)
	   {
	   	delete (*result)[i1];
	   }
	   delete result;
