...
//struct DeleteInvolvedNucleon  { void operator()(G4Nucleon          * aH){ delete aH;} };
...
void G4FTFModel::Init(const G4Nucleus & aNucleus, const G4DynamicParticle & aProjectile)
{
 ...
 G4cout<<"FTF init Proj Mass "<<theProjectile.GetMass()<<" "
                              <<theProjectile.GetMomentum()<<G4endl;
 G4cout<<"FTF init Proj B Q  "<<theProjectile.GetDefinition()->GetBaryonNumber()<<" "
       <<(G4int) theProjectile.GetDefinition()->GetPDGCharge()<<G4endl; 
 G4cout<<"FTF init Target A Z "<<aNucleus.GetA_asInt()<<" "<<
...
//       G4double ProjectileResidualMass=theProjectile.GetMass();
         ...
         {HighEnergyInter=false;} else {HighEnergyInter=true;}
      }
      else
      {
//-----------------------------------------------
...
//-----------------------------------------------
        ...
        { // Projectile is an anti-nucleus
         ...
         {
          if(aNucleon->GetDefinition() == G4Proton::Proton())       // Proton
          {aNucleon->SetParticleType(G4AntiProton::AntiProton());} 

          if(aNucleon->GetDefinition() == G4Neutron::Neutron())     // Neutron
        ...
        G4ThreeVector BoostVector=theProjectile.GetMomentum()
...
{ 
 ...
 G4cout<<"FTF GetResiduals of Nuclei "<<G4endl;
#endif
...
// -------------- Erase the target nucleons -----------
 ...
 G4cout<<"To continue - enter 1, to stop - ^C"<<G4endl;
...
{ //--- To store nucleons involved in the interaction  -------
	...
	}   // end of while ( (aNucleon = theTargetNucleus->GetNextNucleon()) )

#ifdef debugFTFmodel	
  G4cout<<"G4FTFModel::StoreInvolvedNucleon -------------"<<G4endl;
  G4cout<<"NumberOfInvolvedNucleonsOfTarget "<<
           NumberOfInvolvedNucleonsOfTarget<<G4endl<<G4endl;
#endif

        if(!GetProjectileNucleus()) return; // A projectile is a hadron
	...
	theProjectileNucleus->StartLoop();
        ...
        while ( (aProjectileNucleon=theProjectileNucleus->GetNextNucleon()) )
        {
         ...
         {  // Projectile nucleon was involved in the interaction.
           TheInvolvedNucleonsOfProjectile[NumberOfInvolvedNucleonsOfProjectile]=
                                                    aProjectileNucleon;
           NumberOfInvolvedNucleonsOfProjectile++;
         } // End of if ( aProjectileNucleon->AreYouHit() )
        } 
...
{ //--- Implementation of the reggeon theory inspired model-------
 ...
 G4cout<<"theProjectile.GetTotalMomentum() "<<
          theProjectile.GetTotalMomentum()<<G4endl;
 G4cout<<"theProjectile.GetTotalEnergy() "<<
          theProjectile.GetTotalEnergy()<<G4endl;
 G4cout<<"ExcitationE/WN "<<ExcitationE<<G4endl;
#endif
...
// ---------------- Reggeon cascading in target nucleus ---------------------
      ...
      { 
           ...
           G4double CreationTime=
                    aTargetNucleon->GetSplitableHadron()->GetTimeOfCreation();
      ...
      }      // end of for(G4int InvTN=0; InvTN < NumberOfInvolvedNucleon; InvTN++)
...
// ----- Nucleus-Nucleus Interaction ---- Destruction of Projectile ------
      ...
      { 
           ...
           G4double CreationTime=
                    aProjectileNucleon->GetSplitableHadron()->GetTimeOfCreation();
	   ...
	   }   // end of while ( (Neighbour = theProjectileNucleus->GetNextNucleon()) )
      }      // end of for(G4int InvPN=0; 
             //        InvPN < NumberOfInvolvedNucleonsOfProjectile; InvPN++)
...
G4bool G4FTFModel::PutOnMassShell()
{
  ...
  G4cout<<"PutOnMassShell start "<<G4endl;
      ...
      {
//---------------------------------------------------------------------
// ----------- The projectile is hadron or anti-baryon ----------------
// -------------- Properties of the projectile ------------------------

	G4LorentzVector Pprojectile(theProjectile.GetMomentum(),
                                    theProjectile.GetTotalEnergy());
...
//-------------------------------------------------------------
	G4LorentzVector Psum      = Pprojectile;
        ...
        G4double        SumMasses = Mprojectile + 20.*MeV; 
...
//--------------- Target nucleus ------------------------------
	...
	G4LorentzVector PtargetResidual(0.,0.,0.,0.);

        G4double ExcitationEnergyPerWoundedNucleon=
                  theParameters->GetExcitationEnergyPerWoundedNucleon();

#ifdef debugPutOnMassShell
  G4cout<<"ExcitationEnergyPerWoundedNucleon "
        << ExcitationEnergyPerWoundedNucleon<<G4endl;
#endif
	...
	while ( (aNucleon = theNucleus->GetNextNucleon()) )
        {
         ...
         {   // Involved nucleons
          ...
          SumMasses += 20.*MeV;   // Separation energy for a nucleon
...
#ifdef debugPutOnMassShell
  G4cout<<"Target residual: Charge, MassNumber "
        <<TargetResidualCharge<<" "<<TargetResidualMassNumber<<G4endl;
  G4cout<<"Target Initial Momentum "<< Ptarget<<G4endl;
  G4cout<<"Target Residual Momentum   "<< PtargetResidual<<G4endl;
#endif
// ---------------------------------------------------------------
        Psum += Ptarget;   
        PtargetResidual.setPz(0.); PtargetResidual.setE(0.);

        G4double TargetResidualMass(0.);
        if(TargetResidualMassNumber == 0)
        {
         TargetResidualMass=0.;
         TargetResidualExcitationEnergy=0.;
        }
        else
        {
         TargetResidualMass=G4ParticleTable::GetParticleTable()->GetIonTable()->
                 GetIonMass(TargetResidualCharge ,TargetResidualMassNumber);
         if(TargetResidualMassNumber == 1) {TargetResidualExcitationEnergy=0.;}
        }
 
        SumMasses += std::sqrt( sqr(TargetResidualMass)
...
#ifdef debugPutOnMassShell
  G4cout<<"Psum "<<Psum/GeV<<" GeV"<<G4endl;
  G4cout<<"SqrtS "<<SqrtS/GeV<<" GeV"<<G4endl;
  G4cout<<"SumMasses And TargetResidualMass "
        <<SumMasses/GeV<<" "<<TargetResidualMass/GeV<<" GeV"<<G4endl;
#endif
...
//------- Ascribing of the involved nucleons Pt and Xminus ----
  ...
  G4cout<<"Dcor "<<theParameters->GetDofNuclearDestruction()<<" DcorA "<<Dcor
        ...
        do    // while (!OuterSuccess)
        {
          ...
          {     // while DecayMomentum < 0.
            ...
            do      // while(!InerSuccess), while some Xi < 0, or > 1
            {
             InerSuccess=true;
	     ...
	     for(G4int i=0; i < NumberOfInvolvedNucleonsOfTarget; i++ )
             {
               ...
               G4LorentzVector tmp(tmpPt.x(), tmpPt.y(), Xminus,   
...
//---------------------------------------------------------------------------
             G4double DeltaX = (PtSum.x()-PtargetResidual.x())
                             /NumberOfInvolvedNucleonsOfTarget;
             G4double DeltaY = (PtSum.y()-PtargetResidual.y())
                             /NumberOfInvolvedNucleonsOfTarget;
             G4double DeltaXminus(0.);

             if(TargetResidualMassNumber == 0)
             {
              DeltaXminus = (XminusSum-1.)/NumberOfInvolvedNucleonsOfTarget;
             }
             else
             {
  ...
  G4cout<<"SqrtS, Mp+Mt, Mt "<<SqrtS/GeV<<" "<<(Mprojectile + std::sqrt(M2target))/GeV
  ...
  G4cout<<"i YtN Ypr YtN-YtA "<<i<<" "<<YtargetNucleon<<" "<<YtargetNucleus<<" "
        <<YtargetNucleon-YtargetNucleus<<G4endl;
           ...
           if((std::abs(YtargetNucleon-YtargetNucleus) > 2) || 
              (Yprojectile  < YtargetNucleon))        {OuterSuccess=false; break;} 
...
#endif
//-------------------------------------------------------------
        G4ThreeVector TargetResidual3Momentum(0.,0.,1.);
	...
	for(G4int i=0; i < NumberOfInvolvedNucleonsOfTarget; i++ )
        ...
        G4double Mt2Residual=sqr(TargetResidualMass) +
                  sqr(TargetResidual3Momentum.x())+sqr(TargetResidual3Momentum.y());
//==========================
#ifdef debugPutOnMassShell
  G4cout<<"WminusTarget TargetResidual3Momentum.z() "<<WminusTarget<<" "
        ...
        if(TargetResidualMassNumber != 0)
        {
         PzResidual=-WminusTarget*TargetResidual3Momentum.z()/2. + 
                 Mt2Residual/(2.*WminusTarget*TargetResidual3Momentum.z());
         EResidual = WminusTarget*TargetResidual3Momentum.z()/2. + 
                 Mt2Residual/(2.*WminusTarget*TargetResidual3Momentum.z());
        }
//==========================
  ...
  G4cout<<"To continue enter - 1, to break - ^C"<<G4endl;
  G4int Uzhi; G4cin>>Uzhi;
#endif
//-------------------------------------------------------------
        return true;
...
//      ProjectileResidualExcitationEnergy=0.;
	...
	G4LorentzVector Pproj(0.,0.,0.,0.);
	G4LorentzVector PprojResidual(0.,0.,0.,0.);
	...
	while ( (aNucleon = thePrNucleus->GetNextNucleon()) )
        {
         ...
         {   // Involved nucleons
          ...
          ProjectileResidualMassNumber--;
          ProjectileResidualCharge-=
                  std::abs((G4int) aNucleon->GetDefinition()->GetPDGCharge());
         }
         else
         {   // Spectator nucleons
...
#ifdef debugPutOnMassShell
  G4cout<<"Projectile residual: Charge, MassNumber "
        <<ProjectileResidualCharge<<" "<<ProjectileResidualMassNumber<<G4endl;
  G4cout<<"Initial Momentum   "<< Pproj<<G4endl;
  G4cout<<"Residual Momentum0 "<< PprojResidual<<G4endl;
#endif
...
//      TargetResidualExcitationEnergy=0.;
  ...
  G4cout<<"Target residual: Charge, MassNumber "
        <<TargetResidualCharge<<" "<<TargetResidualMassNumber<<G4endl;
  G4cout<<"Initial Momentum   "<< Ptarget<<G4endl;
  G4cout<<"Residual Momentum0 "<< PtargetResidual<<G4endl;
#endif

// ---------------------------------------------------------------
        Psum += Ptarget;   
// ---------------------------------------------------------------
          PprojResidual.setPz(0.);   PprojResidual.setE(0.);
        PtargetResidual.setPz(0.); PtargetResidual.setE(0.);
// ---------------------------------------------------------------

        G4double PrResidualMass(0.);
        ...
        else
        {
         ...
         if(ProjectileResidualMassNumber == 1) {ProjectileResidualExcitationEnergy=0.;}
        }
 

        SumMasses += std::sqrt( sqr(PrResidualMass)
                               +PprojResidual.vect().perp2());
...
//-------------------------------------------------------------
  ...
  G4cout<<"Psum "<<Psum/GeV<<" GeV"<<G4endl;
  G4cout<<"SqrtS "<<SqrtS/GeV<<" GeV"<<G4endl;
  G4cout<<"SumMasses, PrResidualMass and TargetResidualMass "
        <<SumMasses/GeV<<" "<<PrResidualMass/GeV<<" "
...
#ifdef debugPutOnMassShell
  G4cout<<"PrResidualMass ProjResidualExcitationEnergy "
        <<PrResidualMass/GeV<<" "<<ProjectileResidualExcitationEnergy<<" MeV"<<G4endl;
  G4cout<<"TargetResidualMass TargetResidualExcitationEnergy "
        <<TargetResidualMass/GeV<<" "<<TargetResidualExcitationEnergy<<" MeV"<<G4endl;
  G4cout<<"Sum masses "<<SumMasses/GeV<<G4endl;
#endif
	  ...
	  for(G4int i=0; i < NumberOfInvolvedNucleonsOfProjectile; i++ )
          {
            if((G4UniformRand() < ProbDeltaIsobar)&&(NumberOfDeltas < MaxNumberOfDeltas))
            {
              ...
              G4double MassNuc=
                 std::sqrt(sqr(projectileSplitable->GetDefinition()->GetPDGMass())
              ...
              if(SqrtS < SumMasses + MassDel - MassNuc) 
              { // Change cannot be acsepted!
          ...
          }   // end of for(G4int i=0; i < NumberOfInvolvedNucleonsOfProjectile; i++ )
        }   // end of if(thePrNucleus.GetMassNumber() != 1)
...
//-------------------------------------------------------------
//------- Ascribing of the involved nucleons Pt and Xminus ----
        G4double DcorP       = theParameters->GetDofNuclearDestruction()/
                                               thePrNucleus->GetMassNumber();
        G4double DcorT       = theParameters->GetDofNuclearDestruction()/
                                               theNucleus->GetMassNumber();
  ...
  G4cout<<"Y targetNucleus     "<<YtargetNucleus<<G4endl;
  G4cout<<"Dcor "<<theParameters->GetDofNuclearDestruction()<<" DcorP DcorT "<<DcorP
        <<" "<<DcorT<<" AveragePt2 "<<AveragePt2<<G4endl;
#endif
	     ...
	     for(G4int i=0; i < NumberOfInvolvedNucleonsOfProjectile; i++ )
             {
               ...
               G4ThreeVector tmpX=GaussianPt(DcorP*DcorP, 1.);
               Xplus=tmpX.x();
               XplusSum+=Xplus;

               G4LorentzVector tmp(tmpPt.x(), tmpPt.y(), Xplus,   
                                   aNucleon->Get4Momentum().e());
...
//---------------------------------------------------------------------------
             G4double DeltaX = (PtSum.x()-PprojResidual.x())
                             /NumberOfInvolvedNucleonsOfProjectile;
             G4double DeltaY = (PtSum.y()-PprojResidual.y())
                             /NumberOfInvolvedNucleonsOfProjectile;
             G4double DeltaXplus(0.);

             if(ProjectileResidualMassNumber == 0)
             {
              DeltaXplus = (XplusSum-1.)/NumberOfInvolvedNucleonsOfProjectile;
             }
             else
             {
              DeltaXplus = -1./thePrNucleus->GetMassNumber();
             }
	     ...
	     for(G4int i=0; i < NumberOfInvolvedNucleonsOfProjectile; i++ )
             {
               ...
               if(ProjectileResidualMassNumber == 0)               
               {
                if((Xplus <= 0.)   || (Xplus > 1.))    {InerSuccess=false; break;}
               } else
               {
                if((Xplus <= 0.)   || (Xplus > 1.) || 
                   (XplusSum <=0.) || (XplusSum > 1.)) {InerSuccess=false; break;}
               }                                          
	     ...
	     for(G4int i=0; i < NumberOfInvolvedNucleonsOfTarget; i++ )
             {
               ...
               G4ThreeVector tmpX=GaussianPt(DcorT*DcorT, 1.);
               Xminus=tmpX.x();
               XminusSum+=Xminus;
...
//-------------------------------------------------------------
          G4double DecayMomentum2= sqr(S) + sqr(M2proj) + sqr(M2target)
                                    -2.*S*M2proj - 2.*S*M2target 
                                       -2.*M2proj*M2target;

          WminusTarget=(S-M2proj+M2target+std::sqrt(DecayMomentum2))/2./SqrtS;
          WplusProjectile=SqrtS - M2target/WminusTarget;

          G4double Pzprojectile= WplusProjectile/2. - M2proj/2./WplusProjectile;
          G4double Eprojectile = WplusProjectile/2. + M2proj/2./WplusProjectile;
          G4double Yprojectile =0.5*std::log((Eprojectile+Pzprojectile)/
                                             (Eprojectile-Pzprojectile));

          G4double Pztarget    =-WminusTarget/2.    + M2target/2./WminusTarget;
          G4double Etarget     = WminusTarget/2.    + M2target/2./WminusTarget;
          G4double Ytarget     =0.5*std::log((Etarget+Pztarget)/
                                             (Etarget-Pztarget));
...
//--------------------- Now all is O.K. -----------------------
	  for(G4int i=0; i < NumberOfInvolvedNucleonsOfProjectile; i++ )
          {
           ...
           G4double Pz= WplusProjectile*Xplus/2. - Mt2/(2.*WplusProjectile*Xplus);
           G4double E = WplusProjectile*Xplus/2. + Mt2/(2.*WplusProjectile*Xplus);
  ...
  G4cout<<"i YpN Ypr YpN-YtA Ypr0 "<<i<<" "<<YprojNucleon<<" "<<Yprojectile<<" "
        <<YprojNucleon-Yprojectile<<" "<<YprojectileNucleus<<G4endl;
           ...
           if((std::abs(YprojNucleon-YprojectileNucleus) > 2) || 
              (Ytarget > YprojNucleon))        {OuterSuccess=false; break;} 
...
//-------------------------------------------------------------
        G4ThreeVector ProjectileResidual3Momentum(0.,0.,1.);
	...
	for(G4int i=0; i < NumberOfInvolvedNucleonsOfProjectile; i++ )
        ...
        G4double Mt2PrResidual=     sqr(PrResidualMass) +
           sqr(ProjectileResidual3Momentum.x())+sqr(ProjectileResidual3Momentum.y());
//==========================
#ifdef debugPutOnMassShell
  G4cout<<"WplusProjectile ProjectileResidual3Momentum.z() "<<WplusProjectile<<" "
        ...
        G4double PzPrResidual=0.;
        G4double EPrResidual =0.;
        if(ProjectileResidualMassNumber != 0)
        {
         PzPrResidual= WplusProjectile*ProjectileResidual3Momentum.z()/2. - 
                 Mt2PrResidual/(2.*WplusProjectile*ProjectileResidual3Momentum.z());
         EPrResidual = WplusProjectile*ProjectileResidual3Momentum.z()/2. + 
                 Mt2PrResidual/(2.*WplusProjectile*ProjectileResidual3Momentum.z());
        }
//==========================
...
//-------------------------------------------------------------
	...
	for(G4int i=0; i < NumberOfInvolvedNucleonsOfTarget; i++ )
        ...
        G4double Mt2TrResidual=sqr(TargetResidualMass) +
                  sqr(TargetResidual3Momentum.x())+sqr(TargetResidual3Momentum.y());
//==========================
#ifdef debugPutOnMassShell
  G4cout<<"WminusTarget TargetResidual3Momentum.z() "<<WminusTarget<<" "
        ...
        G4double PzTrResidual=0.;
        G4double ETrResidual =0.;
        if(TargetResidualMassNumber != 0)
        {
         PzTrResidual=-WminusTarget*TargetResidual3Momentum.z()/2. + 
                 Mt2TrResidual/(2.*WminusTarget*TargetResidual3Momentum.z());
         ETrResidual = WminusTarget*TargetResidual3Momentum.z()/2. + 
                 Mt2TrResidual/(2.*WminusTarget*TargetResidual3Momentum.z());
        }
//==========================
        TargetResidual4Momentum.setPx(TargetResidual3Momentum.x());
...
G4cout<<"G4FTFModel::ExciteParticipants() "<<G4endl;
...
#ifdef debugBuildString
  G4cout<<G4endl<<"Interaction # Status    "
        <<CurrentInteraction<<" "<<collision.GetStatus()<<G4endl;
  G4cout<<"Pr* Tr* "<<projectile<<" "<<target<<G4endl;
  G4cout<<"projectile->GetStatus target->GetStatus "
        <<projectile->GetStatus()<<" "<<target->GetStatus()<<G4endl;
  G4cout<<"projectile->GetSoftC  target->GetSoftC  "
  <<projectile->GetSoftCollisionCount()<<" "<<target->GetSoftCollisionCount()<<G4endl;
#endif
  ...
  G4cout<<"Elastic scattering"<<G4endl;
             ...
             if(!HighEnergyInter) 
             {
              ...
              G4bool Result=AdjustNucleons(projectile, ProjectileNucleon,
                                               target, TargetNucleon,
                                               Annihilation);
  ...
  G4cout<<"MaxNumOfInelCollisions "
        <<MaxNumOfInelCollisions<<G4endl;
#endif
             ...
             if(G4UniformRand()< (1.-projectile->GetSoftCollisionCount()
                                       /MaxNumOfInelCollisions))
             {
/*
              if(!HighEnergyInter) 
...
//G4cout<<"After  tar "<<target->Get4Momentum()<<" "
//      <<target->Get4Momentum().mag()<<G4endl;
#endif
              } else
              {
#ifdef debugBuildString
  G4cout<<"FTF excitation Non Successfull -> Elastic scattering "<<Successfull<<G4endl;
#endif
                Successfull = theElastic->ElasticScattering(projectile, target, theParameters)
                            || Successfull; 
              }
             } else 
             { // The inelastic interactition was rejected -> elastic scattering
#ifdef debugBuildString
...
*/
  ...
  G4cout<<"Annihilation"<<G4endl;
             ...
             if(theAnnihilation->Annihilate(projectile, target, 
                                              AdditionalString, theParameters))
             {
              Successfull = Successfull || true;
#ifdef debugBuildString
  G4cout<<"Annihilation successfull. "
        <<"*AdditionalString "<<AdditionalString<<G4endl;
...
//G4cout<<"After  tar "<<target->Get4Momentum()<<G4endl;
  ...
  G4cout<<"projectile->GetSoftC  target->GetSoftC  "
        <<projectile->GetSoftCollisionCount()<<" "<<target->GetSoftCollisionCount()<<G4endl;
  G4cout<<"ExciteParticipants() Successfull? "<<Successfull<<G4endl;
#endif
                                  ...
                                  G4bool Annihilation)
{
  ...
  G4cout<<"Proj is nucleus? "<<GetProjectileNucleus()<<G4endl;
  G4cout<<"Proj 4mom "<<SelectedAntiBaryon->Get4Momentum()<<G4endl;
  G4cout<<"Targ 4mom "<<SelectedTargetNucleon->Get4Momentum()<<G4endl;
  G4cout<<"Pr ResidualMassNumber Pr ResidualCharge Pr ResidualExcitationEnergy "
        <<ProjectileResidualMassNumber<<" "<< ProjectileResidualCharge<<" "
        <<ProjectileResidualExcitationEnergy<<G4endl;
  G4cout<<"Tr ResidualMassNumber Tr ResidualCharge Tr ResidualExcitationEnergy  "
        <<TargetResidualMassNumber<<" "<< TargetResidualCharge<<" "
        <<TargetResidualExcitationEnergy<<G4endl;
  G4cout<<"Collis. pr tr "<<SelectedAntiBaryon->GetSoftCollisionCount()
...
//------- Ascribing of the involved nucleons Pt and X --------------------
        ...
        G4double DcorP(0.), DcorT(0.);
        if(ProjectileResidualMassNumber != 0) DcorP=Dcor/
                                               (G4double) ProjectileResidualMassNumber;
        if(TargetResidualMassNumber != 0)     DcorT=Dcor/
  ...
  G4cout<<"case 1, hA prcol=0 trcol=0, AA prcol#0 trcol=0"<<G4endl;
#endif
        if(TargetResidualMassNumber < 1) return false;

        if(SelectedAntiBaryon->Get4Momentum().rapidity() <   // <= Uzhi Feb. 2013
                      TargetResidual4Momentum.rapidity()   ) return false;
        ...
        if(TargetResidualMassNumber == 1)
        {
         ...
         TargetResidual4Momentum=G4LorentzVector(0.,0.,0.,0.);
         return true;
        }

        G4LorentzVector Psum=SelectedAntiBaryon->Get4Momentum() + 
...
//-------------------------------
        ...
        if(TResidualMassNumber <= 1) {TResidualExcitationEnergy=0.;}

        G4double TResidualMass(0.);
        ...
        G4double SumMasses = SelectedAntiBaryon->Get4Momentum().mag() +
  ...
  G4cout<<"TResidualExcitationEnergy "<<TResidualExcitationEnergy<<G4endl;
        ...
        {
#ifdef debugAdjust
  G4cout<<"SqrtS < SumMasses - TNucleonMass "<<SqrtS<<" "<<SumMasses- TNucleonMass<<G4endl;
#endif
         if(SqrtS < SumMasses - TNucleonMass) {return false;} 
#ifdef debugAdjust
...
//        TResidualExcitationEnergy=0.;                // Uzhi Feb. 2013
          Stopping=true;
         }
#ifdef debugAdjust
  G4cout<<"SqrtS < SumMasses "<<SqrtS<<" "<<SumMasses<<G4endl;
  ...
  G4cout<<"Stopping "<<Stopping<<G4endl;
        ...
        if(Stopping)
        {
          ...
          // New projectile
          Ptmp.setPx(0.); Ptmp.setPy(0.); Ptmp.setPz(0.);
  ...
  G4cout<<Ptarget<<G4endl;
  G4cout<<TargetResidual4Momentum<<G4endl;
#endif
          return true;
        }
        ...
        G4ThreeVector PtNucleon(0.,0.,0.);
        G4double XminusNucleon(0.);
        G4ThreeVector PtResidual(0.,0.,0.);
        G4double XminusResidual(0.);

        G4int    NumberOfTries(0);
        G4double ScaleFactor(1.);
        G4bool OuterSuccess(true);
        do    // while (!OuterSuccess), while Ynucleon > Yprojectile
        {
          OuterSuccess=true;
...
*/
            ...
            if(TargetResidualMassNumber > 1)
            {
             do      // while(!InerSuccess), while Xn < 0, or > 1
             {
              InerSuccess=true;
              ...
              G4double Mtarget=
                       std::sqrt(sqr(TNucleonMass)  + PtNucleon.mag2() ) + 
                       std::sqrt(sqr(TResidualMass) + PtResidual.mag2());
              if(SqrtS < Mprojectile + Mtarget) {InerSuccess=false; continue;}

              G4ThreeVector tmpX=GaussianPt(DcorT*DcorT, 1.);
              G4double Xcenter = std::sqrt(sqr(TNucleonMass)  + PtNucleon.mag2())
                                /Mtarget;
              XminusNucleon=Xcenter + tmpX.x();
              if((XminusNucleon <= 0.) || (XminusNucleon >= 1.)) 
                    {InerSuccess=false; continue;}

              XminusResidual = 1. - XminusNucleon;
             } while(!InerSuccess);
            }
            else
            {
             XminusNucleon  =1.;
             XminusResidual =1.;  // It must be 0, but in the case calculation
                                  // of Pz, E is problematic.
            }
  ...
  G4cout<<"Yprojectile "<<Yprojectile<<G4endl;
          ...
          G4double Pz=-WminusTarget*XminusNucleon/2. + Mt2/(2.*WminusTarget*XminusNucleon);
          G4double E = WminusTarget*XminusNucleon/2. + Mt2/(2.*WminusTarget*XminusNucleon);
          G4double YtargetNucleon=0.5*std::log((E+Pz)/(E-Pz)); 

#ifdef debugAdjust
  G4cout<<"YtN Ytr YtN-Ytr "<<" "<<YtargetNucleon<<" "<<YtargetNucleus<<" "
        <<YtargetNucleon-YtargetNucleus<<G4endl;
  G4cout<<"YtN Ypr YtN-Ypr "<<" "<<YtargetNucleon<<" "<<Yprojectile   <<" "
        <<YtargetNucleon-Yprojectile<<G4endl;
#endif

          if((std::abs(YtargetNucleon-YtargetNucleus) > 2) || 
             (Yprojectile  < YtargetNucleon))        {OuterSuccess=false; continue;} 
        } while(!OuterSuccess);
//-------------------------------------------------------------
        G4double Pzprojectile=WplusProjectile/2. - M2projectile/2./WplusProjectile;
        G4double Eprojectile =WplusProjectile/2. + M2projectile/2./WplusProjectile;
...
//--------------------------------------------------------------
  ...
  G4cout<<"TargetResidualMassNumber TargetResidualCharge TargetResidualExcitationEnergy "
        ...
        if(TargetResidualMassNumber != 0)
        {
         ...
         Pz=-WminusTarget*XminusResidual/2. + Mt2/(2.*WminusTarget*XminusResidual);
         E = WminusTarget*XminusResidual/2. + Mt2/(2.*WminusTarget*XminusResidual);
...
#endif
        } else {TargetResidual4Momentum=G4LorentzVector(0.,0.,0.,0.);}
        return true;
      }
      else  //==========================================================
      if((SelectedAntiBaryon->GetSoftCollisionCount()    == 0) && 
  ...
  G4cout<<"case 2,  prcol=0 trcol#0"<<G4endl;
        ...
        if(ProjectileResidual4Momentum.rapidity() <= 
           SelectedTargetNucleon->Get4Momentum().rapidity()) return false;
        ...
        if(ProjectileResidualMassNumber == 1)
        {
         ...
         ProjectileResidual4Momentum = G4LorentzVector(0.,0.,0.,0.);
         return true;
        }

        G4LorentzVector Psum=ProjectileResidual4Momentum +
...
//-------------------------------
  ...
  G4cout<<"Stopping "<<Stopping<<G4endl;
        ...
        G4double M2projectile(0.);
        G4double WminusTarget(0.);
        G4double WplusProjectile(0.);

        G4ThreeVector PtNucleon(0.,0.,0.);
        G4double XplusNucleon(0.);
        G4ThreeVector PtResidual(0.,0.,0.);
        G4double XplusResidual(0.);

        G4int    NumberOfTries(0);
        G4double ScaleFactor(1.);
        G4bool OuterSuccess(true);
        do    // while (!OuterSuccess), while Ypr_nucleon < Ytarget_nucleon
        {
          OuterSuccess=true;
        ...
        <<" "<< Mprojectile<<" "<<Mtarget+Mprojectile<<G4endl;
            ...
            if(ProjectileResidualMassNumber > 1)
            {
             do      // while(!InerSuccess), while Xn < 0, or > 1
             {
              ...
              if((XplusNucleon <= 0.) || (XplusNucleon >= 1.)) {InerSuccess=false; continue;}

              XplusResidual = 1. - XplusNucleon;
             } while(!InerSuccess);
            } else
            {
             XplusNucleon  =1.;
             XplusResidual = 1.; // It must be 0, but in the case determination
                                 // of Pz and E will be problematic.
            }
#ifdef debugAdjust
  G4cout<<"TNucleonMass PtNucleon XplusNucleon "<<TNucleonMass<<" "<<PtNucleon
        <<" "<<XplusNucleon<<G4endl;
  G4cout<<"TResidualMass PtResidual XplusResidual "<<TResidualMass<<" "
        <<PtResidual<<"  "<<XplusResidual<<G4endl;
#endif
  ...
  G4cout<<"SqrtS < Mtarget + std::sqrt(M2projectile) "<<SqrtS<<"  "<<Mtarget
        <<" "<< std::sqrt(M2projectile)<<" "<<Mtarget+std::sqrt(M2projectile)<<G4endl;
#endif
          ...
          WplusProjectile=(S+M2projectile-M2target+std::sqrt(DecayMomentum2))/2./SqrtS;
  ...
  G4cout<<"DecayMomentum2 "<<DecayMomentum2<<G4endl;
  G4cout<<"WminusTarget WplusProjectile "<<WminusTarget<<" "<<WplusProjectile<<G4endl;
  G4cout<<"YtargetNucleon "<<Ytarget<<G4endl;
#endif

          G4double Mt2 = sqr(TNucleonMass) + PtNucleon.mag2();
          G4double Pz= WplusProjectile*XplusNucleon/2. - 
                        Mt2/(2.*WplusProjectile*XplusNucleon);
          G4double E = WplusProjectile*XplusNucleon/2. + 
                        Mt2/(2.*WplusProjectile*XplusNucleon);
  ...
  G4cout<<"YpN Ytr YpN-Ytr "<<" "<<YprojectileNucleon<<" "<<Ytarget   <<" "
        <<YprojectileNucleon-Ytarget<<G4endl;
#endif

          if((std::abs(YprojectileNucleon-YprojectileNucleus) > 2) || 
             (Ytarget  > YprojectileNucleon))        {OuterSuccess=false; continue;} 
        } while(!OuterSuccess);
//-------------------------------------------------------------
//      New target
        G4double Pztarget =-WminusTarget/2. + M2target/2./WminusTarget;
        ...
        Ptarget.transform(toLab);                // The work with the target nucleon
                                                 // is finished at the moment.
        SelectedTargetNucleon->Set4Momentum(Ptarget);
#ifdef debugAdjust
...
//      New projectile
  ...
  G4cout<<"Proj after in Lab "<<Pprojectile<<G4endl;
        ...
        {
         Mt2 = sqr(TResidualMass) + PtResidual.mag2();
         Pz= WplusProjectile*XplusResidual/2. - Mt2/(2.*WplusProjectile*XplusResidual);
         E = WplusProjectile*XplusResidual/2. + Mt2/(2.*WplusProjectile*XplusResidual);
         ...
         ProjectileResidual4Momentum.transform(toLab);
        } else {ProjectileResidual4Momentum = G4LorentzVector(0.,0.,0.,0.);}
...
//    It can be in the case of nucleus-nucleus interaction only!
  ...
  G4cout<<"Targ res Init "<<TargetResidual4Momentum<<G4endl;
  G4cout<<"ProjectileResidualMassNumber ProjectileResidualCharge "
        <<ProjectileResidualMassNumber<<" "<<ProjectileResidualCharge<<G4endl;
  G4cout<<"TargetResidualMassNumber TargetResidualCharge "<<TargetResidualMassNumber<<" "
...
//--------------------------------
        ...
        G4double PResidualExcitationEnergy=ProjectileResidualExcitationEnergy +
                                            ExcitationEnergyPerWoundedNucleon;
        if(PResidualMassNumber <= 1) {PResidualExcitationEnergy=0.;}

        G4double PResidualMass(0.);
...
//------------------------------------------------------
  ...
  G4cout<<"SqrtS < SumMasses+ PResidualExcitationEnergy + TResidualExcitationEnergy "
        ...
        <<G4endl;
         ...
         { 
          Stopping=true; return false;
          if(PResidualExcitationEnergy <= 0.){TResidualExcitationEnergy = SqrtS - SumMasses;}
          else 
          if(TResidualExcitationEnergy <= 0.){PResidualExcitationEnergy = SqrtS - SumMasses;}
          else
          {
           G4double Fraction=(SqrtS - SumMasses)/(PResidualExcitationEnergy +
  ...
  G4cout<<"YprojectileNucleus XcenterP "<<YprojectileNucleus<<G4endl;
        ...
        G4ThreeVector PtNucleonP(0.,0.,0.);
        G4double      XplusNucleon(0.);
        G4ThreeVector PtResidualP(0.,0.,0.);
        G4double      XplusResidual(0.);

        G4ThreeVector PtNucleonT(0.,0.,0.);
        G4double XminusNucleon(0.);
        G4ThreeVector PtResidualT(0.,0.,0.);
        G4double XminusResidual(0.);

        G4int    NumberOfTries(0);
        G4double ScaleFactor(1.);
        G4bool OuterSuccess(true);
        do    // while (!OuterSuccess), while Ytr_nucleon > Y_projectile_nucleon
        {
          OuterSuccess=true;
  ...
  //G4cout<<"NumberOfTries ScaleFactor "<<NumberOfTries<<" "<<ScaleFactor<<G4endl;
            ...
            {
             PtNucleonP =G4ThreeVector(0.,0.,0.);
            }
            ...
            {
             PtNucleonT = G4ThreeVector(0.,0.,0.);
            }
            ...
            { 
             do      // while(!InerSuccess), while Xn < 0, or > 1
             {
              ...
              G4ThreeVector tmpX=GaussianPt(DcorP*DcorP, 1.);
              G4double XcenterP = std::sqrt(sqr(PNucleonMass)  + PtNucleonP.mag2())
                                 /Mprojectile;
...
#ifdef debugAdjust
  //G4cout<<"XplusNucleon XplusResidual 2 "<<XplusNucleon<<" "<<XplusResidual<<G4endl;
  //{G4int Uzhi; G4cin>>Uzhi;}
#endif
            } else
            {
             XplusNucleon  = 1.;
             XplusResidual = 1.; // It must be 0
            }
            ...
            { 
             do      // while(!InerSuccess), while Xn < 0, or > 1
             {
              ...
              G4ThreeVector tmpX=GaussianPt(DcorT*DcorT, 1.);
              G4double XcenterT = std::sqrt(sqr(TNucleonMass)  + PtNucleonT.mag2())
                                 /Mtarget;
              XminusNucleon=XcenterT + tmpX.x();
              if((XminusNucleon <= 0.) || (XminusNucleon >= 1.)) {InerSuccess=false; continue;}
            ...
            {
             XminusNucleon  = 1.;
             XminusResidual = 1.;  // It must be 0
            }
#ifdef debugAdjust
  G4cout<<"PtNucleonP "<<PtNucleonP<<" "<<PtResidualP<<G4endl;
  G4cout<<"XplusNucleon XplusResidual "<<XplusNucleon<<" "<<XplusResidual<<G4endl;
  G4cout<<"PtNucleonT "<<PtNucleonT<<" "<<PtResidualT<<G4endl;
  G4cout<<"XminusNucleon XminusResidual "<<XminusNucleon<<" "<<XminusResidual<<G4endl;
#endif
...
//            if(SqrtS < std::sqrt(M2projectile)+std::sqrt(M2target)) {OuterSuccess=false; break;} // Uzhi 31.08.13
          ...
          G4double YprojectileNucleon=0.5*std::log((E+Pz)/(E-Pz));
                   ...
                   Pz=-WminusTarget*XminusNucleon/2. + 
                       Mt2/(2.*WminusTarget*XminusNucleon);
                   E = WminusTarget*XminusNucleon/2. + 
                       Mt2/(2.*WminusTarget*XminusNucleon);
          G4double YtargetNucleon=0.5*std::log((E+Pz)/(E-Pz)); 
...
//  be duplicate in the List ( to unique G4VSplitableHadrons)
 ...
 G4cout<<"Number of projectile strings "<<primaries.size()<<G4endl;	
#endif
...
//            if(primaries[ahadron]->GetStatus() <= 1) {isProjectile=true; }
  ...
  G4cout<<"Quarks on the FirstString ends "
       <<FirstString->GetRightParton()->GetPDGcode()<<" "
       <<FirstString->GetLeftParton()->GetPDGcode()<<G4endl;
#endif
	 }

#ifdef debugBuildString
  G4cout<<"Check 1 string "
        <<strings->operator[](0)->GetRightParton()->GetPDGcode()<<" "
        <<strings->operator[](0)->GetLeftParton()->GetPDGcode()<<G4endl<<G4endl;
#endif

         std::for_each(primaries.begin(), primaries.end(), DeleteVSplitableHadron());
         primaries.clear();
        }     // end of if(!GetProjectileNucleus())
        else
        {  // Projectile was a nucleus           if(GetProjectileNucleus())
  ...
  G4cout<<"Nucleon #, status, intCount "<<ahadron<<" "
     ...
     <<TheInvolvedNucleonsOfProjectile[ahadron]->GetSplitableHadron()->GetSoftCollisionCount();
#endif
  ...
  G4cout<<"Case1 aProjectile->GetStatus() ==0"<<G4endl;
	     ...
	     if(FirstString  != 0) strings->push_back(FirstString);
             if(SecondString != 0) strings->push_back(SecondString);
            } else if((aProjectile->GetStatus() ==1) && 
                      (aProjectile->GetSoftCollisionCount() !=0)) 
  ...
  G4cout<<"Case2 aProjectile->GetStatus() !=0 St==1 SoftCol!=0"<<G4endl;
	     ...
	     if(FirstString  != 0) strings->push_back(FirstString);
             if(SecondString != 0) strings->push_back(SecondString);
                       ...
                       (aProjectile->GetSoftCollisionCount() ==0)) && HighEnergyInter)
...
#ifdef debugBuildString
  G4cout<<" Strings are build for nucleon marked for an interaction, but"
...
//G4cout<<TheInvolvedNucleonsOfProjectile[ahadron]->GetSplitableHadron()<<G4endl;
  ...
  G4cout<<"Building of target-like strings"<<G4endl;
	...
	for (G4int ahadron=0; ahadron < NumberOfInvolvedNucleonsOfTarget ; ahadron++)
	{
            G4VSplitableHadron * aNucleon=
               TheInvolvedNucleonsOfTarget[ahadron]->GetSplitableHadron();
  ...
  G4cout<<" 1 case A string is build"<<G4endl;
            ...
            else if((aNucleon->GetStatus() ==1            )  &&  
                    (aNucleon->GetSoftCollisionCount() !=0)     )
...
// ?????????             delete aNucleon;
  ...
  G4cout<<"4 case A string is not build"<<G4endl;
            ...
            else if(aNucleon->GetStatus() == 2)
            {// A nucleon was involved in Reggeon cascading
             FirstString=0; SecondString=0;
             theExcitation->CreateStrings(aNucleon,    isProjectile,
                                          FirstString, SecondString,
                                          theParameters);
...
#ifdef debugBuildString
  G4cout<<G4endl<<"theAdditionalString.size() "<<theAdditionalString.size()<<G4endl<<G4endl;
#endif
...
#ifdef debugFTFmodel
  G4cout<<"NumberOfInvolvedNucleonsOfProjectile "
        <<NumberOfInvolvedNucleonsOfProjectile<<G4endl;
  G4cout<<"ProjectileResidualExcitationEnergy ProjectileResidual4Momentum "
        <<ProjectileResidualExcitationEnergy<<"  "
        <<ProjectileResidual4Momentum<<G4endl;
#endif
...
//--------------------------------
        G4double  DeltaExcitationE(0.);
        G4LorentzVector DeltaPResidualNucleus = G4LorentzVector(0.,0.,0.,0.);

        if(NumberOfTargetParticipant != 0)
        {
         DeltaExcitationE=TargetResidualExcitationEnergy/
                          (G4double) NumberOfTargetParticipant;
         DeltaPResidualNucleus = TargetResidual4Momentum/
                          (G4double) NumberOfTargetParticipant;
        }
	...
	for(G4int i=0; i < NumberOfInvolvedNucleonsOfTarget; i++ )
        {
         ...
         else
         {
          ...
          aNucleon->SetBindingEnergy(0.);
         }
        } 

#ifdef debugFTFmodel
  G4cout<<"NumberOfTargetParticipant "<<NumberOfTargetParticipant<<G4endl;
  G4cout<<"TargetResidual4Momentum  "<< TargetResidual4Momentum <<G4endl;
#endif
//================================
...
//--------------------------------
        DeltaExcitationE = 0.;
        DeltaPResidualNucleus = G4LorentzVector(0.,0.,0.,0.);
...
//G4cout<<"DeltaExcitationE DeltaPResidualNucleus "<<DeltaExcitationE<<" "<<DeltaPResidualNucleus<<G4endl;
	for(G4int i=0; i < NumberOfInvolvedNucleonsOfProjectile; i++ )
        {
         G4Nucleon * aNucleon = TheInvolvedNucleonsOfProjectile[i];
         G4VSplitableHadron * projectileSplitable=aNucleon->GetSplitableHadron();
  ...
  G4cout<<"ProjectileResidual4Momentum "<< ProjectileResidual4Momentum<<G4endl;
#endif
    }
#ifdef debugFTFmodel
