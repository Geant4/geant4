...
//
//
// $Id: G4DiffractiveExcitation.cc,v 1.2 2007-12-20 15:38:06 vuzhinsk Exp $
// ------------------------------------------------------------
...
// Without de-excitation
     ...
     G4double ProjectileBackgroundWeight;
     ...
     if( absPDGcode > 1000 )                        //------Projectile is baryon --------
     {
      ...
      ProjectileDiffStateMinMass = 1.1;           // GeV
      ProjectileBackgroundWeight = 0.;
      AveragePt2 = 0.3;                           // GeV^2
     }
     else if( absPDGcode == 211 || PDGcode ==  111) //------Projectile is Pion -----------
     {
      ProbProjectileDiffraction  = 0.; //0.1;                  // Vova
      ProjectileDiffStateMinMass = 0.6;           // GeV
      ProjectileBackgroundWeight = 0.;
      AveragePt2 = 0.3;                           // GeV^2
     }
     ...
     {
      ProbProjectileDiffraction  = 0.1;
      ProjectileDiffStateMinMass = 0.7;           // GeV 1.1
      ProjectileBackgroundWeight = 0.5;
      AveragePt2 = 0.3;                           // GeV^2
     }
     ...
     ProjectileDiffStateMinMass = ProjectileDiffStateMinMass * GeV;
...
// -------------------- Target parameters ----------------------------------------------
     ...
     G4double ProbTargetDiffraction  = 0.; // 0.1;    Vova
     G4double TargetDiffStateMinMass = 1.1*GeV;
     G4double TargetBackgroundWeight = 0.;
...
//G4cout<<" SqrtS P+mim max "<<SqrtS<<" "<<PMinusMin<<" "<<PMinusMax<<G4endl;
             ...
             if(G4UniformRand() < ProjectileBackgroundWeight)
             {
              PMinusNew=PMinusMax-(PMinusMax-PMinusMin)*G4UniformRand();
             }
             else
             {
...
//             if(PZcms2 < 0 ) {PZcms2=0;};
            ...
            if(G4UniformRand() < TargetBackgroundWeight)
            {
             TPlusNew=TPlusMax-(TPlusMax-TPlusMin)*G4UniformRand();
            }
            else
            {
...
//G4cout<<"TargetDiffStateMinMass "<<sqrt(TargetDiffStateMinMass2)<<G4endl;
 ...
 ((Pprojectile+Qmomentum).mag2() <  ProjectileDiffStateMinMass2) && //No double Diffraction
 ((Ptarget    -Qmomentum).mag2() <  TargetDiffStateMinMass2    ));
         }

//G4int Uzhiinp; G4cin>>Uzhiinp;
