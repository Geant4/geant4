...
#include "G4V3DNucleus.hh"        // Uzhi
#include "G4ParticleTable.hh"     // Uzhi
#include "G4IonTable.hh"          // Uzhi
#include "G4PhysicalConstants.hh" // Uzhi
...
// Start copy of FTF Model
  ...
  NumberOfInvolvedNucleonsOfTarget= 0;                                // Uzhi
  ...
  while ( ( NuclearNucleon = theNucleus->GetNextNucleon() ) ) {  /* Loop checking, 07.08.2015, A.Ribon */  
...
//copy G4FTFModel::GetStrings()
  ...
  do
  {
   ...
   } while( (!Success) && ++internalLoopCounter < maxNumberOfInternalLoops );  /* Loop checking, 07.08.2015, A.Ribon */
   if ( internalLoopCounter >= maxNumberOfInternalLoops ) {
     Success = false;
   }
   
   if ( Success ) {
     PerformDiffractiveCollisions();   // G4bool -------------------------------------
     ...
     if( theProjectileSplitable && theProjectileSplitable->GetStatus() == 0) Success = DeterminePartonMomenta(); 
     /*
     G4int attempts(0);
     do 
     { 
     ...
     G4cout<<"if( attempts > 100 ) break; "<<attempts<<" Success "<<Success<<G4endl;
     */
     if(!Success) PrepareInitialState( thePrimary );
   }
  } while( (!Success) && ++loopCounter < maxNumberOfLoops );  /* Loop checking, 07.08.2015, A.Ribon */
  if ( loopCounter >= maxNumberOfLoops ) {
    Success = false;
    #ifdef debugQGSParticipants
      G4cout<<"NOT Success ======" <<G4endl;
    #endif
  }
  ...
  if ( Success ) {
    ...
    // To calculate residual nucleus properties
    GetResiduals();
...
// Erasing of target involved nucleons.
  #ifdef debugQGSParticipants
     G4cout<<"Involved target nucleons "<<NumberOfInvolvedNucleonsOfTarget<<G4endl;
  #endif

  if ( NumberOfInvolvedNucleonsOfTarget != 0 ) {
     for ( G4int i = 0; i < NumberOfInvolvedNucleonsOfTarget; i++ ) {
      G4VSplitableHadron* aNucleon = TheInvolvedNucleonsOfTarget[i]->GetSplitableHadron();
//G4cout<<i<<" aNucleon "<<aNucleon<<G4endl;
      if ( (aNucleon != 0 ) && (aNucleon->GetStatus() >= 1) ) delete aNucleon;
     }
  }

// Erasing of projectile involved nucleons.
//G4cout<<"InvolvedProj "<<NumberOfInvolvedNucleonsOfProjectile<<G4endl;
  if ( NumberOfInvolvedNucleonsOfProjectile != 0 ) {
     for ( G4int i = 0; i < NumberOfInvolvedNucleonsOfProjectile; i++ ) {
       G4VSplitableHadron* aNucleon = TheInvolvedNucleonsOfProjectile[i]->GetSplitableHadron();
       if ( aNucleon ) delete aNucleon;
  ...
  #ifdef debugQGSParticipants
     G4cout<<"Target nucleons from soft interactions "<<theTargets.size()<<G4endl<<G4endl;
  #endif
  std::for_each(theTargets.begin(), theTargets.end(), DeleteSplitableHadron());
  theTargets.clear();
// G4cout<<"End -------------------- "<<G4endl;
// Uzhi	delete aProjectile; // Not defined here
...
void G4QGSParticipants::PrepareInitialState( const G4ReactionProduct& thePrimary ) 
{
  ...
  theNucleus->StartLoop();
  G4Nucleon* aNucleon;
  while ( ( aNucleon = theNucleus->GetNextNucleon() ) )  /* Loop checking, 07.08.2015, A.Ribon */ 
  {
...
/*
G4cout<<"Plab Stot Sel Sin Sdif Sum Diff"<<G4endl;
G4double Plab=0.;
for(G4int i=0; i < 20;i++)
{
 Plab +=1.*GeV; G4double Elab=std::sqrt(sqr(Plab)+sqr(938.0));
 G4double SS= 2.0*sqr(938.0)+ 2.0*938.0*Elab;
 G4double Stot=theProbability.GetTotalCrossSection(SS)/millibarn;
 G4double Sel =theProbability.GetElasticCrossSection(SS)/millibarn;
 G4double Sin =theProbability.GetInelasticCrossSection(SS)/millibarn;
 G4double Sdif=theProbability.GetDiffractiveCrossSection(SS)/millibarn;
 G4cout<<Plab/GeV<<" "<<Stot<<" "<<Sel<<" "<<Sin<<" "<<Sdif<<G4endl;
}

for(G4int i=0; i < 12;i++)
{
 Plab *=2.0; G4double Elab=std::sqrt(sqr(Plab)+sqr(938.0));
 G4double SS= 2.0*sqr(938.0)+ 2.0*938.0*Elab;
 G4double Stot=theProbability.GetTotalCrossSection(SS)/millibarn;
 G4double Sel =theProbability.GetElasticCrossSection(SS)/millibarn;
 G4double Sin =theProbability.GetInelasticCrossSection(SS)/millibarn;
...
G4cout<<"Bimp Ptot Pel Pnd Pdif Pin "<<G4endl;
G4double Imp=-0.1;
...
for(G4int i=0;i<30;i++)
{
 Imp+=0.1;
 Plab =160.*GeV; 
 G4double Elab=std::sqrt(sqr(Plab)+sqr(938.0));
 G4double SS= 2.0*sqr(938.0)+ 2.0*938.0*Elab;
 G4double PbT=theProbability.GetTotalProbability(SS,sqr(Imp*fermi));
 G4double PbE=theProbability.GetElasticProbability(SS,sqr(Imp*fermi));
 G4double PbND=theProbability.GetNondiffractiveProbability(SS,sqr(Imp*fermi));
 G4double PbD=theProbability.GetDiffractiveProbability(SS,sqr(Imp*fermi));
 G4double PbI=theProbability.GetInelasticProbability(SS,sqr(Imp*fermi));

 G4cout<<Imp<<" "<<PbT<<" "<<PbE<<" "<<PbND<<" "<<PbD<<" "<<PbI<<G4endl;
}
G4cout<<G4endl;

G4cout<<"Bimp Pnd P1 P2 P3 P4 P5 P6 P7 P8 P9 P10 Sum Dif"<<G4endl;
Imp=-0.1;
G4double Sum;
for(G4int i=0;i<30;i++)
{
 ...
 {
  Sum+=theProbability.GetCutPomeronProbability(SS,sqr(Imp*fermi),j);
  G4cout<<" "<<theProbability.GetCutPomeronProbability(SS,sqr(Imp*fermi),j);
 }
 G4cout<<" "<<Sum<<" "<<PbND-Sum<<G4endl;
}
...
// Check reaction threshold
  ...
  if ( ! pNucleon ) {
    #ifdef debugQGSParticipants
    G4cout << "BAD situation: pNucleon is NULL ! Leaving immediately!" << G4endl;
    #endif
    return;
  }
  ...
  ModelMode = SOFT;
  if (sqr(ThresholdMass + ThresholdParameter) > s_nucleus) {ModelMode = DIFFRACTIVE;}
  if (sqr(ThresholdMass + QGSMThreshold) > s_nucleus)      {ModelMode = DIFFRACTIVE;}
...
// first find the collisions HPW
  ...
  const G4int maxNumberOfLoops = 1000;
  G4int loopCounter = -1;
  while( (theInteractions.size() == 0) && ++loopCounter < maxNumberOfLoops )  /* Loop checking, 07.08.2015, A.Ribon */
  {
...
// loop over nucleons to find collisions
   ...
   {
    if(totalCuts>1.5*thePrimary.GetKineticEnergy()/GeV) {break;}

    nucleonCount++;

    G4LorentzVector nucleonMomentum=pNucleon->Get4Momentum();
//Uzhi It does not need nucleonMomentum.setE(nucleonMomentum.e()-pNucleon->GetBindingEnergy());
    ...
    #ifdef debugQGSParticipants
//       G4cout << "SqrtS_hN (GeV), Prob. Inter.= "<<std::sqrt(s_nucleon)/GeV<<" "
//                                                 <<Probability<<G4endl;		  
//       G4cout<<"ModelMode "<<ModelMode<<" (0 - soft; 1 - difraction)"<<G4endl;
    #endif
    ...
    if (Probability > rndNumber)
    {
     ...
     #ifdef debugQGSParticipants
        G4cout<<"Target nucleon - "<<pNucleon->GetDefinition()->GetParticleName()<<G4endl;
        G4cout<<"Diffraction Prob.  "
              <<theProbability.GetDiffractiveProbability(s_nucleon, Distance2);
        G4cout<<" Realative DD Prob. "
              <<theProbability.GetDiffractiveProbability(s_nucleon, Distance2)/Probability<<G4endl;
     #endif
//
     if ((theProbability.GetDiffractiveProbability(s_nucleon, Distance2)
...
/* Uzhi
      ...
      {
	theSingleDiffExcitation.ExciteParticipants(theProjectileSplitable, aTarget);
      }
      else
      {
...
*/
      #ifdef debugQGSParticipants
         G4cout<<"Diffractive interaction occurs"<<G4endl;
      #endif
...
//    totalCuts += 1;                                No cuts for diffraction
     }
     else
     {
	...
	// sample nCut+1 (cut Pomerons) pairs of strings can be produced
        #ifdef debugQGSParticipants
           G4cout<<"Non-diffractive interaction occurs, nCutMax "<<nCutMax<<G4endl;
           G4cout<<"Probs of cuts, nCut Prob"<<G4endl;
        #endif
	...
	for(nCut = 0; nCut < nCutMax; nCut++)
	{
	 running[nCut] = theProbability.GetCutPomeronProbability(s_nucleon, Distance2, nCut + 1);
         #ifdef debugQGSParticipants
            G4cout<<"                        "<<nCut<<" "<<running[nCut]<<G4endl;
         #endif
         if(nCut!=0) running[nCut] += running[nCut-1];
	}
        #ifdef debugQGSParticipants
           G4cout<<"                      Sum"<<" "<<running[nCut-1]<<G4endl;
        #endif
...
//nCut=1;
        totalCuts += nCut + 1;
        #ifdef debugQGSParticipants
           G4cout<<"Number of cuts in the interaction "<<nCut+1<<G4endl;
        #endif
	...
	theProjectileSplitable->IncrementCollisionCount(nCut+1);
    ...
    }    // End of if (Probability > rndNumber)
   }     // End of while( (pNucleon = theNucleus->GetNextNucleon()) )
  ...
  #endif
// Uzhi	return theProjectileSplitable;
}
...
{ //To store nucleons involved in the interaction
  ...
  #ifdef debugQGSParticipants
     G4cout << G4endl<<"G4QGSParticipants::StoreInvolvedNucleon() ...."<<G4endl
...
{ // Implementation of the reggeon theory inspired model of nuclear destruction 
  ...
  for ( G4int InvTN = 0; InvTN < InitNINt; InvTN++ ) { 
    ...
    while ( ( Neighbour = theTargetNucleus->GetNextNucleon() ) ) {  /* Loop checking, 07.08.2015, A.Ribon */
      if ( ! Neighbour->AreYouHit() ) {
        ...
        if ( G4UniformRand() < GetCofNuclearDestruction() *
          ...
          #ifdef debugQGSParticipants
             G4cout<<"Target nucleon involved in reggeon cascading - "
  ...
  #ifdef debugQGSParticipants
     G4cout <<"Number of involved nucleons "<<NumberOfInvolvedNucleonsOfTarget - InitNINt<<G4endl;
  #endif
  return;
}   
...
G4bool G4QGSParticipants::PutOnMassShell() {
  ...
  if ( ! isProjectileNucleus ) {  // hadron-nucleus collision
    ...
    if ( theProjectileSplitable ) theProjectileSplitable->Set4Momentum(Pprojectile);   // Uzhi 14.05.2015
/*   Uzhi
...
*/ // Uzhi
    #ifdef debugPutOnMassShell
//Uzhi G4cout << "Final proj. mom in Lab. " << primary->Get4Momentum() << G4endl;
...
void G4QGSParticipants::PerformDiffractiveCollisions()
{
	...
	for(i = 0; i < theInteractions.size(); i++)
	{
		G4InteractionContent* anIniteraction = theInteractions[i];
		#ifdef debugQGSParticipants
		   G4cout<<"Interaction No and its status "
		         <<i<<" "<<theInteractions[i]->GetStatus()<<G4endl;
		#endif
                if(theInteractions[i]->GetStatus() == 1) // Selection of diffractive interactions
                {
		   G4VSplitableHadron* aTarget = anIniteraction->GetTarget();
			...
			G4cout<<"The targ. before inter "
                        ...
                        G4cout<<"IsSingleDiffractive? "<<IsSingleDiffractive()<<G4endl;
		   #endif
		   if(IsSingleDiffractive())
		   {
		    theSingleDiffExcitation.ExciteParticipants(theProjectileSplitable, aTarget);
...
//G4cout<<"theProjectileSplitable->GetStatus() "<<theProjectileSplitable->GetStatus()<<G4endl;
  ...
  for(i = theTargets.begin(); i != theTargets.end(); i++ )
  {
   Psum += (*i)->Get4Momentum();
   #ifdef debugQGSParticipants
      G4cout<<NuclNo<<" "<<(*i)->Get4Momentum()<<G4endl;
   #endif
   NuclNo++;
  }

  G4LorentzRotation toCms( -1*Psum.boostVector() );       // Uzhi 14.05.2015 Start
//  G4LorentzRotation toLab( toCms.inverse() );

  G4LorentzVector Ptmp = toCms*Projectile4Momentum;
//  Projectile4Momentum=Ptmp;
...
//  Ptarget.transform( toCms );
  ...
  for(i = theTargets.begin(); i != theTargets.end(); i++ )
  {
   ...
   (*i)->Set4Momentum( tmp );
   #ifdef debugQGSParticipants
      G4cout<<"Target nucleon NuclNo, 4Mom "<<" "<<NuclNo<<" "<<(*i)->Get4Momentum()<<G4endl;
   #endif
   Target4Momentum += tmp;
   NuclNo++;
  }                                                        // Uzhi 14.05.2015 End
  ...
  #ifdef debugQGSParticipants
     G4cout<<"Sum of tar 4 momentum "<<Target4Momentum<<G4endl<<G4endl;
  #endif
...
//G4cout<<" NumberOfUnsampledSeaQuarks "<<NumberOfUnsampledSeaQuarks<<" ";
     tmp.setPz(SampleX(Xmin, NumberOfUnsampledSeaQuarks, 2*nSeaPair, aBeta)*(1.0-SumZ)); 
                     ...
                     ProjSumMt2perX +=tmp.perp2()/tmp.pz();
...
//For valence quark
   aParton = theProjectileSplitable->GetNextParton();   // for quarks
   #ifdef debugQGSParticipants
      G4cout<<"Val quark :   "<<" "<<aParton->GetDefinition()->GetParticleName();
   #endif
   aPtVector = GaussianPt(SigPt, aHugeValue);
   tmp.setPx(aPtVector.x()); tmp.setPy(aPtVector.y());
   SumPx += aPtVector.x();   SumPy += aPtVector.y();
...
// Work with target nucleons 
   ...
   NuclNo=0;
//G4cout<<"theTargets.size() "<<theTargets.size()<<G4endl;
   for(i = theTargets.begin(); i != theTargets.end(); i++ )
   {
    nSeaPair = (*i)->GetSoftCollisionCount()-1;
    #ifdef debugQGSParticipants
       G4cout<<"nSeaPair of target N "<<nSeaPair<<" "
             <<(*i)->Get4Momentum()<<G4endl;
    #endif
//----------------------
...
SumZw+=tmp.pz();
                     NumberOfUnsampledSeaQuarks--;
                     TargSumMt2perX +=tmp.perp2()/tmp.pz();
                     aParton->Set4Momentum(tmp);
...
//G4cout<<"ProjSumMt2perX "<<ProjSumMt2perX<<" TargSumMt2perX "<<TargSumMt2perX<<G4endl;

   if( ProjSumMt      + TargSumMt      > SqrtS ) {
//G4cout<<"if( ProjSumMt      + TargSumMt      > SqrtS ) "<<ProjSumMt<<" "<<TargSumMt<<" "<<ProjSumMt+TargSumMt<<" "<<SqrtS<<G4endl; 
Success = false; continue;}
   if( std::sqrt(ProjSumMt2perX) + std::sqrt(TargSumMt2perX) > SqrtS ) {
//G4cout<<"if( std::sqrt(ProjSumMt2perX) + std::sqrt(TargSumMt2perX) > SqrtS ) "<<std::sqrt(ProjSumMt2perX)<<" "<<std::sqrt(TargSumMt2perX)<<" "<<std::sqrt(ProjSumMt2perX) + std::sqrt(TargSumMt2perX)<<" "<<SqrtS<<G4endl; 
Success = false; continue;}

//G4cout<<"End loop Success "<<Success<<G4endl;
//G4int Uzhi; G4cin>>Uzhi;
...
//G4cout<<"Tmp.mt2() "<<Tmp.mt2()<<" "<<Tmp.perp2()<<" perp2"<<G4endl;
     Tmp.setPz(projectileWplus*z/2.0 - Tmp.perp2()/(2.0*z*projectileWplus));
     Tmp.setE( projectileWplus*z/2.0 + Tmp.perp2()/(2.0*z*projectileWplus)); 
     Tmp.transform( toLab );                                                     // Uzhi 14.05.2015
//                        Tmp.boost(-bstToCM);                         // Backward transformation
                     aParton->Set4Momentum(Tmp);
...
NuclNo=0;
   for(i = theTargets.begin(); i != theTargets.end(); i++ )
   {
    nSeaPair = (*i)->GetSoftCollisionCount()-1;
    #ifdef debugQGSParticipants
       G4cout<<"nSeaPair of target N "<<nSeaPair<<" "<<NuclNo<<G4endl;
    #endif
NuclNo++;
    for (G4int aSeaPair = 0; aSeaPair < nSeaPair; aSeaPair++)
    {
     ...
     #endif
     Tmp =aParton->Get4Momentum(); z=Tmp.z(); 
     Tmp.setPz(-targetWminus*z/2.0 + Tmp.perp2()/(2.0*z*targetWminus));
     Tmp.setE(  targetWminus*z/2.0 + Tmp.perp2()/(2.0*z*targetWminus)); 
     Tmp.transform( toLab );                                                     // Uzhi 14.05.2015
//                        Tmp.boost(-bstToCM);                         // Backward transformation
                     aParton->Set4Momentum(Tmp);
...
SampleX(G4double anXmin, G4int nSea, G4int totalSea, G4double aBeta)
{
 G4double Xmin=anXmin; G4int Nsea=totalSea;   Xmin*=1.; Nsea++;// Must be erased Uzhi 12.05.2015 
   ...
   G4double result = r1/r12;    // Vova, chto ty delaesh?
//G4cout<<"Sampling results "<<result<<G4endl;
   return result;
} 
...
//?? unused     G4double      CreationTime = theProjectileSplitable->GetTimeOfCreation();
     ...
     #ifdef debugQGSParticipants
	G4cout << "Diffr. Pr. St " << aPair->GetParton1()->GetPDGcode()   << " "
...
//-----------------------------------------
  #ifdef debugQGSParticipants
     G4cout<<"Soft interactions"<<G4endl;
  #endif 
...
G4int IntNo=0;    
	...
	G4PartonPair * aPair = NULL;
        ...
        #ifdef debugQGSParticipants
           G4cout<<"an interaction No soft coll. "<<IntNo<<" "
...
void G4QGSParticipants::GetResiduals() {                  // Uzhi 29.05.2015
...
{
//G4cout<<"G4QGSParticipants::SelectInteractions "<<G4endl;
...
//G4cout<<"aPrMom "<<aPrimaryMomentum<<G4endl;
//G4cout<<"Targ   "<<aTargetNMomentum<<G4endl;
...
//G4cout<<"ModelMode "<<ModelMode<<G4endl;
	// first find the collisions HPW
	std::for_each(theInteractions.begin(), theInteractions.end(), DeleteInteractionContent());
	theInteractions.clear();
	G4int totalCuts = 0;
...
//G4cout<<"theCurrent "<<theCurrent<<G4endl;
        if ( pNucleon ) {
	  G4QGSMSplitableHadron* aTarget = new G4QGSMSplitableHadron(*pNucleon);
//G4cout<<"aTarget "<<aTarget<<G4endl;
          pNucleon->Hit(aTarget);
//G4cout<<"pNucleon->Hit(aTarget)"<<G4endl;
...
*/
//G4cout<<"Gamma List Diffraction"<<G4endl;
      		G4InteractionContent * aInteraction = new G4InteractionContent(theProjectileSplitable);
      		theProjectileSplitable->SetStatus(1*theProjectileSplitable->GetStatus());
	  ...
	  {
		// nondiffractive soft interaction occurs
//G4cout<<"Gamma List Non-Diffraction -----"<<G4endl;
