...
//
//
// $Id: G4LundStringFragmentation.cc 108072 2017-12-19 15:31:21Z gcosmo $
// GEANT4 tag $Name:  $ 1.8
//
// -----------------------------------------------------------------------------
...
G4LundStringFragmentation::G4LundStringFragmentation()
{
    ...
    SetDiquarkBreakProbability(0.5); //(0.05); 
    SetStrangenessSuppression((1.0 - 0.10)/2.0);// 0.446 -> 0.424     Uzhi Nov. 2017 0.12

    SetDiquarkSuppression(0.150);    // 0.067 -> 0.15      Uzhi Nov. 2017
...
// For treating of small string decays
   ...
   pspin_meson = 0.55;  SetVectorMesonProbability(pspin_meson);          // Uzhi Dec. 2017 0.5 
   ...
   vectorMesonMix[1] = 0.45;
   vectorMesonMix[2] = 0.0;  
   vectorMesonMix[3] = 0.45;                                                                // 0.5 Uzhi Dec. 2017
...
G4double Mth = 1250.0;                                 // 2 Mk + Mpi    1130.0 * 1.5  1500
if     ( NumberOfpossibleBaryons == 3 ){Mth = 2000.0;} // Mlambda/Msigma + Mk + Mpi 2520 3400
else if( NumberOfpossibleBaryons == 4 ){Mth = 2550.0;} // 2 Mlambda/Msigma + Mk + Mpi   4620
else {}

//       ActualProb = ProbSaS * (1.0 - sqr(sqr(Mth/StringMass))); if( ActualProb < 0.0 ) ActualProb = 0.0;
       ActualProb = ProbSaS * (1.0 - G4Pow::GetInstance()->powA( Mth/StringMass , 4.0 )); 
//       ActualProb = ProbSaS * (1.0 - G4Exp( -(StringMass -Mth)*500. )); 
if( ActualProb < 0.0 ) ActualProb = 0.0;
       SetStrangenessSuppression((1.0-ActualProb)/2.0);

#ifdef debug_LUNDfragmentation
G4cout<<"StrMass DiquarkSuppression Pow "<<StringMass<<" "<<GetDiquarkSuppress()<<G4endl; 
#endif   
// Uzhi Nov. 2017 End
...
// create new String from old, ie. keep Left and Right order, but replace decay
	    ...
	    if(Iter >= MaxIter) {SetDiquarkSuppression(ProbDqADq);
                                 SetStrangenessSuppression((1.0-ProbSaS)/2.0); 
                                 G4KineticTrack * Hadron =0; 
                                 return Hadron;}                                 // Uzhi Nov. 2017
	    Iter++;

       } while(HadronMass + MinMass > StringMass);

SetDiquarkSuppression(ProbDqADq);                                                // Uzhi Nov. 2017
SetStrangenessSuppression((1.0-ProbSaS)/2.0);                                    // Uzhi Nov. 2017
...
{
G4double StrSup=GetStrangeSuppress();                           // Uzhi Dec. 2017
...
StrangeSuppress=(1.0 - 0.16)/2.0;                               // Uzhi Dec. 2017
      pDefPair QuarkPair = CreatePartonPair(IsParticle,false);  // no diquarks wanted
StrangeSuppress=StrSup;                                         // Uzhi Dec. 2017
...
{ 
  ...
  G4cout<<"Mass of the string is not sufficient to produce the hadron!"<<G4endl;
	...
	G4double HadronMt, Pt, Pt2, phi;             // Uzhi Nov. 2016 Introduction of Mt distribution
	 ...
	 HadronMt = HadronMass - 300.0*G4Log(G4UniformRand()); //200 -> 300                           // Uzhi Dec. 2017
...
//	if(! ( ((std::abs(PDGEncodingOfDecayParton) > 1000) && (HadronEncoding > 1000)) || (HadronEncoding == 113) ) ) // Uzhi Dec. 2017
...
//	if((std::abs(PDGEncodingOfDecayParton) > 1000) || (HadronEncoding == 113))      // Uzhi dec. 2017
	if(std::abs(PDGEncodingOfDecayParton) > 1000)     
	{
		G4double an = 2.5; 
//Uzhi		if(pHadron->GetPDGIsospin() > 0.5) an=0.5;//0.75;
...
// Uzhi 24 June 2017: Sample4Momentum ascribes LeftMom.pz() along positive Z axis for baryons in many cases.
...
//        G4double ProbIsotropy = sqr(sqr(938.0/InitialMass));                       // Uzhi Oct. 2017
#ifdef debug_LUNDfragmentation
...
//	if((Mass > 930. || AntiMass > 930.) && (G4UniformRand() < ProbIsotropy))     // Uzhi Oct. 2017
...
//	{
                ...
                const G4int maxNumberOfLoops = 1000;
G4double SigmaQTw=SigmaQT;                                                                // Uzhi Oct. 2017
if(Mass > 930. || AntiMass > 930.) SigmaQT *=(1.0-0.55*sqr((Mass+AntiMass)/InitialMass)); // Uzhi Oct. 2017
...
if(Mass > 930. || AntiMass > 930.) SigmaQT=SigmaQTw;;                                    // Uzhi Oct. 2017
...
//	}                                                                                // Uzhi Oct. 2017
  ...
  G4cout<<"Smass Mom "<<AntiMom->getX()<<" "<<AntiMom->getY()<<" "<<AntiMom->getZ()<<" "<<AntiMom->getT()<<G4endl;
#endif
}
...
//--------------------------
   ...
   MesonWeight[1][1][4]=pspin_meson*(vectorMesonMix[0]);
//--------------------------
...
//---------------------------------------
   ...
   G4double ProbUUbar = 0.33; // 0.40 -> 0.33 Uzhi Nov. 2017 
