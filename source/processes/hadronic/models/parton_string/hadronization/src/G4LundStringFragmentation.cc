...
void G4LundStringFragmentation::SetMinimalStringMass(const G4FragmentingString  * const string)  
{
   ...
   { 
     if ( Number_of_squarks==0 ) {EstimatedMass += 700.*MeV;}
...
G4KineticTrackVector* G4LundStringFragmentation::FragmentString(const G4ExcitedString& theString)
{
   ...
   //G4cout<<"StringFragm"<<G4endl;
   G4FragmentingString aString(theString);
   //G4cout<<"SetMinMass "<<G4endl;
   SetMinimalStringMass(&aString);
   //G4cout<<"Check for Fragm "<<G4endl;
                                            ...
                                            G4KineticTrackVector * RightVector)
{
   ...
   //G4cout<<"StrMass "<<StringMass<<" q "<<string->GetLeftParton()->GetParticleName()<<" "<<string->GetRightParton()->GetParticleName()<<G4endl;
...
//----------------------------------------------------------------------------------------------------------
void G4LundStringFragmentation::Sample4Momentum(G4LorentzVector* Mom, G4double Mass, G4LorentzVector* AntiMom,
                                                  ...
                                                  G4double Px, G4double Py)
{
   ...
   { // ---------------- Di-quark fragmentation ----------------------
     ...
     do {
          z = zmin + G4UniformRand()*(zmax-zmin);
          //yf = sqr(z-zmin)*(z-zmin)/sqr(zmax-zmin)/(zmax-zmin);
          yf = sqr(z-zmin)/sqr(zmax-zmin);
     } while (G4UniformRand()*maxYf > yf);
     return z;
   }
}
                                                 ...
                                                 G4ParticleDefinition * & RightHadron)
{
   ...
   for ( G4int ProdQ=1; ProdQ < 4; ProdQ++ )
   {
     ...
     do { // while(Meson[AbsIDquark-1][ProdQ-1][StateQ]<>0);
          ...
          LeftHadron=G4ParticleTable::GetParticleTable()->FindParticle(-Baryon[ADi_q1-1][ADi_q2-1][ProdQ-1][StateADiQ]);
          ...
          do { // while(Baryon[Di_q1-1][Di_q2-1][ProdQ-1][StateDiQ]<>0);
               ...
               RightHadron=G4ParticleTable::GetParticleTable()->FindParticle(+Baryon[Di_q1-1][Di_q2-1][ProdQ-1][StateDiQ]);
          ...
          do { // while(Meson[AbsIDanti_quark-1][ProdQ-1][StateAQ]<>0);
               ...
               if ( StringMass > LeftHadronMass + RightHadronMass )
               {
                 ...
                 FS_Weight[NumberOf_FS]=std::sqrt(FS_Psqr)*MesonWeight[AbsIDquark-1][ProdQ-1][StateQ]*
                                                           ...
                                                           Prob_QQbar[ProdQ-1];
                 //G4cout<<AbsIDquark-1<<" "<<ProdQ-1<<" "<<StateQ<<" "<<MesonWeight[AbsIDquark-1][ProdQ-1][StateQ]<<G4endl;
                 //G4cout<<AbsIDanti_quark-1<<" "<<ProdQ-1<<" "<<StateAQ<<" "<<MesonWeight[AbsIDanti_quark-1][ProdQ-1][StateAQ]<<G4endl;
                 //G4cout<<Prob_QQbar[ProdQ-1]<<G4endl;
                 if ( string->GetLeftParton()->GetPDGEncoding()>0 )
                 {
...
G4int G4LundStringFragmentation::SampleState(void) 
{
   ...
   //G4cout<<"NumberOf_F "<<NumberOf_FS<<G4endl;
