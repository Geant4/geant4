...
//
//
// $Id: G4HadronBuilder.cc,v 1.8 2009-04-16 09:17:33 mkossov Exp $
                                                    ...
                                                    G4ParticleDefinition * white)
{
  ...
  else return Barion(black, white, SpinHalf); // SpinThreeHalfBarion for 3 identical quarks
}
                                                     ...
                                                     G4ParticleDefinition * white)
{
  ...
  else return Barion(black, white,SpinThreeHalf);
}
                                             ...
                                             G4ParticleDefinition* white, Spin theSpin)
{
  ...
  if (std::abs(charge) > 2 || std::abs(3*(charge - G4int(charge*1.001))) > perCent )
  {
     ...
     G4cerr << "    PGDcode input quark1/quark2 : " << black->GetPDGEncoding()
...
#ifdef G4VERBOSE
  if ( MesonDef == 0 )
    G4cerr<<" G4HadronBuilder - Warning: No particle for PDGcode= "<< PDGEncoding <<G4endl;
  if ((black->GetPDGCharge() + white->GetPDGCharge() - MesonDef->GetPDGCharge()) > perCent)
    G4cerr << " G4HadronBuilder - Warning: Incorrect Charge : Quark1/2 = " 
           << black->GetParticleName() << " / " << white->GetParticleName() 
           << " resulting Hadron is " << MesonDef->GetParticleName() << G4endl;
#endif
                                              ...
                                              G4ParticleDefinition* white, Spin theSpin)
{
  ...
  G4ParticleDefinition* BarionDef =
                        G4ParticleTable::GetParticleTable()->FindParticle(PDGEncoding);
#ifdef G4VERBOSE
  if (BarionDef == 0 ) G4cerr << " G4HadronBuilder - Warning: No particle for PDGcode= "
                              << PDGEncoding << G4endl;
  if((black->GetPDGCharge() + white->GetPDGCharge() - BarionDef->GetPDGCharge()) > perCent)
    G4cerr << " G4HadronBuilder - Warning: Incorrect Charge : DiQuark/Quark = " 
           << black->GetParticleName() << " / " << white->GetParticleName() 
           << " resulting Hadron " << BarionDef->GetParticleName() << G4endl;
#endif
