...
//
// 230308 Tentative modified in a short-lived particle production by Y-H. Sato and A. Haga
// 230308 Energy difference evaluated by "GetTotalEnergy" calculated in Mean Field (Y-H. Sato and A. Haga)
//
...
/*
G4LightIonQMDCollision::G4LightIonQMDCollision( const G4LightIonQMDCollision& obj )
: fdeltar ( obj.fdeltar )
...
, fepse ( obj.fepse )
{
   ...
   if ( obj.theMeanField != NULL ) {
      theMeanField = new G4LightIonQMDMeanField;
...
G4LightIonQMDCollision & G4LightIonQMDCollision::operator= ( const G4LightIonQMDCollision& obj)
{
   fdeltar = obj.fdeltar;
   fbcmax0 = obj.fbcmax1;
   fepse = obj.fepse;
   ...
   for ( G4int i = 0 ; i < n ; i++ )
   {
      ...
      if ( theSystem->GetParticipant( i )->GetDefinition()->IsShortLived() )
      {
         ...
         G4LorentzVector p40 = theSystem->GetParticipant( i )->Get4Momentum();
          ...
          G4double eini = theMeanField->GetTotalEnergy(); // R-JQMD, Skyrme, RMF
...
*/
                   ...
                   if ( id > 0 )
                   {
                      ...
                      {
                         //081118
                         //G4cout << "G4LightIonQMDCollision id >2; id= " << id  << G4endl; 
                      }
                   }
...
//          EnergyCheck  
             ...
             G4double efin = theMeanField->GetTotalEnergy();  // R-JQMD, Skyrme, RMF
...
G4bool G4LightIonQMDCollision::CalFinalStateOfTheBinaryCollision( G4int i , G4int j )
{

//081103
   //G4cout << "CalFinalStateOfTheBinaryCollision " << i << " " << j << " " << theSystem->GetTotalNumberOfParticipant() << G4endl;

    G4int k = theSystem->GetTotalNumberOfParticipant(); // added by Y-H. Sato and A.H.

   G4bool result = false;
   G4bool energyOK = false; 
   G4bool pauliOK = false; 
   G4bool abs = false;
    G4bool pion_prod = false; // added by Y-H. Sato and A.H.
    //G4bool pion_abs = false; // added by Y-H. Sato and A.H.
...
//071031
   ...
   for ( G4int iitry = 0 ; iitry < 4 ; iitry++ )
   {
      ...
      G4LorentzVector p4kx_new(G4ThreeVector(0,0,0) , 0 ); // added by Y-H. S. and A.H.
      G4KineticTrackVector* secs = NULL;
                ...
                = secs->begin() ; it != secs->end() ; it++ )
            {
               ...
               if ( iti == 1 )
               {
                  ...
                  //theSystem->GetParticipant( j )->SetMomentum( p4jx_new.v() );
                   ...
                   // added by Y-H. S and A.H.
                   if((*it)->GetDefinition()->IsShortLived())
                   {
                       G4KineticTrackVector * dec = (*it)->Decay();
                       
                       G4int ita = 0;
                       for(G4KineticTrackVector::iterator jter=dec->begin(); jter != dec->end(); jter++)
                       {
                           //G4cout << "decay " << (*jter)->GetDefinition()->GetParticleName() << " " << (*jter)->Get4Momentum()/GeV << " " << (*jter)->GetDefinition()->GetBaryonNumber() << G4endl;
                           
                           if(ita == 0)
                           {
                               theSystem->SetParticipant( new G4QMDParticipant( (*jter)->GetDefinition() , (*jter)->Get4Momentum().v()/GeV , (*jter)->GetPosition()/fermi ) );
                               //G4cout << "decay " << (*jter)->GetDefinition()->GetParticleName() << " " << (*jter)->Get4Momentum()/GeV << G4endl;
                               //theMeanField->Update();
                               //G4cout << "decay " << (*jter)->GetDefinition()->GetParticleName() << " " << theMeanField->GetTotalEnergy() << " " << eini << G4endl;
                               theSystem->GetParticipant( k )->SetDefinition( (*jter)->GetDefinition() );
                               //theMeanField->Update();
                               //G4cout << "decay " << (*jter)->GetDefinition()->GetParticleName() << " " << theSystem->GetParticipant( k )->GetNuc() << " " << theMeanField->GetTotalEnergy() << G4endl;
                               p4kx_new = (*jter)->Get4Momentum()/GeV;
                               theSystem->GetParticipant( k )->SetMomentum( p4kx_new.v() );
                               //theSystem->ShowParticipants();
                               
                           }
                           else if(ita == 1)
                           {
                               theSystem->GetParticipant( j )->SetDefinition( (*jter)->GetDefinition() );
                               p4jx_new = (*jter)->Get4Momentum()/GeV;
                               theSystem->GetParticipant( j )->SetMomentum( p4jx_new.v() );
                               pion_prod = true;
                               //theMeanField->Update();
                               //G4cout << "decay " << (*jter)->GetDefinition()->GetParticleName() << " " << theMeanField->GetTotalEnergy() << " " << eini << G4endl;
                           ...
                           {
                               std::cout << "************ Multi-particle decay ************" << std::endl;
                           }
                           ita ++;
                       ...
                       //std::cout << "THESCATTERER " << "dec "<< dec << std::endl;
                       //std::cout << "THESCATTERER " << p4j.m() << " " << p4j << " " << p4i << std::endl;
                       //std::cout << "THESCATTERER " << pdi0->GetParticleName() << " " << pdj0->GetParticleName() << " " << p4i.e() + p4j.e() << std::endl;
                       //std::cout << "THESCATTERER " << theSystem->GetParticipant( k )->GetDefinition()->GetParticleName() << " " << p4kx_new.m() << " " << (*it)->Get4Momentum() << " " << theSystem->GetParticipant( k )->Get4Momentum() << " " << theSystem->GetParticipant( k )->Get4Momentum().m() << std::endl;
                       //std::cout << "THESCATTERER " << theSystem->GetParticipant( i )->GetDefinition()->GetParticleName() << " " << theSystem->GetParticipant( j )->GetDefinition()->GetParticleName() << " " << p4ix_new.e() + p4jx_new.e() << std::endl;
                   }
                   else
                   {
                       theSystem->GetParticipant( j )->SetDefinition( (*it)->GetDefinition() );
...
//081118
             abs = true;
             //G4cout << "G4LightIonQMDCollision pion absrorption " << secs->front()->GetDefinition()->GetParticleName() << G4endl;
              
              // added by Y-H. S and A.H.
              if(secs->front()->GetDefinition()->IsShortLived())
              {
                  G4KineticTrackVector * dec = secs->front()->Decay();
                  G4int ita = 0;
                  for(G4KineticTrackVector::iterator jter=dec->begin(); jter != dec->end(); jter++)
                  {
                      //G4cout << "decay "<<(*jter)->GetDefinition()->GetParticleName()<< " " << (*jter)->Get4Momentum()/GeV << G4endl;
                      if(ita == 0)
                      {
                          theSystem->GetParticipant( i )->SetDefinition( (*jter)->GetDefinition() );
                          p4ix_new = (*jter)->Get4Momentum()/GeV;
                          theSystem->GetParticipant( i )->SetMomentum( p4ix_new.v() );
                      }
                      else if(ita == 1)
                      {
             ...
             theSystem->GetParticipant( i )->SetMomentum( p4ix_new.v() );
              */
              //std::cout << "THESCATTERER " << p4i.e()+p4j.e() << " " << p4j << " " << p4i << std::endl;
              //std::cout << "THESCATTERER " << p4ix_new.e()+p4jx_new.e() << " " << p4ix_new << " " << p4jx_new << std::endl;
              //exit(0);
         } 
...
//081118
         if ( secs->size() > 2 ) 
         {
            ...
            G4cout << "G4LightIonQMDCollision secs size > 2;  " << secs->size() << G4endl;
...
//071031
       //G4double fepse_change = fepse; // Added by Y-H. S and A.H.
       //if(pion_prod || pion_abs) fepse_change = fepse*10; // Added by Y-H. S and A.H.
      ...
      {
          //if(pion_prod || pion_abs) G4cout << "EnergyNotOK " << std::abs ( eini - efin )*1000 << " orig " << std::abs ( eini0 - efin0 )*1000 << G4endl;
           //G4cout << "EnergyNotOK " << std::abs ( eini - efin )*1000 << G4endl;
           //if(iitry == 3) G4cout << "Energy non-conserved and go through" << G4endl;
           /*
           if(std::abs ( eini - efin )*1000 > 20)
           {
             //G4cout << "EnergyNotOK " << std::abs ( eini - efin )*1000 << " orig " << std::abs ( eini0 - efin0 )*1000 << G4endl;
               G4cout << p4ix_new + p4jx_new << " " <<  theSystem->GetParticipant( i )->Get4Momentum() + theSystem->GetParticipant( j )->Get4Momentum() << G4endl;
               G4cout << p4ix_new.m() << " " <<  p4jx_new.m() << " " <<  theSystem->GetParticipant( i )->Get4Momentum().m() << " " << theSystem->GetParticipant( j )->Get4Momentum().m() << G4endl;
            G4cout << "G4QMDRESULT Collsion Really Happened between "
            << i << " and " << j
            << G4endl;
            G4cout << "G4QMDRESULT Collsion initial p4 i and j "
            ...
            << p4i + p4j - theSystem->GetParticipant( i )->Get4Momentum() - theSystem->GetParticipant( j )->Get4Momentum()
            << G4endl;
               G4cout << "G4QMDRESULT Particle Name "
               << theSystem->GetParticipant( i )->GetDefinition()->GetParticleName() << " " << theSystem->GetParticipant( j )->GetDefinition()->GetParticleName() << " mj " <<theSystem->GetParticipant( j )->Get4Momentum().m()
               << G4endl;
            //G4cout << "G4QMDRESULT Collsion initial r i and j "
            //<< ri << " " << rj
            //<< G4endl;
         ...
         else if ( pion_prod ) // added by Y-H. S and A.H.
          {
              //G4cout << "TKDB reinsert j " << G4endl;
              theSystem->EraseParticipant( k );
              theMeanField->Update();
              theSystem->GetParticipant( i )->SetDefinition( pdi0 );
              theSystem->GetParticipant( i )->SetMomentum( p4i.v() );
              theSystem->GetParticipant( j )->SetDefinition( pdj0 );
              theSystem->GetParticipant( j )->SetMomentum( p4j.v() );
              theMeanField->Update();
              pion_prod = false;
          }
         else
...
G4bool G4LightIonQMDCollision::CalFinalStateOfTheBinaryCollisionJQMD( G4double sig , G4double cutoff , G4ThreeVector pcm , G4double prcm , G4double srt , G4ThreeVector beta , G4double gamma , G4int i , G4int j )
{
...
//
      ...
      //G4double efin = epot + pi_new_e + pj_new_e ;
       G4double efin = theMeanField->GetTotalEnergy();
      //std::cout << "Collision NEW epot " << i << " " << j << " " << epot << " " << std::abs ( eini - efin ) - fepse << std::endl;
/*
      G4cout << "Collision efin " << i << " " << j << " " << efin << G4endl;
      G4cout << "Collision " << i << " " << j << " " << std::abs ( eini - efin ) << " " << fepse << G4endl;
