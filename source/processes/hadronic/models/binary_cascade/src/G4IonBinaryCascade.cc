...
//#define debug_G4IonBinaryCascade 1
//
//  C O N S T R U C T O R S   A N D   D E S T R U C T O R S
//

G4IonBinaryCascade::G4IonBinaryCascade() : G4VIntraNuclearTransportModel()
{
...
//  theScatterer = new G4ScattererStub;
  thePropagator = new G4IonRKPropagation;
  theCurrentTime = 0.;
...
G4IonBinaryCascade::G4IonBinaryCascade(const G4IonBinaryCascade& )
: G4VIntraNuclearTransportModel()
{
}


G4IonBinaryCascade::~G4IonBinaryCascade()
{
...
//----------------------------------------------------------------------------
G4HadFinalState * G4IonBinaryCascade::ApplyYourself(const G4HadProjectile & aTrack,
//----------------------------------------------------------------------------
							G4Nucleus & aNucleus)
{
...
// Build a KineticTrackVector with the G4Track
  ...
  if(!getenv("I_Am_G4IonBinaryCascade_Developer") )
  {
    if(definition!=G4Neutron::NeutronDefinition() &&
      definition!=G4Proton::ProtonDefinition() )
    {
      G4cerr << "You are using G4IonBinaryCascade for projectiles other than neutron and proton."<<G4endl;
      G4cerr << "If you want to continue, please switch on the developer environment: "<<G4endl;
      G4cerr << "setenv I_Am_G4IonBinaryCascade_Developer 1 "<<G4endl<<G4endl;
      throw G4HadronicException(__FILE__, __LINE__, "G4IonBinaryCascade - used for unvalid particle type - Fatal");
    }
  }
...
//gfhit    the3DNucleus->Init(aNucleus.GetN(), aNucleus.GetZ());
    BuildTargetList();
...
//----------------------------------------------------------------------------
G4ReactionProductVector * G4IonBinaryCascade::Propagate(
//----------------------------------------------------------------------------
		G4KineticTrackVector * secondaries, G4V3DNucleus * nucleus)
{
  G4ping debug("debug_G4IonBinaryCascade");
  debug.push_back("trial");
  debug.dump();
#ifdef debug_G4IonBinaryCascade
   G4cout << "G4IonBinaryCascade Propagate starting -------------------------------------------------------" <<G4endl;
#endif
...
//#ifdef debug_G4IonBinaryCascade
   PrintKTVector(&theFinalState,std::string(" FinalStatebe4 stepping out"));
...
//#endif
  ...
  {
#ifdef debug_G4IonBinaryCascade
      G4cerr << "G4IonBinaryCascade: Warning, have active particles at end" << G4endl;
#endif
...
// 	if ( ExcitationEnergy < 0. )
 	{
#ifdef debug_G4IonBinaryCascade
 	  G4cerr << "G4IonBinaryCascade-Warning: negative excitation energy ";
...
//----------------------------------------------------------------------------
G4double G4IonBinaryCascade::GetExcitationEnergy()
//----------------------------------------------------------------------------
{
...
// get A and Z for the residual nucleus
  ...
  {
     #ifdef debug_1_BinaryCascade
     G4cout << "G4IonBinaryCascade::GetExcitationEnergy(): Warning - invalid nucleus (A,Z)=("
...
//----------------------------------------------------------------------------
void G4IonBinaryCascade::BuildTargetList()
//----------------------------------------------------------------------------
{

  if(!the3DNucleus->StartLoop())
  {
//    G4cerr << "G4IonBinaryCascade::BuildTargetList(): StartLoop() error!"
//	   << G4endl;
    return;
  }
...
// 	{
// 	   G4cout << "G4IonBinaryCascade::BuildTargetList found free nucleon " <<G4endl
// 	   	  << "   4mom = " << mom << G4endl
// 		  << "    pos = " << pos << G4endl
...
// 	}
  ...
  {
     //G4cout << "G4IonBinaryCascade::BuildTargetList(): Warning - invalid nucleus (A,Z)=("
	//	<< currentA << "," << currentZ << ")" << G4endl;
  }
...
//----------------------------------------------------------------------------
void  G4IonBinaryCascade::FindCollisions(G4KineticTrackVector * secondaries)
//----------------------------------------------------------------------------
{
  G4KineticTrack * pkt;
  G4KineticTrack * tkt;
...
//      G4cerr << "G4IonBinaryCascade::ApplyCollision pre-collision time"
...
//      G4cerr <<" !!!!!! debug collisions "<<collisionTime<<" "<<pkt->GetDefinition()->GetParticleName()<<G4endl;      
    ...
    //  G4cerr << "G4IonBinaryCascade::ApplyCollision post-collision time"<<G4endl;
    // look for decay
...
//----------------------------------------------------------------------------
G4bool G4IonBinaryCascade::ApplyCollision(G4CollisionInitialState * collision)
//----------------------------------------------------------------------------
{
  G4ping debug("debug_ApplyCollision");
  //G4cerr << "G4IonBinaryCascade::ApplyCollision start"<<G4endl;
  ...
  {
       //G4cerr << "G4IonBinaryCascade::ApplyCollision pre-decay"<<G4endl;
    products = primary->Decay();
       //G4cerr << "G4IonBinaryCascade::ApplyCollision post-decay"<<G4endl;
...
//  G4cerr << "G4IonBinaryCascade::ApplyCollision start 1"<<G4endl;
    products = theScatterer->Scatter(*primary, target_reloc);
//  G4cerr << "G4IonBinaryCascade::ApplyCollision start 2"<<G4endl;
...
//     #ifdef debug_G4IonBinaryCascade
       ...
       G4cerr << "G4IonBinaryCascade::ApplyCollision blocked"<<G4endl;
//     #endif
...
//----------------------------------------------------------------------------
G4bool G4IonBinaryCascade::Absorb()
//----------------------------------------------------------------------------
{
...
// Build the vector of G4KineticTracks that must be absorbed
  ...
  for(iter = absorbList.begin(); iter != absorbList.end(); ++iter)
  {
    ...
    if(!absorber.FindAbsorbers(*kt, theTargetList))
      throw G4HadronicException(__FILE__, __LINE__, "G4IonBinaryCascade::Absorb(): Cannot absorb a particle.");
...
// ------ end debug
	...
	throw G4HadronicException(__FILE__, __LINE__, 
	  "G4IonBinaryCascade::Absorb(): Cannot absorb a particle.");
    }
// ------ debug
...
//----------------------------------------------------------------------------
G4bool G4IonBinaryCascade::Capture(G4bool verbose)
//----------------------------------------------------------------------------
{
  ...
  G4cout << "capture " <<  particlesAboveCut <<" "<< particlesBelowCut <<" "<< capturedEnergy<<G4endl;
  if(particlesAboveCut==0 && particlesBelowCut>0 && capturedEnergy/particlesBelowCut<0.2*theCutOnP)
  {
    capture=true;
  PrintKTVector(&theSecondaryList,std::string("capturing the following...")); 
    for(i = theSecondaryList.begin(); i != theSecondaryList.end(); ++i)
    {
      G4KineticTrack * kt = *i;
      if(kt->GetState() == G4KineticTrack::inside) // capture happens only inside the nucleus
...
//----------------------------------------------------------------------------
G4bool G4IonBinaryCascade::CheckPauliPrinciple(G4KineticTrackVector * products)
//----------------------------------------------------------------------------
{
  G4int A = the3DNucleus->GetMassNumber();
  G4int Z = the3DNucleus->GetCharge();
  ...
  if ( myflag  )
  {
	for(i = products->begin(); i != products->end(); ++i)
	{
		...
		(definition == G4Neutron::Neutron()))
		{
			...
			G4LorentzVector mom = (*i)->Get4Momentum();
			G4double field =((G4IonRKPropagation*)thePropagator)->GetField(definition->GetPDGEncoding(),pos);
...
//----------------------------------------------------------------------------
void G4IonBinaryCascade::StepParticlesOut()
//----------------------------------------------------------------------------
{
  G4int counter=0;
  G4int countreset=0;
...
//	           <<kt->GetTrackingMomentum().e()-kt->GetTrackingMomentum().mag()<<G4endl;
          ...
          throw G4HadronicException(__FILE__, __LINE__, "G4IonBinaryCascade::StepParticlesOut() particle not in nucleus");
      }
    }
//    G4cerr << "CaptureCount = "<<counter<<" "<<nsec<<" "<<minTimeStep<<" "<<1*ns<<G4endl;
    ...
    {
#ifdef debug_G4IonBinaryCascade
       G4cerr << "G4IonBinaryCascade.cc: Warning - aborting looping particle(s)" << G4endl;
#endif
...
//----------------------------------------------------------------------------
void G4IonBinaryCascade::CorrectFinalPandE()
//----------------------------------------------------------------------------
{
  ...
  {
#ifdef debug_G4IonBinaryCascade
     G4cout << "G4IonBinaryCascade::CorrectFinalPandE() : error! " << G4endl;
#endif
      return;
  }
...
//----------------------------------------------------------------------------
void G4IonBinaryCascade::UpdateTracksAndCollisions(
//----------------------------------------------------------------------------
			G4KineticTrackVector * oldSecondaries,
			G4KineticTrackVector * oldTarget,
...
//----------------------------------------------------------------------------
G4bool G4IonBinaryCascade::DoTimeStep(G4double theTimeStep)
//----------------------------------------------------------------------------
{
  ...
  G4cerr <<"G4IonBinaryCascade::DoTimeStep: enter "<< theTimeStep << G4endl;
...
//  G4cerr <<"G4IonBinaryCascade::DoTimeStep: exit "<<G4endl;
...
//----------------------------------------------------------------------------
G4KineticTrackVector* G4IonBinaryCascade::CorrectBarionsOnBoundary(
...
//----------------------------------------------
  if (out->size())
  {
     ...
     if (currentA < 0 || currentZ < 0 || currentA < currentZ) 
       { throw G4HadronicException(__FILE__, __LINE__, "G4IonBinaryCascade::CorrectBarionsOnBoundary() - fatal error");}
...
G4Fragment * G4IonBinaryCascade::FindFragments()
//----------------------------------------------------------------------------
{
...
//#ifdef debug_G4IonBinaryCascade
   G4cout <<  a <<" "<< z <<" "<< excitons <<" "<< zCaptured  <<" "<<  holes <<" "
 	  << GetExcitationEnergy() <<" "
 	  << GetFinalNucleusMomentum() <<" "
 	  << CapturedMomentum <<" "
 	  << "Fragment: a, z, particles, Charged,holes, " 
	  << "excitE, Final4Momentum, capturMomentum= " <<G4endl;
//#endif
...
G4LorentzVector G4IonBinaryCascade::GetFinal4Momentum()
//----------------------------------------------------------------------------
{
...
//G4cerr << "what 1 - "<<theInitial4Mom<<" "<<final4Momentum<<G4endl;
  ...
  {
    G4cerr << G4endl;
    G4cerr << "G4IonBinaryCascade::GetFinal4Momentum - Fatal"<<G4endl;
    G4KineticTrackVector::iterator i;
...
//----------------------------------------------------------------------------
G4LorentzVector G4IonBinaryCascade::GetFinalNucleusMomentum()
//----------------------------------------------------------------------------
{
...
//G4cout << "GetFinalNucleusMomentum boost= " <<boost<<G4endl;
  ...
  if(boost.mag()>1.0)
  {
    G4cerr << "G4IonBinaryCascade::GetFinalNucleusMomentum - Fatal"<<G4endl;
...
//----------------------------------------------------------------------------
G4ReactionProductVector * G4IonBinaryCascade::Propagate1H1(
//----------------------------------------------------------------------------
		G4KineticTrackVector * secondaries, G4V3DNucleus * nucleus)
{
    G4ReactionProductVector * products = new G4ReactionProductVector;
...
//----------------------------------------------------------------------------
G4ThreeVector G4IonBinaryCascade::GetSpherePoint(
...
void G4IonBinaryCascade::ClearAndDestroy(G4KineticTrackVector * ktv)
//----------------------------------------------------------------------------
{
  std::vector<G4KineticTrack *>::iterator i;
...
//----------------------------------------------------------------------------
void G4IonBinaryCascade::ClearAndDestroy(G4ReactionProductVector * rpv)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void G4IonBinaryCascade::PrintKTVector(G4KineticTrackVector * ktv, std::string comment)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
G4bool G4IonBinaryCascade::CheckDecay(G4KineticTrackVector * products)
//----------------------------------------------------------------------------
{
  G4int A = the3DNucleus->GetMassNumber();
  G4int Z = the3DNucleus->GetCharge();
...
void G4IonBinaryCascade::PrintWelcomeMessage()
{
  G4cout <<"Thank you for using G4IonBinaryCascade. "<<G4endl;
}
