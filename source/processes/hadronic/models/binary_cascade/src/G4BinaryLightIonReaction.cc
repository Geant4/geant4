...
{ 
	...
	if( (mom.t()-mom.mag())/a1 < 50*MeV )
	{
		...
		m_nucl=G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(z2,a2);
	...
	else
	{
		...
		while(!result)
		{
			...
			mass1=G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(
					projectile->GetCharge(),projectile->GetMassNumber());
			it=toBreit * G4LorentzVector(mass1,G4ThreeVector(0,0,0));
			fancyNucleus = new G4Fancy3DNucleus;
			fancyNucleus->Init(a2, z2);
			mass2=G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(
					fancyNucleus->GetCharge(),fancyNucleus->GetMassNumber());
			m_nucl = ( swapped ) ? mass1 : mass2;
		...
		delete fancyNucleus;
		delete projectile;
		G4ping debug1("debug_G4BinaryLightIonReaction_1");
		debug1.push_back("have the hits. A,Z, excitE");
		debug1.push_back(resA);
		debug1.push_back(resZ);
		debug1.push_back(theStatisticalExEnergy);
		debug1.dump();
		// Calculate excitation energy
		G4LorentzVector iState = mom;
		iState.setT(iState.getT()+mass2);
		...
		for(i=0; i<result->size(); i++)
		{
			if( (*result)[i]->GetNewlyAdded() )
			{
				...
				//          G4cout <<" secondary ... ";
				debug1.push_back("secondary ");
				debug1.push_back((*result)[i]->GetDefinition()->GetParticleName());
				debug1.push_back(G4LorentzVector((*result)[i]->GetMomentum(),(*result)[i]->GetTotalEnergy()));
				debug1.dump();
			}
			else {
				//          G4cout <<" spectator ... ";
		...
		delete result;
		result=0;
		debug1.push_back(" iState - (fState+pspectators) ");
		debug1.push_back(iState-fState-pspectators);
		debug1.dump();
		...
		{
			debug1.push_back("the momentum balance");
			debug1.push_back(iState);
			debug1.push_back(fState);
			debug1.push_back(momentum-pspectators);
			debug1.push_back(momentum);
			debug1.dump();
			...
			debug1.push_back("the momentum balance after correction");
			debug1.push_back(iState);
			debug1.push_back(fState);
			debug1.push_back(momentum-pspectators);
			debug1.push_back(momentum);
		...
		debug1.push_back("the nucleon count balance");
		debug1.push_back(resA);
		debug1.push_back(resZ);
		if(proFrag) debug1.push_back(proFrag->size());
		debug1.dump();
		...
		debug1.push_back("################# done with evaporation"); debug1.dump();
	...
	G4ping debug2("debug_G4BinaryLightIonReactionResults");
	debug2.push_back("Result analysis, secondaries");
	debug2.push_back(theResult.GetNumberOfSecondaries());
	debug2.dump();
	debug2.push_back(" Energy conservation initial/final/delta(init-final) ");
	debug2.push_back(aTrack.GetTotalEnergy() + m_nucl);
	debug2.push_back(aTrack.GetTotalEnergy());
	debug2.push_back(m_nucl);
	debug2.push_back(Etot);
	debug2.push_back(aTrack.GetTotalEnergy() + m_nucl - Etot);
	debug2.dump();
