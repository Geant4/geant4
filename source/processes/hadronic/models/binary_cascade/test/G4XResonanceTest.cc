...
int main()
{
  ...
  G4VXResonanceTable* table = new G4XDeltaNstarTable;

  // Create the ResonanceCrossSection

  G4VCrossSectionSource* source = new G4XResonance(definition1,definition2,
						   deltaPlus,N1440Plus,*table);
  G4cout << "***** " << source->Name() <<  "***** created " << G4endl;
  source->Print();

  const G4PhysicsVector* sigmaVector = table->CrossSectionTable(N1440Plus->GetParticleName());
  ...
  for (i=0; i<n; i++)
    {
      ...
      G4bool dummy = false;
      G4double ppSigma = ((G4PhysicsVector*)sigmaVector)->GetValue(sqrtS,dummy);

      G4cout << definition1->GetParticleName() << " " << definition2->GetParticleName() 
	     << " cross section = " << sigma / millibarn << G4endl;
      G4cout << "pp cross section = " << ppSigma / millibarn << G4endl;
