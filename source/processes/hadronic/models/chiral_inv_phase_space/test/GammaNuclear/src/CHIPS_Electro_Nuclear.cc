...
// @@@ add Electron nuclear reaction here.
    G4ElectroNuclearReaction* theCHIPSElectronNuclear = new G4ElectroNuclearReaction;
...
// Prepare the analysis
    ...
    if(kl == 16)      fileName = "Electron_c";
    //else if(kl == 8)  fileName = "Electron_be";
    //else if(kl == 17) fileName = "Electron_li6";
    else if(kl == 9)  fileName = "Electron_al";
    //else if(kl == 0)  fileName = "Electron_cu";
    //else if(kl == 18) fileName = "Electron_ta";
...
// Analysis prepared
    ...
    for( k = kl; k <= kr; k++ )
    {
	  ...
	  LogicalFrame->SetMaterial( theMaterials[k] ); 
	  G4DynamicParticle aParticle( theElectron, theDirection, incomingEnergy );
	  ...
	  G4int l;
	  for( l=0; l<nEvents; ++l ) {
	    aParticle.SetDefinition( theElectron );
	    ...
	    G4cout << "Event:" << G4std::setw(4) << l+1 << 
	              ...
	              " Particle:" << theElectron->GetParticleName() << '\t' << G4endl;
#endif
#ifdef edebug
	    ...
	    aFinalState=(G4ParticleChange*)(theCHIPSElectronNuclear->ApplyYourself(*aTrack,theTarget));
	    G4int nSec = aFinalState->GetNumberOfSecondaries();
#ifdef debug
        G4cout << "CHIPS_Electro_Nuclear:  A NUMBER OF SECONDARIES = "<<nSec<<G4endl;
#endif
	    timerEvent.Stop();
