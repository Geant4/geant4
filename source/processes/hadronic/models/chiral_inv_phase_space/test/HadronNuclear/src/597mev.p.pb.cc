...
//	th14.push_back(aBin);
    ...
    operator()(MEvent * it) 
    {
      double weight = 1763./10000./29.85; // sigma/10000./binwidth
      int ip = it->getIP();
      if(ip!=2112) return;
	...
	{
	  if( fabs(it->getTheta()-7.5) <accept )  th1[j].second+=weight;
	  if( fabs(it->getTheta()-30.)<accept  ) th2[j].second+=weight;
	  if( fabs(it->getTheta()-60.)<accept  ) th3[j].second+=weight;
	  if( fabs(it->getTheta()-120.)<accept )  th4[j].second+=weight;
//	  if( fabs(it->getTheta()-150.)<accept )  th5[j].second+=weight;
...
//	  if( fabs(it->getTheta()-121.)<accept ) th11[j].second+=weight;
//	  if( fabs(it->getTheta()-134.)<accept ) th12[j].second+=weight;
//	  if( fabs(it->getTheta()-145.)<accept ) th13[j].second+=weight;
//	  if( fabs(it->getTheta()-160.)<accept ) th14[j].second+=weight;
    ...
    void dump()
    {
      ...
      cout << "th=7.5"<<endl;   for_each(th1.begin(),   th1.end(),   print(binWidth/2.));
      cout << "th=30"<<endl;  for_each(th2.begin(),   th2.end(),  print(binWidth/2.));
      cout << "th=60"<<endl;  for_each(th3.begin(),   th3.end(),  print(binWidth/2.));
      cout << "th=120"<<endl;  for_each(th4.begin(),   th4.end(), print(binWidth/2.));
//      cout << "th=150"<<endl;  for_each(th5.begin(),   th5.end(), print(binWidth/2.));
//      cout << "th=56"<<endl;  for_each(th6.begin(),   th6.end(), print(binWidth/2.));
//      cout << "th=69"<<endl;  for_each(th7.begin(),   th7.end(), print(binWidth/2.));
...
//      cout << "th=121"<<endl; for_each(th11.begin(),  th11.end(), print(binWidth/2.));
//      cout << "th=134"<<endl; for_each(th12.begin(),  th12.end(), print(binWidth/2.));
//      cout << "th=145"<<endl; for_each(th13.begin(),  th13.end(), print(binWidth/2.));
...
main()
{
  ...
  analyse theA = for_each(data.begin(), data.end(), analyse(597.));
  theA.dump();
}
