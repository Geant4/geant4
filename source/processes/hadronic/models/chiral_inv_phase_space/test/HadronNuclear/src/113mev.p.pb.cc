...
//	th14.push_back(aBin);
    ...
    operator()(MEvent * it) 
    {
      double weight = 1808./10000./5.65; // sgima/10000./binwidth
      int ip = it->getIP();
      if(ip!=2112) return;
	...
	if(energy<th1[j].first)
	{
	  ...
	  if( fabs(it->getTheta()-150.)<accept )  th4[j].second+=weight;
...
//	  if( fabs(it->getTheta()-160.)<accept ) th14[j].second+=weight;
    ...
    { 
      public:
      print(double halfWidth, ofstream & out) {theHalf = halfWidth;}
      operator()(pair<double, double>& it){out << it.first-theHalf<<" "<<it.second<<endl;}
      private: 
      double theHalf;
    };
    void dump()
    {
      ...
      cout << "th=150"<<endl;  for_each(th4.begin(),   th4.end(), print(binWidth/2.));
...
main()
{
  ...
  analyse theA = for_each(data.begin(), data.end(), analyse(113.));
  theA.dump();
}
