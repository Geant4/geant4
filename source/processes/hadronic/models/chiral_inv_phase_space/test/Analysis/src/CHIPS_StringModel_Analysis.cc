...
//
//
// $Id: CHIPS_StringModel_Analysis.cc,v 1.13 2009-12-09 16:01:06 mkossov Exp $
// GEANT4 tag $Name: not supported by cvs2svn $
//
// Created by Johannes Peter Wellisch, 22 Apr 1997: full test-suite coded
// Modified by Mikhail Kosov, 9 Dec 2009: used for the G4QInelastic process    
...
class KtoPi
{
 public:  
   ...
   void push_back(const G4DynamicParticle * aPart)
   {
     ...
     for(int i=0; i<theData.size(); i++) theData[i]->push_back(aPart);
   }
   
   void dump() {for(int i=0; i<theData.size(); i++) theData[i]->dump();}
...
{    
  ...
  //------ Processes definitions Follow @@ Change to the Physics list definition @@ -------
  ...
  // here the high energy parts are coming
  // the string model; still not quite according to design - Explicite use of the forseen
  // interfaces  will be tested and documented.
  G4VPartonStringModel * theStringModel;
  G4int choice;
  do
  {
    cout<<"Please select String Model to test(1-FTF, 2-QGS, 3-CHIPS): "<<std::flush;
    cin >> choice;
  ...
  cout << "Please select fragmentation model [1=lund, 2=qgsm] "<<endl;
  int ifragmodel;
  cin >> ifragmodel;
  G4VLongitudinalStringDecay* theFragmentation;
  if     (ifragmodel == 1) theFragmentation = new G4LundStringFragmentation;
  else if(ifragmodel == 2) theFragmentation = new G4QGSMFragmentation;
  ...
  G4LambdaInelasticProcess* theLambdaInelasticProcess = new G4LambdaInelasticProcess();
  ...
  G4ProcessManager* theAntiSigmaMinusProcessManager = 
                                                   new G4ProcessManager(theAntiSigmaMinus);
  ...
  G4XiMinusInelasticProcess* theXiMinusInelasticProcess = new G4XiMinusInelasticProcess();
  ...
  G4XiZeroInelasticProcess* theXiZeroInelasticProcess = new G4XiZeroInelasticProcess();
  ...
  G4ProcessManager* theAntiOmegaMinusProcessManager =
                                                   new G4ProcessManager(theAntiOmegaMinus);
  ...
  *condition = NotForced;
  ...
  for( int k = kl; k <= kr; k++ ) for( int i = il; i <= ir; i++ )
  {
    ...
    double kineticEnergy, totalEnergy, currentMass;
    ...
    for(int l=0; l<nEvents; ++l )
    {
      ...
      cout<<"Event#"<<std::setw(4)<<l<<", Material: "<<theMaterials[k]->GetName()
          <<", Particle: "<<theParticles[i]->GetParticleName()<< endl;
      timerEvent.Start();
      aFinalState = (G4ParticleChange*) (theProcesses[i]->PostStepDoIt( *aTrack, aStep ) );
      timerEvent.Stop();

      int nSec = aFinalState->GetNumberOfSecondaries();
      ...
      for(int isec=0; isec<aFinalState->GetNumberOfSecondaries(); ++isec)
      {
