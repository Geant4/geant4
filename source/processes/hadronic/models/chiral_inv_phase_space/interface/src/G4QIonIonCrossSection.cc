...
G4double* G4QIonIonCrossSection::lastLEN=0; // Pointer to the lastArray of LowEn CS
G4double* G4QIonIonCrossSection::lastHEN=0; // Pointer to the lastArray of HighEn CS
G4double  G4QIonIonCrossSection::lastSP=0.; // Last value of ShadowingPomeron(A-dep)
...
G4double  G4QIonIonCrossSection::lastCS=0.; // Last value of the Cross Section
                                                ...
                                                G4int tN, G4int pPDG)
{
...
#ifdef pdebug
  G4cout<<"G4QIICS::GetCS:>>> f="<<fCS<<", p="<<pMom<<", Z="<<tZ<<"("<<lastZ<<") ,N="<<tN
        <<"("<<lastN<<"),PDG="<<pPDG<<"("<<lastPDG<<"), T="<<pMom<<"("<<lastTH<<")"<<",Sz="
        <<colN.size()<<G4endl;
  ...
  if(tN!=lastN || tZ!=lastZ || pPDG!=lastPDG)// The nucleus was not the last used isotope
  {
    ...
    if(lastI) for(G4int i=0; i<lastI; i++) if(colPDG[i]==pPDG) // The partType is found
      ...
      if(colN[i]==tN && colZ[i]==tZ)
        ...
        {
#ifdef pdebug
          G4cout<<"G4QIonIonCS::GetCS: P="<<pMom<<", XS="<<lastCS*millibarn<<G4endl;
#endif
...
#ifdef pdebug
        G4cout<<"G4QIonIonCS::GetCrosSec:*****> New (inDB) Calculated CS="<<lastCS<<G4endl;
        //CalculateCrossSection(fCS,-27,j,lastPDG,lastZ,lastN,pMom); // DUMMY TEST
#endif
        if(lastCS<=0. && pMom>lastTH)    // Correct the threshold
        {
#ifdef pdebug
...
#ifdef pdebug
      G4cout<<"G4QIICS::GetCrosSec: New CS="<<lastCS<<",lZ="<<lastN<<",lN="<<lastZ<<G4endl;
      //CalculateCrossSection(fCS,-27,j,lastPDG,lastZ,lastN,pMom); // DUMMY TEST
#endif
      colN.push_back(tN);
      colZ.push_back(tZ);
...
#ifdef pdebug
      G4cout<<"G4QIICS::GetCS:1st,P="<<pMom<<"(MeV),CS="<<lastCS*millibarn<<"(mb)"<<G4endl;
      //CalculateCrossSection(fCS,-27,j,lastPDG,lastZ,lastN,pMom); // DUMMY TEST
#endif
  ...
  {
#ifdef pdebug
    G4cout<<"G4QIICS::GetCS:OldCur P="<<pMom<<"="<<pMom<<", CS="<<lastCS*millibarn<<G4endl;
    //CalculateCrossSection(fCS,-27,j,lastPDG,lastZ,lastN,pMom); // DUMMY TEST
#endif
    return lastCS*millibarn;     // Use theLastCS
  }
...
#ifdef pdebug
  G4cout<<"G4QIICS::GetCroSec:End,P="<<pMom<<"(MeV),CS="<<lastCS*millibarn<<"(mb)"<<G4endl;
  //CalculateCrossSection(fCS,-27,j,lastPDG,lastZ,lastN,pMom); // DUMMY TEST
#endif
  return lastCS*millibarn;
}
                                                     ...
                                                     G4int tZ, G4int tN, G4double Momentum)
{
...
#endif
  G4int dPDG=pPDG/10;
  G4int zPDG=dPDG/1000;
  G4double pA=dPDG-1000*zPDG;
  //G4int sPDG=zPDG/1000;
  //G4int pZ=zPDG-1000*sPDG;
  //G4int pN=pA-pZ;
...
#ifdef debug
  //    G4cout<<"G4QIonIonCrossSection::CalcCS:lnA="<<lnA<<",lastSP="<<lastSP<<G4endl;
#endif
#ifdef debug3
  //    if(A==3) G4cout<<"G4QIonIonCrossSection::CalcCS: lastSP="<<lastSP<<G4endl;
#endif
...
#ifdef debug
  //    G4cout<<"G4QIonIonCrossSection::CalcCS: GetFunctions er="<<er<<G4endl;
#endif
  ...
  //{
#ifdef debug
	 //  G4cout<<"G4QIINCS::CalcCS:bLEN,A="<<A<<",nL="<<nL<<",TH="<<THmin<<",dP="<<dP<<G4endl;
#endif
  //  if(A<=1.) sigma=0.;
  //  else      sigma=EquLinearFit(Momentum,nL,THmin,dP,lastLEN);
#ifdef debugn
	 //  if(sigma<0.)
  //    G4cout<<"G4QIICS::CalcCS:A="<<A<<",P="<<Momentum<<",Th="<<THmin<<",d="<<dP<<G4endl;
#endif
  //}
...
G4int G4QIonIonCrossSection::GetFunctions(G4double a, G4double* y, G4double* z)
{
  ...
  if(a<=.9)
  {
    G4cout<<"***G4QIonIonCrossSection::GetFunctions:A="<<a<<"(?). No CS returned!"<<G4endl;
    return -1;
  }
...
#ifdef debug
  G4cout<<"G4QIonIonCrosSection::GetFunctions:r="<<r<<",a="<<a<<",y="<<y<<",z="<<z<<G4endl;
#endif
  if(!r)                                // LEN part is not filled
  {
...
#ifdef debug
    G4cout<<"G4QIonIonCrossSection::GetFunctions: nLA="<<nLA<<G4endl;
#endif
    for(G4int m=0; m<nL; m++)
    {
      ...
      //  y[m]=yi+(SL[k][m]-yi)*b;
#ifdef debug
						//if(y[m]<0.)G4cout<<"G4QIonIonCrossSec::GetF:y="<<y[m]<<",k="<<k<<",yi="<<yi<<",ya="
...
#ifdef debug
  G4cout<<"G4QIonIonCrossSection::GetFunctions: r="<<r<<G4endl;
#endif
