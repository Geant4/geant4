...
//
//
// $Id: G4QNuMuNuclearCrossSection.cc,v 1.1 2005-11-25 20:29:25 mkossov Exp $
...
G4double* G4QNuMuNuclearCrossSection::lastLE=0;  // Pointer to the Array of the Energy axis
...
G4double G4QNuMuNuclearCrossSection::GetQEL_ExchangeQ2()
{
  ...
  G4double Emu2=Emu*Emu;              // squared energy of nu/anu
  ...
  G4double Xma=((1.+Q2mi),power);     // X_max(E_nu)
  G4double Xmi=((1.+Q2ma),power);     // X_min(E_nu)
...
G4double G4QNuMuNuclearCrossSection::GetNQE_ExchangeQ2()
{
  ...
  // Direct table
  static const G4double Xmax[nE]={X0[lX],X1[lX],X2[lX],X3[lX],X4[lX],X5[lX],X6[lX],X7[lX],
						X8[lX],X9[lX],XA[lX],XB[lX],XC[lX],XD[lX],XE[lX],XF[lX],XG[lX],XH[lX],XI[lX],XJ[lX]};
...
-1.98842,-1.58049,-1.17256,-.764638,-.356711, .051215,	.459141,	.867068,	1.27499,	1.68292,
  ...
  G4double shift=1.+.9673/(1.+.323/Enu/Enu)/pow(E,.78); //@@ different for anti-nu
  // --- E-interpolation must be done in a log scale ---
  G4double Xmi=((shift-Rmi),power);   // X_min(E_nu)
  G4double Xma=((shift-1.),power);    // X_max(E_nu)
  // Find the integral values integ(Xmi) & integ(Xma) using the direct table
  ...
  G4double bnti=bntil+dE*(bntir-bntil);// interpolated begin of the integral
  //
  ...
  G4double bnta=bntal+dE*(bntar-bntal);// interpolated end of the integral
  //
  // *** Find X using the reversed table ***
  G4double intx=inti+(inta-inti)*G4UniformRand(); 
  G4int    intc=static_cast<int>(intx);
  if(intc<0) intc=0;
  if(intc>bQ2) intc=bQ2;
  G4double dint=intx-intc;
