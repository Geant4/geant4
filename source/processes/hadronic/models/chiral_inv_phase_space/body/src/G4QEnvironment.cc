  ...
  theEnvironment(90000000)                   // User is responsible for projHadrons(Vector)
{
  ...
  {
    G4cerr<<"***G4QEnv::Const: a#of INPUT QHadrons="<<nHadrons<<", tPDG="<<targPDG<<G4endl;
  ...
  else                                        // => "Unique hadron" case
  {
    ...
    else
    {
      ...
      else                                // Now we can get 4Mom &  QC of incedent particle
      {
        ...
        if(tQ<0||targPDG==10)
		{
          G4cerr<<"***G4QEnvironment::Constructor:TargQ<0 or Chipo, PDG="<<targPDG<<G4endl;
...
{//========================================================================================
  ...
  if(targPDG>80000000&&targPDG!=90000000)   // Interaction with a nuclear target
  {
    ...
    if(nCandid<=0)
	{
	  G4cerr<<"***G4QEnv::CrQ:nC="<<nCandid<<",maxP="<<maxP<<",E="<<theEnvironment<<G4endl;
    ...
    else
    {
      ...
      else
	  {
        ...
        else
		{
	      ...
	      G4cerr<<"***G4QE::CrQ: Fill projHadr asItIs QC="<<projQC<<",4M="<<proj4M<<G4endl;
          return;
        }
	  }
	}
      ...
      if(ist)
	  {
        ...
        else                                     // => "Quasmon can't decay" case         ^
	    {
		  ...
		  {                                      //                                       ^
			...
			{                                    //                                       ^
		      G4cerr<<"***G4QE::HQ:T="<<tot4M<<totQC<<",M="<<totQM<<"< gsM="<<gsM<<",d="
                    <<dM<<",Env="<<theEnvironment<<G4endl; //                             ^
        ...
        else
		{
	      ...
	      {//@@ Can (must) be moved to EvaporateResidual ??
            if(NaK==1&&!NPi)                     // ==> "One anti-strange K" case
		    {
              ...
              {
#ifdef pdebug
                G4cerr<<"***G4QEnv::HadronizeQEnv: M="<<aKPDG<<"(m="<<MaK<<") + N="<<totPDG<<"(m="
...
#ifdef pdebug
                  G4cerr<<"***G4QEnv::HadronizeQEnv: Fill AsItIs (1), Hadron="<<hadr<<G4endl;
#endif
                  //throw G4QException("G4QEnvironment::HadronizeQEnv:AntiS-Nuc decay failed");
                }
                delete quasH;  
                return theQHadrons;
              }
...
{//  ================================================================
  ...
  //else if(2>3) // One can easily close this decay as it will be done later... (time of calc?)
  {
    ...
    else
    {
      ...
      if(!G4QHadron(q4M).DecayIn2(h4Mom, g4Mom))
      {
        ...
        throw G4QException("G4QEnvironment::EvaporateResidual: BaryonDecay in Baryon+Gamma failed");
      }
#ifdef debug
	  G4cout<<"G4QE::E:"<<q4M<<"->"<<thePDG<<h4Mom<<"+g="<<g4Mom<<",n="<<theQHadrons.size()<<G4endl;
#endif
  ...
  else if(theBN>0&&!theS&&(theC<0||theC>theBN)) DecayIsonucleus(qH); // "UnavoidIsonucl" case (d.e.)
  else if(thePDG==89999003||thePDG==90002999)    //=> "ISO-dibarion" case (@@ ? Look at prev. line!)
  {
  ...
  { // @@ Should be improved for Sigma+, Sigma-, Ksi0 & Ksi- content in the Total Residual Nucleus
    ...
    //else if(bZ==4&&bN==2&&!bS) DecayAlphaDiN(qH); // Decay alpha+2protons state (del. equivalent)
...
{//              ===============================
...
#ifdef pdebug
    G4cout<<"G4QE::FSI:B nH="<<nHadr<<",PDG="<<curHadr->GetPDGCode()<<",E="<<theEnvironment<<G4endl;
#endif
  ...
  if(nHadr)for(unsigned ipo=0; ipo<theQHadrons.size(); ipo++)// Find theBigestNuclFragm and DecayA
  {
    ...
    G4cout<<"G4QE::FSI:h#"<<ipo<<":h="<<hPDG<<h4Mom<<",F="<<hNF<<",nH="<<theQHadrons.size()<<G4endl;
#endif
    ...
    if(hPDG==89002000||hPDG==89001001||hPDG==89000002)// "2pt dec. of anti-strange (3pt dec.)" case
    {
      ...
      else if(!tPDG)           // 2 particle decay
      {
	    ...
	    {
          G4cerr<<"**G4QEn::FSI:"<<hM<<"->"<<fM<<"("<<fQPDG<<")+"<<sM<<"("<<sPDG<<")="<<sum<<G4endl;
          ...
          if(!theEnvironment.GetA())
          {
            ...
            delete quasH;
            //           5. Decrement jpo index (copy of ipo) to find theBiggestNuclearFrag
            jpo--;
            nHadr=theQHadrons.size();
		  }
          else
    ...
    {
#ifdef pdebug
      G4cout<<"G4QEnv::FSI:***nNmD-/nPmD++***#"<<ipo<<",PDG="<<hPDG<<",B="<<hBN<<",C="<<cBN<<G4endl;
#endif
      G4double hM=h4Mom.m();
      ...
      if(hM<sum || !G4QHadron(h4Mom).DecayIn3(nu4M,ba4M,pi4M))
	  {
        G4cerr<<"***G4QEnv::FSI:IsN M="<<hM<<","<<hPDG<<"->N="<<nuQPDG<<"(M="<<nucM<<")+"<<nN<<"*B="
              <<barPDG<<"(M="<<barM<<")+"<<nPi<<"*pi="<<tPDG<<"(M="<<tM<<")="<<nucM+barM+tM<<G4endl;
        G4cerr<<"***G4QEnv::FSI:IsN BaryoNum="<<hBN<<", Charge="<<cBN<<", Strangness="<<sBN<<G4endl;
        if(!theEnvironment.GetA())                  // Emergency recovery
        {
          ...
          if(!CheckGroundState(quasH,true))         // Try to correct by other hadrons
          {
            G4cerr<<"***G4QEnv::FSI:IN Recovery failed (3) Fill as it is !!!!"<<G4endl;
            theQHadrons.push_back(qH);              // Fill as it is (delete equivalent)
          }
          else delete qH;
          delete quasH;
  ...
  while(bfAct&&bfCount<bfCountM) // "Infinite" LOOP of the ThermoNuclearBackFusion
  {
...
#endif
      if(hPDG==89999003||hPDG==90002999)G4cerr<<"G4QEnv::FSI:***nD-/pD++**(3)***PDG="<<hPDG<<G4endl;
      if(hPDG==89001001||hPDG==89002000||hPDG==89000002)G4cerr<<"G4QE::FSI:*KN**PDG="<<hPDG<<G4endl;
		  ...
		  //if(!bF&&bB&&bB<fL&&bM+hM>tM+.001&&sM-bM-hM<cut)              // Light fragments == ecut
	      {
            ...
            if(!three)
            {
              if(!G4QHadron(s4M).DecayIn2(f4Mom,g4Mom))
              {
                ...
                throw G4QException("***G4QEnvironment::FSInter:Fusion (1) DecIn2 didn't succeed");
              }
              else
			  {
#ifdef pdebug
...
#ifdef pdebug
        G4cout<<"G4QE::FSI: Is Energy&Mom conserved? t4M="<<tot4Mom<<",d2="<<dem<<">10e-6?"<<G4endl;
#endif
		//if(2>3)                                 //@@Energy/Momentum conservation check is closed
	...
	if(nHadr>2 && frag) for(G4int h=nHadr-1; h>=0; h--)  //Collect gammas & kill DecayedHadrons
    {
      ...
      G4int   hF = curHadr->GetNFragments();
      G4int hPDG = curHadr->GetPDGCode();
      if(hPDG==89999003||hPDG==90002999)G4cerr<<"G4QEnv::FSI:***nD-/pD++***(1)**PDG="<<hPDG<<G4endl;
#ifdef pdebug
	  ...
	  //else if(2>3)                           // Close the pair absorbtion of gamma
	  {
...
#ifdef pdebug
          G4cout<<"G4QE::FSI:SO,h="<<sh<<"<"<<nHadr<<",PDG/LV="<<curHadr->GetPDGCode()<<h4M<<G4endl;
#endif
          G4double hM=h4M.m();
        ...
        if(thePrev->Get4Momentum().m()>mGamEva)
        {
          ...
          else
          {
            ...
            {
              G4cerr<<"***G4QE::FSI:GammaSuppres,tM="<<dh4M.m()<<" < A+h="<<n4M.m()+h4M.m()<<G4endl;
  ...
  if(nHadr) for(unsigned hd=0; hd<theQHadrons.size(); hd++)
  {
    ...
    if(hPDG==89999004||hPDG==90003999) G4cout<<"G4QEnv::FSI:**nnD-/ppD++**(0)***PDG="<<hPDG<<G4endl;
#ifdef pdebug
    G4cout<<"G4QE::FSI:Copy's been made with PDG="<<hPDG<<G4endl;// Just to compare with theOriginal
#endif
    ...
    else if(hPDG>91000000)                     // Hypernucleus (@@ Only for the GEANT4 applications)
	{
      ...
      if(reM<rnM+mPi0)
	  {
		...
		G4cerr<<"***G4QE::FSI:Small Hypernuclear mass M="<<reM<<">"<<rnM+mPi0<<",d="<<d<<G4endl;
      ...
      else
	  {
        ...
        if(!G4QHadron(r4M).DecayIn2(n4M,h4M))
        {
          G4double d=rnM+npiM-reM;
          G4cerr<<"***G4QE::FSI:Hypernucleus, M="<<reM<<"<A+n*Pi0="<<rnM+npiM<<",d="<<d<<G4endl;
          throw G4QException("***G4QEnvironment::FSInter: Hypernuclear decay did not succeeded");
        }
        else
		{
#ifdef pdebug
          G4cout<<"*G4QE::FSI: Hypernucleus "<<r4M<<"->A="<<rnPDG<<n4M<<",n*Pi0="<<nPi<<h4M<<G4endl;
#endif
...
{//  ============================================
  ...
  else if(qM<sum || !G4QHadron(q4M).DecayIn2(f4Mom, s4Mom))
  {
    ...
    else throw G4QException("G4QEnvir::DecayIsonucleus: Isonucleus DecIn2 didn't succeed");
  }
#ifdef pdebug
...
{//  ============================================
  ...
  else if(qPDG!=90002000|| rM<dProt)           // Other possibilities (if not a default)
  {
    ...
    else
    {
      ...
      else throw G4QException("G4QEnvir::DecayDibar:Unknown PDG code or small Mass of DB");
    }
  }
  ...
  if(!tPDG&&!four)
  {
    ...
    {
      G4cerr<<"***G4QEnv::DecDibar:fPDG="<<fPDG<<"(fM="<<fMass<<") + sPDG="<<sPDG<<"(sM="
      ...
      if(!theEnvironment.GetA())
      {
	    ...
	    if(!CheckGroundState(quasH,true))
        {
          G4cerr<<"***G4QEnv::DecayDiBaryon:FillAsItIs diBaryon="<<qQC<<q4M<<G4endl;
      ...
      else throw G4QException("***G4QEnv::DecayDibaryon: DiBar DecayIn2 didn't succeed");
    }
#ifdef pdebug
  ...
  else if(four)
  {
    ...
    else if(qM<sum || !G4QHadron(q4M).DecayIn2(f4Mom, s4Mom))
    {
      ...
      if(!theEnvironment.GetA())
      {
	    ...
	    if(!CheckGroundState(quasH,true))
        {
          G4cerr<<"***G4QEnv::DecDibar:FillAsItIs diDel="<<qQC<<qH->Get4Momentum()<<G4endl;
          theQHadrons.push_back(qH); // Fill as it is
        }
        else delete qH;  
        delete quasH;
      ...
      else throw G4QException("***G4QEnv::DecDibaryon: Dibaryon DecayIn2 didn't succeed");
    }
#ifdef pdebug
    ...
    else if(!G4QHadron(q4M).DecayIn2(f4Mom, s4Mom))
    {
      ...
      throw G4QException("G4QEnv::DecayDibaryon: No DecayIn2 didn't succeed for dibaryon");
    }
#ifdef pdebug
  ...
  else
  {
    ...
    else if(qM<sum || !G4QHadron(q4M).DecayIn3(f4Mom, s4Mom, t4Mom))
    {
      ...
      else throw G4QException("G4QEnv::DecayDibaryon: diBar DecayIn3 didn't succeed");
    }
#ifdef pdebug
...
{//  ===============================================
  ...
  static const G4double mK0   = G4QPDGCode(311).GetMass();
  static const G4double eps   = 0.003;
  G4LorentzVector q4M = qH->Get4Momentum();    // Get 4-momentum of the AntiStrangeNuclearState
  G4double         qM = q4M.m();               // Real mass of the AntiStrangeNuclearState
  G4QContent       qQC= qH->GetQC();           // PDG Code of the decaying AntiStrangeNuclearState
  G4int            qS = qH->GetStrangeness();  // Strangness of the AntiStrangeNuclearState
  G4int            qB = qH->GetBaryonNumber(); // Baryon number of the AntiStrangeNuclearState
  G4int            qP = qH->GetCharge();       // Charge of the AntiStrangeNuclState (a#of protons)
#ifdef pdebug
  ...
  if(!n1||!n2)                            // AntiKaons of only one sort are found
  {
    ...
    else if(qM<sum || !G4QHadron(q4M).DecayIn2(f4Mom, s4Mom))
    {
	  ...
	  G4cerr<<"***G4QE::DecayAntiS:fPDG="<<n1<<"*"<<k1PDG<<"(M="<<k1M<<") + sPDG= "<<qPDG
            <<"(M="<<nucM<<" = "<<sum<<" > TotM="<<qM<<q4M<<", n="<<n1<<G4endl;
#endif
      if(theEnvironment.GetA()==0)
	  {
		...
		if(!CheckGroundState(quasH,true))
        {
          G4cerr<<"***G4QEnv::DecayAntiStrange: NoCorDone, Fill2 AsItIs h="<<qH<<G4endl;
      ...
      {
        G4cerr<<"***G4QE::DecAntiSt:NoCorrDone, Fill2 AsItIs Env="<<theEnvironment<<G4endl;
  ...
  else
  {
    ...
    else if(qM<sum || !G4QHadron(q4M).DecayIn3(f4Mom, s4Mom, t4Mom))
    {
      G4cerr<<"***G4QEnv::DecayAntiS: nPDG="<<qPDG<<"(M="<<nucM<<")+PDG1="<<k1PDG<<"(M="<<k1M
            <<")+PDG2="<<k2PDG<<"(M="<<k2M<<")="<<nucM+n1*k1M+n2*k2M<<">tM="<<qM<<q4M<<G4endl;
      if(theEnvironment.GetA()==0)
	  {
        G4Quasmon* quasH = new G4Quasmon(qQC,q4M);
		if(!CheckGroundState(quasH,true))
        {
          G4cerr<<"***G4QEnv::DecayAntiStrange: NoCorDone, Fill3 AsItIs h="<<qH<<G4endl;
          theQHadrons.push_back(qH); //@@ Can cause problems with particle conversion in G4
        }
        else delete qH;  
        delete quasH;
      ...
      {
        G4cerr<<"***G4QEnv::DecayAntiStrange:NoCorrDone, Fill3 AsItIs Env="<<theEnvironment<<G4endl;
        theQHadrons.push_back(qH); // @@ Can cause problems with particle conversion in G4
        return;
...
{//  ===============================================
  ...
  else if(totBN==2)
  {
    ...
    {
      G4cerr<<"***G4QEnv::DecayMultyBaryon: fPDG="<<fPDG<<"(fM="<<fMass<<")*2 = "<<sum
      ...
      if(!theEnvironment.GetA())
      {
	    ...
	    if(!CheckGroundState(quasH,true))
        {
          G4cerr<<"***G4QEnv::DecayMultyBaryon:FillAsItIs diBar(I=1)="<<qQC<<q4M<<G4endl;
          theQHadrons.push_back(qH);               // Fill as it is
        }
        else delete qH;  
        delete quasH;
      ...
      else throw G4QException("G4QEnv::DecayMultyBaryon:diBaryon DecayIn2 didn't succeed");
    }
#ifdef pdebug
  ...
  else if(totBN==3)
  {
    ...
    else if(qM<sum || !G4QHadron(q4M).DecayIn3(f4Mom, s4Mom, t4Mom))
    {
      ...
      if(!theEnvironment.GetA())
      {
	    ...
	    if(!CheckGroundState(quasH,true))
        {
          G4cerr<<"***G4QEnv::DecayMultyBaryon:FillAsItIs threeBaryon="<<qQC<<q4M<<G4endl;
          theQHadrons.push_back(qH);      // Fill as it is
        }
        else delete qH;  
        delete quasH;
      ...
      else throw G4QException("G4QEnv::DecayMultyBar:ThreeBaryon DecayIn3 didn't succeed");
    }
#ifdef pdebug
...
{//  ============================================
  ...
  if(!G4QHadron(q4M).DecayIn3(f4Mom, s4Mom, t4Mom))
  {
    ...
    if(!theEnvironment.GetA())
    {
	  ...
	  if(!CheckGroundState(quasH,true))
      {
        G4cerr<<"***G4QEnv::DecayAlphaDiN: FillAsItIs AlphaNN="<<qQC<<q4M<<G4endl;
        theQHadrons.push_back(qH);      // Fill as it is
      }
      else delete qH;  
      delete quasH;
    ...
    else throw G4QException("G4QEnv::DecayAlphaDiN: Alpha+N+N DecayIn3 didn't succeed");
  }
#ifdef pdebug
...
{//  ============================================
  ...
  else if(qPDG==92000003||qPDG==92003000||qPDG==93000002||qPDG==93002000)
  {
    ...
    else if(qM<sum || !G4QHadron(q4M).DecayIn2(f4Mom, s4Mom))
    {
      ...
      if(!theEnvironment.GetA())
      {
	    ...
	    if(!CheckGroundState(quasH,true))
        {
          G4cerr<<"***G4QEnv::DecayAlphaBaryon:FillAsItIs AlphaBaryon="<<qQC<<q4M<<G4endl;
          theQHadrons.push_back(qH);      // Fill as it is
        }
        else delete qH;  
        delete quasH;
      ...
      else throw G4QException("G4QEnv::DecayAlphaBar: DecayIn2 didn't succeed for 3/2");
    }
#ifdef pdebug
  ...
  else if(qPDG==90004001||qPDG==90001004)
  {
    ...
    else if(qM<sum || !G4QHadron(q4M).DecayIn3(f4Mom, s4Mom, t4Mom))
    {
      ...
      else throw G4QException("G4QEnv::DecayAlphaBar: t/nn,He3/pp DecayIn3 didn't");
    }
#ifdef pdebug
  ...
  else if(qPDG==94000001||qPDG==94001000||qPDG==91000004||qPDG==91004000)
  {
    ...
    {
      G4cerr<<"***G4QEnv::DecayAlphaBaryon: fPDG="<<fPDG<<"(2*fM="<<fMass<<")*2="<<2*fMass
      ...
      else throw G4QException("G4QEnv::DecayAlphaBar:QuintBaryon DecayIn2 didn't succeed");
    }
#ifdef pdebug
  ...
  else if(qPDG==90003002||qPDG==90002003||qPDG==91002002)
  {
    ...
    else if(qM<sum || !G4QHadron(q4M).DecayIn2(f4Mom, s4Mom))
    {
      ...
      else throw G4QException("***G4QE::DecayAlphaBar:Alpha+Baryon DecIn2 didn't succeed");
    }
#ifdef pdebug
    G4cout<<"G4QEnv::DecAlphaBar:DONE a4M="<<f4Mom<<",s4M="<<s4Mom<<",sPDG="<<sPDG<<G4endl;
#endif
    delete qH;
...
{//  ==============================================
  ...
  if(2>3)
  {
    ...
    else if(qM<sum || !G4QHadron(q4M).DecayIn2(f4Mom, s4Mom))
    {
      G4cerr<<"***G4QEnv::DecayAlphaAlpha:gPDG="<<fPDG<<"(gM="<<fMass<<")+PDG="<<sPDG
            <<"(sM="<<sMass<<")="<<sum<<" > TotM="<<q4M.m()<<q4M<<G4endl;
      if(!theEnvironment.GetA())
      {
        G4QContent qQC=qH->GetQC();
        G4Quasmon* quasH = new G4Quasmon(qQC,q4M);
	    if(!CheckGroundState(quasH,true))
        {
          G4cerr<<"***G4QEnv::DecayAlphaAlpha:FillAsItIs AlphaAlpha="<<qQC<<q4M<<G4endl;
          theQHadrons.push_back(qH);      // Fill as it is
        }
        else delete qH;  
        delete quasH;
      ...
      else throw G4QException("G4QEnv::DecayAlphaAlpha: g+diAlph DecayIn2 didn't succeed");
    }
#ifdef pdebug
  ...
  {
    G4cerr<<"***G4QEnv::DecayAlphaAlpha:fPDG="<<fPDG<<"(fM="<<fMass<<")*2="<<sum
          <<" > TotM="<<q4M.m()<<q4M<<G4endl;
    if(!theEnvironment.GetA())
    {
      G4QContent qQC=qH->GetQC();
    ...
    else throw G4QException("G4QEnv::DecayAlphaAlpha: diAlpha DecayIn2 didn't succeed");
  }
#ifdef pdebug
...
{ //   ==================================================================
  ...
  {
    G4QHadron*  theLast = theQHadrons[nOfOUT-1];
    if(theLast->GetNFragments() || theLast->GetPDGCode()==22) return false;// Dec H or gam
    ...
    if(tmpTM>resSMa+hadrMa)                     // resMa contains 2 Hadrs: resQ and Environ
    {
      ...
      if(nOfOUT>1)                              // Cor with Last&Previous can be tryed
	  {
        ...
        if(tmpTLV.m()>tQMa+hadrMa+prevMa)
        {
          ...
          if(!G4QHadron(tmpTLV).DecayIn3(hadr4M,prev4M,nuc4M))
          {
            ...
            G4cerr<<"***G4QE::CGS:Decay in ResidNuc+LastH+PrevH did not succeeded"<<G4endl;
            return false;
          }
          else
          {
