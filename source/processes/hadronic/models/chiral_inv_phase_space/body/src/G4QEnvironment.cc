...
//
//
// $Id: G4QEnvironment.cc,v 1.96 2004-12-14 16:01:10 mkossov Exp $
  ...
  if(targPDG>80000000&&targPDG!=90000000)   // Interaction with a nuclear target
  {
    ...
    if((projPDG==-2212||projPDG==-2112||projPDG==-3122||projPDG==-3112||projPDG==-3212||
...
#endif
        if(!shDFL&&abs(shPDG)>99)                // Final(notDecayed)&notPhot/Lept QHad ^ ^
		      {
#ifdef pdebug
      ...
      if(!efFlag)                                  // =>NotEnergyFlux=MultyQuasmon Case ^
        ...
        for_each(outH->begin(), outH->end(), DeleteQHadron()); // >------------------^=^ ^
        outH->clear();                             //                                ^ ^ ^
        delete outH;                               // >==============================^=^ ^
#ifdef pdebug
...
{//              ===============================
  ...
  if(nHadr)for(unsigned ipo=0; ipo<theQHadrons.size(); ipo++)//FindBigestNuclFragm & DecayA
  {
    ...
    G4cout<<"G4QE::FSI:h#"<<ipo<<",PDG="<<hPDG<<h4Mom<<",m="<<h4Mom.m()<<",F="<<hNF<<",nH="
          <<theQHadrons.size()<<G4endl;
#endif
...
#ifdef pdebug
      G4cout<<"G4QE::FSI:gSum4M="<<sum<<" is added to "<<new4M<<" of PDG="<<newPDG<<G4endl;
#endif
