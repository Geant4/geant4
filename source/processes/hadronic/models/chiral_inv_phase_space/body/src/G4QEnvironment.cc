...
//
//
// $Id: G4QEnvironment.cc,v 1.48.2.2 2003/06/25 10:18:58 gunter Exp $
...
{//              ==========================
  ...
  if(nHadr)for(unsigned ipo=0; ipo<theQHadrons.size(); ipo++)// Find NuclFragm and try to decay/evap
  {
    ...
    {
      G4cerr<<"***nD-/pD++***G4QEnv::Fr: PDG="<<hPDG<<", BaryN="<<hBN<<G4endl;
      G4int  nucPDG = 2112;
      G4double nucM = mNeut;
      ...
      if(!G4QHadron(h4Mom).DecayIn3(n14M,n24M,pi4M))
	  {
	    ...
	    G4Exception("G4QEnv::Fr:ISO-dibaryon DecayIn3 did not succeed");
	  }
