...
//
//
// $Id: G4QInelastic.cc,v 1.5 2009-08-28 17:08:29 mkossov Exp $
...
G4int    G4QInelastic::nCutMax=7; 
G4double G4QInelastic::stringTension=1.*GeV/fermi;
G4double G4QInelastic::tubeDensity  =1./fermi;
...
G4QInelastic::G4QInelastic(G4QNucleus &pNucleus, G4QNucleus &tNucleus)
{
...
#ifdef debug
  G4cout<<"G4QInelastic::Construct: theIntSize="<<theInteractions.size()<<G4endl;
#endif
  G4int attCnt=0;
  G4int maxAtt=27;
  ...
  while(!theInteractions.size() && ++attCnt < maxAtt)// TillAtLeastOneInteractionExists
  {
    std::for_each(theInteractions.begin(),theInteractions.end(), DeleteQInteraction());
    theInteractions.clear();
    // @@ Recover the proj and Targ nuclei !!
    theProjNucleus.StartLoop();                      // Prepare Loop ovder nucleons
    pNucleon = theProjNucleus.GetNextNucleon();      // Get theNextProjNucleon to try
    ...
    G4cout<<"G4QInelastic::Construct: estimated energy, prEn="<<prEn<<G4endl;
#endif
    G4int totalCuts = 0;
    G4int atCn=0;
    G4int maxAt=27;
    while(!theInteractions.size() && ++atCn < maxAt)// Till Interaction is created
    {
      ...
      tNucleon = theTargNucleus.GetNextNucleon(); // Get theNextTargNucleon to try
      G4LorentzVector tNuc4M=tNucleon->Get4Momentum();
#ifdef debug
      G4cout<<"G4QInelastic::Constr:OuterR="<<outerRadius<<", mC="<<maxCuts<<", pA="
            <<theProjNucleus<<", tA="<<theTargNucleus<<G4endl;
#endif
...
#ifdef debug
      G4cout<<"G4QInelastic::Construct: pE="<<prEn<<" < mE="<<maxEn<<", mN="<<maxNuc<<G4endl;
#endif
      ...
      while((tNucleon=theTargNucleus.GetNextNucleon()) && nucCount<nA && totalCuts<maxCuts)
      {
        ...
        // ModelMode = DIFFRACTIVE;
#ifdef sdebug
        G4cout<<"G4QInelastic::Construct: NLOOP prob="<<Probability<<", rndm="<<rndNumber
        ...
        if (Probability > rndNumber) // Inelastic (diffractive or soft) interaction (JOB)
        {
          ...
          else
          {
            ...
            delete [] running;
#ifdef debug
            G4cout<<"G4QInelastic::Construct: NLOOP-Soft Chosen nCut="<<nCut<<G4endl;
#endif
...
#endif
    } // End of WHILE over attempts to find the target nucleon
  ...
  for(G4int i=0; i<nInt; i++)
  {
    ...
    G4cout<<"-EMC-G4QInelastic::Construct: Interaction#"<<i<<",dP4M="
...
#ifdef debug
  G4cout<<"G4QInelastic::Construct: Creation ofSoftCollisionPartonPair STARTS"<<G4endl;
#endif
...
#ifdef debug
    G4cout<<"G4QInelastic::Construct: #0f SOFT collisions ="<<nSoftCollisions<<G4endl;
#endif
    if (nSoftCollisions)
    { 
...
#ifdef debug
  G4cout<<"G4QInelastic::Construct: -> Parton pairs for SOFT strings are made"<<G4endl;
#endif  
  //
  ...
  for(unsigned i = 0; i < theInteractions.size(); i++) // Interactions are reduced bySoft
  {
...
#ifdef debug
    G4cout<<"G4QInelastic::Construct: CreationOfDiffractivePartonPairs, i="<<i<<G4endl;
#endif
...
#ifdef debug
      G4cout<<"G4QInelastic::Construct: proj Diffractive PartonPair is filled"<<G4endl;
#endif
    }
    ...
    if (aParton)
    {
      ...
      thePartonPairs.push_back(aPartonPair);
#ifdef debug
      G4cout<<"G4QInelastic::Constr: target Diffractive PartonPair is filled"<<G4endl;
#endif
    }
  }
#ifdef debug
...
#ifdef debug
    G4cout<<"G4QInelastic::Construct: StringType="<<aPair->GetCollisionType()<<G4endl;
#endif
  ...
  for(G4int i=0; i<nStrings; i++)
  {
    ...
    rChg-=sChg;
    rBaN-=sBaN;
    G4cout<<"-EMC-G4QInelastic::Construct: String#"<<i<<", 4M="<<strI4M<<",LPDG="<<LPDG
  ...
  if(!stringsInitted)
  {
    G4cerr<<"******G4QInelastic::Construct:***** No strings are created *****"<<G4endl;
    G4Exception("G4QInelastic::Construct:","72",FatalException,"NoStrings're created");
  }
#ifdef debug
  G4cout<<"G4QInelastic::Constr: BeforeRotation, #0fStrings="<<strings.size()<<G4endl;
#endif
  //
  ...
  for(ist = strings.begin(); ist < strings.end(); ist++)
  {
...
#ifdef debug
    G4cout<<"G4QInelastic::Construct: Needs Fusion? cLPDG="<<cLPDG<<",cRPDG="<<cRPDG
    ...
    if(bad)                                          // String should be merged with others
    {
...
#ifdef debug
        G4cout<<"->G4QInelastic::Construct: sum4M="<<pS4M<<", M2="<<pSM2<<G4endl;
#endif
...
#ifdef debug
          else G4cout<<"G4QInelastic::Construct:2(QaQ+QDiQ/aQaDiQ) Can't fuse"<<G4endl;
#endif
        }
...
#ifdef debug
          else G4cout<<"G4QInelastic::Construct:3(QDiQ/aQaDiQ+QaQ) Can't fuse"<<G4endl;
#endif
        }
...
#ifdef debug
          else G4cout<<"-G4QInelastic::Construct: 4 (QaQ+aQDiQDiQ) Can't fuse"<<G4endl;
#endif
        }
...
#ifdef debug
          else G4cout<<"-G4QInelastic::Construct: 5 (aQDiQDiQ+QaQ) Can't fuse"<<G4endl;
#endif
        }
...
#ifdef debug
          else G4cout<<"-G4QInelastic::Construct: 6 (QDiQ+aQaDiQ) Can't fuse"<<G4endl;
#endif
        }
#ifdef debug
        G4cout<<"G4QInelastic::Const: ***Possibility***, tf="<<tf<<", af="<<af<<G4endl;
#endif
        if(tf && af)
        {
          ...
          else
          {
            ...
            if(cLT==2 && cRT==2)
            {
              ...
              {
#ifdef debug
                G4cout<<"G4QInelastic::Const:aLPDG="<<aLPDG<<", aRPDG="<<aRPDG<<G4endl;
#endif
                sing=false;
                G4QPDGCode tmp;
      ...
      if(sOrd)                                       // The best pStringCandidate was found
      {
...
#ifdef debug
        G4cout<<"G4QInelastic::Const:cS4M="<<cS4M<<" fused/w pS4M="<<pL4M+pR4M<<G4endl;
#endif
        if(sOrd>0)
        {
          pL4M+=cL4M;
        ...
        G4cout<<"G4QInelastic::Construct:Created,4M="<<ss4M<<",m2="<<ss4M.m2()<<G4endl;
#endif
  ...
  for(G4int i=0; i<nStr; i++)
  {
    ...
    G4int sBaN=strings[i]->GetBaryonNumber();
    rB-=sBaN;
    G4cout<<"-EMCLS-G4QInelastic::Construct: St#"<<i<<", 4M="<<strI4M<<", M="<<strI4M.m()
  ...
  //
#ifdef debug
    G4cout<<"G4QInelastic::Construct: problem="<<problem<<G4endl;
#endif
  if(problem)
  {
    G4int nOfStr=strings.size();
#ifdef debug
    G4cout<<"G4QInelastic::Construct:SecurityDiQaDiQReduction,#OfStr="<<nOfStr<<G4endl;
#endif
    for (G4int astring=0; astring < nOfStr; astring++)
    {
      ...
      {
#ifdef debug
        G4cout<<"G4QInelastic::Constr:TrySelfReduString,L="<<sPDG<<",R="<<nPDG<<G4endl;
#endif
...
G4QHadronVector* G4QInelastic::Fragment()
{ // This is the member function fragmenting Strings & Quasmons (in nuclear matter)
#ifdef debug
  G4cout<<"*******>G4QInelastic::Fragment: ***Called***, Res="<<theResult<<G4endl;
#endif
  ...
  for(G4int i=0; i < striNum; i++)
  {
    ...
    G4int sBaN=strings[i]->GetBaryonNumber();
    totBaN+=sBaN;
    G4cout<<"-EMCLS-G4QInelastic::Fragment:String#"<<i<<", 4M="<<strI4M<<", M="<<strI4M.m()
          <<", C="<<sChg<<", B="<<sBaN<<G4endl;
  }
  for(G4int i=0; i < nQm; i++)
  {
    ...
    totBaN+=hBaN;
    G4cout<<"-EMCLS-G4QInelastic::Fragment: Quasmon#"<<i<<", 4M="<<hI4M<<", C="<<hChg
          <<", B="<<hBaN<<G4endl;
  }
...
#endif
#ifdef debug
  G4cout<<"***>G4QInelastic::Fragment: #OfStr="<<striNum<<", #OfRes="<<hadrNum<<G4endl;
#endif
  if(!striNum && hadrNum)                                   // Quasi-elastic or decoupled p
  {
#ifdef debug
    G4cout<<"***>G4QInelastic::Fragment:**Quasi-Elastic**,#OfResult="<<hadrNum<<G4endl;
#endif
    return theResult;
  }
...
#ifdef debug
  G4cout<<"***>G4QInelastic::Fragment:beforeEnv,#OfQ="<<nQuas<<",#OfR="<<theRS<<G4endl;
#endif
  if(nQuas && theRS)
  {
...
#ifdef pdebug
    G4cout<<"G4QInelastic::Fragment:#OfRemainingHadron="<<theRS<<",A="<<theEnv<<G4endl;
#endif
    ...
    for(G4int j=theRS-1; j>-2; --j)                         // try to reach M_compound>M_GS
    {
...
#ifdef pdebug
      G4cout<<"G4QInelastic::Fragm:rN4M"<<qsum4M<<qsum4M.m()<<",rNQC="<<qsumQC<<G4endl;
#endif
...
#ifdef pdebug
        G4cout<<"G4QInelastic::Fragm: Q#"<<i<<", QQC="<<curQC<<",sQC="<<qsumQC<<G4endl;
#endif
...
#ifdef pdebug
      G4cout<<"G4QInelastic::Fragment: PDG="<<miPDG<<",rM="<<reM<<",GSM="<<gsM<<G4endl;
#endif
      ...
      {
        G4cerr<<"***G4QInelastic::Fragm:PDG="<<miPDG<<",M="<<reM<<",GSM="<<gsM<<G4endl;
        G4Exception("G4QInelastic::Fragment:","27",FatalException,"Can't recover GSM");
      }
    }
...
#ifdef pdebug
    G4cout<<"G4QInelastic::Fragm: nucE="<<nucE<<",nQ="<<nQuas<<G4endl; //                 |
#endif
    ...
    {                                                       //                            |
      ...
      G4cout<<"G4QInelastic::Fragment: Quasmon# "<<i<<" added, 4M="<<cQ4M<<G4endl; // |
#endif
    }                                                       //                            |
    try                                                     //                            |
    {                                                       //                            |
    ...
    {                                                       //                          | |
      G4cerr<<"***G4QInelastic::Fragment: G4QE Exception is catched"<<G4endl; //    | |
      G4Exception("G4QInelastic::Fragment:","27",FatalException,"CHIPSCrash");//    | |
    }                                                       //                          | |
  ...
  else if(!striNum) G4cout<<"-Warning-G4QInelastic::Fragment:Nothing was done"<<G4endl;
#ifdef debug
  G4cout<<"====>G4QInelastic::Fragment: Final #OfResult="<<theResult->size()<<G4endl;
#endif
    ...
    G4cout<<"-EMCLS-G4QInelastic::Fragment: #ofHadr="<<nHd<<", #OfQuasm="<<nQ<<G4endl;
    for(G4int i=0; i<nHd; i++)
    {
      ...
      G4int hBaN=(*theResult)[i]->GetBaryonNumber();
      fBN-=hBaN;
      G4cout<<"-EMCLS-G4QInelastic::Fragment: Hadron#"<<i<<", 4M="<<hI4M<<", PDG="
...
{ // This is the member function, which returns the resulting vector of Hadrons & Quasmons
  ...
  for(G4int i=0; i<nStri; i++)
  {
    ...
    G4int sBaN=strings[i]->GetBaryonNumber();
    totBaN+=sBaN;
    G4cout<<"-EMCLS-G4QInelastic::Breeder: St#"<<i<<", 4M="<<strI4M<<", M="<<strI4M.m()
          <<", C="<<sChg<<", B="<<sBaN<<G4endl;
  }
#endif
  G4int nOfStr=strings.size();
#ifdef debug
  G4cout<<"G4QInelastic::Breeder: BeforeFragmentation, #OfStr="<<nOfStr<<G4endl;
#endif
...
#ifdef debug
    G4cout<<">G4QInelastic::Breeder:1stTest,S#"<<i<<",4M="<<pS4M<<",QC="<<pSQC<<G4endl;
#endif
  }
  if(ftBad)
  {
  ...
  for (G4int astring=0; astring < nOfStr; astring++)
  {
...
#ifdef debug
    G4cout<<"====>G4QInelastic::Breeder: String#"<<astring<<",s4M/m="<<curString4M
    ...
    if (!theHadrons)                           // The string can not be fragmented
    {
      ...
      {
#ifdef debug
        G4cout<<"G4QInelastic::Breeder:TryReduceString, L="<<sPDG<<",R="<<nPDG<<G4endl;
#endif
        if( cLeft->ReduceDiQADiQ(cLeft, cRight) ) // DiQ-aDiQ pair was successfully reduced
        {
          LT=1;
          ...
          nPDG=cRight->GetPDGCode();
#ifdef debug
          G4cout<<"G4QInelastic::Breeder:AfterReduction,L="<<sPDG<<",R="<<nPDG<<G4endl;
#endif
...
#ifdef debug
        else G4cout<<"^G4QInelastic::Breeder: DQ-aDQ reduction to Q-aQ Failed"<<G4endl;
#endif
      } // End of the SelfReduction
#ifdef debug
      G4cout<<"G4QInelastic::Breed:AfterRedAttempt, theH="<<theHadrons<<", L4M="
      ...
      if (!theHadrons)                         // The string can not be fragmented
      {
        ...
        if(next < strings.size())             // TheString isn't theLastString can fuse
        {
...
#ifdef debug
          G4cout<<"G4QInelastic::Breed:TryFuseStringS, q="<<qPDG<<", a="<<dPDG<<", n="
                <<next<<G4endl;
#endif
          ...
          for (restr=next; restr < nOfStr; restr++)
          {
            ...
            if(LT==2 && RT==2 && PLT==2 && PRT==2)    // Have a chance for the reduction
            {
              ...
              G4int pA2=aPDG%10;
#ifdef debug
		  G4cout<<"G4QInelastic::Breeder: cQ="<<cQ1<<","<<cQ2<<", cA="<<cA1<<","<<cA2
              ...
              if  (reduce==2)                  // Two quark pairs can be reduced
              {
                if(sPDG>0 && uPDG<0)           // LL/RR Reduction
                {
                  ...
                  if(!newCL || !newPL)
                  {
                    G4cerr<<"*G4QInelastic::Breeder:CL="<<newCL<<",PL="<<newPL<<G4endl;
                    G4Exception("G4QInelastic::Breeder:","72",FatalException,"2-LL-");
                  }
                  ...
                  if(!newCR || !newPR)
                  {
                    G4cerr<<"*G4QInelastic::Breeder:CR="<<newCR<<",PR="<<newPR<<G4endl;
                    G4Exception("G4QInelastic::Breeder:","72",FatalException,"2-RR-");
                  }
                ...
                else if(sPDG>0 && mPDG<0)                // LR Reduction
                {
                  ...
                  if(!newCL || !newPR)
                  {
                    G4cerr<<"*G4QInelastic::Breeder:CL="<<newCL<<",PR="<<newPR<<G4endl;
                    G4Exception("G4QInelastic::Breeder:","72",FatalException,"2-LR");
                  }
                  ...
                  if(!newCR || !newPR)
                  {
                    G4cerr<<"*G4QInelastic::Breeder:CR="<<newCR<<",PL="<<newPL<<G4endl;
                    G4Exception("G4QInelastic::Breeder:","72",FatalException,"2-LR");
                  }
                  cLeft->SetPDGCode(newCL);    // Reset the left quark of curString
                  cRight->SetPDGCode(-newCR);  // Reset the right quark of curString
                ...
                else                           // RL Reduction
                {
                  ...
                  {
                    G4cerr<<"*G4QInelastic::Breeder:CL="<<newCL<<",PR="<<newPR<<G4endl;
                    G4Exception("G4QInelastic::Breeder:","72",FatalException,"2-RL");
                  }
                  std::pair<G4int,G4int> resRR=ReducePair(nPDG/100, (-uPDG)/100);
                  G4int newCR=resRR.first;
                  G4int newPL=resRR.second;
...
#ifdef debug
              G4cout<<"G4QInelastic::Breeder: StringCand#"<<restr<<", q="<<rPDG<<", a="
          ...
          {
#ifdef debug
            G4cout<<"-G4QInelastic::Breeder:Reduced #"<<astring<<" & #"<<restr<<G4endl;
#endif
          ...
          if(fustr)                            // The partner was found -> fuse strings
          {
...
#ifdef debug
            G4cout<<"G4QInelastic::Breeder:BeforeFuDir,sL="<<sPDG<<",nR="<<nPDG<<",uL="
...
#ifdef debug
              G4cout<<"G4QInelastic::Breeder:LL/RR s4M="<<fuString->Get4Momentum()
...
#ifdef debug
              G4cout<<"G4QInelastic::Breeder:LR/RL s4M="<<fuString->Get4Momentum()
                    <<",S="<<L4M+PL4M+R4M+PR4M<<", L="<<Left->Get4Momentum()<<", R="
                    <<Right->Get4Momentum()<<G4endl;
#endif
            }
#ifdef debug
            else G4cout<<"-Warning-G4QInelastic::Breeder: WrongStringFusion"<<G4endl;
#endif
#ifdef edebug
            G4cout<<"#EMC#G4QInelastic::Breeder:StringFused,F="<<fusionDONE<<",L="<<L4M
            ...
            {
#ifdef debug
              G4cout<<"###G4QInelastic::Breeder: Str#"<<astring<<" fused/w Str#"<<fustr
          ...
          else
          {
            ...
            G4cerr<<"**G4QInelastic::Breeder:*NoPart*M="<<curString->Get4Momentum().m()
        ...
        if(!fusionDONE)                          // Fusion of theString failed, try hadrons
        {
          ...
          while( (nHadr=theResult->size()) && !theHadrons)
          {
...
#ifdef debug
              G4cout<<"G4QInelastic::Breeder: Hadron # "<<reh<<", QC="<<curQC
...
#ifdef debug
                G4cout<<"G4QInelastic::Breeder:*IN*Hadron#"<<reh<<",M2="<<M2<<G4endl;
#endif
...
#ifdef debug
            G4cout<<"G4QInelastic::Breeder: fuh="<<fuhad<<",fus="<<fusDONE<<G4endl;
#endif
...
#ifdef debug
                G4cout<<"G4QInelastic::Breeder: Left, s4M="<<curString->Get4Momentum()
...
#ifdef debug
                G4cout<<"G4QInelastic::Breeder: Right, s4M="<<curString->Get4Momentum()
                      <<", L4M="<<cLeft->Get4Momentum()<<", R4M="<<newRight<<", h4M="
                      <<selH4M<<", newR="<<newPDG<<", oldL="<<cLeft->GetPDGCode()<<G4endl;
#endif
              }
#ifdef debug
              else G4cout<<"-G4QInelastic::Breeder: Wrong String+HadronFusion"<<G4endl;
#endif
#ifdef debug
              if(fusDONE) G4cout<<"####G4QInelastic::Breeder: String #"<<astring
            ...
            {
#ifdef debug
              G4cerr<<"**G4QInelastic::Breeder:*NoH*,M="<<curString->Get4Momentum().m()
                    <<", LPDG="<<curString->GetLeftParton()->GetPDGCode()
                    <<", RPDG="<<curString->GetRightParton()->GetPDGCode()<<G4endl;
              // @@ Temporary exception for the future solution
              //G4Exception("G4QInelastic::Breeder:","72",FatalException,"SHNotFused");
#endif
...
#ifdef debug
            G4cout<<"G4QInelastic::Breeder: before HR, nH="<<theResult->size()<<G4endl;
            ...
            for(ih = theResult->begin(); ih != theResult->end(); ih++)
            {
              ...
              if((*ih)==selHP)
              {
                ...
                curStrChg+=Chg;
                curStrBaN+=BaN;
                G4cout<<"-EMC->>>>G4QInelastic::Breeder: S+=H, 4M="<<curString4M<<",M="
...
#ifdef debug
            if(!found) G4cout<<"*G4QInelastic::Breeder:nH="<<theResult->size()<<G4endl;
#endif
...
#ifdef debug
          G4cout<<"*G4QInelastic::Breeder: *CanTryToDecay?* nH="<<theHadrons<<", next="
            ...
            if(miPDG == 10)                        // ==> Decay Hadron-Chipolino
            {
              ...
              if(h1M+h2M<ttM+eps)                  // Two particles decay of Chipolino
              {
                ...
                else
                {
                  ...
                  {
                    G4cerr<<"***G4QInelastic::Breeder: tM="<<ttM<<"->h1="<<h1QPDG<<"("
                          <<h1M<<")+h2="<<h1QPDG<<"("<<h2M<<")="<<h1M+h2M<<G4endl;
                    G4Exception("G4QInelastic::Breeder:","72",FatalException,"ChiDec");
                  }
                }
                ...
                G4cout<<"-EMC->>>G4QInelastic::Breeder: String=Hadr ChiPro1's filled, f4M="
                ...
                G4cout<<"-EMC->>>G4QInelastic::Breeder: String=Hadr ChiPro2's filled, s4M="
                      <<s4M<<", sPDG="<<sPD<<", sCg="<<sCg<<", sBN="<<sBN<<G4endl;
#endif
#ifdef edebug
                G4cout<<"-EMC-..Chi..G4QInelastic::Breeder: DecayCHECK, Ch4M="
...
#ifdef debug
              G4cout<<"G4QInelastic::Breeder: Decay the Last, Res#H="<<tmpN<<G4endl;
#endif
              if(tmpN>1)
              {
                for(G4int aH=0; aH < tmpN; aH++)
                {
                  ...
                  G4cout<<"-EMC->>>G4QInelastic::Breeder:String=Hadr,H#"<<aH<<" filled,4M="
...
#ifdef debug
      G4cout<<"G4QInelastic::Breeder: theH="<<theHadrons<<"?=0, next="<<next<<G4endl;
#endif
      if(!theHadrons && next < strings.size())       // ForwardInLOOP strings exist
      {
        ...
        G4int miPDG=miQC.GetSPDGCode();// PDG of the Lightest Hadron
#ifdef debug
        G4cout<<">>>G4QInelastic::Breeder: SQC="<<miQC<<", miSPDG="<<miPDG<<G4endl;
#endif
...
#ifdef debug
        G4cout<<">>>G4QInelastic::Breeder: minMass="<<miM<<", realM2="<<cM2<<G4endl;
#endif
        G4double   cM=0.;
        if(cM2>0.)
        {
...
#ifdef debug
              G4cout<<">>>G4QInelastic::Breeder:S->H="<<miPDG<<curString4M<<G4endl;
#endif
            }
            else
            {
              ...
              G4int           fCg=h1H->GetCharge();
              G4int           fBN=h1H->GetBaryonNumber();
              G4cout<<"-EMC->>>G4QInelastic::Breeder:Str=2HadrAR Prod-F is filled,f4M="
                    <<f4M<<", fPDG="<<fPD<<", fCg="<<fCg<<", fBN="<<fBN<<G4endl;
#endif
              G4QHadron* h2H = new G4QHadron(h2QPDG.GetPDGCode(),h24M);
              theResult->push_back(h2H);         // (delete equivalent)  
              ...
              G4int           sCg=h2H->GetCharge();
              G4int           sBN=h2H->GetBaryonNumber();
              G4cout<<"-EMC->>>G4QInelastic::Breeder:Str=2HadrAR Prod-S is filled,s4M="
                    <<s4M<<", sPDG="<<sPD<<", sCg="<<sCg<<", sBN="<<sBN<<G4endl;
#endif
            }
            ...
            for(restr=next; restr < nOfStr; ++restr) if(restr != astring)
            {
              ...
              if(x > 0. && x < 1.)          // We are getting x part of p4M
              {
                ...
                if(delta > 0. && delta > maD)
                {
                  maD=delta;
#ifdef debug
                  G4cout<<"G4QInelastic::Breeder: Subtr,S#"<<restr<<",d="<<maD<<G4endl;
#endif
                  fustr=restr;
                  selX=x;
                  s4M=p4M;
              ...
              else if(x <= 0.)               // We are adding to p4M, so use RelVelocity
              {
                ...
                {
#ifdef debug
                  G4cout<<"G4QInelastic::Breeder: FreeAdd,S#"<<restr<<",x="<<x<<G4endl;
#endif
                  Vmin=dV;
                  fustr=restr;
                  selX=x;
...
#ifdef debug
              G4cout<<"G4QInelastic::Breeder:EndOfLOOP r="<<restr<<"<"<<nOfStr<<G4endl;
#endif
            } // End of the LOOP over string-partners for Correction
#ifdef debug
              G4cout<<"G4QInelastic::Breeder: AfterLOOP fustr="<<fustr<<G4endl;
#endif
            if(fustr)
            {
#ifdef edebug
              G4LorentzVector sum4M=s4M+curString4M;
              G4cout<<"G4QInelastic::Breeder: Found Sum4M="<<sum4M<<G4endl;
#endif
              ...
              if(std::abs(miPDG)%10 > 2)                  // Decay String-Hadron-Resonance
              {
                ...
                G4QHadronVector* tmpQHadVec=Quasm.DecayQHadron(sHad); // It deletes sHad
#ifdef debug
                G4cout<<"G4QInelastic::Breeder:DecStH,nH="<<tmpQHadVec->size()<<G4endl;
#endif
                for(unsigned aH=0; aH < tmpQHadVec->size(); aH++)
                {
                  ...
                  G4int           Chg=prodH->GetCharge();
                  G4int           BaN=prodH->GetBaryonNumber();
                  G4cout<<"-EMC->>>G4QInelastic::Breeder:St=Had,pH#"<<aH<<" filled,4M="
                        <<p4M<<", PDG="<<PDG<<", Chg="<<Chg<<", BaN="<<BaN<<G4endl;
#endif
                }
                tmpQHadVec->clear();
              ...
              else if(miPDG == 10)                   // ==> Decay Hadron-Chipolino
              {
                ...
                if(h1M+h2M<ttM+eps)                  // Two particles decay of Chipolino
                {
                  ...
                  else
                  {
                    if(!G4QHadron(curString4M).DecayIn2(h14M,h24M))
                    {
                      G4cerr<<"***G4QInelastic::Breeder: tM="<<ttM<<"->h1="<<h1QPDG
                            <<"("<<h1M<<")+h2="<<h1QPDG<<"("<<h2M<<")="<<h1M+h2M<<G4endl;
                      G4Exception("G4QInelastic::Breeder:","72",FatalException,"ChDe");
                    }
                  }
                  G4QHadron* h1H = new G4QHadron(h1QPDG.GetPDGCode(),h14M);
                  theResult->push_back(h1H);        // (delete equivalent)  
                  ...
                  G4int           fCg=h1H->GetCharge();
                  G4int           fBN=h1H->GetBaryonNumber();
                  G4cout<<"-EMC->>>G4QInelastic::Breeder:Str=Hadr Prod-F's filled,f4M="
                        <<f4M<<", fPDG="<<fPD<<", fCg="<<fCg<<", fBN="<<fBN<<G4endl;
#endif
                  G4QHadron* h2H = new G4QHadron(h2QPDG.GetPDGCode(),h24M);
                  theResult->push_back(h2H);        // (delete equivalent)  
                  ...
                  G4int           sCg=h2H->GetCharge();
                  G4int           sBN=h2H->GetBaryonNumber();
                  G4cout<<"-EMC->>>G4QInelastic::Breeder:Str=Hadr Prod-S's filled,s4M="
                        <<s4M<<", sPDG="<<sPD<<", sCg="<<sCg<<", sBN="<<sBN<<G4endl;
#endif
#ifdef edebug
                  G4cout<<"-EMC-Chipo.G4QInelastic::Breeder:DecCHECK,c4M="<<curString4M
                ...
                else
                {
                  ...
                  G4Exception("G4QInelastic::Breeder:","72",FatalException,"ChiDecMa");
                }
              }
              else
              {
...
#ifdef debug
                G4cout<<"-EMC->>>G4QInelastic::Breeder:Str=Hadr Filled, 4M="
...
#ifdef edebug
              G4cout<<"-EMC-...Cor...G4QInelastic::Breeder:CorCHECK Sum="<<sum4M
                    <<" =? "<<curString4M+pString->Get4Momentum()<<", M="<<miM<<" ?= "
                    <<curString4M.m()<<G4endl;
#endif
#ifdef debug
              G4cout<<">>>G4QInelastic::Breeder:*Corrected* String->Hadr="<<miPDG
...
#ifdef debug
      else G4cerr<<"***G4QInelastic::Breeder: **No SSCorrection**,next="<<next<<G4endl;
#endif
...
#ifdef debug
      G4cout<<"G4QInelastic::Breeder: theH="<<theHadrons<<", #OfH="<<nofRH<<G4endl;
#endif
      if(!theHadrons && nofRH)                  // Hadrons are existing for SH Correction
      {
#ifdef debug
        G4cerr<<"!G4QInelastic::Breeder:Can try SHCor, nH="<<theResult->size()<<G4endl;
#endif
        // @@ string can be not convertable to one hadron (2,0.0,0,2,0) - To be improved
        G4QContent miQC=curString->GetQC();     // QContent of the Lightest Hadron
        G4int miPDG=miQC.GetSPDGCode();         // PDG of the Lightest Hadron
        ...
        { 
          ...
          if(miM+spM<sM+eps)                    // Decay into CorrectedString+theSameHadron
          {
            ...
            else
            {
              ...
              {
                G4cerr<<"***G4QInelastic::Breeder: *SH*, tM="<<sM<<"->h1=("<<miPDG<<")"
                      <<miM<<" + h2="<<spM<<" = "<<miM+spM<<G4endl;
                G4Exception("G4QInelastic::Breeder:","72",FatalException,"SHChipoDec");
              }
            }
            pHadron->Set4Momentum(sp4M);
#ifdef debug
            G4cout<<"-EMC->...>G4QInelastic::Breeder: H# "<<fuha<<" is updated, new4M="
                  <<sp4M<<G4endl;
#endif
          }
          else
          {
            ...
            G4Exception("G4QInelastic::Breeder:","72",FatalException,"HSChipoDecMass");
          }
          if(std::abs(miPDG)%10 > 2)                  // Decay Hadron-Resonans
          {
            ...
            for(unsigned aH=0; aH < tmpQHadVec->size(); aH++)
            {
              ...
              G4int           Chg=prodH->GetCharge();
              G4int           BaN=prodH->GetBaryonNumber();
              G4cout<<"-EMC->>>G4QInelastic::Breeder:Str+Hadr PrH#"<<aH<<" filled, 4M="
                    <<p4M<<", PDG="<<PDG<<", Chg="<<Chg<<", BaN="<<BaN<<G4endl;
#endif
            }
            tmpQHadVec->clear();
          ...
          else if(miPDG == 10)                   // ==> Decay Hadron-Chipolino
          {
            ...
            if(h1M+h2M<miM+eps)                  // Two particles decay of Chipolino
            {
              ...
              else
              {
                ...
                {
                  G4cerr<<"***G4QInelastic::Breeder: HS tM="<<ttM<<"->h1="<<h1QPDG<<"("
                        <<h1M<<")+h2="<<h1QPDG<<"("<<h2M<<")="<<h1M+h2M<<G4endl;
                  G4Exception("G4QInelastic::Breeder:","72",FatalException,"ChipoDec");
                }
              }
              G4QHadron* h1H = new G4QHadron(h1QPDG.GetPDGCode(),h14M);
              theResult->push_back(h1H);         // (delete equivalent)  
              ...
              G4int           fCg=h1H->GetCharge();
              G4int           fBN=h1H->GetBaryonNumber();
              G4cout<<"-EMC->>>G4QInelastic::Breeder:CorStrHadr Prod-1 is filled, f4M="
                    <<f4M<<", fPDG="<<fPD<<", fCg="<<fCg<<", fBN="<<fBN<<G4endl;
#endif
              G4QHadron* h2H = new G4QHadron(h2QPDG.GetPDGCode(),h24M);
              theResult->push_back(h2H);         // (delete equivalent)  
              ...
              G4cout<<"-EMC->>>G4QInelastic::Breeder:CorStrHadr Prod-2 is filled, n4M="
                    <<n4M<<", nPDG="<<nPD<<", nCg="<<nCg<<", nBN="<<nBN<<G4endl;
#endif
#ifdef edebug
              G4cout<<"-EMC-...HS-Chipo...G4QInelastic::Breeder:DecCHECK, Ch4M="<<mi4M
          ...
          else
          {
            ...
            theResult->push_back(sHad);          // The original string=hadron is filled
#ifdef debug
            G4cout<<">>>>>>G4QInelastic::Breeder: CorStr=Hadr is Filled, 4M="
                  <<curString4M<<", StPDG="<<miPDG<<G4endl;
#endif
          }
#ifdef edebug
          G4cout<<"-EMC-...Cor...G4QInelastic::Breeder:StHadCor CHECK Sum="<<s4M
        ...
        {
#ifdef debug
          G4cout<<"-EMC->>>G4QInelastic::Breeder: Str+Hadr Failed, 4M="<<curString4M
                <<", PDG="<<miPDG<<G4endl;
#endif
        }
    ...
    else // !! Sum Up all strings and convert them in a Quasmon (Exception for development)
    {
      ...
      for(G4int i=astring; i < tnSt; ++i)
      {
        ...
        ssQC+=pSQC;
#ifdef debug
        G4cout<<"====>G4QInelastic::Breeder:S#"<<i<<",4M="<<pS4M<<",QC="<<pSQC<<G4endl;
#endif
      }
#ifdef debug
      G4cout<<"==>G4QInelastic::Breeder:AllStrings are summed up in a Quasmon"<<G4endl;
#endif
...
#ifdef debug
    G4cout<<"G4QInelastic::Breeder: Trying to decay hadrons #ofHRes="<<nHfin<<G4endl;
#endif
    for(G4int aTrack=0; aTrack<nHfin; aTrack++)
    {
...
#endif
#ifdef debug
      G4cout<<">>>>>>>>G4QInelastic::Breeder:S#"<<astring<<",H#"<<aTrack<<",PDG="<<hPDG
      ...
      if(std::abs(hPDG)%10 > 2)
      {
...
#ifdef debug
        G4cout<<"G4QInelastic::Breeder:-DECAY'S DONE-,nH="<<tmpQHadVec->size()<<G4endl;
#endif
        ...
        for(unsigned aH=0; aH < tmpQHadVec->size(); aH++)
        {
          ...
          curHCh-=Chg;
          curHBN-=BaN;
          G4cout<<"-EMC->>>>G4QInelastic::Breeder:Str*Filled, 4M="<<p4M<<", PDG="<<PDG
      ...
      else                                      // Chipolino is not checked here
      {
        ...
        curStrChg-=curCh;
        curStrBaN-=curBN;
        G4cout<<"-EMC->>>>>>G4QInelastic::Breeder: curH filled 4M="<<curH4M<<",PDG="
...
#ifdef edebug
    G4cout<<"-EMC-.........G4QInelastic::Breeder: StringDecay CHECK, r4M="<<curString4M
  ...
  for(G4int i=0; i<nHadr; i++)
  {
    ...
    G4int hBaN=(*theResult)[i]->GetBaryonNumber();
    rBN-=hBaN;
    G4cout<<"-EMCLS-G4QInelastic::Breeder: Hadron#"<<i<<", 4M="<<hI4M<<", PDG="
          <<(*theResult)[i]->GetPDGCode()<<", C="<<hChg<<", B="<<hBaN<<G4endl;
  }
  for(G4int i=0; i<nQuasm; i++)
  {
    ...
    G4int hBaN=theQuasmons[i]->GetBaryonNumber();
    rBN-=hBaN;
    G4cout<<"-EMCLS-G4QInelastic::Breeder: Quasmon#"<<i<<", 4M="<<hI4M<<", C="<<hChg
          <<", B="<<hBaN<<G4endl;
  }
...
#ifdef debug
      G4cout<<"%->G4QInelastic::Breeder: TRY hPDG="<<hPDG<<", h4M="<<h4M<<G4endl;
#endif
...
#ifdef debug
      G4cout<<"%->G4QInelastic::Breeder:Exclude,4M="<<(*nih)->Get4Momentum()<<", & 4m="
      ...
      if(maxEn > 0.)                                         // More hadrons to absorbe
      {
        for(ih = theResult->begin(); ih < theResult->end(); ih++)
        {
          ...
          if(dE < maxEn)
          {
            ...
            qQC+=(*ih)->GetQC();
#ifdef debug
            G4cout<<"%->G4QInelastic::Breeder:Exclude,4M="<<h4M<<",dE="<<maxEn<<G4endl;
#endif
            delete (*ih);
...
#ifdef debug
      G4cout<<"%->G4QInelastic::Breeder:QuasmonIsFilled,4M="<<q4M<<",QC="<<qQC<<G4endl;
#endif
...
G4bool G4QInelastic::ExciteDiffParticipants(G4QHadron* projectile,
                                                G4QHadron* target) const
{
  ...
  {
#ifdef debug
    G4cout<<"G4QInelastic::ExciteDiffParticipants: *1* abort Collision!! *1*"<<G4endl;
#endif
    return false; 
  }         
...
#ifdef debug
  G4cout<<"G4QInelastic::ExciteDiffParticipantts:BeforBoost Pproj="<<Pprojectile<<",Ptarg="
        <<Ptarget<<G4endl;
#endif
  ...
  G4cout<<"G4QInelastic::ExciteDiffParticipants: ProjX+="<<Pprojectile.plus()<<", ProjX-="
  ...
  G4int whilecount=0;
#ifdef debug
  G4cout<<"G4QInelastic::ExciteDiffParticipants: Before DO"<<G4endl;
#endif
  do
  {
    //  Generate pt  
...
#ifdef debug
    G4cout<<"G4QInelastic::ExciteDiffParticipants: maxPtSq="<<maxPtSquare<<G4endl;
    if(whilecount++>=500 && whilecount%100==0) // @@ M.K. Hardwired limits 
    G4cout<<"G4QInelastic::ExciteDiffParticipantts: can loop, loopCount="<<whilecount
          <<", maxPtSquare="<<maxPtSquare<<G4endl;
#endif
    if(whilecount>1000)                        // @@ M.K. Hardwired limits 
    {
#ifdef debug
      G4cout<<"G4QInelastic::ExciteDiffParticipants: *2* abort Loop!! *2*"<<G4endl;
#endif
      return false;    //  Ignore this interaction 
    }
    Qmomentum=G4LorentzVector(GaussianPt(widthOfPtSquare,maxPtSquare),0);
#ifdef debug
    G4cout<<"G4QInelastic::ExciteDiffParticipants: generated Pt="<<Qmomentum<<", ProjPt="
...
#ifdef debug
    G4cout<<"G4QInelastic::ExciteDiffParticip:X-plus="<<Xplus<<",X-minus="<<Xminus<<G4endl;
#endif
...
#ifdef debug
  G4cout<< "G4QInelastic::ExciteDiffParticipants: TargetMass="<<Ptarget.mag()<<G4endl;
#endif
  target->Set4Momentum(Ptarget);  
#ifdef debug
  G4cout<<"G4QInelastic::ExciteDiffParticipants: ProjMass="<<Pprojectile.mag()<<G4endl;
#endif
...
G4bool G4QInelastic::ExciteSingDiffParticipants(G4QHadron* projectile,
                                                    G4QHadron* target) const
{
  G4LorentzVector Pprojectile=projectile->Get4Momentum();
  G4double Mprojectile=projectile->GetMass();
  ...
  {
#ifdef debug
    G4cout<<"--1/2--G4QInelastic::ExSingDiffParticipants: Projectile is fixed"<<G4endl;
#endif
  ...
  {
#ifdef debug
    G4cout<<"---1/2---G4QInelastic::ExSingDiffParticipants: Target is fixed"<<G4endl;
#endif
  ...
  {
#ifdef debug
    G4cout<<"G4QInelastic::ExciteSingDiffParticipants: *1* abort Collision!! *1*"<<G4endl;
#endif
    return false; 
  }         
  toCms.rotateZ(-Ptmp.phi());
  ...
  do
  {
    ...
    if(whilecount++>=500 && whilecount%100==0) // @@ M.K. Hardwired limits 
#ifdef debug
    G4cout<<"G4QInelastic::ExciteSingDiffParticipantts: can loop, loopCount="<<whilecount
          <<", maxPtSquare="<<maxPtSquare<<G4endl;
#endif
    if(whilecount>1000)                        // @@ M.K. Hardwired limits 
    {
#ifdef debug
      G4cout<<"G4QInelastic::ExciteSingDiffParticipants: *2* abort Loop!! *2*"<<G4endl;
#endif
      return false;    //  Ignore this interaction 
    }
    Qmomentum=G4LorentzVector(GaussianPt(widthOfPtSquare,maxPtSquare),0);
#ifdef debug
    G4cout<<"G4QInelast::ExciteSingDiffParticipants: generated Pt="<<Qmomentum<<", ProjPt="
          <<Pprojectile+Qmomentum<<", TargPt="<<Ptarget-Qmomentum<<G4endl;
#endif
    //  Momentum transfer
    G4double Xmin=0.;
  ...
  Ptarget     -= Qmomentum;
#ifdef debug
  G4cout<<"G4QInelastic::ExciteSingDiffParticipan: Proj(Q)="<<Pprojectile<<"(E="
  ...
  Ptarget.transform(toLab);
#ifdef debug
  G4cout<< "G4QInelastic::ExciteSingleDiffParticipants: TargMass="<<Ptarget.mag()<<G4endl;
#endif
  target->Set4Momentum(Ptarget);  
#ifdef debug
  G4cout<<"G4QInelastic::ExciteSingleParticipants: ProjMass="<<Pprojectile.mag()<<G4endl;
#endif
  projectile->Set4Momentum(Pprojectile);
  return true;
} // End of ExciteSingleDiffParticipants

void G4QInelastic::SetParameters(G4int nC, G4double stT,  G4double tbD, G4double SigPt)
{//  ======================================================================================
...
G4double G4QInelastic::ChooseX(G4double Xmin, G4double Xmax) const
{
  ...
  if( Xmin < 0. || Xmax < Xmin) 
  {
    G4cerr<<"***G4QInelastic::ChooseX: Xmin="<<Xmin<<", Xmax="<<Xmax<< G4endl;
    G4Exception("G4QInelastic::ChooseX:","72",FatalException,"Bad X or X-Range");
  }
  //G4double x;
...
#ifdef debug
  G4cout<<"G4QInelastic::ChooseX: DiffractiveX="<<x<<G4endl;
#endif
  return x;
} // End of ChooseX

// Pt distribution @@ one can use 1/(1+A*Pt^2)^B
G4ThreeVector G4QInelastic::GaussianPt(G4double widthSq, G4double maxPtSquare) const
{
#ifdef debug
  G4cout<<"G4QInelastic::GaussianPt:width2="<<widthSq<<",maxPt2="<<maxPtSquare<<G4endl;
#endif
  G4double pt2=0.;
...
G4int G4QInelastic::SumPartonPDG(G4int PDG1, G4int PDG2) const
{
  ...
  else if (PDG1 <-99 && PDG2 < 7 && PDG2 > 0) // Sum up Q and ADiQ in AQ
  {
    ...
    else
    {
      ...
      G4Exception("G4QInelastic::SumPartonPDG:","72",FatalException,"Q&ADiQ notMatch");
    }
  }
  else if (PDG2 <-99 && PDG1 < 7 && PDG1 > 0) // Sum up ADiQ and Q in AQ
  {
    ...
    {
      G4cerr<<"***4QFragmentation::SumPartonPDG: PDG1="<<PDG1<<", PDG2="<<PDG2<<G4endl;
      G4Exception("G4QInelastic::SumPartonPDG:","72",FatalException,"ADiQ&Q notMatch");
    }
  }
  else if (PDG1 > 99 && PDG2 >-7 && PDG2 < 0) // Sum up DiQ and AQ in Q
  {
    ...
    {
      G4cerr<<"***4QFragmentation::SumPartonPDG: PDG1="<<PDG1<<", PDG2="<<PDG2<<G4endl;
      G4Exception("G4QInelastic::SumPartonPDG:","72",FatalException,"DiQ&AQ notMatch");
    }
  }
  else if (PDG2 > 99 && PDG1 >-7 && PDG1 < 0) // Sum up AQ and DiQ in Q
  {
    ...
    {
      G4cerr<<"***G4QInelastic::SumPartonPDG: PDG1="<<PDG1<<", PDG2="<<PDG2<<G4endl;
      G4Exception("G4QInelastic::SumPartonPDG:","72",FatalException,"AQ&DiQ notMatch");
    }
  }
  else
  {
    G4cerr<<"***G4QInelastic::SumPartonPDG: PDG1="<<PDG1<<", PDG2="<<PDG2<<G4endl;
    G4Exception("G4QInelastic::SumPartonPDG:","72",FatalException,"Can'tSumUpPartons");
  }
  return 0;
} // End of SumPartonPDG

// Reduces quark pairs (unsigned 2 digits) to quark singles (unsigned)
std::pair<G4int,G4int> G4QInelastic::ReducePair(G4int P1, G4int P2) const
{
#ifdef debug
  G4cout<<"G4QInelastic::ReducePair: **Called** P1="<<P1<<", P2="<<P2<<G4endl;
#endif
  G4int P11=P1/10;
  G4int P12=P1%10;
  G4int P21=P2/10;
  ...
  //#ifdef debug
  G4cout<<"-Warning-G4QInelastic::ReducePair:**Failed**, P1="<<P1<<", P2="<<P2<<G4endl;
  //#endif
...
G4int G4QInelastic::AnnihilationOrder(G4int LS, G4int MPS, G4int uPDG, G4int mPDG,
...
{//                                             ^L^^ Curent ^^R^
  ...
  {
#ifdef debug
    G4cout<<"G4QInelastic::AnnihilationOrder: QaQ/QaQ->DiQ-aDiQ, uPDG="<<uPDG<<G4endl;
#endif
    ...
    else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong 22 fusion, L="<<uPDG
  ...
  else if ( LS==2 && MPS==3 )
  {
    ...
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihOrder: pLDiQ, sPDG="<<sPDG<<", nPDG="<<nPDG<<G4endl;
#endif
      if     ( sPDG<0 && (-sPDG==uPDG/1000 || -sPDG==(uPDG/100)%10) ) Ord= 1; // LL/RR
      else if( nPDG<0 && (-nPDG==uPDG/1000 || -nPDG==(uPDG/100)%10) ) Ord=-1; // LR/RL
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong pLDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
    else if (mPDG > 7)                               // Fuse pRDiQ
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihOrder: pRDiQ, sPDG="<<sPDG<<", nPDG="<<nPDG<<G4endl;
#endif
      if     ( sPDG<0 && (-sPDG==mPDG/1000 || -sPDG==(mPDG/100)%10) ) Ord=-1; // LR/RL
      else if( nPDG<0 && (-nPDG==mPDG/1000 || -nPDG==(mPDG/100)%10) ) Ord= 1; // LL/RR
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong pRDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
    else if (uPDG <-7)                               // Fuse pLaDiQ
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihOrder: pLaDiQ, sPDG="<<sPDG<<", nPDG="<<nPDG<<G4endl;
#endif
      if     ( sPDG>0 && (sPDG==(-uPDG)/1000 || sPDG==((-uPDG)/100)%10) ) Ord= 1; // LL/RR
      else if( nPDG>0 && (nPDG==(-uPDG)/1000 || nPDG==((-uPDG)/100)%10) ) Ord=-1; // LR/RL
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong pLaDiQ, L="<<uPDG
                 <<", R="<<mPDG<<", cL="<<sPDG<<", cR="<<nPDG<<G4endl;
    }
    else if (mPDG <-7)                              // Fuse pRaDiQ
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihOrder: pRaDiQ, sPDG="<<sPDG<<", nPDG="<<nPDG<<G4endl;
#endif
      if     ( sPDG>0 && (sPDG==(-mPDG)/1000 || sPDG==((-mPDG)/100)%10) ) Ord=-1; // LR/RL
      else if( nPDG>0 && (nPDG==(-mPDG)/1000 || nPDG==((-mPDG)/100)%10) ) Ord= 1; // LL/RR
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong pRaDiQ, L="<<uPDG
                 <<", R="<<mPDG<<", cL="<<sPDG<<", cR="<<nPDG<<G4endl;
    }
    else if( (sPDG<0 && (-sPDG==mPDG || -sPDG==uPDG) ) ||
             (nPDG<0 && (-nPDG==mPDG || -nPDG==uPDG) ) ) Ord= 2; // @@ Annihilation fusion
#ifdef debug
    else G4cout<<"-Warning-G4QInelastic::AnnihilatOrder: Wrong 23StringFusion"<<G4endl;
    G4cout<<"G4QInelastic::AnnihilationOrder: Ord="<<Ord<<",sPDG="<<sPDG<<",nPDG="
  ...
  else if ( LS==3 && MPS==2 )
  {
    ...
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihOrder: cLDiQ, uPDG="<<uPDG<<", mPDG="<<mPDG<<G4endl;
#endif
      if     ( uPDG<0 && (-uPDG==sPDG/1000 || -uPDG==(sPDG/100)%10) ) Ord= 1; // LL/RR
      else if( mPDG<0 && (-mPDG==sPDG/1000 || -mPDG==(sPDG/100)%10) ) Ord=-1; // LR/RL
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong cLDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
    else if (nPDG > 7)                               // Fuse cRDiQ
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihOrder: cRDiQ, uPDG="<<uPDG<<", mPDG="<<mPDG<<G4endl;
#endif
      if     ( uPDG<0 && (-uPDG==nPDG/1000 || -uPDG==(nPDG/100)%10) ) Ord=-1; // LR/RL
      else if( mPDG<0 && (-mPDG==nPDG/1000 || -mPDG==(nPDG/100)%10) ) Ord= 1; // LL/RR
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong cRDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
    else if (sPDG <-7)                               // Fuse cLaDiQ
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihOrder: cLaDiQ, uPDG="<<uPDG<<", mPDG="<<mPDG<<G4endl;
#endif
      if     ( uPDG>0 && (uPDG==(-sPDG)/1000 || uPDG==((-sPDG)/100)%10) ) Ord= 1; // LL/RR
      else if( mPDG>0 && (mPDG==(-sPDG)/1000 || mPDG==((-sPDG)/100)%10) ) Ord=-1; // LR/RL
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong cLaDiQ, L="<<uPDG
                 <<", R="<<mPDG<<", cL="<<sPDG<<", cR="<<nPDG<<G4endl;
    }
    else if (nPDG <-7)                              // Fuse cRaDiQ
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihOrder: cRaDiQ, uPDG="<<uPDG<<", mPDG="<<mPDG<<G4endl;
#endif
      if     ( uPDG>0 && (uPDG==(-nPDG)/1000 || uPDG==((-nPDG)/100)%10) ) Ord=-1; // LR/RL
      else if( mPDG>0 && (mPDG==(-nPDG)/1000 || mPDG==((-nPDG)/100)%10) ) Ord= 1; // LL/RR
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong cRaDiQ, L="<<uPDG
                 <<", R="<<mPDG<<", cL="<<sPDG<<", cR="<<nPDG<<G4endl;
    }
    else if( (uPDG<0 && (-uPDG==sPDG || -uPDG==nPDG) ) ||
             (mPDG<0 && (-mPDG==sPDG || -mPDG==nPDG) ) ) Ord=2; // @@ Annihilation fusion
#ifdef debug
    else G4cout<<"-Warning-G4QInelastic::AnnihilatOrder: Wrong 32StringFusion"<<G4endl;
    G4cout<<"G4QInelastic::AnnihilationOrder: Ord="<<Ord<<",sPDG="<<sPDG<<",nPDG="
          <<nPDG<<", uPDG="<<uPDG<<",mPDG="<<mPDG<<G4endl;
#endif
  }
  else if ( (LS==2 && MPS==4) || (LS==4 && MPS==2) )
  {
    ...
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihilOrder:pLDiQ, sPDG="<<sPDG<<",nPDG="<<nPDG<<G4endl;
#endif
      ...
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong 24 pLDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
    else if (mPDG >7) // Annihilate pRDiQ
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihilOrder:PRDiQ, sPDG="<<sPDG<<",nPDG="<<nPDG<<G4endl;
#endif
    ...
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihilOrder:cLDiQ, uPDG="<<uPDG<<",mPDG="<<mPDG<<G4endl;
#endif
      ...
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong 24 cLDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
    else if (nPDG > 7)   // Annihilate cRDiQ
    {
#ifdef debug
      G4cout<<"G4QInelastic::AnnihilOrder:cRDiQ, uPDG="<<uPDG<<",mPDG="<<mPDG<<G4endl;
#endif
      ...
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong 24 cRDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
#ifdef debug
    else G4cout<<"-Warning-G4QInelastic::AnnihilOrder: Wrong 24 StringFusion"<<G4endl;
    G4cout<<"G4QInelastic::AnnihilationOrder: Ord="<<Ord<<",sPDG="<<sPDG<<",nPDG="
          <<nPDG<<", uPDG="<<uPDG<<",mPDG="<<mPDG<<G4endl;
#endif
  }
  else if ( LS==3 && MPS==3 )
  {
    if     (uPDG > 7)  // Annihilate pLDiQ
    {
      ...
      if     ( sPDG<-7 && (-nPDG==uPDG/1000 || -nPDG==(uPDG/100)%10) &&
               ...
               (mPDG==(-nPDG)/1000 || mPDG==((-nPDG)/100)%10) ) Ord= 1; // LL/RR
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong 33 pLDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
    else if(mPDG > 7)  // Annihilate pRDiQ
    {
      ...
      if     ( sPDG<-7 && (-nPDG==mPDG/1000 || -nPDG==(mPDG/100)%10) &&
               ...
               (uPDG==(-nPDG)/1000 || uPDG==((-nPDG)/100)%10) ) Ord=-1; // LR/RL
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong 33 pRDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
    else if(sPDG > 7)  // Annihilate cLDiQ
    {
      ...
      if     ( uPDG<-7 && (-mPDG==sPDG/1000 || -mPDG==(sPDG/100)%10) &&
               ...
               (nPDG==(-mPDG)/1000 || nPDG==((-mPDG)/100)%10) ) Ord= 1; // LL/RR
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong 33 cLDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
    else if(nPDG > 7)  // Annihilate cRDiQ
    {
      ...
      if     ( uPDG<-7 && (-mPDG==nPDG/1000 || -mPDG==(nPDG/100)%10) &&
               ...
               (sPDG==(-mPDG)/1000 || sPDG==((-mPDG)/100)%10) ) Ord=-1; // LR/RL
      else G4cerr<<"-Warning-G4QInelastic::AnnihilationOrder: Wrong 33 cRDiQ, L="<<uPDG
                 <<",R="<<mPDG<<",cL="<<sPDG<<",cR="<<nPDG<<G4endl;
    }
#ifdef debug
    else G4cout<<"-Warning-G4QInelastic::AnnihilOrder: Wrong 33 StringFusion"<<G4endl;
    G4cout<<"G4QInelastic::AnnihilationOrder: Ord="<<Ord<<",sPDG="<<sPDG<<",nPDG="
          <<nPDG<<", uPDG="<<uPDG<<",mPDG="<<mPDG<<G4endl;
#endif
  }
...
void G4QInelastic::SwapPartons() // Swap string partons, if a string has negative M2
{
...
#ifdef debug
      else G4cout<<"***G4QInelastic::SwapPartons:**Failed**,cLPDG="<<cLPDG<<",cRPDG="
