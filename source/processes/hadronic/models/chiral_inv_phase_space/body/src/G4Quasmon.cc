...
//              ============================================================
{
  ...
  if(!status||q4Mom==zeroLV)                       // This Quasmon is done (Sould not be here)
  {
    ...
    if(addPhoton)
	{
      ...
      throw G4QException("G4Quasmon::HadronizeQuasmon: Overhead photon for the Zero Quasmon");
	}
  ...
  while(theEnvironment.GetPDG()==NUCPDG || start)  // ***=The MAIN LOOP (a LOOP only for vacuum)=***
  {
	...
	//if(2>3)                                        // Corresponds to envA>256 (@@other not debugged)
	{
      ...
      G4int zEn=maxActEnv;                         // Charge of the Limit Active Nuclear Environment
	...
	while(kCount<kCountMax&&kCond)
	{
	  ...
	  //if(addPhoton>0.&&(G4UniformRand()<.7||iniBN<2))// PhotonAbsorbDiagramContrib(@@HiddenPar)
	  //if(addPhoton>0.&&(G4UniformRand()<.5||iniBN<2))// PhotonAbsorbDiagramContrib(@@HiddenPar)
      //if(addPhoton>0.&&(G4UniformRand()<.27||iniBN<2))// PhotonAbsorbDiagramContrib(@@HiddenPar)
	  //if(addPhoton>0.&&iniBN<2)// PhotonAbsorbDiagramContrib(@@HiddenPar)
	  ...
	  //if(addPhoton>0.&&iniBN>1)//PhotonAbsorbDiagramContrib(@@ A photon is captured by N quark)
	  //if(addPhoton>0.)// PhotonAbsorbDiagramContrib(@@ A photon is always captured by quarks)
	  //if(2>3)                                        // Photon capture by quark is closed
      {
		...
		//  //if(cost<.9999)frat=G4UniformRand()>addPhoton/(addPhoton+kMom);
		//  //else frat=G4UniformRand()>addPhoton*addPhoton/(addPhoton+kMom)/(addPhoton+kMom);
        //  pCount++;
		//}
        if(cost>1.) cost=1.;
        if(cost<-1.) cost=-1.;
	  ...
	  if(!G4QHadron(q4Mom).RelDecayIn2(k4Mom, cr4Mom, q4Mom, cost, cost))// Q->ColResidQ+k_parton
      {
#ifdef debug
      ...
      else                                           // ***> The decay succeeded
      {
        ...
        for (G4int index=0; index<totCand; index++)
        {
          ...
          if(cPDG==90000001||cPDG==90001000||cPDG==91000000||cPDG<MINPDG)//@@ k-PreAttempts(Acc)
          {
		    ...
		    if(poss)
            {
              ...
              G4double dR=0.;                       // @@ Temporary no q-acceleration cut is used
     		  if(cPDG==90000001 || cPDG==90001000 || cPDG==90000002 || cPDG==90001001)
	            G4cout<<"G4Q::HQ:i="<<index<<",cPDG="<<cPDG<<",k="<<kMom<<",kLS="<<kLS<<" > kMin="
                      <<kMin<<", bM="<<bnM<<", rEP="<<rEP<<", dR="<<dR<<", kCon="<<kCond<<G4endl;
#endif
              if(kLS>kMin)
			  {
                kCond=false;
...
#endif
    CalculateHadronizationProbabilities(excE,kMom,k4Mom,piF,gaF); //ResonanceMass is randomized here
    ...
    {
#ifdef debug
	  if(status==4) throw G4QException("G4Q::HadrQ: TMP EXCEPTION - no chanals of fragmentation");
#endif
      ...
      else                                           // May be evaporation is possible
	  {
		...
		//else if(envPDG==NUCPDG && quasM>iniQM && iniBN>1) // Convert Quasmon to Moving Environment
		//{
#ifdef debug
		...
		else if(envPDG==NUCPDG && quasM>iniQM)       // Emergency decay in Gamma & GSQuasmon
		{
		  ...
		  //throw G4QException("***G4Q::HadrQ: Emergency Decay in Gamma + Residual GSQuasmon");
#endif
          G4int gamPDG=22;
          G4double gamM=0.;
          if(quasM>mPi0+iniQM)
          ...
          if(!G4QHadron(q4Mom).DecayIn2(r4Mom, s4Mom))
          {
    	    ...
    	    throw G4QException("G4Quasmon::HadronizeQuasmon:G/Pi0+GSQ decay didn't succeed,Env=0");
          }
#ifdef debug
        ...
        //if(excE>8.)throw G4QException("G4Quas::HQ: Why a candidate is not found?");
        //if(totMass>envM+iniQM&&excE>minK&&excE>1.&&minK)throw G4QException("G4Q::HQ:Why (1) ?");
        //throw G4QException("G4Quasmon::HadronizeQuasmon: Why a candidate is not found?");
#endif
        qEnv=theEnvironment;
        return theQHadrons;
	  }
    ...
    if (!fprob)                                      // Prepare final decay, "nothing was selected"
	{
      ...
      else if(!sPDG)
	  {
        ...
        throw G4QException("G4Quasmon::HadronizeQuasmon: Decay particle isn't selected & can't evaporate");
	  }
      else if(sPDG==10)                              // ---> Chipolino case
	  {
        ...
        if(sMass+rMass>quasM)                        //@@ Evaporate but try 3Ptcl decay with Environ
		{
          ...
          else
		  {
            ...
            throw G4QException("G4Quasmon::HadronizeQuasmon: VirtualChipo can't evaporate Nucleus");
		  }
		}
      }
    ...
    else                                             // ---> "Something was selected" case
	{
      ...
      if(sPDG>MINPDG&&sPDG!=NUCPDG)                // ===> "Fragment" case
	  {
        ...
        if (sppm<=0)                                 // Impossible to find a parent cluster
        {
          ...
          G4cerr<<G4endl;
          throw G4QException("G4Quasmon::HadronizeQuasmon: No parent cluster for the fragment");
	    }
      ...
      else
      {
        ...
        if((sPDG<MINPDG&&envPDG>MINPDG&&envPDG!=NUCPDG||sPDG>MINPDG&&sPDG!=NUCPDG&&
          envPDG>pPDG) && (rqPDG>MINPDG&&rqPDG!=NUCPDG||rqPDG==2112||rqPDG==2212||rqPDG==3122))
        {
...
#ifdef debug
		    G4cout<<"G4Q::HQ:reQC="<<reQC<<",rtQC="<<rtQC<<",eA="<<envA<<",pB="<<pBaryn<<",bE="
                  <<bEn<<bEnQC<<G4endl;
#endif
            rtQC+=reQC;
...
#ifdef debug
		    G4cout<<"G4Q::HQ:***NuclearFragm*** M="<<newT<<",rt="<<rtM<<rtQC<<",e="<<envM
				  <<envQC<<",p="<<pMass<<pQC<<",re="<<reM<<reQC<<",E="<<totMass-rtM-sMass<<G4endl;
#endif
		  }
      ...
      if(!minSqT)
      {
        ...
        throw G4QException("G4Quasmon::HadronizeQuasmon: Minimal ResidMass can't be calculated");
      }
      ...
      if(sPDG>MINPDG&&sPDG!=NUCPDG)                // ==> Nuclear fragment candidate hadronization
	  {
#ifdef debug 
        ...
        {
#ifdef debug
		G4cout<<"***G4Q::HQ: Failed to find FragmentMass:"<<ccM2<<"<"<<frM2<<", pM="<<pMass
        ...
        else
        {
...
#ifdef debug
          // --- All this is just for check and debugging messages. Don't add any calculations!
          ...
          if(cta>1.0001)G4cerr<<"***G4Q::HQ: cost_max="<<cta<<">1. Correct HadrProbab"<<G4endl;
          ...
          if(cti<-1.0001)G4cerr<<"***G4Q::HQ: cost_min="<<cti<<"<-1. Correct HadrProbab"<<G4endl;
          G4double kap_i=ex/(1.+kLS*(1.-cti)/pMass); // MinEnergy of the recoil quark (q_min)
          G4double q_lim=(tmpTM2-tCRN.m2())/(tcEP+tcEP);
          if(cti>cta+.0001)G4cerr<<"***G4Q::HQ:cti="<<cti<<">cta="<<cta<<".Cor.HadrProb"<<G4endl;
          G4cout<<"G4Q::HQ:qi="<<kap_i<<",ci="<<cti<<",qa="<<kap_a<<",ca="<<cta<<",q="<<q_lim
                <<",ex="<<ex<<",M2="<<tmpTM*tmpTM<<",R2="<<tCRN.m2()<<",EP="<<tcEP<<G4endl;
#endif
          // Try the maximum possible range of the recoil quark (@@ Temporary!) --- Starts Here
#ifdef debug
          ...
          while(qCount<qCountMax&&qCond)
          {
            ...
            //**PSLtest**//G4double z = pow((loli+hili)/2,pw); // *** TMP Mid q-RANDOMIZATION ***@@@@
...
#endif
#ifdef debug
            G4cout<<"G4Q::HQ:ct="<<ctkk<<",pM="<<pMass<<",z="<<z<<",zl="<<pow(loli,pw)<<",zh="
                  <<pow(hili,pw)<<",ex="<<ex<<",li="<<loli<<",hi="<<hili<<G4endl;
#endif
            if(abs(ctkk)>1.00001)
            {
#ifdef debug
              ...
              //throw G4QException("***TemporaryException***G4Q::HQ: cos(theta) limits problem");
#endif
              if(ctkk> 1.)ctkk= 1.;
              if(ctkk<-1.)ctkk=-1.;
            }
            ...
            if(!G4QHadron(cc4Mom).RelDecayIn2(kp4Mom, fr4Mom, k4Mom, ctc, ctc))
            {
              ...
              throw G4QException("G4Quasmon::HadronizeQuasmon: Can't decay FakeColClust in fr+kap");
            }
#ifdef debug
...
#ifdef debug
	      G4cout<<"G4Q::HQ:t4M="<<tot4Mom<<".Is kt="<<kt<<">"<<minSqB<<" or kn="<<kn<<">"<<minSqN
                <<"? m2="<<m2<<", sPDG="<<sPDG<<G4endl;
#endif
		  ...
		  //if(kt<minSqB) // BindingEnreachment for OnlyNucleon
          {
            hsflag=true;
#ifdef debug
	        G4cout<<"G4Q::HQ:**hsflag=1** No, sPDG="<<sPDG<<", kt="<<kt<<"<"<<minSqB<<" or kn="<<kn
                  <<"<"<<minSqN<<G4endl;
#endif
		  }
#ifdef debug
	      else G4cout<<"G4Q::HQ:Yes kt="<<kt<<">"<<minSqB<<" or kn="<<kn<<">"<<minSqN<<G4endl;
#endif
		}
	  }
...
#ifdef debug
      G4cout<<"G4Q::HQ:k="<<kMom<<".Is("<<kt<<">"<<minSqB<<"for hadron or "<<kn<<">"<<minSqN<<"&tM="
            <<tM<<">rtM="<<rtM<<" for nFragm.) & !hsflag="<<!hsflag<<" to avoid R+S="<<sPDG<<G4endl;
#endif
      ...
      //if((kt>minSqT+.01&&sPDG<MINPDG || kn>minSqN+.01&&tM>rtM)&&!hsflag) // M(RQ), Ma(RNucl)>M_min
      if((kt>minSqB+.01&&sPDG<MINPDG || sPDG>MINPDG&&kn>minSqN+.01&&tM>rtM) && !hsflag) // M>M_min
      //if(kn>minSqN+.01&&(sPDG<MINPDG||tM>rtM)&&!hsflag)// Mass(RQ)>M_min && Mass(RNucleus)>M_min
	  //if(kt>minSqB+.01&&(sPDG<MINPDG||tM>rtM)&&!hsflag)// Mass(RQ)>M_min && Mass(RNucleus)>M_min
      //if(kt>minSqT+.01&&(sPDG<MINPDG||tM>rtM)&&!hsflag)// Mass(RQ)>M_min && Mass(RNucleus)>M_min
	  ...
	  //if(2>3)                                      // *** Close k-correction ***
	  {
...
#ifdef debug
          G4cout<<"G4Q::HQ: YES for the hadron it is big enough: t="<<kt<<" > T="<<minSqB<<", np="
                <<np<<", cM="<<cM<<", uR="<<uR<<", rn="<<rn<<G4endl;
#endif
        ...
        {
#ifdef debug
           G4cout<<"G4Q::HQ: YES for the fragment it is big enough: kn="<<kn<<">"<<minSqN<<G4endl;
#endif
...
#ifdef ppdebug
		G4cout<<"G4Q::HQ:*NO***hsflag=1!***,kt="<<kt<<"<"<<minSqB<<" or tM="<<tM<<"<"<<rtM<<G4endl;
#endif
	  }
#ifdef debug
	  G4cout<<"G4Q::HQ:******>>>>> rMass="<<rMass<<", rQm="<<sqrt(m2)<<", hsflag="<<hsflag<<G4endl;
#endif
      ...
      if(sPDG<MINPDG&&envPDG==NUCPDG)rFl=G4UniformRand()<bs*bs/m2;// ProbabFunct: m_min^2/m^2
#ifdef debug
        ...
        if(rPDG && rPDG!=10)                         // Can the residual be a Hadron ?
	    {
		  ...
		  { // Final H+R decay of QUASMON in vacuum case (should not exist if Environ exists)
            ...
            if(!G4QHadron(q4Mom).DecayIn2(r4Mom, s4Mom))
            {
    	      ...
    	      throw G4QException("G4Quasmon::HadronizeQuasmon:Hadr+Res decay didn't succeed");
            }
#ifdef debug
    ...
    if(fprob)                                        // Calc. PDG of residual Quasmon as S-hadron
    {
      rPDG=curQ.GetSPDGCode();
      G4double rrr=G4UniformRand();                  // The same as for "rqPDG" & previous "rPDG"
    ...
    G4double reMass=sqrt(minSqB);                    // Min Residual Quasmon Mass after decay
    if (!rPDG)
    {
    ...
    G4cout<<"G4Q::HQ:rPDG="<<rPDG<<",QB="<<iniBN<<". Is [hsf="<<hsflag<<"] or [rdf="<<fdul
          <<"] or [rM="<<rMass<<"<"<<reMass<<"+"<<aMass<<" or rM2="<<reTNM2<<"<miM2="<<tmpTM2
          <<" and ePDG="<<envPDG<<">pPDG="<<pPDG<<"] to fail?"<<G4endl;
#endif
    //        *** This is a condition, when fragmentation can not be continued ***
	if(hsflag||sPDG<MINPDG&&rMass<reMass+aMass||sPDG>MINPDG&&envPDG>pPDG&&reTNM2<tmpTM2||fdul)
    // >>>>>>>>>>> Decay Q->S+H or Q/C->H1+H2 or suck in or evaporate or slow down or decay etc.
	{
      ...
      if(sPDG>MINPDG&&sPDG!=NUCPDG)                // "Quark exchange hadronization" case
	  {
        ...
        //if(nQuasms==1&&retNM>tmpNM+tmpRM) //=> "Go out & drop ExcitedNucleus"
		if(2>3)                                      // *** Attempt GoOutDropExcNucl is closed ***
		{
		  ...
		  if(2>3)                                    // *** Closed ***
		  {
            ...
            if(!G4QHadron(tot4M).DecayIn3(rq4M,re4M,fr4Mom))
            {
              ...
              throw G4QException("***G4Quasmon::HadrQuasm: Failed to decay in Frag+ResQ+ResEnv");
            }
            else
            {
        ...
        else if(totBN>1&&totMass>totM&&totS>=0&&envPDG>MINPDG&&envPDG!=NUCPDG) // Evaporation
		{
#ifdef ppdebug
          ...
          //if(excE>.027*fraM) throw G4QException("G4Quasmon::HadrQuasm: Why Fail? (6)"); //@@ TMP
          //if(excE>60.) throw G4QException("G4Quasmon::HadrQuasm: Why Fail? (6)"); //@@ TMP
          //if(totMass<sumM)throw G4QException("G4Quasmon::HadrQuasm: Why? (6). Prod's>TotM");//@@T
          throw G4QException("G4Quasmon::HadrQuasm: Why Fail? (6) Products are more than total");
#endif
#ifdef debug
        ...
        else if(totBN==1&&nQuasms==1) // Decay of the baryonic Total Nucleus state
		{
          ...
          if(abs(totS)==1)
          {
            if(totS==1)               // Decay of the strange hyperstate
			{
              ...
              else
			  {
    	        ...
    	        throw G4QException("G4Quasmon::HadronizeQuasm: Pi + Lambda decay did not succeed");
              }
			}
			else                      // Decay of the anti-strange hyperstate
			{
			  ...
			  {
                G4cerr<<"***G4Q::HQ:tZ="<<totZ<<",tS="<<totS<<",T="<<totQC<<",M="<<totMass<<G4endl;
    	        throw G4QException("G4Quasmon::HadronizeQuasm: K + Nucleon decay did not succeed");
              }
			}
		  }
          else if(totMass>PiNM&&!totS) // Decay in nucleon & pion
          {
            ...
            else
			{
    	      ...
    	      throw G4QException("G4Quasmon::HadronizeQuasm: Pi + Nucleon decay did not succeed");
            }
		  }
          else if(!totS)
		  {
			...
			{
              G4cerr<<"***G4Q::HQ:totZ="<<totZ<<",totB="<<totBN<<",E="<<envQC<<",Q="<<valQ<<G4endl;
    	      throw G4QException("G4Quasmon::HadronizeQuasm:Photon+Nucleon decay did not succeed");
            }
          }
          ...
          if(!G4QHadron(tot4M).DecayIn2(pi4M, nuc4M))
          {
    	    ...
    	    throw G4QException("G4Quasmon::HadronizeQuasm:Gam/Pi + Nucleon decay did not succeed");
          }
#ifdef debug
      ...
      if(rPDG!=10&&rMass>dm&&!rWi) // Try to use a width of the h-resonance or reduce its spin state
	  {
        ...
        if(sWi)                                      // Hadron is a resonance
		{
          ...
          if(fabs(sMM-ddm)<1.5*sWi-.001 && ddm>mmm)
          {
            ...
            {
#ifdef debug
              G4cerr<<"***G4Q::HQ:CHANGE M=0, sPDG="<<sPDG<<",M_new="<<ddm<<",M_old="<<msm<<G4endl;
#endif
              sMass=ddm;
            }
            if(sMass<ddm) sMass=ddm;
#ifdef debug
            G4cout<<"G4Q::HQ: "<<sPDG<<",sM="<<sMass<<",d="<<ddm<<",isM="<<msm<<",W="<<sWi<<G4endl;
#endif
		  }
...
#ifdef debug
      G4cout<<"G4Q::HQ: *BEFORE rPDG correction* dm="<<dm<<",rnd="<<rnd<<", rPDG="<<rPDG<<G4endl;
#endif
	  ...
	  if((rPDG==111&&sPDG!=111||rPDG==221)&&rMass>544.&&dm>544.&&rnd>.5) rPDG=113; // pi0/eta->rho0
	  if((rPDG==111&&sPDG!=111||rPDG==221)&&rMass>782.&&dm>782.&&rnd<.5) rPDG=223; // pi0/eta->omega
	  if(rPDG==331 &&rMass>1020.&&dm>1020.&&rnd<.5) rPDG=333;                      // eta'   ->phi
      if(rPDG== 211 && dm>544. && rnd>.5) rPDG= 213; // M-1.5*W conv. of pi+ to rho+
      if(rPDG==-211 && dm>544. && rnd>.5) rPDG=-213; // M-1.5*W conv. of pi- to rho-
#ifdef debug
      G4cout<<"G4Q::HQ: *After rPDG correction* qM="<<quasM<<",sM="<<sMass<<", rPDG="<<rPDG<<G4endl;
#endif
      if(rPDG<MINPDG&&rPDG!=2212&&rPDG!=2112&&rPDG!=3122&&rPDG!=10) // => ResidQ isn't a NuclClust
      {
        reMass=GetRandomMass(rPDG,dm);               // Randomize mass of the RResidQuasmon-Hadron
#ifdef debug
        G4cout<<"G4Q::HQ:dm="<<dm<<", ResQMass="<<reMass<<" is changed for rPDG="<<rPDG<<G4endl;
#endif
        if(reMass==0.)
	    {
...
#endif
        if(sPDG>MINPDG) theEnvironment.Reduce(pPDG);  // Update NuclearEnviron after Q->RQ+QEXF
        ClearQuasmon();                               // This Quasmon is done
        qEnv=theEnvironment;                         // Update the QEnvironment
...
#ifdef pdebug
	else G4cout<<"G4Q::HQ:NO-OK,hs="<<hsflag<<",d="<<fdul<<",M="<<rMass<<"<"<<reMass<<",M2="
               <<reTNM2<<"<I2="<<tmpTM2<<",sPDG="<<sPDG<<",ePDG="<<envPDG<<",pPDG="<<pPDG<<G4endl;
#endif
	if(!fskip)                                       // Continue search for fragmentation
	{
	  ...
	  G4cout<<"G4Q::HQ:>>"<<sPDG<<fr4Mom<<fr4Mom.m()<<"="<<sMass<<",T="<<frKin<<",E="<<ePDG<<G4endl;
#endif
      ///if(sPDG<MINPDG&&ePDG==NUCPDG)             // ====>>>> "Hadron candidate in Vacuum" case
	  if(sPDG<MINPDG)           // Hadronic decay is always as in vacuum @@??
      {
        ...
        if(!sPDG&&SQ<0)           // decay in K+/aK0 & residual
		{
          ...
          if(!G4QHadron(tot4M).DecayIn2(r4Mom, s4Mom))
          {
            G4cerr<<"***G4Q::HQ: tM="<<tot4M.m()<<totQC<<" => rPDG="<<rPDG<<"(rM="<<rMass
                  <<") + sPDG="<<sPDG<<"(sM="<<sMass<<")"<<G4endl;
    	    throw G4QException("***G4Quasmon::HadrQuasmon:K+ResidNucl DecayIn2 didn't succeed");
          }
#ifdef debug
        ...
        if(!G4QHadron(q4Mom).DecayIn2(resQ4Mom, s4Mom))
        {
	      ...
	      throw G4QException("G4Quasmon::HadronizeQuasmon: Quasmon+Hadron DecayIn2 did not succeed");
        }
#ifdef debug
      ...
      else                                           // ===> "Hadron in NuclMedia or NuclCand" case
	  {
        ...
        if(nucflag) rQ4Mom+=dMass;                   // Make a correction to 4Mom of ResidQuasmon
    ...
    G4cout<<"G4Q::HQ:eZ="<<eZ<<valQ<<"***>>FinalCHECK>>***>> 4M="<<sumLor<<",Charg="<<sumC<<G4endl;
...
{//  ==================================================================================
 ...
 #ifdef pdebug
  G4cout<<"G4Quasmon::DecayQuasmon: after decay (FillHadronVector by itself) nH="<<nHadrs<<G4endl;
#endif
  if(nHadrs) for (int hadron=0; hadron<nHadrs; hadron++)
  {
    G4QHadron* curHadr = new G4QHadron(theQHadrons[hadron]);
    theFragments->push_back(curHadr);                  // (delete equivalent - user must delete)
  }
#ifdef ppdebug
...
{//  ==========================================
  ...
  else if(thePDG>80000000&&thePDG!=90000000)   // === Decay-Evaporation of the BarionicFragment ===
  {
    ...
    else if(abs(fragMas-GSMass)<.1)            // the Nucleus is too close the Ground State
    {
                                                           ...
                                                           ||nS>0&&fragMas>lResM+mLamb))
	  {
        ...
        else if(thePDG!=90004004)
		{
          ...
          throw G4QException("***G4Quasmon::FillHadronVector: Below GSM but can not decay in p or n");
		}
    ...
    else if (fragMas<GSMass)
	{
      ...
      throw G4QException("***G4Quasmon::FillHadronVector: Out Mass is below the Ground State value");
	}
...
//       ==========================================================
{
  ...
  //  if(Mmum2<fM2m2)throw G4QException("G4Quasmon::QPartMom: mR & mC are too big in comp. with mQ");
...
void G4Quasmon::CalculateHadronizationProbabilities(G4double E, G4double kVal, G4LorentzVector k4M,
                                                    G4bool piF, G4bool gaF)
//   ======================================================================E is not used====
{                                                                  //       ^
  static const G4double mPi0 = G4QPDGCode(111).GetMass();          //       |
  ...
  //if(addPhoton) kLSi=kLS-addPhoton;              // @@ now probabilities for k+gamma can be wrong
  ...
  //if(vap>0)vaf = mQk/kVal/vap;                   //@@ Vacuum factor (instead of in G4QNucleus)
  ...
  G4cout<<"G4Q::CalcHadronizationProbab:Q="<<mQ<<valQ<<",v="<<vaf<<",r="<<var<<",mC="<<maxB<<",vap="
        <<vap<<",k="<<kVal<<G4endl;
#endif
  ...
  if(nHC) for (unsigned index=0; index<nHC; index++)
  {
...
#endif
	//if(pos&&(cPDG<80000000||(cPDG>80000000&&cPDG!=90000000&&dUD<1))) // 1 ** never try this **
	//if(pos&&(cPDG<80000000||(cPDG>80000000&&cPDG!=90000000&&dUD<2))) // 2 deuterons too high
	if(pos&&(cPDG<80000000||(cPDG>80000000&&cPDG!=90000000&&dUD<3))) // 3 the best
	{
      ...
      //G4int cNQ= candQC.GetTot()-1-baryn;             // A#of quarks/diquarks in the candidate - 2
      ...
      if(cPDG>80000000&&cPDG!=90000000&&baryn<=envA) //===> "Nuclear Fragment" case (QUark EXchange)
      {
        ...
        if(oQmax>oQmin) for(int iq=iQmin; iq<iQmax; iq++) // Entering (from Quasmon) quark
		{
          ...
          if(oQmax>oQmin) for(int oq=oQmin; oq<oQmax; oq++) // Exiting (to Quasmon) quark
		  {
			...
			//if(resQ>-2&&resPDG&&resPDG!=10&&!rI&&(!piF||cPDG==90000001||cPDG==90002002))//OK
			//if(resQ>-2&&resPDG&&resPDG!=10&&!rI&&(!piF||cPDG==90000001))
			//if(resQ>-2&&resPDG&&resPDG!=10&&!rI&&(!piF||piF&&cPDG!=90001000&&cPDG!=90002001))
			...
			//if(resQ>-2&&resPDG&&resPDG!=10&&!rI&&(!piF||baryn==1)) // bad
			{
              ...
              if(shift!=7&&is<maxC)                      // This quark exchange is possible
			  {
                ...
                if(possib&&charge<=envZ&&barot-charge<=envN)
                {
				  ...
				  //if(abs(dS)<3 || (qIso>0&&dC<0||qIso<0&&dC>0) && (baryn==1||cPDG==90002002))
                  ...
                  //if(3>2)                                   //***>>> ***NO*** Isotope Focusing ***
                  {
					...
					if(resPDG&&minM>0.) // Kinematical analysis of hadronization
                    {
                      ...
                      // Use the following 3 with special precotions and together with "lz" below
                      //G4double ne=1.-dkd/(nucBM+dkLS); // q_min=DEFOULT=bndM*(k-dn)/(bndM+2k)
                      //G4double ne=1.-dkedC/(boundM+dkLS); // q_min=DEFOULT=bM*(k-de+CB)/(bM+2k)
                      //G4double ne=1.-dkdC/(nucBM+dkLS); // q_min=DEFOULT=bndM*(k-dn+CB)/(bndM+2k)
                      G4double kf=1.;
                      if(ne>0.&&ne<1.)kf=pow(ne,cNQ);
#ifdef pdebug
                      if(baryn<5) G4cout<<"G4Q::CHP:<qi_DEF>="<<ne<<",kf="<<kf<<",dked="<<dked
                                        <<",dkLS="<<dkLS<<",bM="<<boundM<<",CB="<<CB<<G4endl;
#endif
                      ...
                      if(baryn<5)G4cout<<"G4Q::CHP:RN="<<tmpEQ<<"="<<envM-boundM<<"=eM="<<envM
                      ...
                      G4bool atrest=(eQ-mQ)/mQ<.001||k3V.dot(rq3V)<-.999; // Q at rest (Pi/GamCap)
                      //G4bool atrest=(eQ-mQ)/mQ<.001; // Q at rest (only PiCap)
                      // ***VTN*** CHECK necessety to recover the ColouredTotal to MinMassTotal
					  ...
					  //if(minBM2>rQ2&&baryn>2) // ==> Check of Residual Virtual Quasmon **OK**
					  //if(minBM2>rQ2&&baryn>1) // ==> Check of Residual Virtual Quasmon **Better**
					  //if(minBM2>rQ2&&piF&&(cPDG==90000001||cPDG==90002002))//CheckResidualVirtualQ
					  //if(minBM2>rQ2&&piF&&(cPDG==90000001||baryn>3)) //Check ResidualVirtualQ
					  //if(minBM2>rQ2&&(piF&&cPDG==90000001||baryn>2))//=>Check ResidualVirtualQuasmon
					  //if(minBM2>rQ2&&baryn>2) // ==> Check of Residual Virtual Quasmon
					  //if(minBM2>rQ2&&!piF&&baryn>2&&cPDG!=90001002) // Residual Virtual Quasmon
					  if(minBM2>rQ2&&!piF&&baryn>2) //  ==> Check of Residual Virtual Quasmon 
					  //if(minBM2>rQ2&&!piF&&baryn>3) // ==> Check of Residual Virtual Quasmon
					  //if(minBM2>rQ2&&!piF&&baryn>1) // ==> Check of Residual Virtual Quasmon
					  //if(minBM2>rQ2&&!piF&&!gaF)    // ==> Check of Residual Virtual Quasmon
					  ...
					  if(minM2>rQ2&&!piF&&baryn>2&&cPDG!=90001002)// ==> Check of Residual Quasmon
					  ...
					  //if(minM2>rQ2&&(baryn>1||!piF)) // ==> Check Residual Quasmon **Better**
...
#ifdef pdebug
          if(abs(cPDG)<3)G4cout<<"G4Q::CHP:comb="<<comb<<",d="<<cmd<<",u="<<cmu<<",s="<<cms<<G4endl;
#endif
        }
        ...
        if(comb&&resPDG && totMass>frM-CB+resTM &&(resPDG>80000000&&resPDG!=90000000||resPDG<10000))
	    {
          ...
          if(resM>0. && resQCode>-2)
	      {
            ...
            if(rndM>0. && resM+rndM<mQ)
	        {
              ...
              G4double zMin=0.;
              //@@ At this point one can use zMin=0 and later return to residM, as it is for qBar==0
...
{ //   ===========================================
...
#ifdef pdebug
  G4cout<<"G4Q::CheckGS: EnvironmentPDG="<<theEnvironment.GetPDG()<<",Quasmon="<<resQPDG<<G4endl;
#endif
  ...
  else                                           //Calculate BaryonSeparetionCondition for vacQuasm.
  {
    ...
    bsCond = tmpQN.SplitBaryon();                // Possibility to split a fragment from the VacuumQ
#ifdef pdebug
    G4cout<<"G4Q::CheckGS: No environment, only Quasmon="<<tmpQN<<", bsCond="<<bsCond<<G4endl;
#endif
    if(bsCond)                                   // Decay rignht away in Fragment & residual
	{
      ...
      G4LorentzVector quas4M = G4LorentzVector(0.,0.,0.,rsMass); // GS Mass of the ResidualQuasmon
      G4QHadron* quasH = new G4QHadron(rsQC, quas4M);
      G4LorentzVector frag4M = G4LorentzVector(0.,0.,0.,fragmM); // GS Mass of the SplittedFragment
...
#ifdef pdebug
  G4cout<<"G4Q::CheckGS: (totM="<<resTMa<<" < rQM+rEM="<<resSMa<<" || rEM="<<resEMa<<"=0 && "
