  ...
  //
    case overflow_bin :
      if (overflownEvents>0) {
        error = overflowTotalWeight/sqrt(overflownEvents);}
      else {
        error = 0.;}
      break;
    case underflow_bin :
      if (underflownEvents>0) {
        error = underflowTotalWeight/sqrt(underflownEvents);}
      else {
        error = 0.;}
      break;
    case inrange : 
      ...
      if (nAllEvents-overflownEvents-underflownEvents>0) {
        ...
        error = error/sqrt(nAllEvents-overflownEvents-underflownEvents);
      }
      break;
  }
  return error;
...
double Histo1DVar::get_bin_error (int i)
{
  ...
  if (i > int(part.total_bins())-1) {
    ...
    if (nEvents[i]>0) {
      error = totalWeight[i]/sqrt(nEvents[i]);}
    else {
      error = 0.;}
  }
  return error;
