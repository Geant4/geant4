...
G4CascadeParameters::G4CascadeParameters()
  : G4CASCADE_VERBOSE(getenv("G4CASCADE_VERBOSE")),
    ...
    G4NUCMODEL_POTENTIAL_THICKNESS(getenv("G4NUCMODEL_POTENTIAL_THICKNESS")),
...
void G4CascadeParameters::Initialize() {
  ...
  POTENTIAL_THICKNESS = (G4NUCMODEL_POTENTIAL_THICKNESS?strtod(G4NUCMODEL_POTENTIAL_THICKNESS,0):1.0); // default 1.0 fm
...
void G4CascadeParameters::DumpConfig(std::ostream& os) const {
  ...
  if (G4NUCMODEL_POTENTIAL_THICKNESS)
    os << "G4NUCMODEL_POTENTIAL_THICKNESS = " << G4NUCMODEL_POTENTIAL_THICKNESS << endl;
  if (DPMAX_2CLUSTER)
