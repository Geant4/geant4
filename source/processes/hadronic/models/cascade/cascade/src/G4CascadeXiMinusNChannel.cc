...
G4CascadeXiMinusNChannel::G4CascadeXiMinusNChannel()
  :G4CascadeChannel()
{
  G4int i, k, m;
  G4int start, stop;
  ...
  for (m = 0; m < 6; m++) {
    ...
    stop = xmnindex[m][1];
    for (k = 0; k < 31; k++) {
      xmnMultiplicities[m][k] = 0.0;
      for (i = start; i < stop; i++) xmnMultiplicities[m][k] += xmnCrossSections[i][k];
    }
  }
  ...
  for (k = 0; k < 31; k++) {
    xmntot[k] = 0.0;
    for (m = 0; m < 6; m++) xmntot[k] += xmnMultiplicities[m][k];
  }
...
G4double G4CascadeXiMinusNChannel::getCrossSection(G4double ke) const
{
  std::pair<G4int, G4double> epair = interpolateEnergy(ke);
  G4int k = epair.first;
  G4double fraction = epair.second;

  return xmntot[k] + fraction*(xmntot[k+1] - xmntot[k]);
}


G4int G4CascadeXiMinusNChannel::getMultiplicity(G4double ke) const
{
  G4double multint(0.);
  ...
  for(G4int m = 0; m < 6; m++) {
    multint = xmnMultiplicities[m][k] 
         + fraction*(xmnMultiplicities[m][k+1] - xmnMultiplicities[m][k]);
...
std::vector<G4int> 
G4CascadeXiMinusNChannel::getOutgoingParticleTypes(G4int mult, G4double ke) const
{
  G4int i;
  G4double sigint(0.);
  std::vector<G4double> sigma;
  ...
  G4int start = xmnindex[mult-2][0];
  G4int stop = xmnindex[mult-2][1];
 
  for(i = start; i < stop; i++) {
      sigint = xmnCrossSections[i][k] 
          + fraction*(xmnCrossSections[i][k+1] - xmnCrossSections[i][k]);
  ...
  if (mult == 2) {
    for(i = 0; i < mult; i++) kinds.push_back(xmn2bfs[channel][i]);
  } else if (mult == 3) {
    for(i = 0; i < mult; i++) kinds.push_back(xmn3bfs[channel][i]);
  } else if (mult == 4) {
    for(i = 0; i < mult; i++) kinds.push_back(xmn4bfs[channel][i]);
  } else if (mult == 5) {
    for(i = 0; i < mult; i++) kinds.push_back(xmn5bfs[channel][i]);
  } else if (mult == 6) {
    for(i = 0; i < mult; i++) kinds.push_back(xmn6bfs[channel][i]);
  } else if (mult == 7) {
    for(i = 0; i < mult; i++) kinds.push_back(xmn7bfs[channel][i]);
  } else {
...
G4double G4CascadeXiMinusNChannel::xmntot[31];

// Multiplicities as a function of kinetic energy
G4double G4CascadeXiMinusNChannel::xmnMultiplicities[6][31];


const G4int G4CascadeXiMinusNChannel::xmnindex[6][2] = 
   {{0, 3}, {3, 21}, {21,74}, {74,76}, {76,78}, {78,80}};

// Outgoing particle types of a given multiplicity

const G4int G4CascadeXiMinusNChannel::xmn2bfs[3][2] =
  {{2, 31}, {21,27}, {25,27}};

const G4int G4CascadeXiMinusNChannel::xmn3bfs[18][3] =
...
const G4int G4CascadeXiMinusNChannel::xmn4bfs[53][4] =
...
const G4int G4CascadeXiMinusNChannel::xmn5bfs[2][5] =
  {{2,7,7,7,31},  {2,3,5,7,31}};

const G4int G4CascadeXiMinusNChannel::xmn6bfs[2][6] =
  {{2,7,7,7,7,31},  {2,3,5,7,7,31}};

const G4int G4CascadeXiMinusNChannel::xmn7bfs[2][7] =
...
// 
const G4float G4CascadeXiMinusNChannel::xmnCrossSections[80][31] = {
 //
