...
G4CascadeXiZeroNChannel::G4CascadeXiZeroNChannel()
  :G4CascadeChannel()
{
  G4int i, k, m;
  G4int start, stop;
  ...
  for (m = 0; m < 6; m++) {
    ...
    stop = x0nindex[m][1];
    for (k = 0; k < 31; k++) {
      x0nMultiplicities[m][k] = 0.0;
      for (i = start; i < stop; i++) x0nMultiplicities[m][k] += x0nCrossSections[i][k];
    }
  }
  ...
  for (k = 0; k < 31; k++) {
    x0ntot[k] = 0.0;
    for (m = 0; m < 6; m++) x0ntot[k] += x0nMultiplicities[m][k];
  }
...
G4double G4CascadeXiZeroNChannel::getCrossSection(G4double ke) const
{
  std::pair<G4int, G4double> epair = interpolateEnergy(ke);
  G4int k = epair.first;
  G4double fraction = epair.second;

  return x0ntot[k] + fraction*(x0ntot[k+1] - x0ntot[k]);
}


G4int G4CascadeXiZeroNChannel::getMultiplicity(G4double ke) const
{
  G4double multint(0.);
  ...
  for(G4int m = 0; m < 6; m++) {
    multint = x0nMultiplicities[m][k] 
         + fraction*(x0nMultiplicities[m][k+1] - x0nMultiplicities[m][k]);
...
std::vector<G4int> 
G4CascadeXiZeroNChannel::getOutgoingParticleTypes(G4int mult, G4double ke) const
{
  G4int i;
  G4double sigint(0.);
  std::vector<G4double> sigma;
  ...
  G4int start = x0nindex[mult-2][0];
  G4int stop = x0nindex[mult-2][1];
 
  for(i = start; i < stop; i++) {
      sigint = x0nCrossSections[i][k] 
          + fraction*(x0nCrossSections[i][k+1] - x0nCrossSections[i][k]);
  ...
  if (mult == 2) {
    for(i = 0; i < mult; i++) kinds.push_back(x0n2bfs[channel][i]);
  } else if (mult == 3) {
    for(i = 0; i < mult; i++) kinds.push_back(x0n3bfs[channel][i]);
  } else if (mult == 4) {
    for(i = 0; i < mult; i++) kinds.push_back(x0n4bfs[channel][i]);
  } else if (mult == 5) {
    for(i = 0; i < mult; i++) kinds.push_back(x0n5bfs[channel][i]);
  } else if (mult == 6) {
    for(i = 0; i < mult; i++) kinds.push_back(x0n6bfs[channel][i]);
  } else if (mult == 7) {
    for(i = 0; i < mult; i++) kinds.push_back(x0n7bfs[channel][i]);
  } else {
...
G4double G4CascadeXiZeroNChannel::x0ntot[31];

// Multiplicities as a function of kinetic energy
G4double G4CascadeXiZeroNChannel::x0nMultiplicities[6][31];


const G4int G4CascadeXiZeroNChannel::x0nindex[6][2] = 
   {{0, 6}, {6,30}, {30,34}, {34,38}, {38,42}, {42,46}};

// Outgoing particle types of a given multiplicity

const G4int G4CascadeXiZeroNChannel::x0n2bfs[6][2] =
  {{2,29}, {1,31}, {21,21}, {21,25}, {25,25}, {23,27}};

const G4int G4CascadeXiZeroNChannel::x0n3bfs[24][3] =
...
const G4int G4CascadeXiZeroNChannel::x0n4bfs[4][4] =
  {{1,7,13,21}, {2,7,17,21}, {1,3,5,31}, {2,3,5,29}};

const G4int G4CascadeXiZeroNChannel::x0n5bfs[4][5] =
  {{1,3,5,13,21}, {2,3,5,17,21}, {1,3,5,7,31}, {2,3,5,7,29}};

const G4int G4CascadeXiZeroNChannel::x0n6bfs[4][6] =
{{1,3,5,7,13,21}, {2,3,5,7,17,21}, {1,3,3,5,5,31}, {2,3,3,5,5,29}};

const G4int G4CascadeXiZeroNChannel::x0n7bfs[4][7] =
...
// 
const G4float G4CascadeXiZeroNChannel::x0nCrossSections[46][31] = {
 //
