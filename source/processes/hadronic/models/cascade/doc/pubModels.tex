...
Intranuclear cascade model developed by Bertini \cite{}, 

Our model is based on re-engineering of INUCL code ~\cite{iljinov94}.
...
Range of targets allowed arbitrary.
...
We give in the following the overview of submodel features.
...
[Mass formula]
Nucleons binding energies are calculated using mass formula. For pions $V_{opt}$ is taken to be constant (about 7 MeV).
...
Pauli exclusion in the INC: Simulated particle-particle interaction is accepted only for secondary nucleons which have $E_n > E_f$.
...
\subsection{Nucleus explosion model}

Fermi breakup is allowed only in some extreme cases, i.e. for light nuclei ($A < 12$ and  $3(a-z) < Z < 6$ ) and $E(exitation) > 3.*Eb$ 

Simple explosion model treating the total nuleus decay into neutrons and protons decreases exotic evaporation processes.
...
The following evaportaion phase implementation is based on Dostrowski model \cite[].
