...
Fast phase ($10^{-23} -- 10^{-22}$) of initial INC results to highly exited nucleus, and possible fission and pre-equilibrium emission. A slower $10^-18 -- 10^-16$ compound nucleus phase follows with evaportaion.
...
The necessary condition of validity og the INC model is $\lambda_{B} / v << \tau_{c} << \Delta t$, 
...
also physical foundation comes approximate at energies less than $\approx 200 MeV$, and needs to be supported with pre-quilibrium model.
...
\begin{enumerate}
\item The spatial point where the incident particle enters selcted uniformly over the projected area of the nucleus.
...
\item After INC, the residual excitation energy of the resulting nucleus (A', Z') are used as input for non-equilibrim model.
\end{enumerate}

In the code the nucleus is composed of three consentric spheres in all cases,
 exept for Hydrogen (H$_1^1$), in which case the direct particle-particle collision is performed.
...
Importan addition to INC was exiton model itroduced by Griffin in 1966 \cite{griffin66, griffin67}. . The compound nucleon states are characterized by the number of exited particles and holes (the exitons)
N-N collisions give rise to a sequence of states characterized by increasing exciton number, eventually leading to a equilibrated nucleus.

Posible selection rules in the cource of the cascade are:
