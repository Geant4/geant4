...
A fast phase ($10^{-23} - 10^{-22}$) of INC results to highly exited nucleus, and is followed possible by fission and pre-equilibrium emission. 
A slower $10^-18 - 10^-16$ compound nucleus phase follows with evaportaion.
...
In the code the nucleus is composed of three consentric spheres in all cases,
 exept for Hydrogen, in which case the direct particle-particle collision is performed.
...
Total reaction cross-sections [mbarn] were calculated by J.R. Letaw's formulation \cite{letaw83, letaw93, pearlstein89}

%$45 A^0.7 (1+0.016 sin(5.3-2.63 log10(A)))^(1-0.62 exp(-E / 200) sin(10.9 E^(-0.28)))

For N-N cross-sections parametrizations based on the experimental data (energy and isospin dependent) are used. 
The parameterizations described in \cite{barashenkov72} are used. 
...
Important addition to INC was exiton model itroduced by Griffin in 1966 \cite{griffin66, griffin67}. 
The his model compound nucleon states are characterized by the number of exited particles and holes (the exitons).
...
{\sc Geant4} cascade model implements exiton model proposed by Griffin \cite{griffin66}.
...
where p is the number of particle, h is number of holes and n = p + h is the number of exitons. % ::: p g exito
...
If light nucleus is higly exited Fermi break-up model is executed. Also, fission is performed is channel is open. 
The main chain of evaporation is followed until  $E_{exitation}$ falls below E$_{cutoff}$ = 0.1 MeV. Finally photon emission chain is followed until
E$_{exitation}$ $<$ E$^{\gamma}_{cutoff}$ = 10$^{-15}$ MeV.
