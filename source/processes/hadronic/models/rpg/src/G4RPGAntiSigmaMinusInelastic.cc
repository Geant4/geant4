...
// ********************************************************************
//
// $Id: G4RPGAntiSigmaMinusInelastic.cc 94214 2015-11-09 08:18:05Z gcosmo $
//
...
G4HadFinalState*
G4RPGAntiSigmaMinusInelastic::ApplyYourself( const G4HadProjectile &aTrack,
...
{ 
  ...
  if( verboseLevel > 1 )
  {
    ...
    G4cout << "G4RPGAntiSigmaMinusInelastic::ApplyYourself called" << G4endl;
...
void G4RPGAntiSigmaMinusInelastic::Cascade(
   ...
   G4bool &quasiElastic )
{
  // Derived from H. Fesefeldt's original FORTRAN code CASASM
  ...
  G4double availableEnergy = centerofmassEnergy-(targetMass+mOriginal);
    ...
    if( G4UniformRand() > anhl[iplab] )
    {
      ...
      if( targetParticle.GetDefinition() == aProton )
      {
        ...
        np--; nneg--; nz--;
        G4int ncht = std::min( 3, std::max( 1, np-nneg+1 ) );
        switch( ncht )
        {
         case 1:
           break;
