...
//
//
// $Id: G4Be7GEMProbability.cc,v 1.1 2003-08-26 18:44:03 lara Exp $
  ...
  G4GEMProbability(7,4,3.0/2.0) // A,Z,Spin
{
  ...
  ExcitLifetimes.push_back(hbar_Planck*log(2.0)/(175.0*keV));

  ExcitEnergies.push_back(6.73e3*keV);
  ExcitSpins.push_back(5.0/2.0);
  ExcitLifetimes.push_back(hbar_Planck*log(2.0)/(1.2*MeV));

  ExcitEnergies.push_back(7210.0*keV);
  ExcitSpins.push_back(5.0/2.0);
  ExcitLifetimes.push_back(hbar_Planck*log(2.0)/(0.5*MeV));

  ExcitEnergies.push_back(9900.0*keV);
  ExcitSpins.push_back(3.0/2.0);
  ExcitLifetimes.push_back(hbar_Planck*log(2.0)/(1.8*MeV));
  ...
  ExcitSpins.push_back(1.0/2.0);
  ExcitLifetimes.push_back(hbar_Planck*log(2.0)/(6.5*MeV));
...
{
  G4Exception("G4Be7GEMProbability::copy_constructor meant to not be accessable");
}
...
{
  G4Exception("G4Be7GEMProbability::operator= meant to not be accessable");
  return *this;
}
