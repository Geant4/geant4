...
// ********************************************************************
//
// $Id: G4ANuTauNucleusCcModel.cc 91806 2015-08-06 12:20:45Z gcosmo $
//
...
using namespace CLHEP;
    ...
    G4Mutex G4ANuTauNucleusCcModel::numuNucleusModel = G4MUTEX_INITIALIZER;
#endif     


G4ANuTauNucleusCcModel::G4ANuTauNucleusCcModel(const G4String& name) 
...
G4ANuTauNucleusCcModel::~G4ANuTauNucleusCcModel()
{}


void G4ANuTauNucleusCcModel::ModelDescription(std::ostream& outFile) const
{

    outFile << "G4ANuTauNucleusCcModel is a anti tau neutrino-nucleus (charge current)  scattering\n"
...
void G4ANuTauNucleusCcModel::InitialiseModel()
{
...
G4bool G4ANuTauNucleusCcModel::IsApplicable(const G4HadProjectile & aPart, 
					        G4Nucleus & )
{
  G4bool result  = false;
...
G4HadFinalState* G4ANuTauNucleusCcModel::ApplyYourself(
		 const G4HadProjectile& aTrack, G4Nucleus& targetNucleus)
{
  ...
  if( p1pi > G4UniformRand()  && fCosTheta > 0.9  ) // && fQtransfer < 0.95*GeV ) // mu- & coherent pion + nucleus
  {
    ...
    if(  pName == "anti_nu_tau" )         aLept = new G4DynamicParticle( theTauPlus, lv2 );  
  ...
  */
  ...
  if( ratio > G4UniformRand() ) // proton is excited
  {
    ...
    if( pName == "anti_nu_tau" ) // (++) state -> p + pi+
    { 
  ...
  else // excited neutron
  {
    ...
    if( pName == "anti_nu_tau" ) // (+) state -> n + pi+
    {      
  ...
  {  
    if     (  fProton && pName == "anti_nu_tau" )      qB =  2;
...
void G4ANuTauNucleusCcModel::SampleLVkr(const G4HadProjectile & aTrack, G4Nucleus& targetNucleus)
{
  fBreak = false;
