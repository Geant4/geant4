...
// ********************************************************************
//
// $Id: G4NuTauNucleusCcModel.cc 91806 2015-08-06 12:20:45Z gcosmo $
//
...
using namespace CLHEP;
    ...
    G4Mutex G4NuTauNucleusCcModel::numuNucleusModel = G4MUTEX_INITIALIZER;
#endif     


G4NuTauNucleusCcModel::G4NuTauNucleusCcModel(const G4String& name) 
  : G4NeutrinoNucleusModel(name)
{
  fData = fMaster = false;
  fMtau = 1.77686*GeV;
  theTauMinus = G4TauMinus::TauMinus();
  theTauPlus = G4TauPlus::TauPlus();
  InitialiseModel();  
}
...
void G4NuTauNucleusCcModel::ModelDescription(std::ostream& outFile) const
{
    ...
    outFile << "G4NuTauNucleusCcModel is a tau neutrino-nucleus (charge current)  scattering\n"
...
G4bool G4NuTauNucleusCcModel::IsApplicable(const G4HadProjectile & aPart, 
					        G4Nucleus & )
{
  G4bool result  = false;
  ...
  if(  pName == "nu_tau"  // || pName == "anti_nu_tau" )  
        &&
        energy > fMinNuEnergy                                )
  {
    result = true;
...
G4HadFinalState* G4NuTauNucleusCcModel::ApplyYourself(
		 const G4HadProjectile& aTrack, G4Nucleus& targetNucleus)
{
  ...
  if( fBreak == true || fEmu < fMtau ) // ~5*10^-6
  {
    // G4cout<<"ni, ";
    theParticleChange.SetEnergyChange(energy);
    theParticleChange.SetMomentumChange(aTrack.Get4Momentum().vect().unit());
  ...
  if( p1pi > G4UniformRand()  && fCosTheta > 0.9  ) // && fQtransfer < 0.95*GeV ) // mu- & coherent pion + nucleus
  {
    ...
    // muMom = sqrt(fEmuPi*fEmuPi-fMtau*fMtau);
    muMom = sqrt(fEmu*fEmu-fMtau*fMtau);
    ...
    if(  pName == "nu_tau" )         aLept = new G4DynamicParticle( theTauMinus, lv2 );  
    // else if( pName == "anti_nu_tau") aLept = new G4DynamicParticle( theTauPlus,  lv2 );
    else
    {
      theParticleChange.SetEnergyChange(energy);
      theParticleChange.SetMomentumChange(aTrack.Get4Momentum().vect().unit());
  ...
  */
  ...
  {  
    if     (  fProton && pName == "nu_tau" )      qB =  2;
    // else if(  fProton && pName == "anti_nu_tau" ) qB =  0;
    else if( !fProton && pName == "nu_tau" )      qB =  1;
    // else if( !fProton && pName == "anti_nu_tau" ) qB = -1;
...
void G4NuTauNucleusCcModel::SampleLVkr(const G4HadProjectile & aTrack, G4Nucleus& targetNucleus)
{
  fBreak = false;
  ...
  if( A == 1 ) // hydrogen, no Fermi motion ???
  {
    ...
    while( ( abs(fCosTheta) > 1. || fEmu < fMtau ) && iTer < iTerMax );
