...
G4bool G4NeutrinoElectronCcModel::IsApplicable(const G4HadProjectile& aPart, G4Nucleus&)
{
  G4bool result = false;
...
G4HadFinalState* G4NeutrinoElectronCcModel::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus&)
{
  ...
  if (pName == "nu_mu")
    massf = theMuonMinus->GetPDGMass();
  else if (pName == "nu_tau")
    massf = theTauMinus->GetPDGMass();
  ...
  if (pName == "nu_mu" || pName == "nu_tau")
  {
    ...
    aNu = new G4DynamicParticle(theAntiNeutrinoMu, lvp2);  // s-channel for mu (tau later)
  ...
  if (aNu)
  {
    theParticleChange.AddSecondary(aNu, secID);
  }
  if (aLept)
  {
    theParticleChange.AddSecondary(aLept, secID);
  }
...
G4double G4NeutrinoElectronCcModel::SampleCosCMS(const G4HadProjectile* aParticle)
{
  ...
  else if (pName == "anti_nu_mu" || pName == "anti_nu_tau")
  {
    ...
    if (pName == "anti_nu_mu")
      massf2 = theMuonMinus->GetPDGMass() * theMuonMinus->GetPDGMass();
    else
      massf2 = theTauMinus->GetPDGMass() * theTauMinus->GetPDGMass();
