                                       ...
                                       G4String& aFSType, G4ParticleDefinition*)
{
  ...
  G4ParticleHPDataUsed aFile =
    theNames.GetName(static_cast<G4int>(A), static_cast<G4int>(Z), M, tString, aFSType, dbool);
                                                 ...
                                                 G4ParticleDefinition* aDefinition)
{
  ...
  G4ReactionProduct incidReactionProduct(
    const_cast<G4ParticleDefinition*>(hadProjectile->GetDefinition()));  // incidReactionProduct
...
#ifdef G4PHPDEBUG
  if (std::getenv("G4ParticleHPDebug"))
    G4cout << this << " G4ParticleHPInelasticCompFS::CompositeApply A " << theBaseA << " Z "
  ...
  theTarget.SetDefinition(
    G4IonTable::GetIonTable()->GetIon(G4int(theBaseZ), G4int(theBaseA), 0.0));  // XX
  ...
  residualMass = G4NucleiProperties::GetNuclearMass(static_cast<G4int>(residualA + eps),
                                                    static_cast<G4int>(residualZ + eps));
  ...
  if (theEnergyDistribution[it] == 0 && theAngularDistribution[it] == 0
      && theEnergyAngData[it] == 0)
  {
    if (theEnergyDistribution[50] != 0 || theAngularDistribution[50] != 0
        || theEnergyAngData[50] != 0)
    {
      it = 50;
    }
  }
  ...
  if (50 == it) {
    ...
    aHadron.SetMomentum(incidReactionProduct.GetMomentum()
                        * (1. / incidReactionProduct.GetTotalMomentum()) * p);
  }
  else {
    iLevel = imaxEx;
  }
  ...
  if (theParticles != 0) {
    ...
    if (totalBaryonNumber
        != G4int(theBaseA + eps + hadProjectile->GetDefinition()->GetBaryonNumber()))
    {
      needsSeparateRecoil = true;
      residualA = G4int(theBaseA + eps + hadProjectile->GetDefinition()->GetBaryonNumber()
                        - totalBaryonNumber);
      residualZ =
        G4int(theBaseZ + eps + hadProjectile->GetDefinition()->GetPDGCharge() - totalCharge);
    }
  }
  ...
  if (theParticles == 0) {
    ...
    theResidual.SetDefinition(G4IonTable::GetIonTable()->GetIon(static_cast<G4int>(residualZ),
...
#ifdef G4PHPDEBUG
      if (std::getenv("G4ParticleHPDebug"))
        G4cout << this << " G4ParticleHPInelasticCompFS::BaseApply add secondary3 "
               << theSec->GetParticleDefinition()->GetParticleName()
               << " E= " << theSec->GetKineticEnergy() << " NSECO "
               << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
    ...
    if (needsSeparateRecoil && residualZ != 0) {
      G4ReactionProduct theResidual;
      theResidual.SetDefinition(G4IonTable::GetIonTable()->GetIon(
        static_cast<G4int>(residualZ), static_cast<G4int>(residualA), 0));
...
#ifdef G4PHPDEBUG
      if (std::getenv("G4ParticleHPDebug"))
        G4cout << this << " G4ParticleHPInelasticCompFS::BaseApply add secondary4 "
               << theSec->GetParticleDefinition()->GetParticleName()
               << " E= " << theSec->GetKineticEnergy() << " NSECO "
               << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
...
#ifdef G4PHPDEBUG
      if (std::getenv("G4ParticleHPDebug"))
        G4cout << this << " G4ParticleHPInelasticCompFS::BaseApply add secondary5 "
               << theSec->GetParticleDefinition()->GetParticleName()
               << " E= " << theSec->GetKineticEnergy() << " NSECO "
               << theResult.Get()->GetNumberOfSecondaries() << G4endl;
#endif
                                                    ...
                                                    G4double resExcitationEnergy)
{
  ...
  // Residual definition:
  G4int resZ = (G4int)(proj->GetDefinition()->GetPDGCharge() + targ->GetDefinition()->GetPDGCharge()
                       - product->GetDefinition()->GetPDGCharge() + 0.1);
  ...
  G4double fmomsquared = 1. / 4. / totE / totE
                         * (totE * totE - (prodmass - resmass) * (prodmass - resmass))
                         * (totE * totE - (prodmass + resmass) * (prodmass + resmass));
  G4double fmom = 0;
  if (fmomsquared > 0) {
  ...
  product->SetMomentum(fmom * sinth * std::cos(phi), fmom * sinth * std::sin(phi),
                       fmom * cosTh);  // CMS
                                                      ...
                                                      G4ReactionProduct& boosted)
{
  if (aDefinition == G4Neutron::Definition())  // If the outgoing particle is a neutron...
  {
    ...
    if (LR[it] > 0)  // If there is breakup of the residual nucleus LR(flag LR in ENDF)>0 (i.e. Z=6
                     // MT=52-91 (it=MT-50)).
    {
      ...
      for (G4int j = 0; j < 4; ++j) {
        ...
        theResult.Get()->AddSecondary(
          new G4DynamicParticle(theProds[j].GetDefinition(), theProds[j].GetMomentum()), secID);
      }
  ...
  else if (aDefinition == G4Alpha::Definition())  // If the outgoing particle is an alpha, ...
  {
    ...
    if (LR[it] == 0)  // If Z=6, an alpha particle is emitted and there is no breakup of the
