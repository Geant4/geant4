...
G4HadFinalState* G4ParticleHPCaptureFS::ApplyYourself(const G4HadProjectile& theTrack)
{
  ...
  if (!G4ParticleHPManager::GetInstance()->GetDoNotAdjustFinalState()) {
    ...
    if (nPhotons == 1 && thePhotons->operator[](0)->GetDefinition()->GetBaryonNumber() == 0) {
      ...
      G4double Q = G4IonTable::GetIonTable()->GetIonMass(static_cast<G4int>(theBaseZ),
                                                         static_cast<G4int>(theBaseA), 0)
                   + G4Neutron::Neutron()->GetPDGMass()
                   - G4IonTable::GetIonTable()->GetIonMass(static_cast<G4int>(theBaseZ),
                                                           static_cast<G4int>(theBaseA + 1), 0);
  ...
  if (!residual) {
    ...
    if (nPhotons - nNonZero > 0) {
      // G4cout << "TKDB G4ParticleHPCaptureFS::ApplyYourself we will create additional " <<
...
#include <sstream>
void G4ParticleHPCaptureFS::Init(G4double A, G4double Z, G4int M, G4String& dirName, G4String&,
