...
G4ParticleHPCaptureData::G4ParticleHPCaptureData() : G4VCrossSectionDataSet("NeutronHPCaptureXS")
{
...
G4bool G4ParticleHPCaptureData::IsIsoApplicable(const G4DynamicParticle* dp, G4int /*Z*/,
...
G4double G4ParticleHPCaptureData::GetIsoCrossSection(const G4DynamicParticle* dp, G4int /*Z*/,
...
#ifdef G4VERBOSE
    if (std::getenv("CaptureDataIndexDebug") != nullptr) {
...
G4double G4ParticleHPCaptureData::GetCrossSection(const G4DynamicParticle* aP, const G4Element* anE,
                                                  G4double aT)
{
  G4double result = 0;
  G4bool outOfRange;
...
{
  outFile << "High Precision cross data based on Evaluated Nuclear Data Files (ENDF) for radiative "
