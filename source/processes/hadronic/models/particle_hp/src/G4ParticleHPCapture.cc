...
G4ParticleHPCapture::G4ParticleHPCapture() : G4HadronicInteraction("NeutronHPCapture")
{
  SetMinEnergy(0.0);
  SetMaxEnergy(20. * MeV);
  /*
  //    G4cout << "Capture : start of construction!!!!!!!!"<<G4endl;
      if(!G4FindDataDir("G4NEUTRONHPDATA"))
         throw G4HadronicException(__FILE__, __LINE__, "Please setenv G4NEUTRONHPDATA to point to
  the neutron cross-section files."); dirName = G4FindDataDir("G4NEUTRONHPDATA"); G4String tString =
  "/Capture"; dirName = dirName + tString; numEle = G4Element::GetNumberOfElements();
  //    G4cout << "+++++++++++++++++++++++++++++++++++++++++++++++++"<<G4endl;
...
void G4ParticleHPCapture::addChannelForNewElement()
{
   ...
   {
      G4cout << "G4ParticleHPCapture Prepairing Data for the new element of " <<
(*(G4Element::GetElementTable()))[i]->GetName() << G4endl; theCapture.push_back( new
G4ParticleHPChannel );
      (*theCapture[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
      (*theCapture[i]).Register(theFS);
   }
...
void G4ParticleHPCapture::BuildPhysicsTable(const G4ParticleDefinition&)
{
  ...
  if (G4Threading::IsMasterThread()) {
    ...
    auto theFS = new G4ParticleHPCaptureFS;
    for (G4int i = numEle; i < (G4int)G4Element::GetNumberOfElements(); ++i) {
      theCapture->push_back(new G4ParticleHPChannel);
      ((*theCapture)[i])->Init((*(G4Element::GetElementTable()))[i], dirName);
      ((*theCapture)[i])->Register(theFS);
