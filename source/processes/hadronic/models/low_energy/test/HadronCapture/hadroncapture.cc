...
int main(int argc,char** argv)
{
  ...
  captureProcess->RegisterMe(new G4LCapture() );
  ...
  for (G4int i = 0; i < materials.size(); i++) {
    ...
    for (G4int loop = 0; loop < Nevents; loop++) {
      aChange = captureProcess->PostStepDoIt(*track,step);
      ...
      for (G4int i = 0; i < nSec; i++) {
        ...
        if (sec->GetDefinition()->GetParticleName() == "gamma") {
          ngnew++;
          eknew = sec->GetKineticEnergy()/MeV;
          ekg += eknew;
          ekg2 += eknew*eknew;
          pnew = sec->GetDynamicParticle()->GetTotalMomentum()/MeV;
          pxnew = pnew*sec->GetMomentumDirection().x();
          pxg += pxnew;
          pxg2 += pxnew*pxnew;
          pynew = pnew*sec->GetMomentumDirection().y();
          pyg += pynew;
          pyg2 += pynew*pynew;
          pznew = pnew*sec->GetMomentumDirection().z();
          pzg += pznew;
          pzg2 += pznew*pznew;
          eknew4 = eknew;
          pxnew4 = pxnew;
    ...
    G4cout << G4endl << " For a " << matName << " target, the total number of gammas = " << ng << G4endl;
    G4cout << (G4double)ng/Nevents << " gammas per event " << G4endl;
    G4cout << " Mean energy, momentum: E = " << ekg/ng
           << " , px = " << pxg/ng << " , py = " << pyg/ng 
           << " , pz = " << pzg/ng << G4endl;
  }
