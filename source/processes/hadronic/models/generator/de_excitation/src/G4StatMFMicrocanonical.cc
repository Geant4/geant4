...
//    _ThePartitionManagerVector.clearAndDestroy();
    while (!_ThePartitionManagerVector.empty()) {
	delete _ThePartitionManagerVector.back();
	_ThePartitionManagerVector.pop_back();
    }
}
...
void G4StatMFMicroCanonical::Initialize(const G4Fragment & theFragment) 
{
    ...
    for (i = 0; i < _ThePartitionManagerVector.size(); i++) {
