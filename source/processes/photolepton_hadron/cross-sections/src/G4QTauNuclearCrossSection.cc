...
//
//
// $Id: G4QTauNuclearCrossSection.cc,v 1.1 2004-03-05 13:24:26 mkossov Exp $
...
G4double  G4QTauNuclearCrossSection::lastTH=0.; // Last energy threshold
G4double  G4QTauNuclearCrossSection::lastSig=0.;// Last value of the Cross Section
G4double  G4QTauNuclearCrossSection::ml=G4TauMinus::TauMinus()->GetPDGMass();// M_TauLepton
G4double  G4QTauNuclearCrossSection::ml2=
                             G4QTauNuclearCrossSection::ml*G4QTauNuclearCrossSection::ml;
G4double  G4QTauNuclearCrossSection::lml=log(G4QTauNuclearCrossSection::ml);// ln(M_Tau)

// The main member function giving the gamma-A cross section (E_kin in MeV, CS in mb)
G4double G4QTauNuclearCrossSection::GetCrossSection(G4double Energy,
                                                       G4int targZ, G4int targN)
{
  static const G4int nE=336;    // !! If change this, change it in GetFunctions() (*.hh) !!
  static const G4int mL=nE-1;
  static const G4double EMi=2.0612;          // Minimum tabulated KineticEnergy of Tau-lept
  static const G4double EMa=50000.;          // Maximum tabulated Energy of the Electron 
  static const G4double lEMi=log(EMi);       // Minimum tabulatedLogarithmKinEnergy of Tau
  static const G4double lEMa=log(EMa);       // Maximum tabulatedLogarithmKinEnergy of Tau
...
#ifdef pdebug
      G4cout<<"G4QTauNuclearCS::GetCrossSection: lastH="<<lastH<<",A="<<A<<G4endl;
#endif
  ...
  if (Energy<=lastTH)                // Once more check that tauKiE is higher than ThreshE
  {
    lastSig=0.;
    return 0.;
  }
  G4double lE=log(Energy);           // log(tauE) (it is necessary for the fit)
  lastG=lE-lml;                     // Gamma of the tau-lepton (used to recover log(tauE))
...
#ifdef pdebug
    G4cout<<"G4QTauNCS::GCS:S="<<lastSig<<",E="<<lE<<",Yi="<<YNi<<",Yj="<<YNj<<",M="<<lEMa
          <<G4endl;
    G4cout<<"G4QTauNCS::GCS:s="<<shift<<",Jb="<<lastJ1[blast]<<",J="<<lastJ1[lastL]<<",b="
          <<blast<<G4endl;
#endif
  }
  else
...
#ifdef pdebug
    G4cout<<"G4QTauNucCS::GetCrossSec:S="<<lastSig<<",lE="<<lE<<",J1="
...
G4double G4QTauNuclearCrossSection::GetEquivalentPhotonEnergy()
{
...
#ifdef debug
  G4cout<<"G4QTauNuclCrossSec::GetEguPhotE:B="<<lastF<<",l="<<lastL<<",J1="<<lastJ1[lastL]
...
#ifdef debug
  G4cout<<"G4QTauNuclCrossSec::GetEquivalentPhotonEnergy: "<<ris<<",Y="<<Y[lastL]<<G4endl;
#endif
...
#ifdef pdebug
	G4cout<<"G4QTauNucCS::GetEfPhE:HighEnergy f="<<f<<",r="<<ris<<",lastH="<<lastH<<G4endl;
#endif
  ...
  if(phLE>lastLE)
  {
    G4cerr<<"***G4QTauNuclearCS::GetEquPhotE:N="<<lastN<<",Z="<<lastZ<<",lpE"<<phLE<<">leE"
...
G4double G4QTauNuclearCrossSection::SolveTheEquation(G4double f)
{
...
#ifdef pdebug
  G4cout<<"G4QTauNucCS::SolveTheEq:e="<<eps<<",f="<<f<<",z="<<z<<",p="<<p<<",lastG="<<lastG
        <<",x="<<x<<G4endl;
#endif
  ...
  for(G4int i=0; i<imax; i++)
  {
    ...
    x=x+d;
#ifdef pdebug
    G4cout<<"G4QTauNCS::SolveTheE:#"<<i<<",d="<<d<<",x="<<x<<",f="<<fx<<",df="<<df<<G4endl;
#endif
    if(x>=lastLE)
	{
      ...
      x=topLim;
      if(i)G4Exception("G4QTauNuclearCrossSec::SolveTheEq()","009",FatalException,"E>eE");
    }
    if(abs(d)<eps) break;
...
// Randomize Q2 for the scattered tau-lepton
G4double G4QTauNuclearCrossSection::GetEquivalentPhotonQ2(G4double nu)
{
  ...
  {
#ifdef edebug
    G4cerr<<"***G4QTauNucCS::GetEquPhQ2:Fy="<<Fy<<"+fr="<<fr<<" <0"<<",iar="<<iar<<G4endl;
#endif
    return 0.;
  }    
  ...
  {
#ifdef edebug
    G4cerr<<"***G4QTauNucCrossSec::GetEquPhQ2:Q2="<<Q2<<" < Q2min="<<Qi2<<G4endl;
#endif
    return Qi2;
  }  
  if(Q2>Qa2)
  {
#ifdef edebug
    G4cerr<<"***G4QTauNucCrossSec::GetEquPhQ2:Q2="<<Q2<<" > Q2max="<<Qi2<<G4endl;
#endif
    return Qa2;
  }  
  return Q2;
