...
//
//
// $Id: G4QIsotope.cc,v 1.1 2004-03-05 13:28:34 mkossov Exp $
...
G4QIsotope::G4QIsotope() 
{
 ...
 std::vector<std::pair<G4int,G4double>*>*c=new std::vector<std::pair<G4int,G4double>*>(n);
   ...
   natIsoCrosS.push_back(c);       // Fill prototypeCrossSec's (0) for theParticularIsotope
#ifdef pdebug
...
{
#ifdef debug
  G4cout<<"G4QIsotope::InitElement is called"<<G4endl;
#endif
  G4int I=abund->size();
  if(I<=0)
  {
