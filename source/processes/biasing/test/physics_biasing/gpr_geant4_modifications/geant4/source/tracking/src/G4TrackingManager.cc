...
//////////////////////////////////////
  : fpVProcessSteppingManager(0), fpUserTrackingAction(NULL), fpTrajectory(NULL),
    StoreTrajectory(0), verboseLevel(0), EventIsAborted(false)
{
  fpVProcessSteppingManager = new G4SteppingManager();
  
  // Stepping manager for biasing
  fpGPRSteppingManager = new G4GPRSteppingManager();
...
{
  delete messenger;
  delete fpVProcessSteppingManager;
  delete fpGPRSteppingManager;
...
////////////////////////////////////////////////////////////////
{
  ...
  G4cout<<"jane start step "<<fpTrack->GetDefinition()->GetParticleName()<<G4endl;
  ...
  if (0 != fpTrack->GetDefinition()->GetGPRManager()) {
    ...
    fpSteppingManager = fpVProcessSteppingManager;
  ...
  if (0 == fpTrack->GetDefinition()->GetGPRManager()) fpSteppingManager->GetProcessNumber();
