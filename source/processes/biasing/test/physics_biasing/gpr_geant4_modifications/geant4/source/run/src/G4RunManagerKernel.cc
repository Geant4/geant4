...
//
//
// $Id: G4RunManagerKernel.cc,v 1.1 2007-09-06 22:19:22 tinslay Exp $
...
G4RunManagerKernel::G4RunManagerKernel()
  :physicsList(0),physicsBiasing(0),currentWorld(0),
...
void G4RunManagerKernel::SetBiasing(G4VUserPhysicsBiasing* biasing)
{
  physicsBiasing = biasing;
}
  
void G4RunManagerKernel::InitializePhysics()
{
  G4cout<<"jane initialising physics "<<G4endl;
  G4StateManager*    stateManager = G4StateManager::GetStateManager();
  G4ApplicationState currentState = stateManager->GetCurrentState();
  if(!(currentState==G4State_Idle||currentState==G4State_PreInit))
  { 
  ...
  if (0 != physicsBiasing) physicsBiasing->Construct();
