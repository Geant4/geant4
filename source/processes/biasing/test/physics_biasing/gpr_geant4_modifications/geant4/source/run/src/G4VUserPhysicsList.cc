...
//
//
// $Id: G4VUserPhysicsList.cc,v 1.1 2007-09-14 16:46:42 tinslay Exp $
...
void G4VUserPhysicsList::BuildPhysicsTable(G4ParticleDefinition* particle)
{
  ...
  if(!particle->IsShortLived()) {
    ...
    if (0 != particle->GetGPRManager()) {
      particle->GetGPRManager()->Fire<G4GPRTriggerTypes::Initialisation::BuildPhysicsTable>(*particle);
    }
    else {
...
void G4VUserPhysicsList::PreparePhysicsTable(G4ParticleDefinition* particle)
{
  ...
  if(!particle->IsShortLived()) {
    ...
    if (0 != particle->GetGPRManager()) {
      particle->GetGPRManager()->Fire<G4GPRTriggerTypes::Initialisation::PreparePhysicsTable>(*particle);
    }
    else {
					      ...
					      G4bool          ascii)
{
  ...
  if (0 != particle->GetGPRManager()) {
    // jane fixme - bool return so need to get observer collection, iterate & check return value
     particle->GetGPRManager()->Fire<G4GPRTriggerTypes::Initialisation::RetrievePhysicsTable>(particle, directory, ascii);
    // jane fixme particle->GetGPRManager()->Fire<G4GPRTriggerTypes::Initialisation::BuildPhysicsTable>(*particle);
  }
  else {
    G4ProcessVector* pVector = (particle->GetProcessManager())->GetProcessList();
    for ( j=0; j < pVector->size(); ++j) {
	...
	//	if (verboseLevel>2){
	  ...
	  //	}
	//#endif
	  G4cout<<"jane failed, building physics table"<<G4endl;
	(*pVector)[j]->BuildPhysicsTable(*particle);
      }
    }
    for ( j=0; j < pVector->size(); ++j) {
