...
//
//
// $Id: exampleB02_para.cc,v 1.1 2007-05-30 17:42:55 ahoward Exp $
...
{  
  ...
  B02ImportanceDetectorConstruction *pdet = 
    new B02ImportanceDetectorConstruction("ParallelBiasingWorld");
  ...
  for (cell=1; cell<=18; cell++) {
    G4cout << " adding importance values " << cell << G4endl;
  ...
  G4cout << " creating B02store scorer " << G4endl;
  G4CellStoreScorer scorer(b02store); 
  G4cout << " created " << G4endl;


  G4GeometrySampler pgs(ghostWorld,
			"neutron");
  if(ifElectron) pgs.SetParticle("e-");
  G4cout << " created geometry sampler " << G4endl;

  pgs.SetParallel(true);
  G4cout << " set parallel " << G4endl;
  pgs.PrepareScoring(&scorer);
  G4cout << " prepared scorer " << G4endl;
  pgs.PrepareImportanceSampling(&aIstore, 0);
  G4cout << " prepared importance sampling " << G4endl;
  pgs.Configure();
  G4cout << " configured sampler " << G4endl;
