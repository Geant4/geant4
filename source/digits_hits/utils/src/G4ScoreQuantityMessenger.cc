...
G4ScoreQuantityMessenger::G4ScoreQuantityMessenger(G4ScoringManager* SManager) : fSMan(SManager)
{
  QuantityCommands();
  FilterCommands();
}
...
void G4ScoreQuantityMessenger::QuantityCommands()
{
  ...
  qPassCellFluxCmd->SetGuidance("[usage] /score/quantity/passageCellFlux qname unit");
  ...
  qTrackLengthCmd->SetGuidance("[usage] /score/quantity/trackLength qname wflag kflag vflag unit");
  ...
  qPassCellCurrCmd->SetGuidance("[usage] /score/quantity/passageCellCurrent qname wflag");
  ...
  qPassTrackLengthCmd->SetGuidance("[usage] /score/quantity/passageTrackLength qname wflag unit");
  ...
  qFlatSurfFluxCmd->SetGuidance("[usage] /score/quantity/flatSurfaceFlux qname dflag unit");
  ...
  qNofCollisionCmd->SetGuidance("[usage] /score/quantity/nOfCollision qname wflag");
  ...
  qTerminationCmd->SetGuidance("[usage] /score/quantity/nOfTerminatedTrack qname wflag");
  ...
  qMinKinEAtGeneCmd->SetGuidance("[usage] /score/quantity/minKinEAtGeneration qname unit");
...
void G4ScoreQuantityMessenger::SetNewValue(G4UIcommand* command, G4String newVal)
{
  ...
  else if (command == qCellFluxCmd)
  {
    if (CheckMeshPS(mesh, token[0], command))
    {
      ...
      {
        auto pps = new G4PSCellFluxForCylinder3D(token[0]);
  ...
  else if (command == qPassCellFluxCmd)
  {
    if (CheckMeshPS(mesh, token[0], command))
    {
      ...
      {
        auto pps = new G4PSPassageCellFluxForCylinder3D(token[0]);
        pps->SetCylinderSize(mesh->GetSize(), mesh->GetStartAngle(), mesh->GetAngleSpan());
        G4int nSeg[3];
        mesh->GetNumberOfSegments(nSeg);
        pps->SetNumberOfSegments(nSeg);
  ...
  else if (command == qdoseDepCmd)
  {
    if (CheckMeshPS(mesh, token[0], command))
    {
      ...
      {
        auto pps = new G4PSDoseDepositForCylinder3D(token[0]);
        pps->SetUnit(token[1]);
        pps->SetCylinderSize(mesh->GetSize(), mesh->GetStartAngle(), mesh->GetAngleSpan());
        G4int nSeg[3];
        mesh->GetNumberOfSegments(nSeg);
...
void G4ScoreQuantityMessenger::FillTokenVec(const G4String& newValues, G4TokenVec& token)
{
...
void G4ScoreQuantityMessenger::FParticleWithEnergyCommand(G4VScoringMesh* mesh, G4TokenVec& token)
{
  ...
  G4double ehigh = StoD(token[2]);
  G4double unitVal = G4UnitDefinition::GetValueOf(token[3]);
  auto filter = new G4SDParticleWithEnergyFilter(name, elow * unitVal, ehigh * unitVal);
...
G4bool G4ScoreQuantityMessenger::CheckMeshPS(G4VScoringMesh* mesh, const G4String& psname,
                                             G4UIcommand* command)
{
  ...
  G4ExceptionDescription ed;
  ed << "WARNING[" << qTouchCmd->GetCommandPath() << "] : Quantity name, \"" << psname
     << "\", is already existing.";
  command->CommandFailed(ed);
