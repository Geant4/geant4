...
// ********************************************************************
//
// $Id: G4EmExtraBertiniPhysics.cc,v 1.4 2010-06-02 17:21:29 vnivanch Exp $
// GEANT4 tag $Name: not supported by cvs2svn $
//
//---------------------------------------------------------------------------
//
// ClassName:   G4EmExtraBertiniPhysics
//
...
#include "G4EmExtraBertiniPhysics.hh"
...
G4EmExtraBertiniPhysics::G4EmExtraBertiniPhysics(G4int ver): 
  ...
  verbose(ver)
{
  theMessenger = new G4EmBertiniMessenger(this);
  SetPhysicsType(bEmExtra);
}

G4EmExtraBertiniPhysics::G4EmExtraBertiniPhysics(const G4String&): 
  G4VPhysicsConstructor("G4GammaLeptoNuclearPhys"), wasBuilt(false), gnActivated(false), 
  munActivated(false), synActivated(false), synchOn(false), gammNucOn(true), muNucOn(false), 
  theElectronSynch(0), thePositronSynch(0), theGNPhysics(0), muNucProcess(0), muNucModel(0),
  verbose(1)
...
G4EmExtraBertiniPhysics::~G4EmExtraBertiniPhysics()
{
  delete theMessenger;
...
void G4EmExtraBertiniPhysics::Synch(G4String & newState)
{
...
void G4EmExtraBertiniPhysics::GammaNuclear(G4String & newState)
{
  if(newState == "on" || newState == "ON") {
    gammNucOn = true;
...
void G4EmExtraBertiniPhysics::MuonNuclear(G4String & newState)
{
  if(newState == "on" || newState == "ON") {
    muNucOn = true;
...
void G4EmExtraBertiniPhysics::ConstructParticle()
{
  G4Gamma::Gamma();
  G4Electron::Electron();
  G4Positron::Positron();
...
void G4EmExtraBertiniPhysics::ConstructProcess()
{
  if(wasBuilt) return;
  wasBuilt = true;
...
void G4EmExtraBertiniPhysics::BuildMuonNuclear()
{
...
void G4EmExtraBertiniPhysics::BuildGammaNuclear()
{
...
void G4EmExtraBertiniPhysics::BuildSynch()
{
