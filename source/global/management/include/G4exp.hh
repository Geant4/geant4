...
inline double G4exp(double initial_x){
    ...
    double x = initial_x;
    double px=G4details::fpfloor(G4details::LOG2E * x +0.5);
    ...
    // Build 2^n in double.
    x *= G4details::uint642dp(( ((uint64_t)n) +1023)<<52);

    if (initial_x > G4details::EXP_LIMIT)
...
inline float G4expf(float initial_x) {
    ...
    float z = G4details::fpfloor( G4details::LOG2EF * x +0.5f ); /* floor() truncates toward -infinity. */
    ...
    z *=  G4details::uint322sp((n+0x7f)<<23);

    if (initial_x > G4details::MAXLOGF) z=std::numeric_limits<float>::infinity();
    if (initial_x < G4details::MINLOGF) z=0.f;
...
void G4expv(const uint32_t size, double const * __restrict__ iarray, double* __restrict__ oarray);
void expfv(const uint32_t size, float const * __restrict__ iarray, float* __restrict__ oarray);
void G4expfv(const uint32_t size, float const * __restrict__ iarray, float* __restrict__ oarray);
