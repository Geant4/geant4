...
inline double G4log(double x){
	...
	double fe;
	x = G4details::getMantExponent(x,fe);

	// blending
	x > G4details::SQRTH? fe+=1. : x+=x ;
	x -= 1.0;

	/* rational form */
	double px =  G4details::get_log_px(x);
	...
	const double qx = G4details::get_log_qx(x);
	...
	if (original_x > G4details::LOG_UPPER_LIMIT)
		res = std::numeric_limits<double>::infinity();
	if (original_x < G4details::LOG_LOWER_LIMIT) // THIS IS NAN!
...
inline float G4logf( float x ) {
	...
	float fe;
	x = G4details::getMantExponentf( x, fe);

	x > G4details::SQRTHF? fe+=1.f : x+=x ;
	x -= 1.0f;

	const float x2 = x*x;

	float res = G4details::get_log_poly(x);
	res *= x2*x;
	...
	if (original_x > G4details::LOGF_UPPER_LIMIT)
		res = std::numeric_limits<float>::infinity();
	if (original_x < G4details::LOGF_LOWER_LIMIT)
