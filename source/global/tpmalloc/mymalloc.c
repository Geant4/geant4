...
static __thread int dev_zero_fd = -1; /* Cached file descriptor for /dev/zero. */
...
static __thread MLOCK_T malloc_global_mutex = { 0, 0, 0};
...
static __thread MLOCK_T malloc_global_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static __thread MLOCK_T malloc_global_mutex;
static __thread volatile long malloc_global_mutex_status;
...
static __thread struct malloc_params mparams;
...
static __thread struct malloc_state _gm_;
#define gm                 (&_gm_)
...
mspace create_mspace(size_t capacity, int locked) {
  ...
  if (capacity < (size_t) -(msize + TOP_FOOT_SIZE + mparams.page_size)) {
    printf("handle capacity paramenter: %d < %d\n", capacity, (size_t) -(msize + TOP_FOOT_SIZE + mparams.page_size));
