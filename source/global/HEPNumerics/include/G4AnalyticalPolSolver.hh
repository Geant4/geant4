...
// Class description:
//
// G4AnalyticalPolSolver allows the user to solve analytically a polynomial equation
// with the order up to 4. This is used by CSG solid tracking functions like G4Torus.
//
// CACM Algorithm 326
//
...
// Assume p[0] != 0. (< or > 0) (overflows otherwise)
//
// History:
//
// 13.05.05 V. Grichine: first implementation in terms of C++ class 
...
inline G4int G4AnalyticalPolSolver::QUADROOTS( G4double p[5], G4double r[3][5] )
{
  G4double b, c, d, tmp;
...
inline G4int G4AnalyticalPolSolver::CUBICROOTS( G4double p[5], G4double r[3][5] )
{
  G4double s,t,b,c,d;
  G4int k;
...
inline G4int G4AnalyticalPolSolver::BIQUADROOTS( G4double p[5], G4double r[3][5])
{
  G4double a, b, c, d, e;
  G4int k, j;
