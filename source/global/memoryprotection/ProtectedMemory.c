...
static void *getPageNumber(void *theAddress) {return (void *) (((long) theAddress >> logpagesize) << logpagesize);}

static __thread  void *trace[100];
...
void BuildProtectedMemory(size_t totalHeap)
{
  ...
  do {
    ...
    if (writable ==  1)
    {
      ...
      if (strstr(filename2, "ld") > 0)
      {
        ...
        printf("Not protected: 0x%lx 0x%lx %d %s\n", start, end, writable, filename2);
      }
      ...
      else if (strstr(filename2, "heap") > 0)
      {
        ...
        if (Heapcount == 1)
        {
          printf("%d Protected: 0x%lx 0x%lx %d %s\n", numOfSegments, start, end, writable, filename2);
  ...
  //remove the last one, it should in the ld library
  //  numOfSegments--;
  //  SetUpAllProtect();
