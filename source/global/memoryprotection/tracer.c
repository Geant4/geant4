...
__thread struct user_regs_struct regs;
__thread struct user_regs_struct regssinglestep;
__thread int detectionFlag = 0;
...
void FetchMap(pid_t pid)
{
  ...
  {
    printf("Maps: 0x%lx 0x%lx %s", memorySegments[i].start, memorySegments[i].end, libname[i]);
  }
}
...
static __thread int isMain = 1;

static int my_waitpid(pid_t pid, int *status, int flags)
{
  int ret;
  do
    {
