...
G4GenericMessenger::G4GenericMessenger(void* obj, const G4String& dir, const G4String& doc)
  : directory(dir), object(obj)
{
...
G4GenericMessenger::DeclareProperty(const G4String& name, const G4AnyType& var, const G4String& doc)
{
  ...
  if (var.TypeInfo() == typeid(G4ThreeVector)) {
    ...
    if (var.TypeInfo() == typeid(int) || var.TypeInfo() == typeid(long)
        || var.TypeInfo() == typeid(unsigned int) || var.TypeInfo() == typeid(unsigned long))
    {
      ptype = 'i';
    }
...
G4GenericMessenger::Command&
G4GenericMessenger::DeclarePropertyWithUnit(const G4String& name, const G4String& defaultUnit,
                                            const G4AnyType& var, const G4String& doc)
{
...
G4GenericMessenger::DeclareMethod(const G4String& name, const G4AnyMethod& fun, const G4String& doc)
{
  ...
  for (std::size_t i = 0; i < fun.NArg(); ++i) {
    ...
    if (tInfo == typeid(int) || tInfo == typeid(long) || tInfo == typeid(unsigned int)
...
G4String G4GenericMessenger::GetCurrentValue(G4UIcommand* command)
{
  ...
  if (methods.find(command->GetCommandName()) != methods.cend()) {
