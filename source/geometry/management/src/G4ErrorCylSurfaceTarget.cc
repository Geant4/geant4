...
//
//
// $Id: G4ErrorCylSurfaceTarget.cc,v 1.1 2007-05-16 12:50:52 gcosmo Exp $
                ...
                const G4ThreeVector& localDir ) const
{
  ...
  G4int inside = G4int(pow(-1,G4int(localPoint.perp() > fradius) ));
  G4double lambda;

  if( eqa*inside > 0. )
  {
    lambda = (-eqb + sqrt(eqb*eqb-4*eqa*eqc) ) / (2.*eqa);
  }
