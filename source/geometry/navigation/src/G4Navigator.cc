      ...
      if ( insideCode==kSurface )
      {
        ...
        if( isExiting )
        {
          ...
          else
          {
            fLastLocatedPointLocal = localPoint;
            fLocatedOutsideWorld = true;
            // No extra transformation for ExitNormal
            // is in frame of Top Volume
            return 0;          // Have exited world volume
          }
        }
        else
  ...
  if( fExiting )
  {
    ...
    {  
      ...
      if (  fHistory.GetTopVolumeType()!=kReplica )
      {
         ...
         // Do not set fValidExitNormal: signifies the solid is convex!
  ...
  if ( fLastTriedStepComputation ) 
  {
    ...
    { 
      ...
      if( candidateLogical ) 
      {
        ...
        {
          // The following code should also work in case of Replica (JA 15/10/12)
    ...
    if ( EnteredDaughterVolume() )
    {
       ...
       if( std::fabs(ExitNormal.mag2()-1.0 ) > CLHEP::perMillion )
       {
          ...
          desc << " Parameters of solid: " << *daughterSolid
               << " Point for surface = " << fLastLocatedPointLocal
               << std::endl;
          G4Exception("G4Navigator::GetLocalExitNormal()",
    ...
    else
    {
      ...
      { 
         *valid = false;
         ...
         edN3 << " Called when *NOT* at a Boundary." << G4endl;
         G4Exception("G4Navigator::GetLocalExitNormal()",
                     "GeomNav003", JustWarning, edN3); 
#endif
      }
    }
  }
                                       ...
                                       G4bool*        pNormalCalculated)
{
  ...
  else
  {
     ...
     *pNormalCalculated = fCalculatedExitNormal;
     ...
     if( !validNormal )
     {
        G4cerr << "G4Navigator::GetGlobaExitNormal: "
               << " Obtained non-valid Normal from LocalExitNormalAndCheck()"
               << G4endl;
     ...
     if( validNormal && (std::fabs(localMag2-1.0)) > perMillion )
     {
#ifdef G4DEBUG_NAVIGATION
        G4ExceptionDescription edN;

        edN << "Using Local Normal - from call to GetLocalExitNormalAndCheck()."
            << G4endl
        ...
        G4Exception("G4Navigator::GetGlobalExitNormal()", "GeomNav0002",
                    JustWarning, edN,
  ...
  if( fLastTriedStepComputation && fExiting)
  {
     ...
     *pNormalCalculated = fCalculatedExitNormal;
     ...
     {
        G4cerr << "Found difference in normals in case of exiting mother "
               << "- when Get is called after ComputingStep " << G4endl;
        G4cerr << "  Magnitude of diff =      " << diffNorm.mag() << G4endl;
        G4cerr << "  Normal stored (Global)     = " << fExitNormalGlobalFrame
               << G4endl;
