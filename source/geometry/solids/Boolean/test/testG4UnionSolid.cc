...
// NOTE: xRot = rotation such that x axis->-x axis & y axis->-y axis
    ...
    G4UnionSolid b1Ib2("b1Unionb2",&b1,&b2),
                        t1Ib2("t1Unionb2",&t1,&b2),
                        c2Ib2("c2Unionb2",&c2,&b2) ;

    // With placement

    G4UnionSolid t1Ib3("t1Unionb3",&t1,&b3,transform) ;


// Check Inside

    assert(b1Ib2.Inside(pzero)==kInside);
    assert(b1Ib2.Inside(pbigz)==kOutside);
    assert(b1Ib2.Inside(ponxside)==kSurface);
    assert(b1Ib2.Inside(ponyside)==kSurface);
    assert(b1Ib2.Inside(ponzside)==kSurface);

    assert(t1Ib2.Inside(pzero)==kInside);
    assert(t1Ib2.Inside(pbigz)==kOutside);
    assert(t1Ib2.Inside(ponb2x)==kInside);
    assert(t1Ib2.Inside(ponb2y)==kInside);
    assert(t1Ib2.Inside(ponb2z)==kInside);

    assert(c2Ib2.Inside(pzero)==kInside);
    assert(c2Ib2.Inside(pbigz)==kOutside);
    assert(c2Ib2.Inside(ponb2x)==kInside);
    assert(c2Ib2.Inside(ponb2y)==kInside);
    assert(c2Ib2.Inside(ponb2z)==kInside);

    // With placement

    assert(t1Ib3.Inside(pzero)==kInside);
    assert(t1Ib3.Inside(G4ThreeVector(0,60,0))==kSurface);
    assert(t1Ib3.Inside(G4ThreeVector(50,0,0))==kSurface);
    assert(t1Ib3.Inside(G4ThreeVector(0,65,0))==kOutside);
    assert(t1Ib3.Inside(G4ThreeVector(0,50,0))==kInside);
...
// Check Surface Normal
    ...
    normal=b1Ib2.SurfaceNormal(ponxside);
    assert(ApproxEqual(normal,G4ThreeVector(1,0,0)));

    normal=b1Ib2.SurfaceNormal(ponmxside);
    assert(ApproxEqual(normal,G4ThreeVector(-1,0,0)));

    normal=b1Ib2.SurfaceNormal(ponyside);
    assert(ApproxEqual(normal,G4ThreeVector(0,1,0)));

    normal=b1Ib2.SurfaceNormal(ponmyside);
    assert(ApproxEqual(normal,G4ThreeVector(0,-1,0)));

    normal=b1Ib2.SurfaceNormal(ponzside);
    assert(ApproxEqual(normal,G4ThreeVector(0,0,1)));

    normal=b1Ib2.SurfaceNormal(ponmzside);
    assert(ApproxEqual(normal,G4ThreeVector(0,0,-1)));

    // Wirh placemenet

    normal=t1Ib3.SurfaceNormal(G4ThreeVector(0,60,0));
    assert(ApproxEqual(normal,G4ThreeVector(0,1,0)));

    normal=t1Ib3.SurfaceNormal(G4ThreeVector(10,55,0));
    assert(ApproxEqual(normal,G4ThreeVector(1,0,0)));

    normal=t1Ib3.SurfaceNormal(G4ThreeVector(-10,55,0));
    assert(ApproxEqual(normal,G4ThreeVector(-1,0,0)));

    normal=t1Ib3.SurfaceNormal(G4ThreeVector(50.0/sqrt(2.0),50.0/sqrt(2.0),0));
...
// DistanceToOut(P)
    ...
    dist=t1Ib3.DistanceToOut(G4ThreeVector(0,55,0));
    assert(ApproxEqual(dist,5));
 
    dist=t1Ib3.DistanceToOut(G4ThreeVector(45,0,0));
    assert(ApproxEqual(dist,5));
 
    dist=t1Ib3.DistanceToOut(G4ThreeVector(0,45,0));
    assert(ApproxEqual(dist,5));
...
// DistanceToOut(P,V)
    ...
    dist=b1Ib2.DistanceToOut(pzero,vx,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,20)&&ApproxEqual(*pNorm,vx)&&*pgoodNorm);

    dist=b1Ib2.DistanceToOut(pzero,vmx,calcNorm,pgoodNorm,pNorm);
    ...
    dist=b1Ib2.DistanceToOut(pzero,vz,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,40)&&ApproxEqual(norm,vz)&&*pgoodNorm);

    dist=b1Ib2.DistanceToOut(pzero,vmz,calcNorm,pgoodNorm,pNorm);
...
//  assert(ApproxEqual(dist,0)&&ApproxEqual(*pNorm,vy)&&*pgoodNorm);
    ...
    dist=t1Ib3.DistanceToOut(G4ThreeVector(0,55,0),vmy,
                             calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,105)&&ApproxEqual(norm,vmy)&&*pgoodNorm);

    dist=t1Ib3.DistanceToOut(G4ThreeVector(0,55,0),vy,
                             calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,5)&&ApproxEqual(norm,vy)&&*pgoodNorm);

    dist=t1Ib3.DistanceToOut(G4ThreeVector(0,55,0),vx,
                             calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,10)&&ApproxEqual(norm,vx)&&*pgoodNorm);

    dist=t1Ib3.DistanceToOut(G4ThreeVector(0,55,0),vmx,
                             calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,10)&&ApproxEqual(norm,vmx)&&*pgoodNorm);

    dist=t1Ib3.DistanceToOut(G4ThreeVector(0,55,0),vz,
                             calcNorm,pgoodNorm,pNorm);
...
//DistanceToIn(P)
    ...
    cout<<"b1Ib2.DistanceToIn(pbigx) = "<<dist<<endl ;
    cout<<"b1.DistanceToIn(pbigx) = "<<b1.DistanceToIn(pbigx)<<endl ;
    cout<<"b2.DistanceToIn(pbigx) = "<<b2.DistanceToIn(pbigx)<<endl ;
    ...
    dist=t1Ib3.DistanceToIn(pbigmz);
    assert(ApproxEqual(dist,50));

    dist=t1Ib3.DistanceToIn(pbigy);
    assert(ApproxEqual(dist,40));

    dist=t1Ib3.DistanceToIn(G4ThreeVector(0,65,0));
    assert(ApproxEqual(dist,5));

    dist=t1Ib3.DistanceToIn(G4ThreeVector(0,60,0));
    assert(ApproxEqual(dist,0));
...
// DistanceToIn(P,V)
    ...
    dist=t1Ib3.DistanceToIn(pbigx,vmx);
    assert(ApproxEqual(dist,50));

    dist=t1Ib3.DistanceToIn(pbigy,vmy);
    assert(ApproxEqual(dist,40));

    dist=t1Ib3.DistanceToIn(pbigmy,vy);
    assert(ApproxEqual(dist,50));

    dist=t1Ib3.DistanceToIn(pbigmx,vx);
    assert(ApproxEqual(dist,50));
