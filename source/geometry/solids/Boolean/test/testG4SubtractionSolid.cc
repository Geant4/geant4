...
// NOTE: xRot = rotation such that x axis->-x axis & y axis->-y axis
    ...
    G4Tubs t2("Hole Tube #2",50,60,50,-M_PI/2,M_PI); 
 
    G4Tubs t3("Hole Tube #3",45,55,50,M_PI*3./8.,M_PI*5./4.);

    G4Cons c1("Hollow Full Tube",50,100,50,100,50,0,2*M_PI),

	   c2("Full Cone",0,50,0,100,50,0,2*M_PI) ;

    G4SubtractionSolid b1Ib2("b1Subtractionb2",&b1,&b2),

                       t1Ib2("t1Subtractionb2",&t1,&b2),

                       t2It3("t2Subtractiont3",&t2,&t3),

                       c2Ib2("c2Subtractionb2",&c2,&b2) ;

    // Tube t1 \ box t3, which was rotated by -pi and translated +30y

    G4SubtractionSolid   t1Ib3("t1Subtractionb3",&t1,&b3,transform) ;

// Check Inside

    assert(b1Ib2.Inside(pzero)==kOutside);
    assert(b1Ib2.Inside(pbigz)==kOutside);
    assert(b1Ib2.Inside(ponb2x)==kSurface);
    assert(b1Ib2.Inside(ponb2y)==kSurface);
    assert(b1Ib2.Inside(ponb2z)==kSurface);

    assert(t1Ib3.Inside(pzero)==kInside);
    assert(t1Ib3.Inside(pbigz)==kOutside);
    assert(t1Ib3.Inside(ponb2x)==kInside);
    assert(t1Ib3.Inside(ponb2y)==kSurface);
    assert(t1Ib3.Inside(ponb2z)==kInside);

    assert(c2Ib2.Inside(pzero)==kOutside);
    assert(c2Ib2.Inside(pbigz)==kOutside);
    assert(c2Ib2.Inside(ponb2x)==kSurface);
    assert(c2Ib2.Inside(ponb2y)==kSurface);
    assert(c2Ib2.Inside(ponb2z)==kSurface);

// Check Surface Normal

    G4ThreeVector normal;

    normal=b1Ib2.SurfaceNormal(ponb2x);
    assert(ApproxEqual(normal,G4ThreeVector(-1,0,0)));

    normal=b1Ib2.SurfaceNormal(ponb2mx);
    assert(ApproxEqual(normal,G4ThreeVector(1,0,0)));

    normal=b1Ib2.SurfaceNormal(ponb2y);
    assert(ApproxEqual(normal,G4ThreeVector(0,-1,0)));

    normal=b1Ib2.SurfaceNormal(ponb2my);
    assert(ApproxEqual(normal,G4ThreeVector(0,1,0)));

    normal=b1Ib2.SurfaceNormal(ponb2z);
    assert(ApproxEqual(normal,G4ThreeVector(0,0,-1)));

    normal=b1Ib2.SurfaceNormal(ponb2mz);
    assert(ApproxEqual(normal,G4ThreeVector(0,0,1)));

    normal=b1Ib2.SurfaceNormal(ponb2zy);
    assert(ApproxEqual(normal,G4ThreeVector(0,0,-1)));

    normal=b1Ib2.SurfaceNormal(ponb2mzy);
    assert(ApproxEqual(normal,G4ThreeVector(0,0,1)));


    normal=t1Ib3.SurfaceNormal(ponb2y);
    assert(ApproxEqual(normal,G4ThreeVector(0,1,0)));


// DistanceToOut(P)

    dist=b1Ib2.DistanceToOut(ponb2x);
    assert(ApproxEqual(dist,0));
    dist=b1Ib2.DistanceToOut(ponb2y);
    assert(ApproxEqual(dist,0));
    dist=b1Ib2.DistanceToOut(ponb2z);
    assert(ApproxEqual(dist,0));
    dist=b1Ib2.DistanceToOut(ponxside);
    assert(ApproxEqual(dist,0));

    dist=t1Ib3.DistanceToOut(ponb2y);
    assert(ApproxEqual(dist,0));

    dist=t1Ib3.DistanceToOut(pzero);
    assert(ApproxEqual(dist,10));

    dist=t1Ib3.DistanceToOut(ponb2x);
    assert(ApproxEqual(dist,10));

    dist=t1Ib3.DistanceToOut(ponb2z);
    assert(ApproxEqual(dist,10));


// DistanceToOut(P,V)

    dist=b1Ib2.DistanceToOut(ponb2x,vx,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,10)&&ApproxEqual(*pNorm,vx)&&*pgoodNorm);

    dist=b1Ib2.DistanceToOut(ponxside,vmx,calcNorm,pgoodNorm,pNorm);
    ...
    dist=b1Ib2.DistanceToOut(ponyside,vmy,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,20)&&ApproxEqual(norm,vmy)&&*pgoodNorm);

    dist=b1Ib2.DistanceToOut(ponb2z,vz,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,30)&&ApproxEqual(norm,vz)&&*pgoodNorm);

    dist=b1Ib2.DistanceToOut(ponzside,vmz,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,30)&&ApproxEqual(norm,vmz)&&*pgoodNorm);

    dist=b1Ib2.DistanceToOut(ponb2x,vxy,calcNorm,pgoodNorm,pNorm);
...
//  assert(ApproxEqual(dist,0)&&ApproxEqual(*pNorm,vy)&&*pgoodNorm);
    ...
    dist=b1Ib2.DistanceToOut(ponb2my,vmy,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,20)&&ApproxEqual(norm,vmy)&&*pgoodNorm);

    dist=b1Ib2.DistanceToOut(ponb2z,vz,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,30)&&ApproxEqual(norm,vz)&&*pgoodNorm);

    dist=b1Ib2.DistanceToOut(ponb2mz,vmz,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,30)&&ApproxEqual(norm,vmz)&&*pgoodNorm);

    // With placement

    dist=t1Ib3.DistanceToOut(pzero,vy,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,10)&&ApproxEqual(norm,vy)&&*pgoodNorm);

    dist=t1Ib3.DistanceToOut(pzero,vx,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,50)&&ApproxEqual(norm,vx)&&*pgoodNorm);

    dist=t1Ib3.DistanceToOut(pzero,vmy,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,50)&&ApproxEqual(norm,vmy)&&*pgoodNorm);

    dist=t1Ib3.DistanceToOut(pzero,vz,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(dist,50)&&ApproxEqual(norm,vz)&&*pgoodNorm);


//DistanceToIn(P)

    dist=b1Ib2.DistanceToIn(pbigx);
    assert(ApproxEqual(dist,80));
    dist=b1Ib2.DistanceToIn(ponb2x);
    assert(ApproxEqual(dist,0));
    dist=b1Ib2.DistanceToIn(ponxside);
    assert(ApproxEqual(dist,0));
    ...
    dist=t1Ib3.DistanceToIn(pbigy);
    assert(ApproxEqual(dist,50));

    dist=t1Ib3.DistanceToIn(pbigmy);
    assert(ApproxEqual(dist,50));

    dist=t1Ib3.DistanceToIn(G4ThreeVector(0,20,0));
    assert(ApproxEqual(dist,10));

    dist=t1Ib3.DistanceToIn(G4ThreeVector(0,15,0));
    assert(ApproxEqual(dist,5));

// DistanceToIn(P,V)

    dist=b1Ib2.DistanceToIn(pbigx,vmx);
    assert(ApproxEqual(dist,80));
    dist=b1Ib2.DistanceToIn(pbigmx,vx);
    assert(ApproxEqual(dist,80));
    ...
    dist=t2It3.DistanceToIn(G4ThreeVector(1,-50,0),vy) ;
    cout<<"t2It3.DistanceToIn(G4ThreeVector(1,-50,0),vy) = "<<dist<<endl ;

    dist=t2It3.DistanceToIn(G4ThreeVector(1,-61,0),vy) ;
    cout<<"t2It3.DistanceToIn(G4ThreeVector(1,-61,0),vy) = "<<dist<<endl ;

    // With placement

    dist=t1Ib3.DistanceToIn(G4ThreeVector(1,100,0),vmy);
    assert(ApproxEqual(dist,90));

    dist=t1Ib3.DistanceToIn(G4ThreeVector(0,36,0),vmy);
    assert(ApproxEqual(dist,26));

    dist=t1Ib3.DistanceToIn(G4ThreeVector(0,36,0),vx);
    assert(ApproxEqual(dist,10));
