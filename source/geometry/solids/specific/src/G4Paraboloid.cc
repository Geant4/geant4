...

{
  if( (pDz <= 0.) || (pR2 <= pR1) || (pR1 < 0.) )
  {
    ...
    G4Exception("G4Paraboloid::G4Paraboloid()", "InvalidArgument", 
...
G4ThreeVector G4Paraboloid::SurfaceNormal( const G4ThreeVector& p) const
{
  ...
  if(n.mag2() == 0)
  {
    ...
    message << "No normal defined for this point p." << G4endl
            << "          p = " << 1 / mm * p << " mm";
    G4Exception("G4Paraboloid::SurfaceNormal(p)", "InvalidNormal",
                                    ...
                                    const G4ThreeVector& v  ) const
{
  ...
  else
  {
    ...
    message << "          p = " << p * (1/mm) << " mm" << G4endl
            << "          v = " << v * (1/mm) << " mm";
    G4Exception("G4Paraboloid::DistanceToIn(p,v)", "InvalidSetup",
                JustWarning, G4String(message.str()));
    return 0;
  }
}
                                          ...
                                          G4ThreeVector *n  ) const
{
    ...
    && std::fabs(p.z()) < dz - kCarTolerance)
  {
    ...
    message << "There is no intersection between given line and solid!"
            ...
            << "          p = " << p << G4endl
            << "          v = " << v;
    G4Exception("G4Paraboloid::DistanceToOut(p,v,...)", "InvalidSetup",
                JustWarning, G4String(message.str()));
...
G4double G4Paraboloid::DistanceToOut(const G4ThreeVector& p) const
{
  ...
  if( Inside(p) == kOutside )
  {
     ...
     G4Exception("G4Paraboloid::DistanceToOut(p)", "InvalidSetup",
                 JustWarning, G4String(message.str()));
  }
#endif
