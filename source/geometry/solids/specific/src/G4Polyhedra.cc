...
//
//
// $Id: G4Polyhedra.cc,v 1.24 2005-08-08 10:38:20 danninos Exp $
...
//This code is incomplete yet, still being worked on by Dionysios Anninos
...
//  Get Point On Plane --> Auxiliary method for get point on surface
//
G4ThreeVector G4Polyhedra::GetPointOnTriangle(G4ThreeVector p1, G4ThreeVector p2,
...
G4ThreeVector G4Polyhedra::GetPointOnSurface() const
{
  ...
  G4double chose, totArea=0., Achose1, Achose2, rad1, rad2, sinphi1, sinphi2, cosphi1, cosphi2; 
  G4double a, b, l2, flag, ksi = (endPhi-startPhi)/(double)numSide,area, aTop=0., aBottom=0.,zVal=0.;
  ...
  for(j=0; j<numPlanes-1; j++)
  {
    ...
    l2 = sqr(original_parameters->Z_values[j]-original_parameters->Z_values[j+1])+sqr(b-a);
  ...
  for(j=0; j<numPlanes-1; j++)
  {
    if(phiIsOpen == true)
    {
      aVector3.push_back(0.5*(original_parameters->Rmax[j]-original_parameters->Rmin[j]+
			      original_parameters->Rmax[j+1]-original_parameters->Rmin[j+1])*
			 std::fabs(original_parameters->Z_values[j+1]-original_parameters->Z_values[j]));
    }
  ...
  {  
    ...
    flag = std::floor((chose-startPhi)/ksi-0.01);
    flag = std::fabs(flag);  
    sinphi1 = std::sin(startPhi+flag*ksi); sinphi2 = std::sin(startPhi+(flag+1)*ksi);
    cosphi1 = std::cos(startPhi+flag*ksi); cosphi2 = std::cos(startPhi+(flag+1)*ksi);
  ...
  else
  {
    for (j=0; j< numPlanes-1; j++)
    {
      ...
      Achose2 = Achose1 + numSide*(aVector1[j+1]+aVector2[j+1])+2.*aVector3[j+1];
    }
  }

  // at this point we have chosen a subsection between to adjacent plane cuts...	
  j = Flag; 
  ...
  else if (chose >= numSide*aVector1[j]  && chose<=numSide*(aVector1[j]+aVector2[j]))
  {
    chose = RandFlat::shoot(startPhi,endPhi);
    flag = std::floor((chose-startPhi)/ksi-0.01);
    flag = std::fabs(flag);
  ...
  else
  {
    ...
    if(chose>=0. && chose < 1.){ flag = startPhi;}
    else { flag = endPhi;} 
    
    cosphi1 = std::cos(flag); rad1 = original_parameters->Rmin[j];
    sinphi1 = std::sin(flag); rad2 = original_parameters->Rmax[j];
    
    p0 = G4ThreeVector(rad1*cosphi1,rad1*sinphi1,original_parameters->Z_values[j]);
    p1 = G4ThreeVector(rad2*cosphi1,rad2*sinphi1,original_parameters->Z_values[j]);
    
    rad1 = original_parameters->Rmax[j+1];
    rad2 = original_parameters->Rmin[j+1];
     
    p2 = G4ThreeVector(rad1*cosphi1,rad1*sinphi1,original_parameters->Z_values[j+1]);
    p3 = G4ThreeVector(rad2*cosphi1,rad2*sinphi1,original_parameters->Z_values[j+1]);
    
    return GetPointOnPlane(p0,p1,p2,p3);
  }
}

//    for(j=0; j<numPlanes-2; j++){
//     G4cout <<" Rmax "<<j<<" is" <<original_parameters->Rmax[j]<<
//              " and corners is "<<corners[numPlanes+j].r<<G4endl;
//     G4cout <<" Rmin "<<j<<" is" <<original_parameters->Rmin[j]<<
//              " and corners is "<<corners[numPlanes-1-j].r<<G4endl;
//     G4cout <<" Zvalue "<<j<<" is "<<original_parameters->Z_values[j]<<
//              " and corners is "<<corners[numPlanes+j].z<<G4endl;
//    }
