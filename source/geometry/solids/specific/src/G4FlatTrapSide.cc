...
//
//
// $Id: G4FlatTrapSide.cc,v 1.1 2004-10-07 07:33:55 link Exp $
  ...
  : G4VSurface(name)
{
   ...
   fXHalfLength = HalfSides[0] ;
   fYHalfLength = HalfSides[1] ;
   fPhiTwist = PhiTwist ;
   ...
   fAxis[0] = kXAxis ;
   fAxis[1] = kYAxis ;
   fAxisMin[0] = -fXHalfLength ;  // x-Axis
   fAxisMax[0] =  fXHalfLength ;
   fAxisMin[1] = -fYHalfLength ;
   fAxisMax[1] =  fYHalfLength ;
...
void G4FlatTrapSide::SetBoundaries()
{
  ...
  if (fAxis[0] == kXAxis && fAxis[1] == kYAxis) {
    ...
    // sAxis0 & sAxisMax
    direction = GetCorner(sCMax1Max) - GetCorner(sCMax1Min)  ; // inverse
    direction = direction.unit();
    SetBoundary(sAxis0 & (sAxisX | sAxisMax), direction, 
		GetCorner(sCMax1Min), sAxisY);
