...
//
//
// $Id: G4PolyPhiFace.cc,v 1.14 2008-05-14 15:39:29 tnikitin Exp $
                                    ...
                                    G4double phiOther )
{
  ...
  do
  {
    ...
    if(corn==corners){ corn->prev=corners+numEdges-1;}
    else corn->prev=helper;
    //add pointer on next corner
    if(corn<corners+numEdges-1){corn->next=corn+1;}
      else corn->next=corners;
    helper=corn;
...
//
// Auxilary Functions used for Finding the PointOnFace using Triangulation
//
    ...
    { corner_next=corner->next;
      ...
      if((corner!=a)&&(corner_next!=a)&&
        (corner!=b)&&(corner_next!=b)){
          ...
          rz1=G4TwoVector(a->r,a->z);rz2=G4TwoVector(b->r,b->z);
...
void G4PolyPhiFace::EarInit()
{
 ...
 G4PolyPhiFaceVertex *c_prev,*c_next;
  ...
  do{
    // We need to determine three consecutive verticis
     c_next=corner->next;
     c_prev=corner->prev; 
     //Calculation of ears
...
// !!!! have a list of triangles. This list is used for GetPointOnFace().
 ...
 // Make copy for prev/next for triang=corners
  ...
  do
  {
    ...
    triang->y= helper->y;
     //add pointer on prev corner
    if(helper==corners){ triang->prev=triangles+numEdges-1;}
    else triang->prev=helper2;
    //add pointer on next corner 
    if(helper<corners+numEdges-1){triang->next=triang+1;}
    else triang->next=triangles;
    helper2=triang;
    helper=helper->next;
 ...
 while(n>3){
   ...
   i++;
    if(i>=max_n_loops) G4Exception( "G4PolyPhiFace::Triangulation", "Bad Definition of Polycone",
                   FatalException, "Maximum number of steps is reached for triangulation" );
  ...
  //Third Step = Get Point On Choosen Surface
