...
// ********************************************************************
//
// $Id: G4SurfaceVoxelizer.cc 95301 2016-02-04 11:25:33Z gcosmo $
//
// --------------------------------------------------------------------
// GEANT 4 class header file
//
// G4SurfaceVoxelizer implementation
//
// History:
...
void G4SurfaceVoxelizer::BuildVoxelLimits(std::vector<G4VFacet *> &facets)
{
  ...
  if (G4int numNodes = facets.size()) // Number of nodes
  {
    ...
    G4ThreeVector toleranceVector;
    toleranceVector.set(10 * fTolerance,10 * fTolerance,10 * fTolerance);
...
//______________________________________________________________________________
void G4SurfaceVoxelizer::CreateSortedBoundary(std::vector<G4double> &boundary,
                                              G4int axis)
{
...
void G4SurfaceVoxelizer::BuildBoundaries()
{
  ...
  if (G4int numNodes = fBoxes.size())
  {
    ...
    vector<G4double> sortedBoundary(2*numNodes);
    ...
    for (G4int j = 0; j <= 2; ++j)
    {
      ...
      vector<G4double> &boundary = fBoundaries[j];
      boundary.clear();
...
//______________________________________________________________________________
void G4SurfaceVoxelizer::DisplayBoundaries(std::vector<G4double> &boundaries)
{
...
//______________________________________________________________________________
void G4SurfaceVoxelizer::BuildBitmasks(std::vector<G4double> boundaries[],
                                       G4SurfBits bitmasks[], G4bool countsOnly)
{
  ...
  {
    G4int total = 0;
    vector<G4double> &boundary = boundaries[k];
    ...
    vector<G4int> &candidatesCount = fCandidatesCounts[k];
...
{
  G4ThreeVector toleranceVector;
  toleranceVector.set(fTolerance/100,fTolerance/100,fTolerance/100);
  for (G4int i = 0; i <= 2; ++i)
  {
    G4double min = fBoundaries[i].front();
    G4double max = fBoundaries[i].back();
...
//______________________________________________________________________________
void G4SurfaceVoxelizer::BuildReduceVoxels(std::vector<G4double> boundaries[],
                                           G4ThreeVector reductionRatio)
{
  for (G4int k = 0; k <= 2; ++k)
  {
    ...
    vector<G4VoxelInfo> voxels(max);
    G4VoxelComparator comp(voxels);
    set<G4int, G4VoxelComparator> voxelSet(comp);
    vector<G4int> mergings;
    ...
    if (mergings.size())
    {
      ...
      vector<G4double> &boundary = boundaries[k];
      vector<G4double> reducedBoundary(boundary.size() - mergings.size());
...
//______________________________________________________________________________
void G4SurfaceVoxelizer::BuildReduceVoxels2(std::vector<G4double> boundaries[],
                                            G4ThreeVector reductionRatio)
{
  for (G4int k = 0; k <= 2; ++k)
  {
...
//______________________________________________________________________________
void G4SurfaceVoxelizer::CreateMiniVoxels(std::vector<G4double> boundaries[],
                                          G4SurfBits bitmasks[])
{
...
//______________________________________________________________________________
G4int
G4SurfaceVoxelizer::GetCandidatesVoxelArray(const std::vector<G4int> &voxels,
...
//______________________________________________________________________________
G4double
G4SurfaceVoxelizer::DistanceToFirst(const G4ThreeVector &point,
...
//______________________________________________________________________________
G4double
G4SurfaceVoxelizer::DistanceToBoundingBox(const G4ThreeVector &point) const
{
  G4ThreeVector pointShifted = point - fBoundingBoxCenter;
...
//______________________________________________________________________________
G4double
G4SurfaceVoxelizer::MinDistanceToBox (const G4ThreeVector &aPoint,
...
//______________________________________________________________________________
G4double
G4SurfaceVoxelizer::DistanceToNext(const G4ThreeVector &point,
...
//______________________________________________________________________________
G4bool
G4SurfaceVoxelizer::UpdateCurrentVoxel(const G4ThreeVector &point,
                                       const G4ThreeVector &direction,
...
//______________________________________________________________________________
void G4SurfaceVoxelizer::SetMaxVoxels(const G4ThreeVector &ratioOfReduction)
{
  fMaxVoxels = -1;
