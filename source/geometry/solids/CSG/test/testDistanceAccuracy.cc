...
G4ThreeVector GetVectorAroundTubs(G4Tubs& tubs)
{
  ...
  G4double pRmax = part*tubs.GetOuterRadius   ();
  G4double tubsZ = part*tubs.GetZHalfLength   ();
...
int test_one_solid ( Esolid useCase,  int num_points, int directions_per_point )
{
  ...
  switch (useCase)
  {
    ...
    for( i = 0; i < iMax; i++)
    {
      ...
      G4ThreeVector p = GetVectorAroundOrb(o1);    
      ...
      if(surfaceP != kSurface)
      {
        ...
        else
        {
          for( j = 0; j < jMax; j++ )
          {
            ...
            surfaceP = o1.Inside(p + distOut*v);
    ...
    for( i = 0; i < iMax; i++)
    {
      ...
      G4ThreeVector p = GetVectorAroundTubs(t4);    
      ...
      if(surfaceP != kSurface)
      {
        ...
        else
        {
          for( j = 0; j < jMax; j++ )
          {
            ...
            surfaceP = t4.Inside(p + distOut*v);
