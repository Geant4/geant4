...
//
//
// $Id: G4Sphere.cc,v 1.51 2005-08-03 16:00:37 danninos Exp $
...
G4ThreeVector G4Sphere::GetPointOnSurface() const
{
  ...
  slant1  = std::sqrt(sqr((fRmax - fRmin)*std::sin(fSTheta)) + height1*height1);
  slant2  = std::sqrt(sqr((fRmax - fRmin)*std::sin(fSTheta+fDTheta)) + height2*height2);
  ...
  if(chose>=0. && chose<aOne){
    return G4ThreeVector(fRmax*sintheta*cosphi,fRmax*sintheta*sinphi,fRmax*costheta);
  }
  else if(chose>=aOne && chose<aOne+aTwo){
    return G4ThreeVector(fRmin*sintheta*cosphi,fRmin*sintheta*sinphi,fRmin*costheta);
  }
    ...
    else{return G4ThreeVector(rRand*cosphi, rRand*sinphi, 0.);}    
  }
  else if(chose>=aOne+aTwo+aThr && chose < aOne+aTwo+aThr+aFou){
    if(fSTheta + fDTheta != 0.5*pi){
      zRand = RandFlat::shoot(fRmin*std::cos(fSTheta+fDTheta),fRmax*std::cos(fSTheta+fDTheta));
