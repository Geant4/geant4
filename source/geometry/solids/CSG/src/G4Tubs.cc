...
//
//
// $Id: G4Tubs.cc,v 1.67 2008-06-23 11:42:44 tnikitin Exp $
    ...
    else 
    {
      ...
      if (t3 > tolIRMin2 && t2 < 0 && std::fabs(p.z()) <= tolIDz)
      {
        ...
        if (seg)
        {
          ...
          // old version :
	  //  small negative tagent for the point on Surface not taken in account
          //  return 0.0 ;
          // new version : 
          //  check the tangent for the point on Surface and 
          //  if not intersection return Infinity, if intersection : return s
	    c=t3 - fRMax*fRMax; 
            if(c<=0.0)return 0.0;
            else{
             c=c/t1 ;
             d=b*b-c;
              if(d>=0.0){
	        snxt= c/(-b+std::sqrt(d));// using save solution for quadratic equation 
                if ( snxt<kCarTolerance*0.5 )  { snxt=0; }
                return snxt ;
              }      
