...
{
    if(pr_obj_errors > 3)
      G4cout << "Registry::ObjCreate() db ptr: " << db << G4endl;
	...
	if(pr_obj_errors > 3)
	{
	    G4cout << "Created entity: " << se->EntityName() << G4endl;
	    G4cout << "os_database::of(se) in Registry::ObjCreate(): "
	      << os_database::of(se) << G4endl;
	}
