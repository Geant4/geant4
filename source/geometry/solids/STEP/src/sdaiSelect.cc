...
#endif
#ifdef SCL_LOGGING
    *logStream << "Exiting SCLP23(Select) constructor." << G4endl;
#endif
}
...
{
  // changed cast from 'int' to 'long' -  GC
		     ...
		     ErrorDescriptor *err, InstMgr * instances)
{
	...
	switch (base_type)  {
	  ...
	  {  
	    STEPentity * tmp = 
	      ReadEntityRef (Val, err, (char*)(",)"), instances, 0);
...
#ifdef SCL_LOGGING
//    *logStream << "DAVE ERR Entering SCLP23(Select)::STEPread." << G4endl;
#endif
...
//  c = in.peek();
	...
	{   // Assign the value to the underlying type.  CanBeSet() is a
		...
		in.putback (c);
#ifdef SCL_LOGGING
//    *logStream << "DAVE ERR Exiting SCLP23(Select)::STEPread for " << _type->Name() << G4endl;
#endif
		return SEVERITY_WARNING;
	    }
#ifdef SCL_LOGGING
...
//    *logStream << "DAVE ERR Exiting SCLP23(Select)::STEPread for " << _type->Name() << G4endl;
	  ...
	  case '-':
	    // ****************************************************************
	    // The following check is commented out for compatibility with STEP
	    // entities with single attributes for aggregates. See definition
	    // for TRIMMED_CURVE. Part of ISO 10303-203 AIM EXPRESS schema - GC
	    // ****************************************************************
...
//	  err->GreaterSeverity( SEVERITY_INCOMPLETE );
	...
	{  // you don\'t know if this is an ENTITY or a SELECT
	    ...
	    STEPentity * tmp = 
		ReadEntityRef (in, err, (char*)(",)"), instances, addFileId);
