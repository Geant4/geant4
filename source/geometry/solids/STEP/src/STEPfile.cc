...
// and all its terms.
//
// $Id: STEPfile.cc,v 2.2 1998/07/13 16:53:54 urbi Exp $
...
/********************************************************************
 ...
 - ReadWorkingFile does not handle references to entities which
   ...
   it prints error messages to G4cerr which may not need to be printed. It 
...
STEPfile::ReadHeader (istream& in) 
{
	...
	{ //create and read the header instance 
	  // STEPentity::STEPread now reads the opening parenthesis
	  ...
	  //create header instance
		...
		G4cerr << "User defined entity in header section " <<
		...
		++_errorCount;
		SkipInstance(in, strbuf);
		G4cerr << "Unable to create header section entity: \'" <<
		  keywd << "\'.\n\tdata lost: " << strbuf << "\n";
		_error.GreaterSeverity(SEVERITY_WARNING);
	    }
...
// FILE_NAME <== (N279)FILE_IDENTIFICATION
    ...
    oldse = oldinst.GetSTEPentity("N279_File_Identification");
    if (oldse == ENTITY_NULL) 
      {
	  G4cerr << "Warning: STEPfile::HeaderConvertToNew. Unable to get " <<
...
// FILE_DESCRIPTION <== (N279)FILE_DESCRIPTION & (N279)IMP_LEVEL
   p21DIS_File_description * fd = new p21DIS_File_description;
    ...
    oldse = oldinst.GetSTEPentity("N279_File_Description");
    s_N279_file_description * ofd = (s_N279_file_description*)oldse;
    if (oldse != ENTITY_NULL) 
      {
//  DAVE, is there a new way TODO this?
	  // copy a STEPstring value into a StringAggregate
	  const char* tmpstr = ofd->description();
	  int len = strlen(tmpstr) + 2;	  
	  char *str = new char[len];
	  str[0]='\'';  str[1]='\0';
	  strncat(str,tmpstr,len);  
	  strncat(str,"\'",len);
    ...
    oldse = oldinst.GetSTEPentity("N279_Imp_Level");
...
//append any extra instances from oldinst onto imtmp
    for (int n = oldinst.InstanceCount() - 1; n>=0; --n) 
      {
	  imtmp->Append(oldinst.GetSTEPentity(oldinst[n]),completeSE);
      }
...
STEPfile::HeaderVerifyInstances(InstMgr* im)
{
    ...
    if (!(im->FindFileId(fileid))) 
      {
	  ++err;
	  G4cerr << "FILE_NAME instance not found in header section\n";
    ...
    if (!(im->FindFileId(fileid))) 
      {
	  ++err;
	  G4cerr << "FILE_DESCRIPTION instance not found in header section\n";
    ...
    if (!(im->FindFileId(fileid))) 
      {
	  ++err;
	  G4cerr << "FILE_SCHEMA instance not found in header section\n";
...
header section entites. The file id numbers are important.
  ...
  #2 = FILE_DESCRIPTION
  #3 = FILE_SCHEMA
  #4 = CLASSIFICATION (this entity is optional in the header section)
***************************/
void
STEPfile::HeaderMergeInstances(InstMgr* im)
{
    ...
    if(se = _headerInstances->GetSTEPentity(_headerInstances->FindFileId(idnum))) 
      {
	  from = im->GetSTEPentity(im->FindFileId(idnum));
...
STEPfile::ReadData1(istream& in)
{
	...
	if(!endsec)
	{
		...
		G4cerr << Error().UserMsg();
		G4cerr << Error().DetailMsg();
		Error().ClearErrorMsg();
...
// fixing above here
	  switch (c) 
		...
		return instance_count;
	      ...
	      default:
		G4cerr << "Error: in ReadWorkingData1. Unexpected input. \'" <<
		    c << "\'\n";
		endsec = 1;
		return instance_count;
	    }
...
*/
	...
	if(!endsec)
	{
		...
		obj =  ReadInstance(in, G4cout, cmtStr);
...
STEPfile::CreateInstance(istream& in, ostream &out) 
{
	...
	obj = CreateSubSuperInstance(in,fileid);
    }
...
STEPfile::CreateScopeInstances(istream& in, STEPentityH ** scopelist)
{
    ...
    char c;
    int exportid;
    STEPentityH inscope [BUFSIZ];  int i =0;
    SCLstring keywd;    
	...
	if (se != ENTITY_NULL) 
	...
	else 
	  {
	    ...
	    SkipInstance(in, tmpbuf);
	    G4cerr << "instance in SCOPE not created.\n\tdata lost: " 
		 << tmpbuf << "\n";
	    ++_errorCount;
	  }
    ...
    *scopelist = new STEPentityH [i];
	...
	SkipInstance(in, tmpbuf);
	G4cerr << "ERROR: " << "\'ENDSCOPE\' expected."
	     << "\n\tdata lost: " << tmpbuf << "\n";
	++_errorCount;
	return SEVERITY_INPUT_ERROR;
      }
...
STEPfile::ReadScopeInstances (istream& in)
{
    ...
    while (c == '#') 
      {
	  se = ReadInstance (in, G4cout, cmtStr);
	  if (se != ENTITY_NULL) 
	    {
    ...
    if (c == '/') 
	  ...
	  if (c != '/') 
		...
		//ERROR: '/' expected while reading export List
	      SkipInstance(in, tmpbuf);
	      G4cerr << " \'/\' expected while reading export List.\n\tdata lost: "
		   << tmpbuf << "\n";
	      ++_errorCount;
	      rval =  SEVERITY_WARNING;
	    }
...
STEPfile::ReadInstance(istream& in, ostream& out, SCLstring &cmtStr)
{
    ...
    if ((!node) || ((obj =  node -> GetSTEPentity ()) == ENTITY_NULL))
    {
	SkipInstance(in, tmpbuf);
...
//	  ReadSubSuperInstance(in);
	sev = obj->STEPread (fileid, idIncrNum, &instances (), in);
...
//	  return ENTITY_NULL;
	...
	case SEVERITY_INCOMPLETE:
	  if( (_fileType == VERSION_CURRENT) || (_fileType == VERSION_OLD) ) 
	    {
		G4cerr << "ERROR in EXCHANGE FILE: incomplete instance #" 
		     << obj -> STEPfile_id << ".\n";
		if(_fileType != WORKING_SESSION)
...
***************************/
Severity
STEPfile::WriteExchangeFile(const char* filename, int validate)
{
    Severity rval = SEVERITY_NULL;
...
{
/* // default initialization doesn't work DAS PORT
    char* nms[3] = { 
	"FILE_DESCRIPTION\0",
	"FILE_NAME\0",
	"FILE_SCHEMA\0",
...
*/
    const char* nms[3];
    nms[0] = "FILE_DESCRIPTION";
    nms[1] = "FILE_NAME";
    nms[2] = "FILE_SCHEMA";
//    nms[3] = 0; // Aaaaaaahhhhhhh!!
...
STEPfile::WriteHeader(ostream& out) 
{
    ...
    for (int i =0; i < n; ++i) 
      {
	  se = (*_headerInstances)[i]->GetSTEPentity();
	  if (!((se->FileId() == HeaderId("File_Name")) ||
	      (se->FileId() == HeaderId("File_Description")) ||
	      (se->FileId() == HeaderId("File_Schema"))))
	      WriteHeaderInstance((*_headerInstances)[i] -> GetSTEPentity(), out );
      }
    out << "ENDSEC;\n";
}
...
//	  (p21DIS_File_name*)se = HeaderDefaultFileName();
	  se = (STEPentity *)HeaderDefaultFileName();
      }
...
/**************************************************
  DAVE, should this section be deleted now?
	...
	// ERROR: no File_Name instance in _headerInstances
	// create a File_Name instance
	(STEPentity*)fn = HeaderDefaultFileName();
    }
    else 
      {
...
// Write the values for the FileName instance to the ostream    
    ...
    // write name
    //out << "\'" << (char*)fn->name() << "\',";
    STEPstring * s = new STEPstring((char*)fn->name());
    s->STEPwrite(out);
    delete s;
    out << ",";
    ...
    s = new STEPstring((char*)fn->preprocessor_version());
    s->STEPwrite(out);
    delete s;
    out << ",";
    
    // write originating_system
    s = new STEPstring((char*)fn->originating_system());
    s->STEPwrite(out);
    delete s;
    out << ",";
    
    // write authorization
    s = new STEPstring((char*)fn->authorization());
    s->STEPwrite(out);
    delete s;
    out << ");\n";
...
//	  (s_File_Description*)se = HeaderDefaultFileDescription();
	  se = (STEPentity*)HeaderDefaultFileDescription();
      }
...
/**************************************************
   ...
   p21DIS_File_description * fd;
    ...
    if (!mn) 
	  ...
	  (STEPentity*)fd = HeaderDefaultFileDescription();
      }
    else 
      {
...
// Write the values for the FileDescription instance to the ostream    
    ...
    STEPstring *s = new STEPstring((char*)fd->implementation_level());
    s->STEPwrite(out);
    delete s;
    out << ");\n";
...
//	  (p21DIS_File_schema*)se = HeaderDefaultFileSchema();
	  se = (STEPentity*) HeaderDefaultFileSchema();
      }
...
/**************************************************    
    ...
    if (!mn) 
	  ...
	  (STEPentity*)fs = HeaderDefaultFileSchema();
      }
    else 
      {
...
STEPfile::WriteData(ostream& out)
{
    ...
    for (int i = 0; i < n; ++i) 
	instances ()[i] -> GetSTEPentity()->STEPwrite(out);
...
STEPfile::AppendFile (istream* in) 
{
    ...
    G4cout << "Reading Data from " << FileName () << "...\n";
...
/**************************************************
 description:
    This function sends messages to 'G4cerr'
    based on the values in the given ErrorDescriptor (e), which is
    supposed to be from a STEPentity object, being manipulated by one
