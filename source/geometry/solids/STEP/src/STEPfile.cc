...
STEPfile::ReadData1(G4std::istream& in)
{
	...
	while (x < instance_count)
	{
	    ...
	    G4cout << "os_database::of(STEPent): " << os_database::of(STEPent) 
	      << G4endl;
	    x++;
	}
...
*/
	...
	if(!endsec)
	{
		...
		obj =  ReadInstance(in, G4cout, cmtStr, useTechCor);
...
STEPfile::CreateInstance(G4std::istream& in, G4std::ostream &out) 
{
	...
	else 
	{
	    ...
	    if (pr_obj_errors > 3)
	    {
		G4cout << "STEPfile::CreateInstance() db ptr: " << db << G4endl;
	    }
	    ...
	    if (pr_obj_errors > 3)
	    {
		G4cout << "os_database::of(obj) in STEPfile::CreateInstance(): "
		  << os_database::of(obj) << G4endl;
		G4cout << "****" << G4endl;
	    }
#else
