...
// and all its terms.
//
// $Id: Tst17PhysicsListMessenger.cc,v 1.2 1999-12-15 14:54:56 gunter Exp $
...
Tst17PhysicsListMessenger::Tst17PhysicsListMessenger(Tst17PhysicsList * List)
:Tst17List(List)
{
...
Tst17PhysicsListMessenger::~Tst17PhysicsListMessenger()
{
...
void Tst17PhysicsListMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  if(command == cutGCmd)
    { Tst17List->SetGammaCut(cutGCmd->GetNewDoubleValue(newValue));}
  if(command == cutECmd)
    { Tst17List->SetElectronCut(eCmd->GetNewDoubleValue(newValue));}
  if(command == cutPCmd)
    { Tst17List->SetProtonCut(eCmd->GetNewDoubleValue(newValue));}
  if(command == eCmd)
    { Tst17List->SetCutsByEnergy(cutECmd->GetNewDoubleValue(newValue));}
  if(command == rCmd)
    { Tst17List->GetRange(rCmd->GetNewDoubleValue(newValue));}

  if(command == cutGLowLimCmd)
    { Tst17List->SetGammaLowLimit(cutGLowLimCmd->GetNewDoubleValue(newValue));}

  if(command == cutELowLimCmd)
    { Tst17List->SetElectronLowLimit(cutELowLimCmd->GetNewDoubleValue(newValue));}

  if(command == cutGELowLimCmd)
    { Tst17List->SetGELowLimit(cutGELowLimCmd->GetNewDoubleValue(newValue));}

  if(command == cutSecPhotCmd)
    { Tst17List->SetLowEnSecPhotCut(cutSecPhotCmd->GetNewDoubleValue(newValue));}

  if(command == cutSecElecCmd)
    { Tst17List->SetLowEnSecElecCut(cutSecElecCmd->GetNewDoubleValue(newValue));}
