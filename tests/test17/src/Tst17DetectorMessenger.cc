...
// and all its terms.
//
// $Id: Tst17DetectorMessenger.cc,v 1.1 1999-11-30 18:01:55 stesting Exp $
...
Tst17DetectorMessenger::Tst17DetectorMessenger(Tst17DetectorConstruction * Tst17Det)
:Tst17Detector(Tst17Det)
{ 
  Tst17detDir = new G4UIdirectory("/calor/");
  Tst17detDir->SetGuidance("Tst17 detector control.");
...
Tst17DetectorMessenger::~Tst17DetectorMessenger()
{
  delete AbsMaterCmd; 
  delete AbsThickCmd; 
  delete AbsRadCmd;  
...
void Tst17DetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{ 
  if( command == AbsMaterCmd )
   { Tst17Detector->SetAbsorberMaterial(newValue);}
   
  if( command == WorldMaterCmd )
   { Tst17Detector->SetWorldMaterial(newValue);}
   
  if( command == AbsThickCmd )
   { Tst17Detector->SetAbsorberThickness(AbsThickCmd->GetNewDoubleValue(newValue));}
   
  if( command == AbsRadCmd )
   { Tst17Detector->SetAbsorberRadius(AbsRadCmd->GetNewDoubleValue(newValue));}
   
  if( command == AbsZposCmd )
   { Tst17Detector->SetAbsorberZpos(AbsZposCmd->GetNewDoubleValue(newValue));}
   
  if( command == WorldZCmd )
   { Tst17Detector->SetWorldSizeZ(WorldZCmd->GetNewDoubleValue(newValue));}
   
  if( command == WorldRCmd )
   { Tst17Detector->SetWorldSizeR(WorldRCmd->GetNewDoubleValue(newValue));}
   
  if( command == UpdateCmd )
   { Tst17Detector->UpdateGeometry(); }

  if( command == MagFieldCmd )
   { Tst17Detector->SetMagField(MagFieldCmd->GetNewDoubleValue(newValue));}
