...
// and all its terms.
//
// $Id: Tst17PrimaryGeneratorMessenger.cc,v 1.2 1999-12-15 14:54:56 gunter Exp $
...
Tst17PrimaryGeneratorMessenger::Tst17PrimaryGeneratorMessenger(Tst17PrimaryGeneratorAction* Tst17Gun)
:Tst17Action(Tst17Gun)
{ 
...
Tst17PrimaryGeneratorMessenger::~Tst17PrimaryGeneratorMessenger()
{
  delete RndmCmd;
...
void Tst17PrimaryGeneratorMessenger::SetNewValue(G4UIcommand * command,G4String newValue)
{ 
  if( command == RndmCmd )
   { Tst17Action->SetRndmFlag(newValue);}
  if( command == setxvertexCmd)
   { Tst17Action->Setxvertex(setxvertexCmd->GetNewDoubleValue(newValue));}
  if( command == setyvertexCmd)
   { Tst17Action->Setyvertex(setyvertexCmd->GetNewDoubleValue(newValue));}
  if( command == setzvertexCmd)
   { Tst17Action->Setzvertex(setzvertexCmd->GetNewDoubleValue(newValue));}
}
