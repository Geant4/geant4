...
G4double YUzhi[100][7];                 // -2 -- 8 rapidity
...
//        G4double costcm = std::cos(fm.theta());
        ...
        G4int NxUzhi=int(Xplus/0.02);
        if(NxUzhi <  0) NxUzhi= 0;  
        if(NxUzhi > 49) NxUzhi=49;
...
// ----------------------------- Xplus distributions----------------------
    G4cout<< "******** Xplus distr ******* at Plab "<<Plab<<" Xin " << sigIn<< G4endl;
    hAx<<G4Version<<G4endl;
    hAx<<" Xplus  Prot  Neut  Pip Pim Kp Km Pbar"<< G4endl;

    for(G4int ii=0; ii <50; ii++)
    {for(G4int jj=0;jj < 7; jj++){XUzhi[ii][jj]*= 1./Ninelast/0.02;}} //*sigIn/12.;}}     
    ...
    { 
     Xplus+=0.02; 
     hAx<<Xplus<<" "; 
     for(G4int jj=0;jj < 7; jj++){hAx << XUzhi[ii][jj]<<" ";}
     hAx<<G4endl;
    }  
