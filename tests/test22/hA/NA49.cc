...
G4double XUzhi[100][7];                 // -1 -- +1 x=2 Pz/sqrt(S)
for(G4int ii=0; ii<100; ii++){for(G4int j=0;j<7;j++) XUzhi[ii][j]=0.;}
...
//      G4double costcm = std::cos(fm.theta());
        ...
        G4int NxUzhi=int((xF+1.)/0.02);
        if(NxUzhi <  0) NxUzhi= 0;  
        if(NxUzhi > 99) NxUzhi=99;
              
        G4int NyUzhi=int((rapidity+5.)/0.1); 
...
// ----------------------------- xF distributions----------------------
    ...
    hAx<<" xF  Prot  Neut  Pip Pim Kp Km Pbar"<< G4endl;
