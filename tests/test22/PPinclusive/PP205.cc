...
//-------------------------- Global histograms  -------------------------
std::ofstream PP205y("PP205y.dat",std::ios::out);
std::ofstream PP205x("PP205x.dat",std::ios::out);
std::ofstream PP205pt2("PP205pt2.dat",std::ios::out);
...
//      G4double PartWeight=1.;
        ...
        G4int NyUzhi=int(rapidity/0.1);  // For CMS
//      if(NyUzhi <  0) NyUzhi= 0;  
        if(NyUzhi > 99) NyUzhi=99;

        G4int NxUzhi=int(feynmanX/0.02);
...
//----------------------------- Rapidity distributions------------------// Uzhi ++++
    ...
    PP205y<<G4Version<<G4endl;
    PP205y<<"  Y            P            Neut         Pip          Pim          Kp"
         <<"          Km       Pbar "<< G4endl;

    for(G4int ii=0; ii <100; ii++)
    {for(G4int jj=0;jj < 7; jj++){YUzhi[ii][jj]*= 10./Ninelast*sigIn;};};

    G4double Yu=-0.05;  // For CMS
    for(G4int ii=0; ii <100; ii++)
    { 
     Yu+=0.1; 
     if(Yu < 6.) 
     {
      PP205y<<Yu<<" ";
      for(G4int jj=0;jj < 7; jj++){PP205y<<YUzhi[ii][jj]<<" ";}
      PP205y<<G4endl;
     }
    }
//
...
// ----------------------------- Xf distributions----------------------// Uzhi ++++
    ...
    PP205x<<G4Version<<G4endl;
    PP205x<<"  Xf           P          Neut            Pip          Pim          Kp"
        <<"          Km             Pbar"<< G4endl;
    ...
    { 
     ...
     {
      PP205x<<Xu<<" "; 
      for(G4int jj=0;jj < 7; jj++){PP205x << XfUzhi[ii][jj]<<" ";}
      PP205x<<G4endl;
     }
    }  
...
//----------------------------- Pt2 distributions----------------------// Uzhi ++++
    G4cout<< "******** Pt2 ******* at Plab "<<Plab<<" Xin " << sigIn<< G4endl;
     PP205pt2<<G4Version<<G4endl;
     PP205pt2<<"  Pt2          P          Neut            Pip          Pim          Kp"
           <<"          Km     Pbar"<< G4endl;
    ...
    { 
     Pt2u+=0.015; 
     PP205pt2<<Pt2u<<" ";                                    
     for(G4int jj=0;jj < 7; jj++){PP205pt2 << Pt2Uzhi[ii][jj]<<" ";}
     PP205pt2<<G4endl;
    }
//
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Uzhi 
