...
//-------------------------- Global histograms  -------------------------
std::ofstream PipPtopo("PipPtopo.dat",std::ios::out);
std::ofstream PipPchan("PipPchan.dat",std::ios::out);
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      for(G4int i=0; i<n; ++i)              // Loop over produced particles
      {
        ...
        if(verbose>=1) G4cout<<" Part  "<<i<<" "<<pname                             // if(verbose>=1) 
...
/*
G4int Nother=     NprotonBar+NneutronBar +
                  Npim+Npip+Npi0 + 
                  NKm+NKp+NK0s+NK0l + 
                  NLambda+NLambdaBar + 
                  NSigma0+NSigma0Bar +
...
G4int Nother=     Nproton+NprotonBar +
                  NneutronBar+
                  Npim+Npi0 + 
                  NKm+NKp+NK0s+NK0l + 
                  NLambda+NLambdaBar + 
                  ...
                  NSigmap+NSigmapBar +
                  NSigmam+NSigmamBar +
                  Neta+Neta_prime + Ngamma;
//G4cout<<"Nch Other Nneutron Npi0 "<<Ncharged<<' '<<Nother<<" "<<Nneutron<<" "<<Npi0<<G4endl;
if((Ncharged == 2) && (Nother == 0) && (Nneutron == 1) && (Npip ==2)) 
{Xs[Uzhi_run][4]++;} // G4cout<<" 4 "<<G4endl;}

      Nother=     NprotonBar +
                  Nneutron+NneutronBar+
                  Npim + 
                  NKm+NKp+NK0s+NK0l + 
                  NLambda+NLambdaBar + 
                  NSigma0+NSigma0Bar +
                  ...
                  Neta+Neta_prime + Ngamma;
//G4cout<<"Nch Other Nneutron Npi0 "<<Ncharged<<' '<<Nother<<" "<<Nneutron<<" "<<Npi0<<G4endl;
if((Ncharged == 2) && (Nother == 0) && (Nproton == 1) && (Npip ==1) && (Npi0 == 1)) 
{Xs[Uzhi_run][5]++;} // G4cout<<" 5 "<<G4endl;}
      ...
      Nother=     NprotonBar +
                  ...
                  NSigmam+NSigmamBar +
                  Neta+Neta_prime + Ngamma;
//G4cout<<"Nch Other Nneutron Npip Npim "<<Ncharged<<' '<<Nother<<" "<<Nneutron<<" "<<Npip<<" "<<Npim<<G4endl;
if((Ncharged == 4) && (Nother == 0) && (Nproton == 1) && (Npip == 2) && (Npim == 1)) 
...
//
                  ...
                  NSigmam+NSigmamBar +
                  Neta+Neta_prime + Ngamma;
//G4cout<<"Nch Other Nproton Npi0 Npim "<<Ncharged<<' '<<Nother<<" "<<Nproton<<" "<<Npi0<<" "<<Npim<<G4endl;
if((Ncharged == 4) && (Nother == 0) && (Nproton == 1) && (Npip == 2) && (Npim == 1) && (Npi0 == 1)) 
...
//G4cout<<"Nch Other NSigmap NKp "<<Ncharged<<' '<<Nother<<" "<<NSigmap<<" "<<NKp<<G4endl;
if((Ncharged == 2) && (Nother == 0) && (NSigmap == 1) && (NKp == 1)) 
{Xs[Uzhi_run][8]++;}// G4cout<<" 8 "<<G4endl;G4int Uzhi; G4cin>>Uzhi;}
...
//G4cout<<"Nch Other Nproton Npi0 Npim "<<Ncharged<<' '<<Nother<<" "<<Nproton<<" "<<Npi0<<" "<<Npim<<G4endl;
if((Ncharged == 2) && (Nother == 0) && (NLambda == 1) && (NKp == 1) && (Npip == 1) ) 
{Xs[Uzhi_run][9]++;} // G4cout<<" 7 "<<G4endl;}
...
//G4cout<<"Nch Other Nproton Npim "<<Ncharged<<' '<<Nother<<" "<<Nproton<<" "<<Npim<<G4endl;
if((Ncharged == 2) && (Nother == 0) && (Nproton == 1) && (Npip == 1)) 
{Xs[Uzhi_run][10]++; TopoUzhi[Uzhi_run][2]--;} // G4cout<<" 8 "<<G4endl;} Ntotal--; 
...
if(Ntotal != 0) 
{
 for(G4int ii=1;ii<16;ii++)  {TopoUzhi[Uzhi_run][ii]*=sigTot/Ntotal;}; // sigIn ->sigTot
      ...
      Xs[Uzhi_run][4]*=sigTot/Ntotal; //sigIn/Ntotal; 
...
PipPtopo<<G4Version<<G4endl;
PipPtopo<<" Plab   S0   S2   S4   S6   S8   S10"
...
for(G4int ii=0;ii<Uzhi_run;ii++) 
 ...
 PipPchan<<G4Version<<G4endl;  
PipPchan<<" Plab    Xtot    Xel    Xin     N2Pip    PPipPi0    P2pipPim  P2pipPimPi0  SpKp  LamKpPip  PipP"<< G4endl;

  for(G4int ii=0;ii<Uzhi_run;ii++) //--------------------------------- Uzhi
  {
   for(G4int jj=0;jj<11;jj++) {PipPchan<<" "<<Xs[ii][jj];};
    PipPchan<<G4endl;
  };
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//  delete pFrame;
