...
//-------------------------- Global histograms  -------------------------
std::ofstream PbA_1p22En("PbA_1p22En.dat",std::ios::out);

//G4double SqrtS;
//G4double Ycms;

G4double EnerUzhi[100][4];                 // 0 / 1200 MeV/c momentum
for(G4int ii=0; ii<100; ii++){for(G4int j=0;j<4;j++) EnerUzhi[ii][j]=0.;}
...
//      G4double Pmod=mom.mag();
        ...
        G4int IenerUz=int(e/4.); 
        if(IenerUz < 0) IenerUz=0; if(IenerUz > 99) IenerUz=99;

//+++++++++++++++++ For each particle in the event ++++++++++++++++++++++

	 if  (pname == "proton")     
         {
          if(( 37.5 < theta) && (theta <  62.5)) EnerUzhi[IenerUz][0]++; //  50 grad
          if((132.5 < theta) && (theta < 157.5)) EnerUzhi[IenerUz][2]++; // 145 grad
         };

	 if ( pname == "neutron" )  
         {
          if(( 37.5 < theta) && (theta <  62.5)) EnerUzhi[IenerUz][1]++; //  50 grad
          if((132.5 < theta) && (theta < 157.5)) EnerUzhi[IenerUz][3]++; // 145 grad
         };
...
// ----------------------------- Momentum distributions----------------------
    G4cout<< "******** Neutron Energy distr ******* at Plab "<<Plab<<" Xin " << sigIn<< G4endl;

    PbA_1p22En<<G4Version<<G4endl;
    PbA_1p22En<<" Energy    Prot50      Neut50     Prot145    Neut145"<<G4endl;

    G4double dOmega50 =2.*pi*(std::cos( 37.5*pi/180.)-std::cos( 62.5*pi/180.));   //  50 grad
    G4double dOmega145=2.*pi*(std::cos(132.5*pi/180.)-std::cos(157.5*pi/180.));   // 145 grad

    for(G4int ii=0; ii <100; ii++)
    {for(G4int jj=0;jj < 2; jj++){EnerUzhi[ii][jj]*=sigIn/Ninelast/4./dOmega50;}} //sigIn     

    for(G4int ii=0; ii <100; ii++)
    {for(G4int jj=2;jj < 4; jj++){EnerUzhi[ii][jj]*=sigIn/Ninelast/4./dOmega145;}} //sigIn
    ...
    { 
     Tkin+=4.; 
     PbA_1p22En<<Tkin<<"     "; 
     for(G4int jj=0;jj < 4; jj++){PbA_1p22En << EnerUzhi[ii][jj]<<"     ";}
     PbA_1p22En<<G4endl;
    }  
