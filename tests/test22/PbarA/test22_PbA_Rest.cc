...
for(G4int ii=0; ii<50; ii++){for(G4int j=0;j<2;j++) MomUzhi[ii][j]=0.;}
...
*/
         ...
         G4int ImomUz=int(e/4.); 
         if(ImomUz < 0) ImomUz=0; if(ImomUz > 49) ImomUz=49;
...
// ----------------------------- Momentum distributions----------------------
    ...
    PbARest<<G4Version<<G4endl;
    PbARest<<" Tkin     Prot      Neut "<< G4endl;

    for(G4int ii=0; ii <50; ii++)
    {for(G4int jj=0;jj < 2; jj++){MomUzhi[ii][jj]*=1./Ninelast/4.;}} //sigIn/12.     
    ...
    { 
     Tkin+=4.; 
     PbARest<<Tkin<<"    "; 
     for(G4int jj=0;jj < 2; jj++){PbARest << MomUzhi[ii][jj]<<"    ";}
     PbARest<<G4endl;
    }  
