...
G4VProcess* Test49Physics::GetProcess(const G4String& gen_name, const G4String& part_name,
		                              G4Material* mat, G4bool atrest, G4bool elweak)
{
  G4cout<<"Test49Physics::GetProcess's called Gen="<<gen_name<<",Proj="<<part_name<<G4endl;
  ...
  if(!man)
  {
    G4cout<<"Test49Physics::GetProcess: Projectile="<<part_name<<" is not found"<<G4endl;
    return 0;
  }
  ...
  if(elweak) // The ElectroWeek Process is selected for this RUN
  {
    ...
    {
      G4cout<<"Test49Physics::GetProcess: ElectroWeakDiscreteProcess isn't made"<<G4endl;
      exit(1);
  ...
  else       // The Hadronic Process is selected for this RUN
  {
    if(atrest)
    {
      G4cout<<"Test49Physics::GetProcess:HadronicAtRestProcess is not implemented"<<G4endl;
      exit(1);
    }
    else
    {
      theHDProcess = new Test49HadronProduction();
      G4cout<<"HadronicProcessPrototype is created gen="<<gen_name<<G4endl;

      // Definition of the Model from the Hadronic Collection
      Test49VSecondaryGenerator* sg = 0; //@@ Make similar for the capture at rest (?)

      // Choose the generator from the HadronicModelCollection (add more if necessary)
      if(gen_name == "LEparametrisation")
      {
        if     (part_name == "proton")
          sg = new Test49VSecondaryGenerator(new G4LEProtonInelastic(),mat);
        else if(part_name == "pi+")
          sg = new Test49VSecondaryGenerator(new G4LEPionPlusInelastic(),mat);
        else if(part_name == "pi-")
          sg = new Test49VSecondaryGenerator(new G4LEPionMinusInelastic(),mat);
        else if(part_name == "neutron")
          sg = new Test49VSecondaryGenerator(new G4LENeutronInelastic(),mat);
        theHDProcess->SetSecondaryGenerator(sg);
        man->AddDiscreteProcess(theHDProcess);
      }
      //else if(gen_name == "CHIPS")
      //{
      //  sg = new Test49QSecondaryGenerator(new G4QCaptureAtRest(),mat);
      //  theHDProcess->SetSecondaryGenerator(sg);
      //  man->AddDiscreteProcess(theHDProcess);
      //}
      else if(gen_name == "stringCHIPS")
      {
        sg = new Test49VSecondaryGenerator(new G4StringChipsInterface(),mat);
      ...
      else if(gen_name == "preCompound")
      {
        ...
        thePreCompound = pcm;
        sg = new Test49VSecondaryGenerator(pcm,mat);
        theHDProcess->SetSecondaryGenerator(sg);
        man->AddDiscreteProcess(theHDProcess);
      }
      else if(gen_name == "binary")
      {
        ...
        sg = new Test49VSecondaryGenerator(hkm, mat);
        theHDProcess->SetSecondaryGenerator(sg);
        man->AddDiscreteProcess(theHDProcess);
      ...
      else G4cout<<"Test49Physics::GetProcess: "<<gen_name<<" generator is unkown"<<G4endl;
    }
