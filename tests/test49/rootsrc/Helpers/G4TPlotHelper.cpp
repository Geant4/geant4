...
{
  if(fCanvas != 0) delete fCanvas;

  cout<<"G4TPlotHelper::PrepareCanvas: Start preparing canvas..." << endl;
...
{
  Int_t xy = static_cast<Int_t>(std::sqrt(number));
  if(xy*xy < number) ++xy;
...
//______________________________________________________________________________
TH1F* G4TPlotHelper::PrepareFrame(Double_t hxmin, Double_t hymin, Double_t hxmax,
                                  Double_t hymax,TString const& Title)
{
  cout<<"G4TPlotHelper::PrepareFrame: xmi="<<hxmin<<", xma="<<hxmax<<", ymi="<<hymin
      <<", yma="<<hymax<<", Title="<<Title<<endl;
  ...
  cout<<"G4TPlotHelper::PrepareFrame: Title is updated"<<endl;
  ...
  cout<<"G4TPlotHelper::PrepareFrame: Size is updated"<<endl;
...
void G4TPlotHelper::DrawAnglesLegend(G4TData* publication)
{
  ...
  Double_t y1 = 0.2;
  Double_t y2 = y1 + (0.05 * cuts.size());
  TLegend* legend = new TLegend(0.20, y1, 0.35, y2);
...
//______________________________________________________________________________
void G4TPlotHelper::DrawRightAxis(TH1F* frame, Double_t hxmax, Double_t hymin,
                                  Double_t  hymax)
{
  // Draw the axis on the right side
  TAxis*  yAxis = frame->GetYaxis();
  TGaxis* nAxis = new TGaxis(hxmax, hymin, hxmax , hymax, yAxis->GetXmin(),
                             yAxis->GetXmax(), yAxis->GetNdivisions(), "+LG");
...
{
  vector<Double_t> cuts = publication->GetCutValues();
  cout<<"G4TPlotHelper::PrepareCutEnumeration: Cuts"<<endl;
  TString result = "";
  for(UInt_t i = 0; i< cuts.size(); ++i)
  {
...
void G4TPlotHelper::SetTitlePosition(Int_t align , Float_t x , Float_t y )
{
  ...
  if(x) gStyle->SetTitleX(x);
  if(y) gStyle->SetTitleY(y);
}

//______________________________________________________________________________
TCanvas* G4TPlotHelper::GetCanvas()
{
  if(!fCanvas) return PrepareCanvas();
 return fCanvas;
}


//______________________________________________________________________________
G4TPlotHelper::~G4TPlotHelper () { if(gPlotHelper == this) gPlotHelper = 0; }
