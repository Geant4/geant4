...
void G4TModelParams::Load(const TString& pf)
{
  ...
  if (!asciiFile)
  {
    cout<<"***Warning>>>>G4TModelParams::Load: Error in opening file = "<<pf.Data()<<endl;
    return;
  }
  // process
...
void G4TModelParams::Save(const TString& pf )
{
  ...
  if(!out)
  {
    cout<<"G4TModelParams::Save: Cannot open output chips file="<<pf.Data()<<endl;
    return;
  }
  stringstream stream;
  stream << fData.temp    << ' ';
...
//______________________________________________________________________________
inline void G4TModelParams::Tokenize(const string& str, vector<string>& tokens,
