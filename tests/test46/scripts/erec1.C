...
Int_t npo[3] = {15, 15, 15};

Double_t e0[nd] = {.555,.583,.613,.627,.647,.658,.673,.677,.683,.686,.697,.767,.799,.808,.815,.826,-.865};
Double_t e1[nd] = {.555,.580,.622,.626,.647,.663,.669,.673,.680,.696,.696,.767,.795,.807,.816,.821,.825};
Double_t e2[nd] = {.567,.586,.623,.637,.649,.664,.677,.692,.703,.706,.728,.790,.819,.830,.843,-.859,-.862};
...
for(int i=0; i<3; i++) {
  if(i == 0) {
    gr[i] = new TGraphErrors(npo[i],ener,e0,err0,err0);
    //gr[i+3] = new TGraphErrors(nd,ener,e3,err0,err0);
  } else if(i == 1) { 
    gr[i] = new TGraphErrors(npo[i],ener,e1,err0,err0);
    //gr[i+3] = new TGraphErrors(nd,ener,e4,err0,err0);
  } else if(i == 2) { 
    gr[i] = new TGraphErrors(npo[i],ener,e2,err0,err0);
