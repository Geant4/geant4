...
Tst34MaterialManager::Tst34MaterialManager()
{
  initialize();
}

Tst34MaterialManager* Tst34MaterialManager::GetMaterialManager()
{
  if (!mpointer)
...
void Tst34MaterialManager::storeElement(G4String name,G4String symbol,
...
G4Element* Tst34MaterialManager::getElement(G4String symbol)
{
  if (elist.find(symbol)==elist.end())
  {
...
void Tst34MaterialManager::storeMaterial(G4String name, G4double z,
                                         G4double a, G4double d)
{
...
void Tst34MaterialManager::storeMaterial(G4String name,
                                         G4double d, G4int ncomponent)
{
  if (mlist.find(name)!=mlist.end())
  {
    G4cout << "attempt of redefining an existing material " << G4endl;
...
G4Material* Tst34MaterialManager::getMaterial(G4String name)
{
  if (mlist.find(name)==mlist.end())
  {
...
void Tst34MaterialManager::addMaterial(G4String n1,G4String n2,G4double fraction)
{
  G4Material *mat=getMaterial(n1);
  mat->AddMaterial(getMaterial(n2),fraction);
}

void Tst34MaterialManager::addElement(G4String n1,G4String n2,G4double fraction)
{
  G4Material *mat=getMaterial(n1);
  mat->AddElement(getElement(n2),fraction);
}

void Tst34MaterialManager::addElement(G4String n1,G4String n2,G4int natoms)
{
  G4Material *mat=getMaterial(n1);
  mat->AddElement(getElement(n2),natoms);
}

void Tst34MaterialManager::printMaterialTable()
{
  MaterialList::iterator it;
  for (it=mlist.begin();it!=mlist.end();it++) G4cout << (*it).second << G4endl;
}

void Tst34MaterialManager::printElementTable()
{
  ElementList::iterator it;
  for (it=elist.begin();it!=elist.end();it++) G4cout << (*it).second << G4endl;
}

void Tst34MaterialManager::initialize()
{
