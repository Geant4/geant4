...
:pPhysicsList(pPhys)
{
  ...
  thcut = new G4UIcmdWithADoubleAndUnit("/testem/phys/setTh",this);
  thcut->SetGuidance("Set displacement threshold for all.");
  thcut->SetParameterName("th",false);
  thcut->SetUnitCategory("Energy");
  thcut->SetRange("th>=0.0");
  thcut->AvailableForStates(G4State_PreInit,G4State_Idle);
   ...
   if( command == thcut)
    {
     ...
     G4double th = thcut->GetNewDoubleValue(newValue);
     pPhysicsList->SetEth(th);
