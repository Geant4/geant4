...
//
//
// $Id: TrackingAction.cc,v 1.1 2009-11-27 16:06:28 mkossov Exp $
...
void TrackingAction::PostUserTrackingAction(const G4Track* aTrack)
{
  ...
  if (charged && (transmit || reflect)) 
  {
    G4double x = std::fabs(direction.x());
    G4double tet = std::atan(std::fabs(direction.y())/x);
    histoManager->FillHisto(0, tet);
    tet = std::atan(std::fabs(direction.z())/x);
    histoManager->FillHisto(0, tet);
  }
  ...
  //  histoManager->FillHisto(1,theta,weight);
  //}
  ...
  //  histoManager->FillHisto(3, y);
  //  histoManager->FillHisto(3, z);
  //  histoManager->FillHisto(4, r);
  //}
  // 
  ...
  //if (id>0) histoManager->FillHisto(id, aTrack->GetKineticEnergy());
  ////energy leakage
  //if (notabsor) 
  //{
