...
//
//
// $Id: Tst38DetectorMessenger.cc,v 1.1 2006-05-03 07:09:59 mkossov Exp $
// GEANT4 tag $Name: not supported by cvs2svn $
//

#include "Tst38DetectorMessenger.hh"

Tst38DetectorMessenger::Tst38DetectorMessenger(Tst38DetectorConstruction* myDC)
                                                                         : myDetector(myDC)
{
  G4String defParam;
...
void Tst38DetectorMessenger::SetNewValue(G4UIcommand * command,G4String newValues)
{
  if(command == selMatCmd)  myDetector->SelectMaterial(newValues);
  return;
}
