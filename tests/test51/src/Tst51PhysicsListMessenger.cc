...
Tst51PhysicsListMessenger::Tst51PhysicsListMessenger(Tst51PhysicsList * List)
:Tst51List(List)
{
...
Tst51PhysicsListMessenger::~Tst51PhysicsListMessenger()
{  
  delete cutECmd;
...
void Tst51PhysicsListMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  if (command == physicsListCmd)
   { Tst51List->AddPhysicsList(newValue); }

 if (command == cutECmd)
   {Tst51List->SetParticleCut(cutECmd->GetNewDoubleValue(newValue)); }
}
