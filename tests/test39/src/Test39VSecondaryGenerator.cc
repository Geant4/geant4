...
Test39VSecondaryGenerator::Test39VSecondaryGenerator(G4HadronicInteraction* hadi,
                                                     G4Material* mat):
  hInteraction(hadi), material(mat)
{
  G4cout<<"Test39VSecondaryGenerator::Constructor: material="<<material->GetName()<<G4endl;
  generatorName = "";
  const G4ElementVector* elv = material->GetElementVector();
  G4int Z = (G4int)(((*elv)[0])->GetZ() + 0.5);
  G4int N = (G4int)(((*elv)[0])->GetN() + 0.5);
  G4cout<<"Test39VSecondaryGenerator::Constructor: Nucleus with N="<<N<<", Z="<<Z<<G4endl;
  targetNucleus.SetParameters((G4double)N, (G4double)Z);
  mass = targetNucleus.AtomicMass((G4double)N, (G4double)Z);
  G4cout<<"Test39VSecondaryGenerator::Constr: Mass(targetNucleus)(MeV)="<<mass/MeV<<G4endl;
  mass = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(Z, N);
  G4cout << "Test39VSecondaryGenerator::Construct: Mass(IonTable)(MeV)="<<mass/MeV<<G4endl;
}

Test39VSecondaryGenerator::~Test39VSecondaryGenerator()
{}

// This member function starts the Generator from the HadronicModelCollection
G4HadFinalState* Test39VSecondaryGenerator::Secondaries(const G4Track& track)
{
  ...
  {
    G4cout<<"**Test39VSecondaryGenerator::Secondaries:Not initialized Interaction"<<G4endl;
