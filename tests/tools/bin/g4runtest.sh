#!/usr/local/bin/bash
######################################
# Submit remote run sequentially (g4allsys NOT in background)
######################################
...
do
    ...
    command="rsh -n -l $account $host $g4sbr $devprod $debopt $tag $action $actarg1 $actarg2 $actarg3 $nonincremental"
    if [ X$host = X ]
    then
