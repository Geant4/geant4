...
#    print $line;
    ...
    if ( $line =~ m#Creating/replacing object files in .*\/(\w+\.a)\s*# && $flag_comp eq 1 && $flag_warn eq 1) {
	 $flag_comp = 0;
	 $flag_warn = 0;
	 open(TMP,">>./buffer2.txt");
	 close TMP;
    ...
    if ( $line =~ m#Creating/replacing object files in .*\/(\w+\.a)\s*# && $flag_comp eq 1 && $flag_warn eq 0) {
	 $flag_comp = 0;
	 system("rm ./buffer1.txt");
	 next;
     }
