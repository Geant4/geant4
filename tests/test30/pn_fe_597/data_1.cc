...
int main(int argc, char** argv)
{
  ...
  const int nmax = 200;
  char line[nmax]; 
  std::string line1, line2, word1, word2, word3, word4, word5;
  G4bool end = true;
  ...
  double an, e1, e2, e3, y1, y2, ct1, ct2, xs, de;
  double e0 = 0.0;
  double x =  -bin0;
  ...
  do {
    ...
    if(fin->eof() || line[0] == 'E' && line[1] == 'N' && line[2] == 'D') {
      ...
      (*fin) >> line2 >> word1 >> word2 >> word3 >> word4 >> word5;
      (*fin) >> line2 >> word1 >> word2 >> word3 >> word4 >> word5;

      ibin= 0;
      inum= 0;
      for(i=0; i<n; i++) {
        (*fin) >> e1 >> e2 >> e3 >> e3 >> x >> xs;
        x *= 1000.;
        if(1 < verbose) {
      ...
      cs.push_back(cross);
      if(0 < verbose) {
        (*fout_c) << "#####..Result.of.parcing..#######..E(MeV)=590 " 
