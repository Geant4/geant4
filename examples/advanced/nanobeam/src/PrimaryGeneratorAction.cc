...
if (emission==2)
{
  ...
  while (aR < 0)
   aR = G4RandGauss::shoot(0.10e-3 , 0.06e-3/2.35) * rad;
   // old =0.08e-3 displacement
...
if (shoot)
{
  ...
  G4cout << "-> Event= " << numEvent<< ": X0(mm)= " << x0/mm
         << " X0(mm) = " << y0/mm << " Z0(m) = " << z0/m  
         << " THETA(mrad)= " << theta/mrad << " PHI(mrad)= " << phi/mrad
         << " E0(MeV)= " << e0/MeV << G4endl;
...
{                                      //  returns position in micrometers
