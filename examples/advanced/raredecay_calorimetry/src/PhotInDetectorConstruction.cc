...
//
//
// $Id: PhotInDetectorConstruction.cc,v 1.1 2005-05-11 10:37:19 mkossov Exp $
 ...
 gapSolid(0),worldLogical(0),worldPhysical(0),serial(false)
{
  ...
  gapParam = new PhotInGapParameterisation;
  gapParam->SetNumberOfLayers(numberOfLayers);
  PhotInCalorimeterSD::SetNumberOfLayers(numberOfLayers);
  DefineMaterials();
  detectorMessenger = new PhotInDetectorMessenger(this);
}
...
void PhotInDetectorConstruction::DefineMaterials()
{ 
...
G4VPhysicalVolume* PhotInDetectorConstruction::Construct()
{
  ...
  for(i=0;i<3;i++)
  {
    G4VSensitiveDetector* calorSD = new PhotInCalorimeterSD(detName[i]);
...
G4String PhotInDetectorConstruction::GetAbsorberMaterial() const
...
G4String PhotInDetectorConstruction::GetGapMaterial() const
