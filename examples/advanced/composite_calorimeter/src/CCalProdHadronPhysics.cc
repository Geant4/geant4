...
void CCalProdHadronPhysics::ConstructProcess()
{
  ...
  G4CrossSectionDataStore * thePStore = theProtonInelastic.GetCrossSectionDataStore();
  thePStore->AddDataSet(&thePData);
  thePStore->AddDataSet(&thePAlData);
  thePStore->AddDataSet(&thePCData);
  thePStore->AddDataSet(&thePHData);
  thePStore->AddDataSet(&thePOData);
  thePStore->AddDataSet(&thePSiData);
 // anti-Proton
  ...
  G4CrossSectionDataStore * theNStore = theNeutronInelastic.GetCrossSectionDataStore();
  theNStore->AddDataSet(&theNData);
