...
void XrayFluoAnalysisManager::book()
{
  ...
  if (phaseSpaceFlag) 
    {   
      ...
      //FIXME: dummy histogram. Required because otherwise the output file 
      //(which contains only a ntuple) is deleted
      man->CreateH1("hDummy","Dummy histogram", 100,0.,10.); 
  }
  else {
    // Book histograms
      ...
      if(aStep->GetTrack()->GetNextVolume())
	{
	  ...
	  if(aStep->GetTrack()->GetNextVolume()->GetName() == "HPGeDetector")
		   ...
		   ->GetDefinition()) == G4Gamma::Definition() ) 
		{
		  ...
		  gammaAtTheDetPre = 
		    aStep->GetPreStepPoint()->GetKineticEnergy();
		  man->FillH1(6,gammaBornInSample/keV);
		}
	      else if ((aStep->GetTrack()->GetDynamicParticle()
			->GetDefinition() ) == G4Proton::Definition() ) 
		{
