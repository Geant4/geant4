...
UltraPrimaryGeneratorAction::UltraPrimaryGeneratorAction()
{
  ...
  particleGun->SetEnergyDisType("Mono");
  particleGun->SetMonoEnergy(3.0*eV);

  // SET POSITION DISTRIBUTION TYPE
  particleGun->SetPosDisType("Plane");
  particleGun->SetPosDisShape("Circle");
  particleGun->SetRadius(10.0*cm);
...
#define ULTRA_REFLECTION_USE
  ...
  particleGun->SetPosRot1(G4ThreeVector(1.,0.,0.));
  particleGun->SetPosRot2(G4ThreeVector(0.,0.,-1.));
  particleGun->SetCentreCoords(G4ThreeVector(0.0*cm,90.0*cm,150.0*cm));

#else
  particleGun->SetParticleMomentumDirection(G4ThreeVector(0.0,0.0,-1.0)) ;
  particleGun->SetCentreCoords(G4ThreeVector(0.0*cm,0.0*cm,150.0*cm));
...
void UltraPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if ( iEvent == 0 ){
    ...
    G4cout << particleGun->GetEnergyDisType() << " " ;
    G4cout << particleGun->GetPosDisType() << G4endl ;
