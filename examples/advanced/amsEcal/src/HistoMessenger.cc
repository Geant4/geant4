...
// ********************************************************************
//
// $Id: HistoMessenger.cc 68740 2013-04-05 09:56:39Z gcosmo $
//
...
{
  histoDir = new G4UIdirectory("/ams/histo/");
  histoDir->SetGuidance("histograms control");

  factoryCmd = new G4UIcmdWithAString("/ams/histo/setFileName",this);
  factoryCmd->SetGuidance("set name for the histograms file");

  typeCmd = new G4UIcmdWithAString("/ams/histo/setFileType",this);
  typeCmd->SetGuidance("set histograms file type: hbook, root, XML");
  typeCmd->SetCandidates("hbook root XML");

  optionCmd = new G4UIcmdWithAString("/ams/histo/setFileOption",this);
  optionCmd->SetGuidance("set option for the histograms file");
     
  histoCmd = new G4UIcommand("/ams/histo/setHisto",this);
  ...
  prhistoCmd = new G4UIcmdWithAnInteger("/ams/histo/printHisto",this);
  prhistoCmd->SetGuidance("print histo #id on ascii file");
  prhistoCmd->SetParameterName("id",false);
  prhistoCmd->SetRange("id>0");
    
  rmhistoCmd = new G4UIcmdWithAnInteger("/ams/histo/removeHisto",this);
  rmhistoCmd->SetGuidance("desactivate histo  #id");
  rmhistoCmd->SetParameterName("id",false);
  rmhistoCmd->SetRange("id>0");
    
  ntuplCmd = new G4UIcmdWithAnInteger("/ams/histo/setNtuple",this);
  ntuplCmd->SetGuidance("set nTuple  #id");
  ntuplCmd->SetParameterName("id",false);
  ntuplCmd->SetRange("id>0");  
}
...
void HistoMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
  ...
  if (command == ntuplCmd)
   { histoManager->SetNtuple(ntuplCmd->GetNewIntValue(newValues));}            
}
