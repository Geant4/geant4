...
ExN03SteppingAction::ExN03SteppingAction() : IDold(-1), IDout(-1)
{;}
...
void ExN03SteppingAction::initialize(G4int Nev) {
  EventNo = Nev;
  NTracks = 0;
  NSecondaries = 0;
...
G4double ExN03SteppingAction::GetOutOfWorldTracks(G4int i, G4int j){
  return OutOfWorldTracksData[i][j];
}

G4double ExN03SteppingAction::GetSecondaries(G4int i, G4int j){
  return Secondaries[i][j];
}

G4double ExN03SteppingAction::GetEdepFCAL(G4String FCAL) {
