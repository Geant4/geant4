...
G03ColorReader::G03ColorReader() : G4GDMLReadStructure() {}
...
void G03ColorReader::ExtensionRead(const xercesc::DOMElement* const extElement)
{
       ...
       iter = iter->getNextSibling())
  {
    ...
    if (tag == "color") {
      ...
      G4Exception("G03ColorReader::ExtensionRead()", "ReadError", FatalException, error_msg);
    }
  }
}
...
G4VisAttributes* G03ColorReader::GetVisAttribute(const G4String& ref)
{
  ...
  if (pos != fAttribs.end()) {
    ...
    G4Exception("G03ColorReader::GetVisAttribute()", "ReadError", FatalException, err_mess);
  }
  return col;
}
