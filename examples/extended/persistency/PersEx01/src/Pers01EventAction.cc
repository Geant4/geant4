...
//
//
// $Id: Pers01EventAction.cc,v 1.1 2002-12-04 02:44:26 morita Exp $
...
Pers01EventAction::Pers01EventAction()
:calorimeterCollID(-1),drawFlag("all"),printModulo(1),
 eventMessenger(0)
{
  eventMessenger = new Pers01EventActionMessenger(this);
}
...
void Pers01EventAction::BeginOfEventAction(const G4Event* evt)
{
...
void Pers01EventAction::EndOfEventAction(const G4Event* evt)
{
  G4int evtNb = evt->GetEventID();
  ...
  Pers01CalorHitsCollection* CHC = 0;
  G4int n_hit = 0;
  G4double totEAbs=0, totLAbs=0, totEGap=0, totLGap=0;
    
  if (HCE) CHC = (Pers01CalorHitsCollection*)(HCE->GetHC(calorimeterCollID));
  ...
  if (evtNb%printModulo == 0) {
    ...
    G4cout << "\n     " << n_hit
	   << " hits are stored in Pers01CalorHitsCollection." << G4endl;
  }
