...
ExN04CalorimeterSD::ExN04CalorimeterSD(G4String name)
  : G4VSensitiveDetector(name), fCalCollection(NULL), fnumberOfCellsInZ(20), fnumberOfCellsInPhi(48)
{
  G4String HCname;
...
{
  fCalCollection = new ExN04CalorimeterHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool ExN04CalorimeterSD::ProcessHits(G4Step* aStep, G4TouchableHistory* ROhist)
{
  ...
  if (fCellID[copyIDinZ][copyIDinPhi] == -1) {
    ExN04CalorimeterHit* calHit =
      new ExN04CalorimeterHit(physVol->GetLogicalVolume(), copyIDinZ, copyIDinPhi);
    calHit->SetEdep(edep);
