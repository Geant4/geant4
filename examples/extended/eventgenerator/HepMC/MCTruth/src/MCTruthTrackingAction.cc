...
MCTruthTrackingAction::MCTruthTrackingAction() : G4UserTrackingAction() {}
...
void MCTruthTrackingAction::PostUserTrackingAction(const G4Track* track)
{
  ...
  if (TrackToBeStored(track)) {
    MCTruthTrackInformation* mcinf = (MCTruthTrackInformation*)track->GetUserInformation();

    MCTruthManager::GetInstance()->AddParticle(
      fmom, prodpos, endpos, track->GetDefinition()->GetPDGEncoding(), track->GetTrackID(),
    ...
    for (unsigned int index = 0; index < childrens->size(); ++index) {
      ...
      //
      MCTruthTrackInformation* mcinf = (MCTruthTrackInformation*)tr->GetUserInformation();
...
G4bool MCTruthTrackingAction::TrackToBeStored(const G4Track* track)
{
  ...
  if (std::find(types.begin(), types.end(), track->GetDefinition()->GetPDGEncoding())
      != types.end())
    return true;
