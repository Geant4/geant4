...
G4VPhysicalVolume* F02DetectorConstruction::ConstructCalorimeter()
{
  ...
  fSolidAbsorber = new G4Tubs("Absorber", 0., fAbsorberRadius, fAbsorberThickness / 2., 0.0, twopi);
  ...
  fPhysiAbsorber = new G4PVPlacement(nullptr, G4ThreeVector(0., 0., fZAbsorber), "Absorber",
                                     fLogicAbsorber, fPhysiWorld, false, 0);
...
void F02DetectorConstruction::SetFieldValue(G4ThreeVector value)
{  
  fFieldVector = value;

  G4UniformElectricField* elField = nullptr;
  if (fFieldVector != G4ThreeVector(0.,0.,0.)) {
    elField = new G4UniformElectricField(fFieldVector);
  }
