...
F01CalorimeterSD::F01CalorimeterSD(G4String name, F01DetectorConstruction* det)
...
{
  fCalCollection = new F01CalorHitsCollection(SensitiveDetectorName, collectionName[0]);
  for (G4int j = 0; j < 1; j++) {
    fHitID[j] = -1;
  };
}
...
G4bool F01CalorimeterSD::ProcessHits(G4Step* step, G4TouchableHistory*)
{
  ...
  if (fHitID[number] == -1) {
    ...
    if (verboseLevel > 0) G4cout << " New Calorimeter Hit on F01: " << number << G4endl;
  }
  else {
    if (physVol == fDetector->GetAbsorber()) (*fCalCollection)[fHitID[number]]->AddAbs(edep, stepl);
    if (verboseLevel > 0) G4cout << " Energy added to F01: " << number << G4endl;
  }
  return true;
}
