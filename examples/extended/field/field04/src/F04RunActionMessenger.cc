...
F04RunActionMessenger::F04RunActionMessenger(F04RunAction* runAction) : fRunAction(runAction)
{
...
{
  if (command == fRndmSaveCmd) fRunAction->SetRndmFreq(fRndmSaveCmd->GetNewIntValue(newValue));
