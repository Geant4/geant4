...
#ifdef USE_ALTERNATIVE_FIELD_PARAMS
...
#ifdef USE_ALTERNATIVE_FIELD_PARAMS
  // Alternative FieldManager for low energy particles, 
  //    Changing  stepper (and thus chord finder)
  //              changing minimum step (by a factor of 10x -- to test)
  //    BUT       sharing the same field  (mandatory for now)

  // Alternate Stepper, sharing the same Equation of motion
  G4MagIntegratorStepper*
    // pAlternateStepper = new G4HelixImplicitEuler( pEquation ); 
    pAlternateStepper = new G4HelixExplicitEuler( pEquation ); 
  G4cout << "Alternative Stepper is " 
	 << "HelixExplicitEuler" << G4endl;
                // << "HelixImplicitEuler" << G4endl;

  G4double 
  newStepMinimum= 10.0 * fMinChordStep;

  G4MagInt_Driver*
  pAlternateMagIntDriver= new G4MagInt_Driver( newStepMinimum,
					       pAlternateStepper,
					       pAlternateStepper->GetNumberOfVariables() );
  pAlternateMagIntDriver->SetMaxNoSteps( 50 );  //  Else it is 250 / order

  G4ChordFinder* alterChordFinder; 
  alterChordFinder= new G4ChordFinder( pAlternateMagIntDriver );
  // The following would create a new equation of motion -- ok, but why!
  // alterChordFinder= new G4ChordFinder( (G4MagneticField *)&field, 
  //			                  10.0 * fMinChordStep,
  //                                      pAlternateStepper );

  // Alternative FieldManager for low energy particles, sharing the same field
  G4FieldManager* alterLowEnerFldMgr= 
      new G4FieldManager( &field, alterChordFinder, false ); 
  // OLDER: new G4FieldManager( (G4MagneticField *)&field ); // and line below 
  // alterLowEnerFldMgr->SetChordFinder( alterChordFinder );

  // Now create a new Field Manager, to take over for the old global One,
  //   which will simply become the primary Field Manager
  G4FieldManager* newGlobalFldMgr= 
    new NTSTFieldManager( &field, globalFieldManager, alterLowEnerFldMgr, 10.0*MeV); 

  // Finally set this Field Manager to the world volume, taking over ...
  transportMgr->SetFieldManager( newGlobalFldMgr );

  // End of USE_ALTERNATIVE_FIELD_PARAMS
#endif 
