...
//
// $Id: PhysicsListMessenger.cc 67994 2013-03-13 11:05:39Z gcosmo $
//
...
:fPhysicsList(pPhys)
{
  ...
  fPositCutCmd = new G4UIcmdWithADoubleAndUnit("/testem/phys/setPCut",this);  
...
{       
  ...
  if( command == fPositCutCmd )
   { fPhysicsList->SetCutForPositron(fPositCutCmd->GetNewDoubleValue(newValue));}
  ...
  if( command == fVerbCmd )
   { G4LossTableManager::Instance()->SetVerbose(fVerbCmd->GetNewIntValue(newValue));}
