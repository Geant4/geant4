...
void Dicom2PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if (!fEnvelopeBox) {
    G4LogicalVolume* envLV = G4LogicalVolumeStore::GetInstance()->GetVolume("phantomContainer");
  ...
  if (fEnvelopeVol) {
    ...
    G4Exception("Dicom2PrimaryGeneratorAction::GeneratePrimaries()", "DICOM20002", JustWarning,
                msg);
  }
  ...
  if (fEnvelopeBox) {
    fPosDelta =
      G4ThreeVector(2.0 * fEnvelopeBox->GetXHalfLength(), 2.0 * fEnvelopeBox->GetYHalfLength(),
    ...
    msg << "The gun will be place at the center.";
    G4Exception("Dicom2PrimaryGeneratorAction::GeneratePrimaries()", "DICOM20003", JustWarning,
                msg);
  }
