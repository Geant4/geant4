...
{
  fUserVolumeCmd =
                new G4UIcmdWithAString("/dicom/intersectWithUserVolume",this);
  G4String s1("Intersects a phantom with a user-defined volume and outputs ");
  G4String s2("the voxels that are totally inside the intersection as a new ");
  G4String s3("phantom file. It must have the parameters: POS_X ");
  G4String s4("POS_Y POS_Z ANG_X ANG_Y ANG_Z SOLID_TYPE SOLID_PARAM_1 ");
  G4String s5("(SOLID_PARAM_2 ...)");
  G4String ss = s1+s2+s3+s4+s5;
  fUserVolumeCmd->SetGuidance(ss);
...
{ 
  ...
  if (command == fUserVolumeCmd) {
    ...
    if( params.size() < 8 ) {
      G4String s1(" There must be at least 8 parameter: ");
      G4String s2("SOLID_TYPE POS_X POS_Y POS_Z ANG_X ANG_Y ANG_Z ");
      G4String s3("SOLID_PARAM_1 (SOLID_PARAM_2 ...)");
      G4String ss = s1 + s2 + s3;
    ...
    if( params.size() !=1 ) G4Exception("DicomIntersectVolume::SetNewValue",
                                        ...
                                        FatalErrorInArgument,
                                        G4String("Command: "
      + command->GetCommandPath() + "/" + command->GetCommandName() + 
      " " + newValues + "  needs 1 argument: VOLUME_NAME").c_str()); 
    ...
    theVolumeTransform = G4AffineTransform( pv->GetFrameRotation(),
  ...
  fout << -voxelHalfWidthX*nx+thePhantomTransform.NetTranslation().x()
       << " "  
       << voxelHalfWidthX*nx+thePhantomTransform.NetTranslation().x()
       << G4endl;
  fout << -voxelHalfWidthY*ny+thePhantomTransform.NetTranslation().y()
       << " " 
       << voxelHalfWidthY*ny+thePhantomTransform.NetTranslation().y()
       << G4endl;
  fout << -voxelHalfWidthZ*nz+thePhantomTransform.NetTranslation().z()
       << " " 
       << voxelHalfWidthZ*nz+thePhantomTransform.NetTranslation().z()
       << G4endl;
  ...
  for( G4int iz = 0; iz < nz; iz++ ){
    for( G4int iy = 0; iy < ny; iy++) {
      ...
      for(G4int ix = 0; ix < nx; ix++ ){
        ...
        if( bVoxelIsInside ) {
          if( !bPrevVoxelInside ) {
            G4String s1("Volume cannot intersect phantom in discontiguous ");
            G4String s2("voxels, please use other voxel");
            G4String ss = s1 + s2;
            G4Exception("DicomIntersectVolume::SetNewValue","",
                        FatalException,
                        ss);
...
{
  // take out position and rotation angles
                       ...
                       DicomIntersectVolume::GetPhantomParam(G4bool bMustExist)
{
  ...
  for( cite = pvs->begin(); cite != pvs->end(); cite++ ) {
    //    G4cout << " PV " << (*cite)->GetName() << " " 
    //           << (*cite)->GetTranslation() << G4endl;
...
std::vector<G4VPhysicalVolume*> 
     DicomIntersectVolume::GetPhysicalVolumes( const G4String& name,
                                               bool exists, G4int nVols )
{
  ...
  if( ial != std::string::npos ) {
    ...
    if( ial2 != std::string::npos ) {
      ...
      volcopy =
         G4UIcommand::ConvertToInt(name.substr(ial+1, name.length() ).c_str());
    }
  } else {
    volname = name;
    volcopy = -1;
  ...
  if( vvolu.size() == 0 ) {
    if(exists) {
      ...
      G4String s1("!!WARNING: DicomIntersectVolume::GetPhysicalVolumes: ");
      G4String s2("no physical volume found with name ");
      G4String ss = s1 + s2;
      G4cerr << ss << name << G4endl;
    }
  }
  ...
  if( nVols != -1 && G4int(vvolu.size()) != nVols ) {
    G4Exception("DicomIntersectVolume::GetLogicalVolumes:",
                ...
                ("Number of physical volumes " + 
                 ...
                 ", requesting " + 
                 G4UIcommand::ConvertToString(nVols)).c_str());
  }
...
std::vector<G4LogicalVolume*>
            DicomIntersectVolume::GetLogicalVolumes( const G4String& name,
                                                     bool exists, G4int nVols )
{
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
std::vector<G4String>
                 DicomIntersectVolume::GetWordsInString( const G4String& stemp)
{
  ...
  for( ii = 0; ii < siz; ii++ ){
    if(cstr[ii] == '\"' ){
      ...
      //                   << nQuotes << " lastIsBlank " << lastIsBlank 
      //                   << G4endl;
      if( nQuotes%2 == 0 ){
        if( !lastIsBlank && !lastIsQuote ) {
          ...
          //                 << wordlist[wordlist.size()-1] 
          //                 << " lastIsBlank " << lastIsBlank << G4endl;
        }
