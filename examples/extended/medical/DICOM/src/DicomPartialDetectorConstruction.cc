...
void DicomPartialDetectorConstruction::ReadPhantomDataFile(const G4String& fname)
{
 ...
 G4int nMaterials;
  ...
  G4cout << "DicomPartialDetectorConstruction::ReadPhantomData nVoxel X/Y/Z " << fNVoxelX << " " << fNVoxelY << " " << fNVoxelZ << G4endl;
  fin >> fOffsetX >> fDimX;
  ...
  G4cout << "DicomPartialDetectorConstruction::ReadPhantomData voxelDimX " << fDimX << " fOffsetX " << fOffsetX << G4endl;
  G4cout << "DicomPartialDetectorConstruction::ReadPhantomData voxelDimY " << fDimY << " fOffsetY " << fOffsetY << G4endl;
  G4cout << "DicomPartialDetectorConstruction::ReadPhantomData voxelDimZ " << fDimZ << " fOffsetZ " << fOffsetZ << G4endl;
  ...
  for( G4int iz = 0; iz < fNVoxelZ; iz++ ) {
    ...
    for( G4int iy = 0; iy < fNVoxelY; iy++ ) {
      ...
      //      fFilledIDs.insert(std::multimap<G4int,size_t>::value_type(ifxdiff+fNVoxels, ifxmin1));
      fFilledIDs.insert(std::pair<size_t,size_t>(ifxdiff+fNVoxels-1, ifxmin1));
      G4cout << "DicomPartialDetectorConstruction::ReadPhantomData insert FilledIDs " << ifxdiff+fNVoxels-1 << " min " << ifxmin1 << " N= " << fFilledIDs.size() << G4endl;
...
void DicomPartialDetectorConstruction::ReadVoxelDensitiesPartial( std::ifstream& fin )
{
  ...
  for( G4int iz = 0; iz < fNVoxelZ; iz++ ) {
    ...
    for( G4int iy = 0; iy < fNVoxelY; iy++ ) {
      ...
      for( G4int ix = 0; ix < fNVoxelX; ix++ ) {
        if( ix >= G4int(ifxmin1) && ix <= G4int(ifxmax1) ) {
          fin >> dens1;
          //        G4cout << ix << " " << iy << " " << iz << " filling fMateIDs " << copyNo << " = " <<  atoi(stemp.c_str())-1 << " " << stemp << G4endl;
        ...
        //--- store the minimum and maximum density for each material (just for printing)
          ...
          copyNo++;
        //        G4cout << ix << " " << iy << " " << iz << " filling fMateIDs " << copyNo << " = " << atoi(cid)-1 << G4endl;
  ...
  for( mppite= newMateDens.begin(); mppite != newMateDens.end(); mppite++ ){
    ...
    fPhantomMaterials.push_back( BuildMaterialWithChangingDensity( (*mppite).first.first, averdens, mateName ) );
  }
...
void DicomPartialDetectorConstruction::ConstructPhantom()
{
  ...
  if( OptimAxis == "kUndefined" ) {
    phantom_phys = new G4PVParameterised(voxelName,phantom_logic,fContainer_logic,
  ...
  //-  fPartialPhantomParam->BuildContainerSolid(cont_phys);
