...
int main(int argc, char** argv)
{
  ...
  if ((commandLine = parser->GetCommandIfActive("-mt"))) {
    ...
    G4cout << "===== neuron is started with " << runManager->GetNumberOfThreads()
  ...
  // calculation time
  timer->Stop();
  G4cout << " Calculation time                      = " << timer->GetRealElapsed() << " s \n"
         << G4endl;
...
void Parse(int& argc, char** argv)
{
  ...
  parser->AddCommand("-dnaliv", Command::OptionNotCompulsory, "Activate Livermore + DNAPhysics");
  ...
  parser->AddCommand("-swc", Command::WithOption, "Give a SWC file to simulation", "fileName.swc");

  parser->AddCommand("-network",
                     Command::WithOption,  // OptionNotCompulsory,
                     "Give a DAT file to simulation", "fileName.dat");
  ...
  parser->AddCommand("-out", Command::OptionNotCompulsory, "Output files", exec);
