...
SteppingAction::SteppingAction() : G4UserSteppingAction(), fpEventAction(0)
{
...
void SteppingAction::UserSteppingAction(const G4Step* theStep)
{
  if (theStep->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName() != "Transportation") {
    ...
    G4Region* currentRegion = theStep->GetPreStepPoint()
                                ->GetTouchableHandle()
                                ->GetVolume()
                                ->GetLogicalVolume()
                                ->GetRegion();
    ...
    if (edepStep > 0. && (fRegion == currentRegion)) {
      ...
      if ("Wrappede-_G4DNAIonisation" == processName || "e-_G4DNAIonisation" == processName) {
        // Get the flag/label
        ...
        fpEventAction->AddEdepEvent(edepStep * theStep->GetPreStepPoint()->GetWeight());
      }
    }
  }
}
