...
SteppingMessenger::SteppingMessenger(SteppingAction* step) : fSteppingAction(step)
{
  fStepDir = new G4UIdirectory("/step/");
  fStepDir->SetGuidance("step control");

  fKillCmd = new G4UIcmdWithAnInteger("/step/recordOnlyFirstStep", this);
  fKillCmd->SetGuidance(" Choice: 0=no kill; 1=kill track and secondaries after 1st step;");
  fKillCmd->SetParameterName("choice", true);
  fKillCmd->SetRange("choice>=0");
  fKillCmd->SetDefaultValue(0);
}
