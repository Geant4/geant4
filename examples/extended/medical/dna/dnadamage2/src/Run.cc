...
Run::Run() : G4Run()
{
  ...
  G4int CollectionSB = G4SDManager::GetSDMpointer()->GetCollectionID("mfDetector/StrandBreaks");
...
void Run::Merge(const G4Run* aRun)
{
  ...
  ScoreSpecies* masterSpeciesScorer = dynamic_cast<ScoreSpecies*>(this->fScorerRun);

  ScoreSpecies* localSpeciesScorer = dynamic_cast<ScoreSpecies*>(localRun->fScorerRun);

  masterSpeciesScorer->AbsorbResultsFromWorkerScorer(localSpeciesScorer);

  ScoreStrandBreaks* masterSBScorer = dynamic_cast<ScoreStrandBreaks*>(this->fStrandBreakRun);

  ScoreStrandBreaks* localSBScorer = dynamic_cast<ScoreStrandBreaks*>(localRun->fStrandBreakRun);
