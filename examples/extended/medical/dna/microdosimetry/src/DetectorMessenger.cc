  ...
  : G4UImessenger(), fpDetector(Det), fpPhysList(PL)
{
  ...
  fpMaterCmd->SetGuidance("Select material");
  ...
  fpPhysCmd->SetGuidance("Add Physics list");
  ...
  fpTrackingCutCmd->SetGuidance("Add ion tracking cut");
...
void DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  else if (command == fpPhysCmd)
    fpPhysList->AddPhysics(newValue);
