...
void DetectorConstruction::DefineMaterials()
{
  ...
  /*
   ...
   Both materials are created and can be selected from psp.mac
   */
  fpWaterMaterial = H2O;
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  fpLogicWorld = new G4LogicalVolume(solidWorld,  // its solid
  ...
  // Scorer is a sphere
  G4Orb* solidScorer = new G4Orb("Scorer",  // its name
                                 fScorerRadius); // its size

  fpLogicScorer = new G4LogicalVolume(solidScorer,  // its solid
                                      fpWaterMaterial,  // its material
                                      "Scorer");  // its name

  fpPhysiScorer = new G4PVPlacement(0,  // no rotation
                                    G4ThreeVector(),  // at (0,0,0)
                                    "Scorer",  // its name
                                    fpLogicScorer,  // its logical volume
  ...
  G4VisAttributes* scorerVisAtt = new G4VisAttributes(G4Colour(1.0, 0.0, 0.0));
  scorerVisAtt->SetVisibility(true);
  fpLogicScorer->SetVisAttributes(scorerVisAtt);
...
void DetectorConstruction::SetMaterial(const G4String& materialChoice)
{
  ...
  if (pttoMaterial) {
    ...
    if (fpLogicWorld) {
      fpLogicWorld->SetMaterial(fpWaterMaterial);
    }
...
void DetectorConstruction::SetScorerRadius(G4double value)
{
  fScorerRadius = value;
  G4RunManager::GetRunManager()->ReinitializeGeometry();
}
