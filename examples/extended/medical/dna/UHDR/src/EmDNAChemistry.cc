...
void EmDNAChemistry::ConstructProcess()
{
  ...
  while (iterator()) {
    ...
    if (moleculeDef == G4Hydrogen::Definition()) {
      ...
      //------------------------------------------------------------------
      // H + O2(B) -> HO2
      auto reactionData = new G4DNAMolecularReactionData(1.3e10 * (1e-3 * m3 / (mole * s)), H, O2);
    ...
    if (moleculeDef == G4Electron_aq::Definition()) {
      ...
      reactionData =
        new G4DNAMolecularReactionData(2.25e10 * (1e-3 * m3 / (mole * s)), e_aq,
    ...
    if (moleculeDef == G4ParticleTable::GetParticleTable()->FindParticle("OHm")) {
      auto scanvergerProcess = new G4DNAScavengerProcess("G4DNAScavengerProcess", *confinedBox);
      G4DNAMolecularReactionData* reactionData = nullptr;
      //------------------------------------------------------------------
    ...
    if (moleculeDef == G4ParticleTable::GetParticleTable()->FindParticle("HO_2m")) {
      auto scanvergerProcess = new G4DNAScavengerProcess("G4DNAScavengerProcess", *confinedBox);
      G4DNAMolecularReactionData* reactionData = nullptr;
      //------------------------------------------------------------------
    ...
    if (moleculeDef == G4Oxygen::Definition()) {
      ...
      reactionData =
        new G4DNAMolecularReactionData(4.78e10 * (1e-3 * m3 / (mole * s)), Om,
