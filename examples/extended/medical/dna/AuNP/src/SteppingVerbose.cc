...
// ********************************************************************
//
/// \file medical/dna/range/src/SteppingVerbose.cc
...
void SteppingVerbose::TrackingStarted()
{
  ...
  if (verboseLevel > 0) {
    ...
    G4cout << std::setw(5) << fTrack->GetCurrentStepNumber() << " " << std::setw(6)
           ...
           << fTrack->GetVolume()->GetName() << "   initStep" << G4endl;
  }
...
void SteppingVerbose::StepInfo()
{
  ...
  if (verboseLevel >= 1) {
    ...
    if (verboseLevel == 2) {
      ...
      G4int nbtrk = (G4int)(*secondary).size();
      if (nbtrk) {
        G4cout << "\n    :----- List of secondaries ----------------" << G4endl;
        G4cout.precision(4);
        for (G4int lp = 0; lp < nbtrk; lp++) {
          G4cout << "   " << std::setw(13) << (*secondary)[lp]->GetDefinition()->GetParticleName()
                 << ":  energy =" << std::setw(6)
