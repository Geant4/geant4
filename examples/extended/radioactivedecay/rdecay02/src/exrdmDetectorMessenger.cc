...
{ 
  ...
  fDetectThicknessCmd = new G4UIcmdWithADoubleAndUnit("/exrdm/det/setDetectorThickness",this);
  ...
  fDetectLengthCmd = new G4UIcmdWithADoubleAndUnit("/exrdm/det/setDetectorLength",this);
...
{ 
  if( command == fTargMatCmd )
    ...
    { fMyDetector->SetDetectorLength(fDetectLengthCmd->GetNewDoubleValue(newValue));}
  else if (command == fDetectThicknessCmd ) 
    { fMyDetector->SetDetectorThickness(fDetectThicknessCmd->GetNewDoubleValue(newValue));}
}
