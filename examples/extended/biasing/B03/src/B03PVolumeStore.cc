...
{
  B03SetGeometryCell::iterator it = fSetGeometryCell.find(cell);
  if (it != fSetGeometryCell.end()) {
    G4cout << "B03PVolumeStore::AddPVolume: cell already stored" << G4endl;
    return;
  }
...
const G4VPhysicalVolume* B03PVolumeStore::GetPVolume(const G4String& name) const
{
  ...
  if (!pvol) {
    G4cout << "B03PVolumeStore::GetPVolume: no physical volume named: " << name << ", found"
           << G4endl;
  }
  return pvol;
}
