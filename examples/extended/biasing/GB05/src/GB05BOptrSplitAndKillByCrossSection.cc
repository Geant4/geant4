...
GB05BOptrSplitAndKillByCrossSection::GB05BOptrSplitAndKillByCrossSection(G4String particleName,
                                                                         G4String name)
...
void GB05BOptrSplitAndKillByCrossSection::StartRun()
{
  ...
  if (fSetup) {
    ...
    if (sharedData) {
      for (size_t i = 0; i < fProcessesToEquipoise.size(); i++) {
        ...
        for (size_t j = 0; j < (sharedData->GetPhysicsBiasingProcessInterfaces()).size(); j++) {
          const G4BiasingProcessInterface* wrapperProcess =
            (sharedData->GetPhysicsBiasingProcessInterfaces())[j];
          if (fProcessesToEquipoise[i] == wrapperProcess->GetWrappedProcess()->GetProcessName()) {
        ...
        if (!processFound) {
          ...
          G4ExceptionDescription ed;
          ed << "Process `" << fProcessesToEquipoise[i] << "' not found for particle `"
             << particleName << "'" << G4endl;
          G4Exception("GB05BOptrSplitAndKillByCrossSection::StartRun(...)", "exGB05.02",
                      JustWarning, ed);
        }
      }
    }
  ...
  if (fProcessesToEquipoise.size() == 0 || fProcesses.size() == 0) {
    ...
    ed << "No processes to counterbalance for defined or found ! "
       << "Biasing will do nothing." << G4endl;
    G4Exception("GB05BOptrSplitAndKillByCrossSection::StartRun(...)", "exGB05.03", JustWarning, ed);
  }
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

G4VBiasingOperation* GB05BOptrSplitAndKillByCrossSection::ProposeNonPhysicsBiasingOperation(
