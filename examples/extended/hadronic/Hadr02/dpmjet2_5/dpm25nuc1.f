...
C-------------------------------------------------------------
      CALL DMINIT(NCASES,EPN,PPN,NCOUNT,IGLAUB)
C----------------------------------------------------------------
C----------------------------------------------------------------
...
C         IF ( TLEFT .LE. 3.*TMAX+TLIM )                       GO TO 190
      IF(NUMEV.LE.-5)THEN
          WRITE(6,'(A,4I5)')' KKINC call ',IIT,IITZ,IIP,IIPZ
      DO 7779 KK=1,NHKK
      WRITE(6,'(2I4,I6,4I4,5F10.2,2I3,I2,I4)')KK,ISTHKK(KK),IDHKK(KK),
     *JMOHKK(1,KK),JMOHKK(2,KK),JDAHKK(1,KK),JDAHKK(2,KK), 
     *(PHKK(LL,KK),LL=1,5),IDRES(KK),IDXRES(KK),NOBAM(KK),IDBAM(KK)
 7779 CONTINUE      
      ENDIF
  180   CONTINUE
C
...
c                      IPOMTA=0    PMERON TABLES FOR 20 ENERGIES
...
C
	WRITE(6,'(A,4E12.4)')' SAMPPT:ECM,PTCUT,SIGS,DSIGH',
     *	ECM,PTCUT,SIGS,DSIGH
        IF( ISAMPT.EQ.0 ) THEN
     ...
     * ,C,SIGSOF,SIGHAR,RMIN
        ELSEIF( ISAMPT.EQ.4)THEN
          ...
          IF (AAAA.LE.0.00001D0) THEN
            ...
            WRITE(6,5559)PTCUT,SIGSOF,SIGHAR
 5559       FORMAT(' SAMPPT:PTCUT,SIDSOF.SIGHATD:',3E12.3)
          ENDIF
          C=SIGHAR/AAAA
