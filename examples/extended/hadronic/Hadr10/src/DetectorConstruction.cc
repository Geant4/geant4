...
DetectorConstruction::DetectorConstruction()
  : G4VUserDetectorConstruction(),
    ...
    fLogicExperimentalHall(nullptr),
    fPhysExperimentalHall(nullptr),
    fLogicTargetLayer(nullptr),
    fPhysTargetLayer(nullptr),
    fFieldMgr(nullptr),
    fUniformMagField(nullptr),
    fDetectorMessenger(nullptr),
    fTargetInnerRadius(9.0 * mm),
    fTargetOuterRadius(11.0 * mm)  //***LOOKHERE*** Default values
{
...
G4VPhysicalVolume* DetectorConstruction::ConstructLayer()
{
  ...
  G4cout << G4endl << "DetectorConstruction::ConstructLayer() : " << G4endl
         ...
         << "\t Layer (cylinder) size : " << G4endl << "\t \t radii : " << fTargetInnerRadius
         << " , " << fTargetOuterRadius << " mm ;"
         << "\t \t length (along z) : " << 2.0 * halfLength << " mm ;" << G4endl << G4endl
         << G4endl;
...
void DetectorConstruction::SetTargetMaterial(const G4String name)
{
  ...
  if (!fTargetMaterial) {
    G4cout << G4endl << G4endl << "WARNING: the name of the material has not been recognized!"
           << G4endl << "     ===> the default  * G4_Be *  will be used." << G4endl << G4endl;
    //***LOOKHERE*** Default material
    fTargetMaterial = G4NistManager::Instance()->FindOrBuildMaterial("G4_Be");
  }
