...
BiasingOperation::BiasingOperation(G4String name) : G4VBiasingOperation(name)
{
  ...
  fProtonInelasticProcess = new G4HadronInelasticProcess("protonInelastic", G4Proton::Definition());
                                                            ...
                                                            const G4Step* step, G4bool&)
{
  if (track->GetParticleDefinition() == G4Proton::Definition()) {
    ...
    fProtonInelasticProcess->GetCrossSectionDataStore()->ComputeCrossSection(particle, material);
    return fProtonInelasticProcess->PostStepDoIt(*track, *step);
  }
  else if (track->GetParticleDefinition() == G4Neutron::Definition()) {
    auto particle = track->GetDynamicParticle();
    auto material = track->GetMaterial();
    fNeutronInelasticProcess->GetCrossSectionDataStore()->ComputeCrossSection(particle, material);
    return fNeutronInelasticProcess->PostStepDoIt(*track, *step);
  }
  else if (track->GetParticleDefinition() == G4PionPlus::Definition()) {
    auto particle = track->GetDynamicParticle();
    auto material = track->GetMaterial();
    fPionPlusInelasticProcess->GetCrossSectionDataStore()->ComputeCrossSection(particle, material);
    return fPionPlusInelasticProcess->PostStepDoIt(*track, *step);
  }
  else if (track->GetParticleDefinition() == G4PionMinus::Definition()) {
    auto particle = track->GetDynamicParticle();
    auto material = track->GetMaterial();
    fPionMinusInelasticProcess->GetCrossSectionDataStore()->ComputeCrossSection(particle, material);
