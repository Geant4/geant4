...
G4MPIscorerMerger::G4MPIscorerMerger()
  : outputBuffer(nullptr),
    ...
    bytesSent(0),
    ownsBuffer(false),
    scoringManager(nullptr),
    commSize(0),
    destinationRank(G4MPImanager::kRANK_MASTER),
    verbose(0)
{}

G4MPIscorerMerger::G4MPIscorerMerger(G4ScoringManager* mgr, G4int destination, G4int verbosity)
  : outputBuffer(nullptr),
    outputBufferSize(0),
    outputBufferPosition(0),
    bytesSent(0),
...
void G4MPIscorerMerger::Pack(const G4ScoringManager* sm)
{
  ...
  if (outputBuffer == nullptr || outputBufferPosition >= outputBufferSize) {
    G4Exception("G4MPIscorerMerger::Pack(const G4ScoringManager*)", "MPI001", FatalException,
...
void G4MPIscorerMerger::UnPackAndMerge(const G4ScoringManager* sm)
{
  assert(sm != nullptr);
  if (outputBuffer == nullptr || outputBufferPosition >= outputBufferSize) {
    G4Exception("G4MPIscorerMerger::UnPack(const G4ScroingManager*)", "MPI001", FatalException,
