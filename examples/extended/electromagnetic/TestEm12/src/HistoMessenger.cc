...
// ********************************************************************
//
/// \file electromagnetic/TestEm12/src/HistoMessenger.cc
...
:fHistoManager (manager)
{
  ...
  fCsdaCmd = new G4UIcmdWithADoubleAndUnit("/testem/histo/setcsdaRange",this);
  fCsdaCmd->SetGuidance("Set csda range for normalisation");
  fCsdaCmd->SetParameterName("csda",false);
  fCsdaCmd->SetRange("csda>0.");
  fCsdaCmd->SetUnitCategory("Length");  
}
...
void HistoMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
  ...
  if (command == fCsdaCmd)
    fHistoManager->SetcsdaRange(fCsdaCmd->GetNewDoubleValue(newValues));         
}
