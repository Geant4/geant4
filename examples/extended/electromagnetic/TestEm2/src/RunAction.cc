...
RunAction::RunAction(DetectorConstruction* det, PrimaryGeneratorAction* kin)
 :G4UserRunAction(),fDet(det),fKin(kin),fRunMessenger(0),NbOfEvents(0),
...
void RunAction::FillPerEvent()
{
  ...
  fAnalysisManager->FillH1(1, 100.*dLCumul/(Ekin+mass));
  fAnalysisManager->FillH1(2, fChargTrLength/radl);
  fAnalysisManager->FillH1(3, fNeutrTrLength/radl);
}
...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  for(size_t it = 0; it < runActions.size(); ++it) { 
    ...
    for (G4int i=0; i<f_nLbin; i++) {
      ...
      fSumE2Longit[i] += anAction->fSumE2Longit[i];
      fSumELongitCumul[i] += anAction->fSumELongitCumul[i];
      fSumE2LongitCumul[i] += anAction->fSumE2LongitCumul[i];
    }

    for (G4int j=0; j<f_nRbin; j++) {
      fSumERadial[j] += anAction->fSumERadial[j];
      fSumE2Radial[j] += anAction->fSumE2Radial[j];
      fSumERadialCumul[j] += anAction->fSumERadialCumul[j];
      fSumE2RadialCumul[j] += anAction->fSumE2RadialCumul[j];
    }

    fSumChargTrLength  += anAction->fSumChargTrLength;
    fSum2ChargTrLength += anAction->fSum2ChargTrLength;
    fSumNeutrTrLength  += anAction->fSumNeutrTrLength;
    fSum2NeutrTrLength += anAction->fSum2NeutrTrLength;
  }
  ...
  for (i=0; i<f_nLbin; i++) {
    ...
    fAnalysisManager->FillH1(4, bin,MeanELongit[i]/dLradl);
    fAnalysisManager->FillH1(5, bin, rmsELongit[i]/dLradl);      
    bin = (i+1)*dLradl;
    fAnalysisManager->FillH1(6, bin,MeanELongitCumul[i]);
    fAnalysisManager->FillH1(7, bin, rmsELongitCumul[i]);
  }
  ...
  for (i=0; i<f_nRbin; i++) {
    ...
    fAnalysisManager->FillH1(8, bin,MeanERadial[i]/dRradl);
    fAnalysisManager->FillH1(9, bin, rmsERadial[i]/dRradl);      
    bin = (i+1)*dRradl;
    fAnalysisManager->FillH1(10, bin,MeanERadialCumul[i]);
    fAnalysisManager->FillH1(11, bin, rmsERadialCumul[i]);      
  }
