...
// ********************************************************************
//
// $Id: MuCrossSections.cc,v 1.5 2004-12-03 09:38:31 vnivanch Exp $
...
//    
    ...
    double sqrte=1.64872127; // std::sqrt(2.71828...)
    double btf=183.;
    double btf1=1429.;
    double bh=202.4;
    double bh1=446.;
...
//***		nuclear size and excitation, screening parameters
	...
	else
	{
	  b=btf;
	  b1=btf1;
	  dn_star=std::pow(dn,(1.-1./z)); // with Bugaev's correction
	}
//***		nucleus contribution logarithm
	rab1=b*z_13;
	fn=std::log(rab1/(dn_star*(ame+rab0*rab1))*(amu+delta*(dn_star*sqrte-2.)));
	if(fn < 0.) fn=0.;
//***		electron contribution logarithm
	...
	rab2=b1*z_13*z_13;
	fe=std::log(rab2*amu/((1.+delta*rmass/(ame*sqrte))*(ame+rab0*rab2)));
	if(fe < 0.) fe=0.;
//***
label10:
...
//***
    ...
    double alam =0.632456; // std::sqrt(alam2)
...
//***
	...
	aeff=0.22*a+0.78*std::pow(a,0.89); // shadowing
	...
	crn_g4=coeffn*aeff/a*sigph/ep*(-v1+(v1+0.5*v2*(1.+2.*amu2/alam2))*std::log(up/down));
...
//***		zeta calculation
	...
	zeta1=0.073*std::log(e/(amu+g1*z13*z13*e))-0.26;
	if(zeta1 > 0.)
	{
	  zeta2=0.058*std::log(e/(amu+g2*z13*e))-0.14;
	  zeta=zeta1/zeta2;
	}
	else
	{
...
//***
	...
	del=c8/a0; // 6mu^2/EE'
	tmn=std::log((alf+2.*del*a3)/(1.+(1.-del)*std::sqrt(a3))); // std::log(1-rmax)
	sum=0.;
	for(int i=0; i<=7; i++) // integration
	{
	  a4=std::exp(tmn*xgi[i]); // 1-r
	  a5=a4*(2.-a4); // 1-r2
	  a6=1.-a5; // r2
	  a7=1.+a6; // 1+r2
	  a9=3.+a6; // 3+r2
	  ...
	  yeu=5.-a6+4.*bet*a7;
	  yed=2.*(1.+3.*bet)*std::log(3.+xii)-a6-a1*(2.-a6);
	  ye1=1.+yeu/yed;
	  ale=std::log(bbb/z13*std::sqrt(xi1*ye1)/(1.+screen*ye1));
	  cre=0.5*std::log(1.+(1.5/rmass*z13)*(1.5/rmass*z13)*xi1*ye1);
	  if(xi <= 1e3) //
	  {
	    be=((2.+a6)*(1.+bet)+xi*a9)*std::log(1.+xii)+(a5-bet)/xi1-a9;
	  }
	  else
	  {
	  ...
	  ymu=4.+a6+3.*bet*a7;
	  ymd=a7*(1.5+a1)*std::log(3.+xi)+1.-1.5*a6;
	  ym1=1.+ymu/ymd;
	  alm_crm=std::log(bbb*rmass/(1.5*z13*z13*(1.+screen*ym1)));
	  if(xi >= 1e-3) //
	  {
	    a10=(1.+a1)*a5; // (1+2b)(1-r2)
	    bm=(a7*(1.+1.5*bet)-a10*xii)*std::log(xi1)+xi*(a5-bet)/xi1+a10;
	  }
	  else
	  {
